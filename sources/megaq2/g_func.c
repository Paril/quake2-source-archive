...
void plat_blocked (edict_t *self, edict_t *other)
{
	if (!(other->svflags & SVF_MONSTER) && (!other->client) )
	{
		...
		{
			//peewee start
			if (other->owner) //if it has an owner
				if (other->owner->client) //if it's owner is a client
					if (other->owner->client->pers.shot_limit > 0) //if it was a shot
						other->owner->client->pers.shot_limit--; //decrement limit
			//peewee end
