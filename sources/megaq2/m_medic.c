...
void medic_fire_blaster (edict_t *self)
{
	...
	//monster_fire_rocket (self, start, dir, 2, 1000, MZ2_CHICK_ROCKET_1);
...
//peewee start
	//spawn weapon from dead monster
	gitem_t *item;
	edict_t *monster, *timer;
	if (strcmp(self->classname, "Hyperblaster") == 0)
	{
		item = FindItem("Cells");
		Drop_Item (self, item);
		...
		if (deathmatch->value)
		{
			...
			timer = G_Spawn();
			timer->activator = self->activator;
			timer->item = item;
			timer->think = ItemSpawnMonsterTimer;
			timer->nextthink = level.time + 20;
		}
	}

	//always gib so comment out following ling
