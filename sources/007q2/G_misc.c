...
// Bugfix - Removed LccHack.c from project!!
...
void SP_light (edict_t *self)
{
	...
	if (self->spawnflags & 2)
	{
		...
		if (self->color[0] <= 0 && self->color[1] <= 0 && self->color[2] <= 0) //color
			self->color[0] = self->color[1] = self->color[2] = 1;

		if (!self->health)	// size
			self->health = 20;

		if (!self->mass)	//flare image
			self->mass = 1;
	
		self->color[0] *= 1000; red = (int)self->color[0];
		self->color[1] *= 1000; green = (int)self->color[1];
		self->color[2] *= 1000; blue = (int)self->color[2];
		
		sprintf( str, 
                        "%i %i %i %i %i %i %i %i", 
                        self->mass,
                        red, green, blue,
                        (int)self->s.origin[0], (int)self->s.origin[1], (int)self->s.origin[2], self->health);
...
/*id func_explosive_explode (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
    vec3_t origin;
    vec3_t chunkorigin;
    vec3_t size;
    ...
    vec3_t old_origin;
    vec3_t old_velocity;
	int intSP_RespawnTime;

	intSP_RespawnTime = 30;


    VectorCopy(self->s.origin,old_origin);
    VectorCopy(self->velocity,old_velocity);
    ...
    VectorCopy(old_velocity,self->velocity);
    self->solid = SOLID_NOT;
    self->svflags |= SVF_NOCLIENT;
    self->think = func_explosive_respawn;
    self->nextthink = level.time + intSP_RespawnTime;
    self->use = NULL;
    gi.linkentity(self);
}*/
...
void SP_func_explosive (edict_t *self)
{
	...
	if (deathmatch->value && g_glass->value == 0)
