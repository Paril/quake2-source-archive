...
//static cvar_t*  modelindex     = 0;
//  modelindex     = gi.cvar("mi", "0", 0);
...
//		ent->s.effects |= RF_FRAMELERP;
		ent->s.modelindex2 = 0;
//		ent->s.modelindex2 = modelindex;
...
/*	if (muzzleon->value)
		ent->s.modelindex4 = gi.modelindex("models/muzzleflash/tris.md2");
		ent->s.modelindex4.frame
...
/*#define SURF_METAL  1 //Metal floor  
#define SURF_DIRT   2 //Dirt, sand, rock  
#define SURF_VENT   3 //Ventillation duct
#define SURF_GRATE  4 //Metal grating
#define SURF_TILE   5 //Floor tiles
#define SURF_GRASS  6 //Grass
#define SURF_SNOW   7 //Snow
#define SURF_CARPET 8 //Carpet
#define SURF_FORCE  9 //Force field
	...
	qboolean Surface(char *name, int type)
	{
		...
		case SURF_DIRT:
			...
			break;
		case SURF_VENT:
			if (strcmpwld (name, "tilefloor10"))
				return true;
			if (strcmpwld (name, "cdirtysq"))
				return true;
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	//
  if (!ent->isabot) { // Maj++ - Don't modify bot's angles!!
  ...
  SetBotXYSpeed(ent,&xyspeed); // Maj++ - Totally Critical!
...
//ZOID
	...
	// chase cam stuff
//	if (ent->client->resp.spectator && ctf->value) {
	if (ent->client->resp.spectator) {
		G_SetSpectatorStats(ent);
	} else {
		if (!ent->client->chase_target)
//		if (!ent->client->chase_target && ctf->value)
			G_SetStats (ent);}
