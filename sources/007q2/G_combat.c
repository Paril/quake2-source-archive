    ...
    if (!VectorEmpty(point))
      if (IsIn(mod, MOD_BLASTER, MOD_RAILGUN)) {
        ...
        if (z_rel < CHEST_DAMAGE)
          return 0.70;  // Scale up by 1/5
        else
          return 1.0;} // Scale up by 10X
...
//ZOID*/
//ION
	if (teamdm->value && targ->client && attacker->client)
		if (targ->client->resp.teamdm_team == attacker->client->resp.teamdm_team &&
			targ != attacker)
			return true;
//ION
...
void T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t dir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
{
  ...
  CheckBotCrushed(targ,inflictor,mod); // Maj++
...
// Maj++ ---------------------------------------
    ...
    if (targ->client && !targ->isabot)
      if (attacker->client)
        targ->client->current_enemy = attacker; // go after this guy!
//----------------------------------------------------------
