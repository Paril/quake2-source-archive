...
void Use_Target_SpeakerBig (edict_t *ent, edict_t *other, edict_t *activator)
{
	int		chan;
...
void SP_target_otherspeaker (edict_t *ent)
{
	char	buffer[MAX_QPATH];
...
/*QUAKED target_weather (1 0 0) (-8 -8 -8) (8 8 8)
Creates a weather effect when used.
...
*/
void use_target_weather (edict_t *self, edict_t *other, edict_t *activator)
{
	vec3_t	center;
...
*/	if (!self->scale)
	...
	if (!self->velocity)
		VectorSet(self->velocity, 0, 0, -25);
	if (!self->color)
		VectorSet(self->color, 255, 255, 255);
	
	VectorAdd(self->bleft,self->tright,center);
	VectorMA(self->s.origin,0.5,center,center);

	gi.WriteByte (svc_temp_entity);
	gi.WriteByte (TE_WEATHERFX);
	gi.WritePosition (self->s.origin);
	gi.WritePosition (self->velocity);
	gi.WritePosition (self->color);
	gi.WriteByte (self->sounds);
	gi.WriteFloat (self->scale);
	gi.WriteFloat (time);
	gi.WriteByte (self->count);
	gi.WritePosition(self->bleft);
	gi.WritePosition(self->tright);

//	gi.dprintf("target_splash\n");

	gi.multicast (self->s.origin, MULTICAST_PVS);
}

void SP_target_weather (edict_t *self)
{
	self->use = use_target_weather;
	G_SetMovedir (self->s.angles, self->movedir);
