...
void SV_CalcBlend (edict_t *ent)
{
	...
	if(ent->health > 0)
	if(ent->client->pers.explode_time - level.time < 5.0)
	{
		remaining = (ent->client->pers.explode_time - level.time)*20;

		if(remaining & 2)
			SV_AddBlend (1.0, 0.2, 0.0, 0.1, ent->client->ps.blend);
	}
...
void G_SetClientEffects (edict_t *ent)
{
	...
	if((ent->client->pers.explode_time - level.time) < 4)
	{
			ent->s.effects |= EF_COLOR_SHELL;
