...
//-----------------------------------------------------------------------------
// Return a pointer on a client struct with a given name (or NULL if no match)
//-----------------------------------------------------------------------------
gclient_t *GetClientByName(char *szName)
{
	int i;
	...
	for ( i=0 ; i<maxclients->value ; i++ )
	{
		...
		if ( game.clients[i].pers.connected || cl_ent->inuse )
			if ( stricmp(game.clients[i].pers.netname, szName)==0 )
	...
	if ( XML_Parser(pszResult, "client_set", szData, sizeof(szData)) )
	{
		for ( i=0 ; i<maxclients->value ; i++ )
		{
			if (    XML_Parser(szData, va("client%d", i), szSubData, sizeof(szSubData))
				 ...
				 && (pClient = GetClientByName(szName)) != NULL )
			{
				if ( XML_Parser(szSubData, "health", szTmp,    sizeof(szTmp)) )
					pClient->pers.health = atoi(szTmp);

				if ( XML_Parser(szSubData, "inventory", szTmp, sizeof(szTmp)) )
				{
					// Reset inventory, before filling it
					memset(pClient->pers.inventory, 0, sizeof(game.clients[0].pers.inventory));

					// Force Blaster in all cases
					//$$ pClient->pers.inventory[7] = 1;
					...
					while ( pszItem )
					{
						...
						if ( pszNb )
						{
							...
							pClient->pers.inventory[iItem] = iNb ;
						}
						pszItem = strtok(NULL, ":") ;
					}
				}
				...
				if ( XML_Parser(szSubData, "spectator", szTmp, sizeof(szTmp)) )
					pClient->pers.spectator = atoi(szTmp) != 0;
			}
		}
	}
}
...
void WSrv_Killed(edict_t *targ, edict_t *inflictor, edict_t *attacker)
{
	...
	Com_sprintf(szData, sizeof(szData), "%s,%d,%d,%d",
		level.mapname,
		(int)targ->s.origin[0], 
		(int)targ->s.origin[1], 
		(int)targ->s.origin[2]);
...
//$$	cvar_t	*tag_worldserver;
	...
	if ( worldserver->value )
	{
		if ( ent )
		{
			...
			gi.WriteByte (svc_stufftext);
			gi.WriteString("map startmap\n");
			gi.unicast (ent, true);
		}
		else
			gi.AddCommandString ("map startmap\n");
		return;
	}
	...
	{
//$$		l_pWsrvData = gi.TagMalloc(sizeof(*l_pWsrvData), TAG_WORLDSERVER);
		l_pWsrvData = gi.TagMalloc(sizeof(*l_pWsrvData), TAG_GAME);

//<worldserver> FIXME : bad hack for preserve worldserver data throught a dll reload
//$$	    tag_worldserver = gi.cvar ("tag_worldserver",  va("%i",(int)l_pWsrvData), CVAR_NOSET);

		if ( RPC_Login_OK(&l_RpcData, szMsg, sizeof(szMsg)) )
		{
			Com_Printf("Connected to WorldServer\n");
			gi.cvar_set("worldserver", "1");
//$$            gi.cvar_forceset("worldserver", "1");
...
void	Cmd_WSrv_Logout_f(edict_t *ent)
{
	...
	if ( l_RpcData.byRpcStatus == RPC_STATUS_OK )
	{
		...
		l_pWsrvData = NULL;
//$$		gi.FreeTags (TAG_WORLDSERVER);	// Free the internal data used by the WorldServer
