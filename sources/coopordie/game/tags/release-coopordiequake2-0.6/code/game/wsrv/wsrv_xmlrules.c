...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static t_XMLRULES_Status Check1Condition(char *pnodeValue1, char *pnodeValue2, DOM_Node *pOp, t_key *pKey)
{
	int i ;
	...
	for ( i=0 ; i<pKey->nbkey ; i++ )
	{
		if ( strcmp(pKey->name[i], pnodeValue1)==0 )
		{
			...
			{
				if ( strstr(pKey->value[i], pnodeValue2) )
					return XMLRULES_Exec ;
				else
					return XMLRULES_Continue ;
			}
