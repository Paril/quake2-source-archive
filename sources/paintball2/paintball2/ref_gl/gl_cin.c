...
void CIN_StopCinematic (void)
{
	...
	{
		ri.FS_FreeFile(cin->cinematic_file);
...
byte *CIN_ReadNextFrame (void)
{
	...
	if (size > sizeof(compressed) || size < 1)
		ri.Sys_Error (ERR_DROP, "Bad compressed frame size");
	memcpy(compressed, cin->offset, size);
	cin->offset += size;

	// read sound
	start = cin->frame*cin->s_rate*0.071428571428571428571428571428571;
	end = (cin->frame+1)*cin->s_rate*0.071428571428571428571428571428571;
	count = end - start;
...
void CIN_RunCinematic (void)
{
	...
	if (frame > cin->frame+1)
	{
		...
		cin->time = Sys_Milliseconds() - cin->frame*1000*0.071428571428571428571428571428571;
	}
	if (cin->pic)
		free(cin->pic);
