...
//#define DEBUG_DUMP_IRC // jit - testing
...
// jit - forward declares because I rearranged some things and am lazy
 ...
 */
static qboolean IRC_CheckEventRate( int event_type )
{
 ...
 */
static void IRC_UpdateRateLimiter( )
{
	int i;
 ...
 */
static void IRC_InitRateLimiter( )
{
	int i;
	for ( i = 0 ; i < sizeof( IRC_RateLimiter ) / sizeof( unsigned int ) ; i ++ )
		IRC_RateLimiter[ i ] = 0;
...
static void IRC_NeutraliseString( char * buffer , const char * source )
{
	while ( *source ) {
		...
		if ( IS_CNTRL( c ) ) {
			...
			*( buffer ++ ) = '?';
#if 0 // jit - not sure what the escape color is for
...
static void IRC_Display( int event , const char * nick , const char *message )
{
	...
	switch ( IRC_EventType( event ) ) {
...
#else // jit - we don't use this formatting
			fmt_string = "<%s> %s";
#endif
			break;
		case IRC_EVT_ACT:
		...
		case IRC_EVT_JOIN:
			...
			if ( has_nick ) {
				fmt_string = "-> %s has entered the channel.";//"^5-> ^7%s^5 has entered the channel.";
			} else {
				fmt_string = "Joined IRC chat.";//"^2Joined IRC chat.";
			}
			break;
		case IRC_EVT_PART:
			...
			if ( has_message ) {
				fmt_string = "<- %s has left the channel: %s.";//"^5<- ^7%s^5 has left the channel: %s.";
			} else {
				fmt_string = "<- %s has left the channel.";//"^5<- ^7%s^5 has left the channel.";
			}
			break;
		case IRC_EVT_QUIT:
			has_nick = !IRC_EventIsSelf( event );
			if ( has_nick ) {
				has_message = ( message[0] != 0 );
				if ( has_message ) {
					fmt_string = "<- %s has quit: %s.";//"^5<- ^7%s^5 has quit: %s.";
				} else {
					fmt_string = "<- %s has quit.";
				}
			} else {
				has_message = true;
				fmt_string = "Quit IRC chat: %s.";
			}
			break;
		case IRC_EVT_KICK:
			has_nick = has_message = true;
			if ( IRC_EventIsSelf( event ) ) {
				fmt_string = "Kicked by %s: %s.";
			} else {
				fmt_string = "<- %s has been kicked: %s.";
			}
			break;
		case IRC_EVT_NICK_CHANGE:
			has_nick = has_message = true;
			if ( IRC_EventIsSelf( event ) ) {
				fmt_string = "** %s is now known as %s.";
			} else {
				fmt_string = "** %s is now known as %s.";
			}
			break;
	...
	Com_Printf("IRC: %s\n", buffer); // jitodo - print this to some other buffer that can be displayed in a widget or something.
}
...
static int IRCH_Kick( )
{
	if ( !strcmp( IRC_String( arg_values[ 1 ] ) , IRC_User.nick ) ) {
		...
		if ( cl_IRC_kick_rejoin->value > 0 ) {
			IRC_ThreadStatus = IRC_THREAD_CONNECTED;
			IRC_SetTimeout( &IRC_JoinChannel , cl_IRC_kick_rejoin->value );
		} else {
...
// jit - IRC notice
static int IRCH_Notice (void)
{
	if (IRC_ReceivedMessage.arg_count > 1)
	{
		IRC_Display(IRC_EVT_SAY, IRC_String(arg_values[0]), IRC_String(arg_values[1]));
	}
	else if (IRC_ReceivedMessage.arg_count > 0)
	{
		IRC_Display(IRC_EVT_SAY, IRC_String(pfx_nickOrServer), IRC_String(arg_values[0]));
	}
	else
	{
		IRC_Display(IRC_EVT_SAY, IRC_String(pfx_nickOrServer), "NOTICE"); // does this ever even happen?
	}
...
static int IRC_HandleMessage( qboolean is_channel , const char * string )
{
	...
	if ( IRC_CheckEventRate( IRC_RL_MESSAGE ) )
		return IRC_Send( "PRIVMSG %s :Sorry, Paintball2's IRC client does not support private messages" , IRC_String( pfx_nickOrServer ) );
	return IRC_CMD_SUCCESS;
}
...
static int CTCP_Version( qboolean is_channel , const char * argument )
{
	...
	return IRC_Send( "NOTICE %s :\001VERSION Paintball2 IRC client - v" BUILD_S "\001" , IRC_String( pfx_nickOrServer ) );
}
...
void CL_IRCSay( )
{
	...
	if ( ( m_sendstring[ 0 ] == '/' || m_sendstring[ 0 ] == '.' ) && !Q_strncasecmp( m_sendstring + 1 , "me " , 3 ) && m_sendstring[ 4 ] != 0 ) {
...
static qboolean IRC_InitialiseUser( const char * name )
{
	...
	ovrnn = cl_IRC_override_nickname->value && strlen( cl_IRC_nickname->name );
...
#if 0 // jit -- not sure what this is used for
...
static qboolean IRC_InitialConnect( )
{
	...
	int rc_delay = cl_IRC_reconnect_delay->value;
	if ( rc_delay < 5 )
		rc_delay = 5;
	...
	do {
		do {
			...
			if ( IRC_QuitRequested && IRC_ThreadStatus != IRC_THREAD_QUITTING ) {
				...
				err_code = IRC_Send( "QUIT : Paintball2 IRC " BUILD_S );
			} else {
...
void CL_IRCSetup(void)
{
	...
	cl_IRC_server = Cvar_Get( "cl_IRC_server" , "irc.globalgamers.net" , 0 );
	cl_IRC_channel = Cvar_Get( "cl_IRC_channel" , "paintball" , 0 );
	...
	cl_IRC_kick_rejoin = Cvar_Get( "cl_IRC_kick_rejoin" , "0" , 0 );
...
#else // jit - made this more straightforward -- not really enough commands to warrant a hash table and a bunch of convoluted macros.
	const char *cmd_string = IRC_ReceivedMessage.cmd_string.string;

	if (Q_streq(cmd_string, "PING"))
		return IRCH_Ping();		// Ping request
	else if (Q_streq(cmd_string, "ERROR"))
		return IRCH_ServerError();	// Server error
	else if (Q_streq(cmd_string, "JOIN"))
		return IRCH_Joined();		// Channel join
	else if (Q_streq(cmd_string, "PART"))
		return IRCH_Part();		// Channel part
	else if (Q_streq(cmd_string, "QUIT"))
		return IRCH_Quit();		// Client quit
	else if (Q_streq(cmd_string, "PRIVMSG"))
		return IRCH_PrivMsg();		// Message or CTCP
	else if (Q_streq(cmd_string, "KICK"))
		return IRCH_Kick();		// Kick
	else if (Q_streq(cmd_string, "NICK"))
		return IRCH_Nick();		// Nick change
	else if (Q_streq(cmd_string, "NOTICE")) // jit
		return IRCH_Notice();
	else if (Q_streq(cmd_string, "MODE")) // jit
		return IRC_CMD_SUCCESS;		// todo
	else if (Q_streq(cmd_string, "001"))
		return IRCH_Connected();	// Connection established
	else if (Q_streq(cmd_string, "002"))
		return IRC_CMD_SUCCESS;		// Host
	else if (Q_streq(cmd_string, "003"))
		return IRC_CMD_SUCCESS;		// Server creation date.
	else if (Q_streq(cmd_string, "005"))
		return IRC_CMD_SUCCESS;		// RPL_BOUNCE	"Try server <server name>, port <port number>"
	else if (Q_streq(cmd_string, "251"))
		return IRC_CMD_SUCCESS;		// RPL_LUSERCLIENT	":There are <integer> users and <integer> services on <integer> servers"
	else if (Q_streq(cmd_string, "252"))
		return IRC_CMD_SUCCESS;		// 252	RPL_LUSEROP	"<integer> :operator(s) online"
	else if (Q_streq(cmd_string, "254"))
		return IRC_CMD_SUCCESS;		
	else if (Q_streq(cmd_string, "255"))
		return IRC_CMD_SUCCESS;		
	else if (Q_streq(cmd_string, "265"))
		return IRC_CMD_SUCCESS;		
	else if (Q_streq(cmd_string, "266"))
		return IRC_CMD_SUCCESS;		
	else if (Q_streq(cmd_string, "372"))
		return IRC_CMD_SUCCESS;		// 372	RPL_MOTD	":- <text>"
	else if (Q_streq(cmd_string, "375"))
		return IRC_CMD_SUCCESS;		// 375	RPL_MOTDSTART	":- <server> Message of the day - "
	else if (Q_streq(cmd_string, "376"))
		return IRC_CMD_SUCCESS;		
	else if (Q_streq(cmd_string, "404"))
		return IRCH_Banned();		// Banned (when sending message)
	else if (Q_streq(cmd_string, "432"))
		return IRCH_FatalError();	// Erroneous nick name
	else if (Q_streq(cmd_string, "433"))
		return IRCH_NickError();	// Nick name in use
	else if (Q_streq(cmd_string, "474"))
		return IRCH_Banned();		// Banned (when joining)
	else
	{
		Com_Printf("Unhandled IRC command string: \"%s\".\n", cmd_string);
		return IRC_CMD_SUCCESS;
	}
#endif
}
...
#else // jit - made this more straightforward -- no need for hashes or crazy macros.
	if (Q_streq(command, "ACTION"))
		return CTCP_Action(is_channel, argument);	// "/me"
	else if (Q_streq(command, "PING"))
		return CTCP_Ping(is_channel, argument);
	else if (Q_streq(command, "VERSION"))
		return CTCP_Version(is_channel, argument);
	else
	{
		Com_Printf("Unhandled IRC CTCP command: \"%s\".\n", command);
		return IRC_CMD_SUCCESS;
	}
#endif
}
