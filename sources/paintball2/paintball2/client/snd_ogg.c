...
extern cvar_t   *fs_gamedir;		/* Game path (viciouz) */
...
qboolean OGG_Open(ogg_seek_t type, int offset)
{
	...
	/* Find file. */
	if ((size = FS_LoadFile(va("../%s",ogg_filelist[pos]), (void **)&ogg_buffer)) == -1) { // viciouz - ../ nasty hacky ogg support
		Com_Printf("OGG_Open: could not open %d (%s): %s.\n", pos, ogg_filelist[pos], strerror(errno));
		return (false);
	}

	/* Open ogg vorbis file. */
	if ((res = ov_open_callbacks(NULL, &ovFile, (char *)ogg_buffer, size, OV_CALLBACKS_DEFAULT)) < 0) {
