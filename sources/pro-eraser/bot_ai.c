...
//if (false)
	...
	{	// don't attack someone else if current enemy has our flag!
		...
		{
			if (	!(search->flags & FL_NOTARGET) && (search != self) && (self->enemy != search) && (search->health > 0) && (search->bot_client || (search->light_level > 5)) && search->solid && !SameTeam(self, search)
			...
			{
				if (((carrying_flag) || ((this_dist = entdist(self, search)) < closest_dist)) && visible(self, search) && CanSee(self, search))
				{	// go for em!
...
//	if (/*(num_players < 3) &&*/ /*(!botCheckStuck(self)) && (ClosestNodeToEnt(self, false, true) > -1)*/)	// if can't see self, then probably stuck
//	{
...
/*
	if (self->bored_suicide_time > -1)
	{
		if (self->bored_suicide_time < level.time)	// suicide
		{
botDebugPrint("Bot suicide (bored)\n");
			T_Damage(self, self, self, tv(0,0,0), self->s.origin, tv(0,0,0), self->health + 1, 0, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
		}
	}
	else
	{	// suicide if still can't find a reachable item/enemy/node within 15 seconds
		self->bored_suicide_time = level.time + 30;
	}
*/
}
		...
		if (self->client->ctf_grapple)	// it could have been killed in CTFGrapplePull()
		{
			...
			if (self->client->ctf_grapplestate > CTF_GRAPPLE_STATE_FLY)
			{
				...
				if (self->client->ctf_grapple)
				{
					...
					if (self->goalentity && self->last_movegoal && (self->goalentity->node_type != NODE_LANDING))
					{
...
//		(!self->movetarget || !self->movetarget->item || (self->movetarget->item->pickup != CTFPickup_Flag)))
	{
		...
		if (last_checkhelp < (level.time - 0.5))
		{
			for (i=0; i<num_players; i++)
			{
				...
				{
					if ((plyr->enemy != self) && (entdist(plyr, self) < 1000))
...
gotvel:
			...
			{	// if directly ahead is blocked we should jump higher
				...
				VectorAdd(self->s.origin, tv(0,0,8), org);
				VectorCopy(dir, vec);
				vec[2] = 0;
			...
			{	// see if we've reached the marker
				if (bot_ReachedTrail(self))
				{
					...
					else
					{
						...
						else if (		(grapple->value)
									&&	(bot_tarzan->value || CarryingFlag(self))
...
// END, SABIN code
	...
	if (self->waterlevel && ((self->air_finished - 2.5) < level.time))	// go for air!
	{
		// can we get to air?
	...
	else if (!self->groundentity && (self->waterlevel < 3) && self->goalentity && ((self->goalentity->s.origin[2]+32) > self->s.origin[2]))
	{
		ucmd.upmove = sv_maxvelocity->value;
	}
...
//		dir[2] = 0;
	...
	else if ((self->velocity[2] > 310) && !self->client->ctf_grapple)
	{
		self->velocity[2] = 310;

		// have we gone passed the goalentity? if so slow down velocity
		if (self->goalentity && (fabs(self->s.origin[2] - self->goalentity->s.origin[2]) > 32))
		{
	...
	if ((!start_waterlevel) && !self->groundentity && last_groundentity)
	{
		...
		if (self->goalentity && (entdist(self, self->goalentity) < 256) && (self->s.origin[2] > (self->goalentity->s.origin[2]-48)))
		{
			if (self->s.origin[2] < (self->goalentity->s.origin[2]+32))	// only jump if goal is above us
			{
				...
				tr = gi.trace(self->s.origin, self->mins, self->maxs, dropdest, self, MASK_PLAYERSOLID);

				if ((tr.fraction == 1) || (tr.plane.normal[2] < 0.5) || (gi.pointcontents(tr.endpos) & (CONTENTS_LAVA | CONTENTS_SLIME)))
				{	// jump to be safe
		...
		// if monster had the ground pulled out, go ahead and fall, but only if safe (No lava or slime)
			...
			if (	(	(	(trace.fraction == 1) 
				...
				|| (gi.pointcontents(dest) & (CONTENTS_LAVA | CONTENTS_SLIME)))	// falling into lava
...
{
	if (other->client && SameTeam(other, self))
		return false;
...
int	botCheckStuck(edict_t	*self)
{
	...
	trace = gi.trace(self->s.origin, self->mins, self->maxs, self->s.origin, self, MASK_PLAYERSOLID);

	if (trace.startsolid)
	{
		// stuck, suicide
botDebugPrint("Bot suicide (stuck)\n");
		T_Damage(self, self, self, VEC_ORIGIN, self->s.origin, VEC_ORIGIN, 100000, 0, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
		return true;
	}
...
int CanStand(edict_t	*self)
{
	...
	trace = gi.trace(self->s.origin, self->mins, tv(16,16, 32), self->s.origin, self, MASK_PLAYERSOLID);
...
{	// get a bot to head for the enemy flag
	int i, team;
	edict_t	*enemy_flag;
...
// Who cares if it's not there! Just invade their fuckin base!
//	if (enemy_flag->solid != SOLID_TRIGGER)
//		goto done;

	// start an invasion! summon all bots
	...
	for (i=0; i<num_players; i++)
	{
		...
		if (players[i]->client->resp.ctf_team != team)
			continue;

		if (players[i]->target_ent)
			continue;

		if (players[i]->movetarget == enemy_flag)
			continue;

		if (	(	(players[i]->bot_fire == botBlaster)
				 ||	(players[i]->bot_fire == botShotgun))
			&&	(players[i]->movetarget_want >= WANT_SHITYEAH))
			continue;

		if ((entdist(players[i], enemy_flag) > 2000) && (random() < 0.5))
			continue;

		players[i]->movetarget = enemy_flag;
		players[i]->movetarget_want = WANT_SHITYEAH;
//		break;
	}
...
//done:
	...
	ent->nextthink = level.time + 1 + random();
}
*/
