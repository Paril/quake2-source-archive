...
//							self->s.origin[2] += 1;
				...
				if ((rnd >= 1) && (self->maxs[2] == 4) && (CanStand(self)) && (!self->goalentity || (self->goalentity->maxs[2] > 4)))
				{	// resume standing
					self->maxs[2] = 32;
				}
			}
...
void botMachineGun (edict_t *self)
{
	...
	if (!self->dmg)
	{
		...
		self->dmg = 1;	// fire again during next client frame
...
void botRocketLauncher (edict_t *self)
{
	...
	if (self->enemy && infront(self, self->enemy))
	{
		...
		if ((skill->value > 1) && (self->enemy->health > 0) && (self->enemy->client && !self->enemy->bot_client) && (dist > 64))
		{
			...
			// ***************************** MODIFIED FOR PRO-ERASER
			//VectorMA (self->enemy->s.origin, (float) dist / 650, vel, target);
			VectorMA (self->enemy->s.origin, (float) dist / 1000, vel, target);
			// ***************************** MODIFIED FOR PRO-ERASER
			target[2] += self->enemy->viewheight - 8;
...
//		AngleVectors (self->s.angles, forward, NULL, NULL);
	...
	if (self->enemy->client && !self->enemy->bot_client)
	{
		...
		VectorMA (self->enemy->s.origin, (float) dist / 1000, self->enemy->velocity, self->enemy->s.origin);
		// ***************************** MODIFIED FOR PRO-ERASER
	...
	fire_rocket (self, start, forward, damage, 1000, damage_radius, radius_damage);
	// ***************************** MODIFIED FOR PRO-ERASER
	self->client->pers.inventory[self->client->ammo_index]--;
