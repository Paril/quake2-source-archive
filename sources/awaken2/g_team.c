...
//CW++
	if (self->deadflag || (self->s.modelindex != 255))
		return;
//CW--
...
//	Determine if it's near or above/below.
	...
	if (nearteam == CTF_TEAM1)
		sprintf(teamname, "the %s ", sv_team1_name->string);										//CW
	else if (nearteam == CTF_TEAM2)
		sprintf(teamname, "the %s ", sv_team2_name->string);										//CW
	else
		sprintf(teamname, "the ");																	//CW

	strcat(buf, teamname);																			//CW++
...
//CW--
			case MATCH_SETUP :
				...
				if (competition->value < 3)
					sprintf(text, "%02d:%02d SETUP: %d not ready", (int)(t / 60), (int)(t % 60), j);
				else
			...
			case MATCH_PREGAME :
				sprintf(text, "%02d:%02d UNTIL START", (int)(t / 60), (int)(t % 60));
				gi.configstring(CONFIG_CTF_MATCH, text);
			...
			case MATCH_GAME:
				sprintf(text, "%02d:%02d MATCH", (int)(t / 60), (int)(t % 60));
				gi.configstring(CONFIG_CTF_MATCH, text);
				if ((t <= 10) && !teamgame.countdown)
				{
					teamgame.countdown = true;
	...
	else
	{
		...
		if (warn_unbalanced->value)
		{
			...
			if ((team1 - team2 >= 2) && (team2 >= 2))
			{
				...
				{
					sprintf(msg, "WARNING: %s has too many players", sv_team1_name->string);		//CW++
			...
			else if ((team2 - team1 >= 2) && (team1 >= 2))
			{
				...
				{
					sprintf(msg, "WARNING: %s has too many players", sv_team2_name->string);		//CW++
...
//CW++
	...
	if (ban)
	{
		...
		sprintf(text, "sv addip %s\n", targ->client->pers.ip);
		gi.AddCommandString(text);
		sprintf(text, "sv writeip\n");
		gi.AddCommandString(text);
	}
//CW--
	...
	else	// this is only done in non-match (public) mode
	{
		...
		if (warn_unbalanced->value)
		{
			...
			if ((team1 - team2 >= 2) && (team2 >= 2))
			{
				if (teamgame.warnactive != CTF_TEAM1)
				{
					sprintf(msg, "WARNING: %s has too many players", sv_team1_name->string);
					teamgame.warnactive = CTF_TEAM1;
			...
			else if ((team2 - team1 >= 2) && (team1 >= 2))
			{
				if (teamgame.warnactive != CTF_TEAM2)
				{
					sprintf(msg, "WARNING: %s has too many players", sv_team2_name->string);
					teamgame.warnactive = CTF_TEAM2;
					gi.configstring(CONFIG_CTF_TEAMINFO, msg);
				}
			}
...
//	Search through map entities until the appropriate [info_mission] is found.
	...
	sprintf(title, "*Objective #%d", mission->obj);
