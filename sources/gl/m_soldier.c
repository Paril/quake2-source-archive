...
void soldier_fire (edict_t *self, int flash_number)
{
	...
	if (self->s.skinnum <= 1)
	{
                monster_fire_blaster (self, start, aim, 15, 600, flash_index, EF_BLASTER);
	}
	else if (self->s.skinnum <= 3)
	{
                monster_fire_shotgun (self, start, aim, 3, 1, DEFAULT_SHOTGUN_HSPREAD, DEFAULT_SHOTGUN_VSPREAD, DEFAULT_SHOTGUN_COUNT, flash_index);
	}
	else
	{
...
if (!CheckBounds (owner, 36, 100, 2)) return;
	...
	self->monsterinfo.power_armor_type = POWER_ARMOR_SCREEN;
        self->monsterinfo.power_armor_power = 3000;
...
if (!CheckBounds (owner, 36, 100, 4)) return;
