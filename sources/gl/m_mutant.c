...
void mutant_check_refire (edict_t *self)
{
	if (!self->enemy || !self->enemy->inuse || self->enemy->health <= 0)
        ...
        if ( ((skill->value == 3) && (random() < 0.8)) || (range(self, self->enemy) == RANGE_MELEE) )
...
void mutant_jump_takeoff (edict_t *self)
{
	...
	self->s.origin[2] += 1;
        VectorScale (forward, 590, self->velocity);
        self->velocity[2] = 300;
...
if (!CheckBounds (owner, 36, 100, 20)) return;
