...
void TankMachineGun (edict_t *self)
{
	...
	{
        monster_fire_grenade (self, start, forward, 50, 600, MZ2_GUNNER_GRENADE_1);
        }
        else
...
mmove_t tank_move_attack_grenade = {FRAME_attak401, FRAME_attak429, tank_frames_attack_grenade, tank_run};
...
{                                                              
	...
	if (range <= 125)
	{
            ...
            if (strcmp(self->classname, "monster_tank_commander") == 0)
            {
            self->monsterinfo.currentmove = &tank_move_attack_grenade;
            }
            else
...
void SP_monster_tank2 (edict_t *owner, int commander)
{
      edict_t *self;
        ...
        if (commander == 1)
	{
if (!CheckBounds (owner, 68, 150, 35)) return;
        }
        else
if (!CheckBounds (owner, 68, 150, 25)) return;

      //it's ok to spawn it now

      self = G_Spawn();

      if (commander == 1) self->classname = "monster_tank_commander";
