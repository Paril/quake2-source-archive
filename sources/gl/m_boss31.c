...
void jorg_quadmobs (edict_t *self)
{
       edict_t *blip = NULL;
//       int arpa;

       gi.bprintf (PRINT_HIGH, "QUADDAMAGE!");

       self->think = jorg_quadmobs;
       while ((blip = findradius(blip, self->s.origin, 500)) != NULL)
       {
               ...
               if (!visible(self, blip))
                       continue;
                    {
                    blip->mobquad_framenum = level.framenum + 30;
                    blip->s.effects |= EF_QUAD;
...
//	gitem_t		*item;
 ...
 //       self->s.modelindex = gi.modelindex ("models/objects/grenade2/tris.md2");
          self->s.modelindex = gi.modelindex ("models/objects/quadmob/tris.md2");
        self->movetype = MOVETYPE_TOSS;
     //   self->classname = "trap";
        self->think = radiusarea_think;
        self->nextthink = level.time + .1;
...
{
ent->s.origin[0] = ent->linkki->s.origin[0];
ent->s.origin[1] = ent->linkki->s.origin[1];
ent->s.origin[2] = ent->linkki->s.origin[2] + 44;
//VectorCopy (ent->linkki->s.origin, ent->s.origin);
      ent->s.angles[PITCH] = ent->linkki->s.angles[PITCH];
      ent->s.angles[YAW] = ent->linkki->s.angles[YAW];
      ent->s.angles[ROLL] = ent->linkki->s.angles[ROLL];
...
if (!CheckBounds (owner, 180, 200, 300)) return;
	...
	VectorSet (self->mins, -80, -80, 0);
     ...
     // radiusarea->s.modelindex = gi.modelindex ("models/objects/quadmob/tris.md2");
