#include "g_local.h"
//RaVen  needed some declares !!!(linux is tough on this stuff!!)
#include "q_devels.h"
...
void Touch_Plat_Center (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (ent->moveinfo.state == STATE_BOTTOM)
		plat_go_up (ent);
        else if (ent->moveinfo.state == STATE_TOP && (!other->svflags & SVF_MONSTER))
...
void button_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->health <= 0)
        ...
        if (self->endlevel) {
                        ...
                        centerprint_all ("Demon has been Defeated!");
                        if (other->client) other->client->resp.score+= self->endlevel;
 			EndDMLevel ();
			return;
                      }
...
void button_killed (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	self->takedamage = DAMAGE_NO;
        ...
        if (self->endlevel) {
                        ...
                        centerprint_all ("Demon has been Defeated!");
                        if (attacker->client) attacker->client->resp.score+= self->endlevel;
 			EndDMLevel ();
			return;
                      }
