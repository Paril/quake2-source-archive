...
//======================================================================
void Checktospawn(edict_t *ent, gitem_t *item)
{
if (ent->gameleader) {
...
if (item->tag == 524) {
        ...
        {
            gi.cprintf (ent, PRINT_HIGH, "You are not Gameleader, so you cannot create these\n");
...
qboolean Add_Ammo (edict_t *ent, gitem_t *item, int count)
{
	...
	if (ent->client->pers.inventory[index] == max)
        ...
        if (item->tag == AMMO_CELLS) if (ent->gameleader) return false;
...
void Touch_Item (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (!ent->item->pickup)
        ...
        if (other->gameleader) return; //gameleader is not allowed to pick weapons
...
/*QUAKED item_monsu (.3 .3 1) (-16 -16 -16) (16 16 16)
*/
	{
                "Light Soldier", 
                NULL,
