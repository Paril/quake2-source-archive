...
// CYBERSLASH: If it's the BFG, give em the PIG
...
// CYBERSLASH: this is defined later
...
void Weapon_Generic (edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, int FRAME_IDLE_LAST, int FRAME_DEACTIVATE_LAST, int *pause_frames, int *fire_frames, void (*fire)(edict_t *ent))
{
	...
	if (ent->client->weaponstate == WEAPON_READY)
	{
		if ( ((ent->client->latched_buttons|ent->client->buttons) & BUTTON_ATTACK) )
		{
			...
			{
// CYBERSLASH: If the PIG2K, check for a target first
...
qboolean pig_gettarg (edict_t *ent)
{
        ...
        if (!tr.ent || (!(tr.ent->svflags & SVF_MONSTER) && !tr.ent->client))
        {
                ...
                {
                        gi.cprintf(ent, PRINT_HIGH, "No target\n");
