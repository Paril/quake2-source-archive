...
// CYBERSLASH: give everyone the pig and one free shot
        if (deathmatch->value && ((int)dmflags->value & DF_FREE_PIG))
        {
                item = FindItem("PIG2K");
...
// CYBERSLASH: just cos they're not in the player model it doesn't mean they're gibbed. Now.
                else if ((ent->s.modelindex != 255) && !client->pigged)
			client->ps.pmove.pm_type = PM_GIB;
                else if (ent->deadflag || (ent->gravity < 0))
...
// CYBERSLASH: roll if inverted grav
                else if (ent->gravity < 0)
                {
                        if (client->v_angle[ROLL] < 180)
