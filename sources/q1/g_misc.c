...
void ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	ClipGibVelocity (gib);
	gib->avelocity[0] = random()*800;
	gib->avelocity[1] = random()*800;
	gib->avelocity[2] = random()*800;
...
{
	self->takedamage = DAMAGE_NO;
	self->nextthink = level.time + 0 * FRAMETIME;
...
void SP_misc_explobox (edict_t *self)
{
	...
	self->model = "models/objects/big_explo/tris.md2";
...
void SP_misc_smexplobox (edict_t *self)
{
	if (deathmatch->value)
	{	// auto-remove for deathmatch
		G_FreeEdict (self);
	...
	self->model = "models/objects/sma_explo/tris.md2";
	self->s.modelindex = gi.modelindex (self->model);
	VectorSet (self->mins, -16, -16, 0);
