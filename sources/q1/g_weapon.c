...
void fire_nail (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed)
{
	...
	VectorClear (bolt->mins);
	VectorClear (bolt->maxs);
	bolt->s.modelindex = gi.modelindex ("models/weapons/s_spike/tris.md2");
	bolt->owner = self;
...
=================
*/
void gibby_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	int		mod;
	...
	if (other->takedamage)
	{
		if (self->spawnflags & 1)
			...
			mod = MOD_BLASTER;
		T_Damage (other, self, self->owner, self->velocity, self->s.origin, plane->normal, self->dmg, 1, DAMAGE_ENERGY, mod);
		gi.sound(self, CHAN_VOICE, gi.soundindex("zombie_q1/z_hit.wav"), 1, ATTN_NORM, 0);
	}
	else
	{
		gi.sound(self, CHAN_VOICE, gi.soundindex("zombie_q1/z_miss.wav"), 1, ATTN_NORM, 0);
	}

	G_FreeEdict (self);
}

void fire_zomgib (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	edict_t	*bolt;
	trace_t	tr;
	...
	VectorClear (bolt->mins);
	VectorClear (bolt->maxs);
	bolt->s.modelindex = gi.modelindex ("models/monsters/zombiegib/tris.md2");
	bolt->s.sound = 0;
	bolt->owner = self;
	bolt->touch = gibby_touch;
	bolt->nextthink = level.time + 2;
	bolt->think = G_FreeEdict;
	bolt->dmg = damage;
	bolt->classname = "bolt";
...
Fires a Hknight spike bolt.
=================
*/
void hspike_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	int		mod;
	...
	gi.sound (self, ATTN_NORM, gi.soundindex ("hknight_q1/hit.wav"), 1, ATTN_NORM, 0);

	G_FreeEdict (self);
}

void fire_hspike (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	edict_t	*bolt;
	trace_t	tr;
	...
	VectorClear (bolt->mins);
	VectorClear (bolt->maxs);
	bolt->s.modelindex = gi.modelindex ("models/monsters/kspike/tris.md2");
	bolt->owner = self;
	bolt->touch = hspike_touch;
	bolt->nextthink = level.time + 7;
	bolt->think = G_FreeEdict;
	bolt->dmg = damage;
	bolt->classname = "bolt";
	if (hyper)
