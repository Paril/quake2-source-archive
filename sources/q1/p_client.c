...
int i;
  ...
  for (i=1; i<game.num_items; i++) 
  {
	...
	if (quantity>0)
      pack->packitems[i]+=quantity; 
  } // stuff the pack with items
...
//	self->solid = SOLID_NOT;
	...
	{	// normal death
		if (!self->deadflag)
		{
			...
			{
				if (Q_stricmp(self->client->pers.weapon->pickup_name, "Axe") == 0)
				{
					self->s.frame = 41;
			...
			gi.sound (self, CHAN_VOICE, gi.soundindex(va("*death%i.wav", (rand()%5)+1)), 1, ATTN_NORM, 0);
		}
	}
...
void Invis_Skin_MakeSure_Forced(edict_t *ent, char *s)
{
	...
	switch (ent->client->pers.invis) {
	case 0:
		gi.configstring (CS_PLAYERSKINS+playernum, va("%s\\%s%s", 
			ent->client->pers.netname, t, "player") );
		break;
	case 1:
		gi.configstring (CS_PLAYERSKINS+playernum,
			va("%s\\%s%s", ent->client->pers.netname, "eyes/", "skin") );
		break;
	default:
		gi.configstring (CS_PLAYERSKINS+playernum, 
	...
	//{
	//	gi.configstring (CS_PLAYERSKINS+playernum, va("%s\\%s%s", ent->client->pers.netname, "eyes/", "skin") );
//	}
//	else
//	{
	//	gi.configstring (CS_PLAYERSKINS+playernum, va("%s\\%s%s", ent->client->pers.netname, "ranger/", "player") );
//	}
}
...
void PutClientInServer (edict_t *ent)
{
	...
	Invis_Skin_MakeSure_Forced(ent, s);
...
//		}
	...
	ent->model = "players/ranger/tris.md2";
