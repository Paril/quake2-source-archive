...
{
	gi.sound (self, CHAN_VOICE, gi.soundindex ("fiend_q1/sight2.wav"), 1, ATTN_NORM, 0);
}

void fiend_search (edict_t *self)
{
	gi.sound (self, CHAN_VOICE, gi.soundindex ("fiend_q1/idle1.wav"), 1, ATTN_NORM, 0);
}
...
void fiend_hit_left (edict_t *self)
{
	...
	VectorSet (aim, MELEE_DISTANCE, self->mins[0], 8);
	if (fire_hit (self, aim, (10 + (rand() %5)), 100))
		gi.sound (self, CHAN_WEAPON, gi.soundindex ("fiend_q1/dhit2.wav"), 1, ATTN_NORM, 0);
}
...
mmove_t fiend_move_attack = {54, 68, fiend_frames_attack, fiend_run};
...
void fiend_jump_takeoff (edict_t *self)
{
	...
	gi.sound (self, CHAN_VOICE, gi.soundindex ("fiend_q1/djump.wav"), 1, ATTN_NORM, 0);
	AngleVectors (self->s.angles, forward, NULL, NULL);
	self->s.origin[2] += 1;
	VectorScale (forward, 600, self->velocity);
	self->velocity[2] = 250;
...
mframe_t fiend_frames_jump [] =
{
	ai_charge,	 0,	NULL,
	ai_charge,	17,	NULL,
	ai_charge,	15,	fiend_jump_takeoff,
	ai_charge,	15,	NULL,
	ai_charge,	15,	NULL,
	ai_charge,	 0,	fiend_check_landing,
	ai_charge,	 3,	NULL,
	ai_charge,	 0,	NULL,
	ai_charge,	 3,	NULL,
	ai_charge,	 0,	NULL,
...
qboolean fiend_check_melee (edict_t *self)
{
	if (range (self, self->enemy) == RANGE_MELEE)
		return true;
	return false;
...
qboolean fiend_check_jump (edict_t *self)
{
	vec3_t	v;
	float	distance;
...
mmove_t fiend_move_pain1 = {39, 44, fiend_frames_pain1, fiend_run};
...
void fiend_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	r = random();
	gi.sound (self, CHAN_VOICE, gi.soundindex ("fiend_q1/dpain1.wav"), 1, ATTN_NORM, 0);
	self->monsterinfo.currentmove = &fiend_move_pain1;
}
...
mmove_t fiend_move_death1 = {45, 53, fiend_frames_death1, fiend_dead};
...
void fiend_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	gi.sound (self, CHAN_VOICE, gi.soundindex ("fiend_q1/ddeath"), 1, ATTN_NORM, 0);
	self->deadflag = DEAD_DEAD;
	self->takedamage = DAMAGE_YES;
	self->s.skinnum = 1;

	self->monsterinfo.currentmove = &fiend_move_death1;
}
...
/*QUAKED monster_fiend (1 .5 0) (-32 -32 -24) (32 32 32) Ambush Trigger_Spawn Sight
*/
void SP_monster_fiend (edict_t *self)
{
	if (deathmatch->value)
	...
	self->s.modelindex = gi.modelindex ("models/monsters/fiend_q1/tris.md2");
