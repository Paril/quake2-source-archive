...
//
//	RipVTide 12-Aug-2000
//	Credit where credit is due. Found this on a Q2 dll board
//	And couldn't help but implement it. Darned helpfull little
//	bit of code this...
//
...
_stuffcmd 
===================
by manuel 'spanishfly' gonzalez

: USAGE:
: it is a mix between stuffcmd and printf-like functions
: so the following are all valid uses of my _stuffcmd:

: REGULAR _stuffcmd (ent, "abcdefg\n");
: STRINGS _stuffcmd (ent, "01%s23%s\n", str, str);
: INTS _stuffcmd (ent, "012%d 012%d\n", num1, num1);
: FLOATS/DOUBLES _stuffcmd (ent, "%f\n", num2);

: NOTE any of the above may be combined and there
: is no limit on the number of variables used.
...
void _stuffcmd(edict_t* ent, char *s, ...)
{
	...
	for (c=0, j=0; s[j] != '\0'; j++)
	{
		if (s[j] == '%') 
		{
			if (s[j+1] == 's')
			{ 
				c+=sprintf (&buff[c], "%s", va_arg (vl, char*));
			}
			else if (s[j+1] == 'd')
			{
				c+=sprintf (&buff[c], "%d", va_arg (vl, int));
			}
			else if (s[j+1] == 'f')
			{
				int decimal; 
...
#ifndef AMIGA
				fcvt (va_arg (vl, double), 5, &decimal, &sign);
				vl -= sizeof(double);
#else
				c+=sprintf(&buff[c], "%f", va_arg(vl, double));
#endif
	...
	if (ctf_team == other->client->resp.ctf_team) {
		...
		if (!(ent->spawnflags & DROPPED_ITEM)) {
			...
			if (other->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) {
				...
				if (ctf_team == CTF_TEAM1)
					gi.sound (ent, CHAN_RELIABLE+CHAN_NO_PHS_ADD+CHAN_VOICE, gi.soundindex("ctf/jawacapt.wav"), 1, ATTN_NONE, 0);
				else
					gi.sound (ent, CHAN_RELIABLE+CHAN_NO_PHS_ADD+CHAN_VOICE, gi.soundindex("ctf/ewokcapt.wav"), 1, ATTN_NONE, 0);
		...
		if (ctf_team == CTF_TEAM1)
			gi.sound (ent, CHAN_RELIABLE+CHAN_NO_PHS_ADD+CHAN_VOICE, gi.soundindex("ctf/jawartrn.wav"), 1, ATTN_NONE, 0);
		else
			gi.sound (ent, CHAN_RELIABLE+CHAN_NO_PHS_ADD+CHAN_VOICE, gi.soundindex("ctf/ewokrtrn.wav"), 1, ATTN_NONE, 0);
...
//		dropped->nextthink = level.time + CTF_AUTO_FLAG_RETURN_TIMEOUT;
		VectorSet (dropped->mins, -16, -16, -24);
		VectorSet (dropped->maxs, 16, 16, 32);
		dropped->movetype = MOVETYPE_STEP;
		dropped->solid = SOLID_BBOX;
		dropped->yaw_speed = 45;

		dropped->mass = 50;

		dropped->pain = flagsnap;
		//dropped->die = CTFDropFlagThink;
		dropped->touch = CTFDropFlagTouch;
		dropped->think = Critter_Panic;
		dropped->nextthink = level.time + 0.01;
	}
}
...
//	}
	...
	if (player->client->pers.inventory[ITEM_INDEX(flag1_item)])
		player->s.modelindex3 = gi.modelindex("players/male/w_jawa.md2");
	else if (player->client->pers.inventory[ITEM_INDEX(flag2_item)])
		player->s.modelindex3 = gi.modelindex("players/male/w_ewok.md2");
	else
...
void SetCTFStats(edict_t *ent)
{
	...
	ent->client->ps.stats[STAT_CTF_TEAM1_HEADER] = gi.imageindex ("ctf/teamlight");
	ent->client->ps.stats[STAT_CTF_TEAM2_HEADER] = gi.imageindex ("ctf/teamdark");
	...
	if (e != NULL)
	{
		if (e->solid == SOLID_NOT)
		{
			...
			// not at base
			// check if on player
			p1 = gi.imageindex ("ctf/i_ctf1d"); // default to dropped
			for (i = 1; i <= maxclients->value; i++)
				if (g_edicts[i].inuse &&
					g_edicts[i].client->pers.inventory[ITEM_INDEX(flag1_item)])
				{
					// enemy has it
					p1 = gi.imageindex ("ctf/i_ctf1t");
					break;
				}
		}
		else if (e->spawnflags & DROPPED_ITEM)
			p1 = gi.imageindex ("ctf/i_ctf1d"); // must be dropped
	}
	p2 = gi.imageindex ("ctf/i_ctf2");
	...
	if (e != NULL)
	{
		if (e->solid == SOLID_NOT)
		{
			...
			// not at base
			// check if on player
			p2 = gi.imageindex ("ctf/i_ctf2d"); // default to dropped
			for (i = 1; i <= maxclients->value; i++)
				if (g_edicts[i].inuse &&
					g_edicts[i].client->pers.inventory[ITEM_INDEX(flag2_item)])
				{
					// enemy has it
					p2 = gi.imageindex ("ctf/i_ctf2t");
					break;
				}
		}
		else if (e->spawnflags & DROPPED_ITEM)
			p2 = gi.imageindex ("ctf/i_ctf2d"); // must be dropped
	}
	...
	if (ent->client->resp.ctf_team == CTF_TEAM1 &&
		...
		(level.framenum & 8))
		ent->client->ps.stats[STAT_CTF_FLAG_PIC] = gi.imageindex ("ctf/i_ctf2");

	else if (ent->client->resp.ctf_team == CTF_TEAM2 &&
		ent->client->pers.inventory[ITEM_INDEX(flag1_item)] &&
		(level.framenum & 8))
		ent->client->ps.stats[STAT_CTF_FLAG_PIC] = gi.imageindex ("ctf/i_ctf1");

//	ent->client->ps.stats[STAT_CTF_JOINED_TEAM1_PIC] = 0;
//	ent->client->ps.stats[STAT_CTF_JOINED_TEAM2_PIC] = 0;
//	if (ent->client->resp.ctf_team == CTF_TEAM1)
//		ent->client->ps.stats[STAT_CTF_JOINED_TEAM1_PIC] = gi.imageindex ("ctf/i_ctf1j");
//	else if (ent->client->resp.ctf_team == CTF_TEAM2)
//		ent->client->ps.stats[STAT_CTF_JOINED_TEAM2_PIC] = gi.imageindex ("ctf/i_ctf2j");
...
void CTFWeapon_Grapple (edict_t *ent)
{
	...
	Weapon_Generic (ent, 5, 9, 31, 36, 36, pause_frames, fire_frames, 
		CTFWeapon_Grapple_Fire, Reload_NULL);
...
pmenu_t joinmenu[] = {
	{ "*Star Wars Quake2",			PMENU_ALIGN_CENTER, NULL, NULL },
	{ "*Capture The Office Chair",	PMENU_ALIGN_CENTER, NULL, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL, NULL },
	{ "Join Light Side",		PMENU_ALIGN_LEFT, NULL, CTFJoinTeam1 },
	{ NULL,					PMENU_ALIGN_LEFT, NULL, NULL },
	{ "Join Dark Side",		PMENU_ALIGN_LEFT, NULL, CTFJoinTeam2 },
	{ NULL,					PMENU_ALIGN_LEFT, NULL, NULL },
...
void SP_trigger_teleport (edict_t *ent)
{
	...
	s->s.sound = gi.soundindex ("world/amb_ionus_light_hum2_loop.wav");
	gi.linkentity(s);
...
//
//	RipVTide 29-Dec-2000 When in panic or in doubt, run in circles scream
//	and shout!
//
//	Could of course have done this with a switch/case statement but I'm an
//	if/else person at heart =)
//
...
//	Walk
//
	else if(ent->s.frame == 53)
	{
		Set_Panic(ent);
	}

	else if((ent->s.frame < 53) && (ent->s.frame > 41))
	{
		ent->s.angles[1] = ((random() - 0.5));
...
//	Run
//
	else if(ent->s.frame == 59)
	{
		Set_Panic(ent);
	}

	else if((ent->s.frame < 59) && (ent->s.frame > 53))
	{
		ent->s.angles[1] = ((random() - 0.5));
...
//
	...
	else if((ent->s.frame < 76) && (ent->s.frame > 69))
	{
		if(!ent->groundentity)
		{
		}
...
void flagsnap (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if(random() >= 0.5)
		gi.sound (self, CHAN_VOICE, gi.soundindex ("jawa/j_cry1.wav"), 1, ATTN_NORM, 0);
	else
		gi.sound (self, CHAN_VOICE, gi.soundindex ("jawa/j_cry2.wav"), 1, ATTN_NORM, 0);
	...
	if(damage > 20)
	{
		...
		VectorAdd(dir, self->velocity, self->velocity);
		self->s.frame = 70;
	}
	else
		self->s.frame = 65;
