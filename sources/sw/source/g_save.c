...
field_t fields[] = {
	...
	{"fog_fog", FOFS(fog_fog), F_VECTOR},
	{"solid", FOFS(solid_sec), F_INT},
...
void InitGame (void)
{
	...
	zoomall = gi.cvar ("zoomall", "0", 0);
	fog = gi.cvar ("fog", "1", 0);
	trailtime = gi.cvar ("trailtime", "0.3", 0);
	capturelimit = gi.cvar ("capturelimit", "0", CVAR_SERVERINFO);
	ctf = gi.cvar("ctf", "0", CVAR_SERVERINFO);
	ctf_forcejoin = gi.cvar("ctf_forcejoin", "0", CVAR_SERVERINFO);
	sv_botskill = gi.cvar ("botskill", "1", 0);
	// dm map list
	sv_maplist = gi.cvar ("sv_maplist", "", 0);

	saberonly = gi.cvar ("saberonly", "", CVAR_SERVERINFO);
	theforce = gi.cvar ("theforce", "", CVAR_SERVERINFO);
	coolgrav = gi.cvar ("coolgrav", "", CVAR_SERVERINFO);
	p_weap = gi.cvar ("p_weap", "", CVAR_SERVERINFO);
	mouse_s = gi.cvar ("mouse_s", "0", CVAR_USERINFO);
	advanced = gi.cvar ("advanced", "", CVAR_SERVERINFO);

	opengl = gi.cvar ("vid_ref", "", 0);

	if(!strcmp(opengl->string, "gl"))
	{
		FullGL = 1;
	}
	else
