...
void SV_CalcBlend (edict_t *ent)
{
	...
	/* *** NUKES *** */
	if (ent->client->nukeblind)
	{
		SV_AddBlend(1,1,1,0.9,ent->client->ps.blend);
		ent->client->nukeblind--;
	}
	/* *** */

	/* *** IR GOGGLES *** */
	if (ent->client->goggles)
		SV_AddBlend(-0.5,-1,-1,0.5,ent->client->ps.blend);
	/* *** */

	/* *** CLOAKING *** */
	if (ent->client->cloakable==1 && (ent->svflags & SVF_NOCLIENT))
		SV_AddBlend (-1, -1, -1, 0.3, ent->client->ps.blend);
	/* *** */

	/* *** CLOAKING / CLOAKING JAMMER *** */
	if (ent->client->cloaking==2)
		SV_AddBlend (1, 0, 1, 0.3, ent->client->ps.blend);
	/* *** */
	...
	/* *** KAMIKAZE MODE *** */
		...
		if (remaining > 30 || (remaining & 4))
			SV_AddBlend (1, 0, 0, 0.08, ent->client->ps.blend);
	}
	/* *** */
	/* *** FREEZER *** */
	else if (ent->client->frozen)
	{
		SV_AddBlend (0, 0, 1, 0.20, ent->client->ps.blend);
	}
	/* *** */
...
void P_WorldEffects (void)
{
	...
	{
		/* *** JETPACK *** */
		if ( Jet_Active(current_player) ) /*don't jet and dive and stay alive*/
			T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, current_player->health+1, 0, DAMAGE_NO_ARMOR,MOD_JETPACK_EXPLODE);
		/* *** */
...
//ZOID
    ...
    /* *** KAMIKAZE MODE *** */
	if (ent->client->kamikaze_mode & 1)
	{
		ent->s.effects |= EF_COLOR_SHELL;
		ent->s.renderfx |= (RF_SHELL_RED|RF_FULLBRIGHT);
	}
	/* *** */
	...
	{
		ent->s.effects |= EF_COLOR_SHELL;
		ent->s.renderfx |= (RF_SHELL_GREEN|RF_SHELL_BLUE|RF_FULLBRIGHT);
	}
	/* *** */
	...
	{
		ent->s.effects |= EF_COLOR_SHELL;
		ent->s.renderfx |= (RF_SHELL_BLUE|RF_FULLBRIGHT);
	}
	/* *** */
}
...
void G_SetClientSound (edict_t *ent)
{
	...
	/* *** SNIPER RIFLE *** */
	&& !ent->client->resp.sniper
	/* *** */)
		ent->s.sound = gi.soundindex("weapons/rg_hum.wav");
	/* *** LIGTH SABER *** */
//	else if (strcmp(weap, "weapon_saber") == 0)
//		ent->s.sound = gi.soundindex("weapons/lightsaber/saber_hum.wav");
	/* *** */
...
//ZOID
	...
	/* *** RADAR *** */
	if (ent->client->showradar && !(level.framenum & 2))
	{
		radar_update(ent);
