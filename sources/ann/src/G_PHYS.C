...
// add gravity
	...
	&& ent->movetype != MOVETYPE_FLYRICOCHET)	/* *** RICOCHET *** */
		SV_AddGravity (ent);
...
// move origin
	...
	if (trace.fraction < 1)
	{
		...
		/* *** RICOCHET + RICOCHET SOUND *** */
		if (ent->movetype == MOVETYPE_FLYRICOCHET)
		{
			vectoangles (ent->velocity, ent->s.angles);
			rnum = rand();
			if (rnum < (RAND_MAX/2))	/* Will make a sound about 1/2 the time */
			{
...
// move teamslaves
	...
	if (ent->mynoise2!=NULL /* *** TOM *** */ && stricmp(ent->classname,"chamber")==0 /* *** */)
...
void G_RunEntity (edict_t *ent)
{
	...
	case MOVETYPE_FLYRICOCHET:	/* *** RICOCHET *** */
		SV_Physics_Toss (ent);
		break;
	case MOVETYPE_BOUNCE:		/* *** IMPROVED BOUNCE PHYSICS *** */
