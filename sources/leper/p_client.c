	...
	if (deathmatch->value || coop->value)
	{
		...
		if (attacker && attacker->client)
		{
			...
			case MOD_MACHINEGUN+LOC_LEFTARM:
				message = " was machinegunned in the arm by";
				break;
			case MOD_MACHINEGUN+LOC_RIGHTARM:
				message = " was machinegunned in the arm by";
				break;
			case MOD_MACHINEGUN+LOC_LEFTLEG:
				message = " was machinegunned in leg the by";
				break;
		...
		////////////////////
			...
			case MOD_RAILGUN+LOC_FACE:
				message = " was railed through the head by";
				message2 = "";
				break;
			case MOD_RAILGUN+LOC_HEAD:
				message = " took a slug to the brain";
				break;
			case MOD_RAILGUN:
				message = " was railed by";
				break;
			...
			case MOD_TEETH:
				...
				message2 = " because of starvation";
				break;
			case MOD_TEETH+LOC_LEFTARM:
				message = " lost an arm to";
				message2 = "'s teeth";
				break;
			case MOD_TEETH+LOC_RIGHTARM:
				message = " lost an arm to";
				message2 = "'s teeth";
				break;
			case MOD_TEETH+LOC_LEFTLEG:
				message = " was bitten in the leg by";
				message2 = "'s teeth";
				break;
			case MOD_TEETH+LOC_RIGHTLEG:
				message = " was eaten by";
				message2 = " in the leg!";
				break;
			case MOD_TEETH+LOC_BACK:
				message = " was eaten by";
				message2 = " in the back!";
				break;
			case MOD_TEETH+LOC_CHEST:
				message = " was bitten by";
				message2 = " in the chest!";
				break;
			case MOD_TEETH+LOC_FACE:
				message = " kissed";
				message2 = " and died";
				break;
			case MOD_TEETH+LOC_HEAD:
				message = " got hickies from";
				message2 = "'s teeth";
				break;
				//////////////////
			case MOD_HG_SPLASH:
			...
			if (message)
			{
				...
				if (deathmatch->value)
				{
					...
					{   
						oldscore = attacker->client->resp.score;
						attacker->client->resp.score++;
						if(!self->leftarm)
							attacker->client->resp.score++;
						if(!self->leftleg)
						...
						if(!self->rightleg)
							attacker->client->resp.score++;
						if(!self->leftarm && !self->leftleg && !self->rightarm &&!self->rightleg)
							{
							attacker->client->resp.score++;
							gi.bprintf (PRINT_MEDIUM,"%s gets a quadraplegic bonus off %s\n",attacker->client->pers.netname, self->client->pers.netname);
							}
						
						gi.bprintf (PRINT_MEDIUM,"%s wins %i points from %s\n",attacker->client->pers.netname, attacker->client->resp.score - oldscore, self->client->pers.netname);
					}
				}
				return;
			}
...
void TossClientWeapon (edict_t *self)
{
	...
	if (item && (strcmp (item->pickup_name, "Blaster") == 0))
		item = NULL;
	if (item && (strcmp (item->pickup_name, "Teeth") == 0))
		item = NULL;
...
void player_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if(!firstkill && goldenlimb->value)
	{
		firstkill = true;
		drop = Drop_Item (self, FindItemByClassname ("item_goldenleg"));
	}
...
//	self->solid = SOLID_NOT;
	...
	{	// gib
		...
		{
		self->rightarm= 0;
		drop = Drop_Item (self, FindItemByClassname ("item_arm"));
		}
		if(self->rightleg)
		{
		self->rightleg=0;
		drop = Drop_Item (self, FindItemByClassname ("item_leg"));
		}
		if(self->leftarm)
		{
		self->leftarm = 0;
...
//		}
	...
	ent->model = "players/male/torso.md2";//leper this was tris.md2
...
Leper_Think(ent);
//leper
if(!ent->leftleg && !ent->rightleg && ucmd->upmove > 0)
{
	hook_fire (ent);
	ucmd->upmove = -20;
}
