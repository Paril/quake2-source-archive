...
{
	if (!(other->svflags & SVF_MONSTER) && (!G_ClientExists(other)))
	{
...
{ 
	...
	{
		if (ent->ripstate != other->client->resp.s_team)
			return;
	}
	else if (ent->playerclasss)
	{
		if (other->client)
		{
			if (other->client->pers.inventory[ITEM_INDEX(FindItem(ent->playerclasss))] == 0)
				return;
		}
	}
...
void button_fire (edict_t *self)
{
	...
	if (self->playerclasss)
	{
		if (self->activator->client)
		{
			if (self->activator->client->pers.inventory[ITEM_INDEX(FindItem(self->playerclasss))] == 0)
				return;
		}
	}
...
void Touch_DoorTrigger (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
    ...
    if (self->ripstate && other->client)
	{
		if (other->client->resp.s_team != self->ripstate)
			return;
	}
	else if (self->playerclasss)
	{
		if (other->client)
		{
			if (other->client->pers.inventory[ITEM_INDEX(FindItem(self->playerclasss))] == 0)
				return;
		}
	}
