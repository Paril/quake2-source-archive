...
void CheckChasecam_Viewent (entity *ent);
...
//	gi.cprintf (player, PRINT_HIGH, "%i %i\n", player->health, player->client->pers.inventory[ArmorIndex (player)]);
...
void SV_CalcViewOffset (edict_t *ent)
{
	...
	bubble = YDirection = PDirection = RDirection = DizzyYaw = DizzyPitch = DizzyRoll = 0;
//===================================
	...
	else
	{
		...
		if (ent->DrunkTime > level.time)
		{
			...
			if (DizzyRoll > 25)
				RDirection = 0;
			angles[YAW] += DizzyYaw;
			angles[PITCH] += DizzyPitch;
			angles[ROLL] += DizzyRoll;
		}
...
void SV_CalcBlend (edict_t *ent)
{
	...
	else if (ent->ripstate & STATE_CLOAKING)
	...
	if (ent->client->bonus_alpha > 0)
        ...
        if (ent->client->smoke > 0)  // add the smoke effect?
        {
                if (ent->client->smoke > .99) // don't make them completly blinded, you can lower this value a bit if you 
                                                          // want them to be able to see more
                        ent->client->smoke = .99;
                smoke = random()*.2 - .1;  // a random offset to the color so things don't get boring, you can tweak
                                                       // this to your liking as well
                SV_AddBlend (0.6+smoke, 0.6+smoke, 0.6+smoke, ent->client->smoke, ent->client->ps.blend);
                ent->client->smoke = 0;
        }
...
void P_FallingDamage (edict_t *ent)
{
	...
	if (delta > 30)
	{
	    ...
	    i = (delta + 300) / 11;
//		gi.cprintf(ent, PRINT_HIGH, "You fell %i FT\n", i);
	}
	else
	{
...
void Cloaking (edict_t *ent)
{
	...
	if (ent->ripstate & STATE_CLOAKING)
	{
		if (ent->client->pers.inventory[ITEM_INDEX(FindItem("cells"))] == 1)
		{
			...
			gi.cprintf (ent, PRINT_HIGH, "Cloacking field deactivated: not enough energy.\n");
		}
	}
}
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	if (ent->client->showscores && deathmatch->value && !(level.framenum & 31)  && !(ent->client->showmenu) && !(ent->client->showmsg))
	{
		DeathmatchScoreboardMessage (ent, ent->enemy);
		gi.unicast (ent, false);
	}
   	else if (!ent->client->showscores && (ent->client->showmenu || ent->client->showmsg))
        Menu_Close(ent);
