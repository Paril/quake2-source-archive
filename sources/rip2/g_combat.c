...
void SpawnDamage (int type, vec3_t origin, vec3_t normal, int damage)
{
    ...
    G_ImpactEntity (type, origin, normal, MULTICAST_PVS);
}
...
static qboolean PlayingSameTeam (edict_t *targ, edict_t *attacker)
{
	if (!teamplay->value)
		return false;

	if (!G_ClientExists(targ) || !G_ClientExists(attacker))
		return false;

	if ((targ->client->resp.s_team == attacker->client->resp.s_team) && (targ == attacker))
		return true;
