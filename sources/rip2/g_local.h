...
#define	GAMEVERSION	"Rip II v " RIP_VER
...
#define WEAP_FL                 12
#define WEAP_PL                 13
#define WEAP_GORELKA            14
#define WEAP_MAGIC              15
...
typedef struct
{
    ...
    char        capture_message[MAX_QPATH];
    char        capturer_message[MAX_QPATH];
    char        take_message[MAX_QPATH];
    char        taker_message[MAX_QPATH];
    char        return_message[MAX_QPATH];
    char        flag_at_base[MAX_QPATH];
    char        flag_own[MAX_QPATH];
    char        flag_lying[MAX_QPATH]; 
    char        flag_corrupt[MAX_QPATH];
	edict_t     *last_captured;
	char        goal[MAX_QPATH];
	int         FLAG_RECOVERY_BONUS;
	int         FLAG_TAKE_BONUS;
	int         FLAG_CAPTURE_BONUS;
	int         FLAG_TEAM_BONUS;
} level_locals_t;
...
#define MOD_RAILBOMB        34
#define MOD_PLASMA          35
#define MOD_MEDIC_CABLE     36
#define MOD_ZYLON_GAS       37 // NEW FLAG HERE 
#define MOD_BOLT            38
#define MOD_ELSE            39
#define MOD_ELECTRIC        40
#define MOD_PIPEBOMB        41
#define MOD_SENSOR_GRENADES 42
...
#define ETOFS(x) (int)&(((entity_state_t *)0)->x)
...
extern	cvar_t	*ripflags;
extern  cvar_t  *capturelimit;
extern  cvar_t  *bhole_life;
extern  cvar_t  *eject;
...
struct gclient_s
{
   ...
   // ++QMENU++ Handles the menu
	...
	float       smoke; // percentage of the player's view obscured by smoke
...
void G_PointEntity (int type, vec3_t origin, int multicast);
void G_ImpactEntity (int te_impact, vec3_t origin, vec3_t dir, int multicast);
void G_LineEntity (int line, vec3_t start, vec3_t end, vec3_t origin, int multicast);
void G_SplashEntity (int splash, int count, vec3_t origin, vec3_t dir, int style, int multicast);
void G_TraceEntity (int line, edict_t *self, vec3_t start, vec3_t end, vec3_t origin, int multicast);
