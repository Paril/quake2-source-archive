...
void CheckDMRules (void)
{
		...
		if (level.time >= timelimit->value*60)
		{
			if (extratime->value)
			{
				if (hit == 0)
				{
					...
					{
						gi.bprintf (PRINT_HIGH, "Tie. Starting extra time...\n");
						hit = 1;
						return;
					}
				}
	...
	if (extratime->value && hit)
	{
		if (level.time >= ((extratime->value*60)+(timelimit->value*60)))
		{
			gi.bprintf (PRINT_HIGH, "Extra time hit.\n");
			EndDMLevel ();
			return;
		}
	}
...
/*-----/ NEW /-----/ Remove all flames before leaving level. /-----*/
    PBM_KillAllFires ();
/*-----------------------------------------------------------------*/
