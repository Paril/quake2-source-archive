...
void turret_blocked(edict_t *self, edict_t *other)
{
	...
	if (other->takedamage)
	{
		...
		T_Damage (other, self, attacker, vec3_origin, other->s.origin, vec3_origin, self->teammaster->dmg, 10, 0, MOD_ROCKET);
	}
}
...
void turret_breach_think (edict_t *self)
{
	...
	if (self->owner)
	{
		...
		{		// a player is controlling the turret, move towards view angles
			...
			if ((self->owner->client->buttons & BUTTON_MOVEUP) && (self->delay < level.time))	// 3 second break between shots
			{
				turret_breach_fire (self);
			...
			if (self->owner->client->ps.pmove.velocity[2] > 30) {
	...
	{	// check if a player has mounted the turret
		...
		for (i=0 ; i<maxclients->value ; i++, ent++)
		{
			...
			if (VectorLength(dir) < 16)
			{
				...
				{
					gi.cprintf(ent, PRINT_HIGH, "Only scientist can drive turrets\n");
					return;
				}
...
//	self->model = "models/weapons/turret/gun/tris.md2";
...
/*
void turret_base_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	gi.bprintf (PRINT_HIGH, "Turrent base touched\n");
}
*/
...
void SP_turret_base (edict_t *self)
{
	self->solid = SOLID_BSP;
	self->movetype = MOVETYPE_PUSH;
//	self->model = "models/weapons/turret/base/tris.md2";
	gi.setmodel (self, self->model);
	self->blocked = turret_blocked;

//	self->touch = turret_base_touch;
