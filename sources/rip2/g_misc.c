...
{
    G_PointEntity(TE_EXPLOSION2, self->s.origin, MULTICAST_PVS);
...
void SP_misc_explobox (edict_t *self)
{
	...
	self->ripstate = 0;
/*-----/ NEW /-----/ Set gib health and fireflags. /-----*/
        self->gib_health = 0;
...
void SP_misc_blue_banner (edict_t *ent)
{
	ent->movetype = MOVETYPE_NONE;
	ent->solid = SOLID_NOT;
	ent->s.modelindex = gi.modelindex ("models/banner_bl/tris.md2");
	ent->s.frame = rand() % 16;
	gi.linkentity (ent);
...
/*-----/ NEW /-----/ Set gib health, mass, and fireflags. /-----*/
        ent->gib_health = -80;
        ent->mass       = 200;
...
/* TELEPORT MODIFICATION BEGINS (1/2) */

extern void teleporter_touch2(edict_t*,edict_t*,cplane_t*,csurface_t*);
