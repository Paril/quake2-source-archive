...
void ChickSlash (edict_t *self)
{
	...
	fire_hit (self, aim, (damage + (rand() %6)), 100);
}
...
void ChickRocket (edict_t *self)
{
	...
	monster_fire_rocket (self, start, dir, damage, 500, MZ2_CHICK_ROCKET_1);
}	
...
{
	gi.sound(self, CHAN_ITEM, sound_missile_reload, 1, ATTN_NORM, 0);
}

void Chick_setpause(edict_t *self)
{
	if (skill->value <= 3)
		return;
...
void ChickWait(edict_t *self)
{
	if (skill->value <= 3)
		return;

	if (self->monsterinfo.aiflags & AI_SHOOTSROCKET)
	{
		if (level.time >= self->monsterinfo.pausetime)
			self->monsterinfo.aiflags &= ~AI_HOLD_FRAME;
		else
...
mframe_t chick_frames_start_attack1 [] =
{
	...
	ai_charge, 0,	monster_skip_frame,
	ai_charge, 4,	monster_skip_frame,
	ai_charge, 0,	monster_skip_frame,
	ai_charge, -3,  monster_skip_frame,
	ai_charge, 3,	monster_skip_frame,
	ai_charge, 5,	monster_skip_frame,
	ai_charge, 7,	monster_skip_frame,
	ai_charge, 0,	NULL,
	ai_charge, 0,	Chick_setpause,
	ai_charge, 0,	ChickWait,
