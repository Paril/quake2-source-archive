...
void monster_fire_blaster (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int flashtype, int effect)
{
	...
	if (flashtype >= MZ2_TANK_BLASTER2_1)
	{
		gi.sound(self, CHAN_VOICE, gi.soundindex("tank/Tnkatck3.wav"), 1, ATTN_NORM, 0);
...
void monster_think (edict_t *self)
{
	...
	if (strcmp(self->classname, "monster_jorg") && level.time > 1)
	{
...
void walkmonster_start_go (edict_t *self)
{
	...
	if (!self->yaw_speed)
		self->yaw_speed = 30 + (mt_ldrand() * 30);
	self->viewheight = 25;
