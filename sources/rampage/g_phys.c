...
void SV_Physics_Toss(edict_t *ent)
{
	...
	if (isinwater)
		...
		ent->waterlevel = 0;
	if (strcmp(ent->classname, "debris") == 0 )
	{
		if (rand() % 4 == 3)
		{
	...
	else if (strcmp(ent->classname, "bubble") != 0)
	{
		if (!wasinwater && isinwater)
			gi.positioned_sound(old_origin, g_edicts, CHAN_AUTO, gi.soundindex("misc/h2ohit1.wav"), 1, 1, 0);
		else if (wasinwater && !isinwater)
			...
			if (!wasonground)
				if (hitsound)
				{
					if(ent->monsterinfo.scale)
						gi.sound(ent, 0, gi.soundindex("player/land1.wav"), 1, ATTN_IDLE, 0);

					else
						gi.sound(ent, 0, gi.soundindex("world/land.wav"), 1, ATTN_IDLE, 0);
...
================
*/
void MOVETYPE_PLAYER_EXT(edict_t *ent)
{
}
