...
void makron_prerailgun (edict_t *self)
{
	...
	if ((random() * 0.25) + (get_dist(self, self->enemy) * 0.002) < 1)
...
void MakronHyperblaster (edict_t *self)
{
	...
	monster_fire_blaster (self, start, forward, damage, 1000, MZ2_MAKRON_BLASTER_1, EF_BLASTER);
}	
...
void makron_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	{
		gib_target(self, damage, (GIB_MED | GIB_MECH), point);
