...
void floater_fire_blaster(edict_t *self)
{
	...
	monster_fire_blaster(self, start, dir, damage, 1000, MZ2_FLOAT_BLASTER_1, effect);
}
...
void floater_wham (edict_t *self)
{
	...
	fire_hit (self, aim, damage + rand() % 6, -50);
}
...
//	G_ProjectSource (self->s.origin, monster_flash_offset[flash_number], forward, right, origin);
	...
	T_Damage (self->enemy, self, self, dir, self->enemy->s.origin, vec3_origin, damage + rand() % 6, -10, DAMAGE_ENERGY, MOD_UNKNOWN);
}
