...
// HOLY WARS
	int makeanother = 0;
	// If an halo is about to be destroyed, prepare to spawn another one
	if (self->item && (self->item->flags & IT_HALO))
		makeanother = 1;
// end HOLY WARS
...
// HOLY WARS
	// Respawn the halo if you destroyed it
	if (makeanother)
...
// HOLY WARS
	if (other->client->plstatus == SAINT)
		hw.teleported = 1;
// end HOLY WARS
}
...
// HOLY WARS
// HWv2.1
	// Remove visible plats from map HW6
	if (!Q_stricmp(level.mapname, "hw6"))
		gi.setmodel (ent, "models/invis/tris.md2");
// end HOLY WARS
