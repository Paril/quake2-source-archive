...
// HOLY WARS
// inventory is now handled in the menu
	OpenMainMenu(ent);
// end HOLY WARS
}
...
// this is called from player_die()
//	HW_PlayerDie(ent);
// end HOLY WARS
...
// HWv2.1 : modified next line
	...
	else if (Q_stricmp (cmd, "hw_fix") == 0)  // toggles the telefrag hack
	{
		if (hw_edit->value)
		{
			if (telefraghack)
				...
				telefraghack = 1;
			gi.bprintf(PRINT_HIGH, "*** Telefrag fix is now %s for this map ***\n",
				telefraghack ? "ON" : "OFF");
		}
		else
			gi.cprintf(ent, PRINT_HIGH, "No effect - HW_EDIT is off\n");
	}
