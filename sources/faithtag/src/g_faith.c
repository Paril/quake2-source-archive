...
pmenu_t joinmenu[] = {
	...
	{ "*Faith Betrayer",	PMENU_ALIGN_CENTER, NULL, NULL },
...
qboolean CheckforIt (void){
	...
	for (i=1, target=g_edicts+i; i < globals.num_edicts; i++,target++)
	{
		if (!target->client)
			continue;
		if (target->client->it){
...
void SetIt (void){
	if(random_player(NULL))
		SetItEffects(random_player(NULL));
}
...
void SetItEffects (edict_t *ent){
	...
	{
		gi.centerprintf (joe_bloggs, "%s is the Betrayer\npurge him!!!!", ent->client->pers.netname);
	}
}
...
void Rules (edict_t *killer, edict_t *victim){
	...
	//The killer was the betrayer so gets 3 points
	if (killer->client->it){
		for (i = 0; i < 3; i++){
	...
	//The vitim was the betrayer so killer gets 5 points
	if (victim->client->it){
		for (i = 0; i < 5; i++){
			killer->client->resp.score++;
			holylevel(killer);
		}
		RemoveItEffects(victim);
		SetIt();
		return;
	}

	// neither the killer or the victim was the betrayer
}
