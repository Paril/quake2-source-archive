...
static void Touch_Multi (edict_t &self, edict_t &other, const cplane_t *plane, const csurface_t *surf)
{
	if (other.client)
	{
	...
	{
		const vec3_t forward = other.s.angles.Forward();
...
void SP_trigger_once(edict_t &ent)
{
	...
	if (ent.spawnflags & 1)
	{
		const vec3_t v = ent.mins + (ent.size * 0.5f);
		ent.spawnflags &= ~1;
		ent.spawnflags |= 4;
...
static void trigger_counter_use (edict_t &self, edict_t &other, edict_t &activator)
{
	...
	if (self.count)
	{
		if (!(self.spawnflags & 1))
		{
			activator.client->CenterPrint("%i more to go...", self.count);
	...
	if (!(self.spawnflags & 1))
	{
		activator.client->CenterPrint("Sequence completed!");
		activator.PlaySound(gi.soundindex ("misc/talk1.wav"));
	}
...
==============================================================================
*/
constexpr int32_t PUSH_ONCE		= bit(0);

static soundindex_t windsound;

static void trigger_push_touch (edict_t &self, edict_t &other, const cplane_t *plane, const csurface_t *surf)
{
...
static void hurt_touch (edict_t &self, edict_t &other, const cplane_t *plane, const csurface_t *surf)
{
	if (!other.takedamage)
		return;
	...
	if (!(self.spawnflags & 4))
	{
		if ((level.framenum % 10) == 0)
			other.PlaySound(self.noise_index);
	}
...
*/
static void trigger_gravity_touch (edict_t &self, edict_t &other, const cplane_t *plane, const csurface_t *surf)
{
...
void SP_trigger_gravity (edict_t &self)
{
	...
	InitTrigger (self);
	self.gravity = strtol(st.gravity, nullptr, 10);
...
*/
static void trigger_monsterjump_touch (edict_t &self, edict_t &other, const cplane_t *plane, const csurface_t *surf)
{
