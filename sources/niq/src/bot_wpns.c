...
//#ifdef NIQ
	if(niq_enable->value)
	{
	// niq: weapon doesn't change when you run out of ammo, just return and try again later
...
#ifdef NIQ
	// niq: hack to fix bots being *WAY* too strong with HB
	if(skill->value == 0)
		// my grandmother should be able to win most of the time
		fire_blaster (self, start, forward, (int)(damage*0.20), 1000, effect, true);
	else if(skill->value == 1)
		// I should be able to win most of the time
		fire_blaster (self, start, forward, (int)(damage*0.30), 1000, effect, true);
	else if(skill->value == 2)
		// should be pretty tough
		fire_blaster (self, start, forward, (int)(damage*0.50), 1000, effect, true);
	else
		{
		// default values normally used for skill >=2: 
		// f***ing impossible unless I suck more than I realize
...
void botPickBestWeapon(edict_t *self)
{
	...
	if(niq_enable->value)
		{
		// niq: no choice but to keep using the current weapon
		gi.dprintf("NIQ: botPickBestWeapon called with niq_enable=1!\n");
		return;
		}
	else
#endif
...
void botPickBestCloseWeapon(edict_t *self)
{
	...
	if(niq_enable->value)
		{
		// niq: no choice but to keep using the current weapon
		gi.dprintf("NIQ: botPickBestCloseWeapon called with niq_enable=1!\n");
		return;
		}
	else
#endif
...
void botPickBestFarWeapon(edict_t *self)
{
	...
	if(niq_enable->value)
		{
		// niq: no choice but to keep using the current weapon
		gi.dprintf("NIQ: botPickBestFarWeapon called with niq_enable=1!\n");
		return;
		}
	else
#endif
