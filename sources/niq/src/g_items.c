...
static void drop_make_touchable (edict_t *ent)
{
	...
	{
#ifdef EBOTS
		if(niq_ebots->value && !niq_enable->value)
		{
			// Set visible nodes
			if ((ent->item->pickup == Pickup_Weapon) ||
				(ent->item->pickup == Pickup_Health) ||
...
{
	// !!! I should call niq_noprecache here and only pre-cache items
	// which can't be in the map or in a client's inventory. This will
	// fix the problem with city3 and visible_weapons=1 where there
	// are too many calls gi.to modelindex. This will not fix the pblm
	// if niq_enable=0 -- TBD block loading city3 in this case?
	PrecacheItem (item);
...
#ifdef NIQ
	// 14/07/98: moved here from just below the if (deathmatch->value) below
	if(niq_enable->value)
	{
	// niq: most items are never spawned
	if(niq_zapitem(ent, item))
		return;
	}
#endif
