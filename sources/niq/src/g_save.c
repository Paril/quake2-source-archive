...
//ZOID
	...
	bot_free_clients = gi.cvar ("bot_free_clients", "1", CVAR_ARCHIVE);
    ...
    if ( ctf_auto_teams->value > 0 && ctf_special_teams )  // ~~JLH
...
//#ifdef NIQ
    ...
    // niq: initialize NIQ stuff (returns if niq_enable is 0)
	niq_initall();
//#endif
...
// BUNGLE Standard Logging Support
    ...
    {
        // Required to set patch name
        if (ctf->value)
            sl_Logging( &gi, "NIQ CTF" );
        else
            sl_Logging( &gi, "NIQ"     );
    }
#endif
