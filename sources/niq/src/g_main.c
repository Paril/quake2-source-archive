...
//ZOID
		for (i=0 ; i<maxclients->value ; i++)
		{
			...
			if ( (cl->resp.score >= fraglimit->value) || ((fraglimit->value - cl->resp.score) <= 0.001) )
#else
...
//	AI_SetSightClient ();
	...
	if (level.intermissiontime && niq_inttime->value && (level.time > level.intermissiontime + niq_inttime->value))
	{
...
#ifdef EBOTS
	if (niq_ebots->value && dedicated->value)
		OptimizeRouteCache();
#endif

//#ifdef NIQ
	if(niq_enable->value)
	    // niq: central change is to have this function change weapon from time to time for all clients
