...
*/
edict_t *dm_spots[64];
int		num_dm_spots = 0;	// niq: initialized this here
...
#ifdef EBOTS
	else if (niq_ebots->value && deathmatch->value && teamplay->value && attacker->client && SameTeam(self, attacker))
	{
		meansOfDeath |= MOD_FRIENDLY_FIRE;
	}
#endif
...
//#ifdef NIQ
			...
			{
				// niq: handle kill
				niq_kill(self, attacker, ff, message, message2);
			}
//#else
			else
			{
...
//#ifdef NIQ
	if(niq_enable->value)
		// niq: dead players can never drop weapons
		return;
//#else
	else
	{
...
#ifdef NIQ
        if (niq_enable->value && niq_logfile->value)
...
//#ifdef NIQ
	if(niq_enable->value)
	    // niq: remove all weapons and ammo
		niq_removeallweapons (self);
//#endif
}
...
//#ifdef NIQ
	if(niq_enable->value)
		niq_InitClientPersistant (client);
//#else
	else
	{
	gitem_t		*item;
...
//ZOID
//#ifdef NIQ
//    int	niq_helpmode	= client->resp.niq_helpmode;
//#endif
...
//    client->resp.niq_helpmode = niq_helpmode;
	if(is_bot || client->pers.niq_isbot)
	    client->resp.niq_helpmode = 0; // bots don't go into observer mode
	else
		client->resp.niq_helpmode = 1; // force observer mode up for humans
//#endif
...
#ifdef NIQ
		// clear helpmode in case it was set when client got killed otherwise will go into observer mode
		self->client->resp.niq_helpmode = 0;
#endif
...
//#ifndef NIQ
	if(!niq_enable->value)
		// niq: do this *after* setting up gun
		ShowGun(ent);
//#endif
...
#ifdef NIQ
	// clients are initially in observer mode
	if(NIQStartClient(ent))
		return;
#endif
...
//ZOID
	...
	// 26/09/98 mfox: this should make the scoreboard go away faster when respawning
	if (!ent->bot_client)
...
//#ifdef NIQ
	if(niq_enable->value)
	{
	// niq: per-client timers
	niq_settimers(client);

	// niq: do this after setting the weapon
	ShowGun(ent);
	}

#ifdef OHOOK
	if(niq_tractor->value && client->hook)
		{
	    hook_reset(client->hook);

		// just in case
		client->hook_state = HOOK_READY;
		}
#endif
...
//#endif
	...
	if(niq_ebots->value)
	{
		...
		if (!ent->bot_client)
		{
			...
			// niq: added check for trail[0]!!
			if ((bot_calc_nodes->value) && trail[0] && trail[0]->timestamp)
...
#ifdef NIQ
	    // niq: fix for comparing user password against empty one on server!
		if ((passwd->string[0] != '\0') && (strcmp(passwd->string, value) != 0))
			return false;
#else
...
#ifdef NIQ
	gi.bprintf (PRINT_HIGH, "%s exits NIQ, %d client(s)\n", ent->client->pers.netname, niq_getnumclients()-1);
#else
...
#ifdef EBOTS
	// fill this spot in with a bot if possible
	if(niq_ebots->value)
	{
		if (teamplay->value && ent->client->team)
			ent->client->team->num_players--;
	}
#endif
...
#ifdef OHOOK
	if (niq_tractor->value && client->hook_state == HOOK_ON && client->hook) 
...
#ifdef NIQ
	// quick hack to use attack button to quit observer mode
	if((ent->svflags & SVF_NOCLIENT) && !(client->buttons & BUTTON_ATTACK) && (ucmd->buttons & BUTTON_ATTACK))
		niq_putaway(ent);		
#endif
...
//#ifdef NIQ
	if(niq_enable->value)
		// niq: see if time to do anything to health, ammo, df
		niq_checktimers(ent);
//#endif

#ifdef OHOOK
	// attempt to compensate for hook messing up prediction
	...
	// ack, this is ugly
	// changed from 0.1 to 1 in v0.4
	if (niq_tractor->value && client->hook_state == HOOK_ON && VectorLength(ent->velocity) < 1) {
...
//	if (gi.pointcontents(ent->s.origin) & CONTENTS_LADDER)
	...
	if (niq_ebots->value && bot_calc_nodes->value)
	{
...
//		(!ent->movetarget || !ent->movetarget->item || (ent->movetarget->item->pickup != CTFPickup_Flag)))
	{
		...
		if (last_checkhelp < (level.time + 0.5))
		{
			for (i=0; i<num_players; i++)
			{
				...
				{
					if ((plyr->bot_client) && (plyr->enemy != ent) && (entdist(plyr, ent) < (200 * plyr->bot_stats->combat)))
			...
			if (count < ideal)
			{
				for (i=0; (i<num_players && count<ideal); i++)
				{
					...
					if (entdist(plyr, ent) > 700)
						continue;

					if (!gi.inPVS(plyr->s.origin, ent->s.origin))
						continue;
