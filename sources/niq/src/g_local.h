...
#ifdef NIQ
    // niq: these go here so they are reset when a game is started and can be saved/restored
    int         niq_curweapon;
    int         niq_monsterkills;
    int         niq_wlindex;
    float	    niq_wsecs;
    
    // Actual list of weapons to use in user-specified order. 
    // Defaults to all 10 weapons, in usual order (entry 0 not used).
    int         niq_weaponsindx[NIQMAXLISTWEAPONS+1];
    int         niq_weaponsused[NIQMAXLISTWEAPONS+1];
    int         niq_numweapons;
#endif
} game_locals_t;
...
#ifdef NIQ
	// The following will be saved between spawns/level changes

    // niq: score is a float
	float		score;			// for calculating total unit score in coop games

    float       niq_fDamage;	// client's damage factor
	int			niq_sbmode;		// client's preferred scoreboard
	int			niq_hudmode;	// client's preferred HUD
    qboolean    niq_didmotd;	// did we show motd already (for sp/coop)
	qboolean    niq_isbot;		// set ==> client is a bot
#else
...
#ifdef NIQ
    // niq: score is a float
	float		score;				// frags, etc

	// niq: track kills, killied, suicides
    int         niq_kills;
    int         niq_killed;
    int         niq_suicides;
    float       niq_fDamageTotal;	// niq1.3: average out df
    int			niq_fDamageTics;	// niq1.3: average out df
    int			niq_helpmode;		// set ==> motd/help/info screen is up
#else
	int			score;				// frags, etc
#endif
} client_respawn_t;
...
//ZOID
	// niq:
	float		niq_sectimer;		// used to wait exactly 1 second
    int         niq_ammotics;       // used for finer resolution
...
// 29/11/98 mfox: actually, it seems I can't add *anything* below this point
// even an int, or the game blows up on startup.
};
