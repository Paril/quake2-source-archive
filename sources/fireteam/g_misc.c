...
void gib_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (plane)
	{
		gi.sound (self, CHAN_VOICE, FHIT3_SOUND, 1, ATTN_NORM, 0);
...
void ThrowClientHead (edict_t *self, int damage)
{
	...
	if (rand()&1)
	{
		gibname = GIB_HEAD2_MODEL;
		self->s.skinnum = 1;		// second skin is player
	}
	else
	{
		gibname = GIB_SKULL_MODEL;
		self->s.skinnum = 0;
	}
...
//PSY: CTF
	...
	G_Spawn_Explosion(TE_EXPLOSION1, self->s.origin, self->s.origin );
...
void SP_viewthing(edict_t *ent)
{
	...
	ent->s.modelindex = gi.modelindex (BANNER_MODEL);
	gi.linkentity (ent);
...
void func_explosive_explode (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (mass >= 100)
	{
		...
		while(count--)
		{
			...
			ThrowDebris (self, DEBRIS1_MODEL, 1, chunkorigin);
		}
	}
	...
	while(count--)
	{
		...
		chunkorigin[1] = origin[1] + crandom() * size[1];
		chunkorigin[2] = origin[2] + crandom() * size[2];
		ThrowDebris (self, DEBRIS2_MODEL, 2, chunkorigin);
	}
...
void SP_func_explosive (edict_t *self)
{
	...
	gi.modelindex (DEBRIS1_MODEL);
	gi.modelindex (DEBRIS2_MODEL);
...
void barrel_explode (edict_t *self)
{
	...
	ThrowDebris (self, DEBRIS1_MODEL, spd, org);
	org[0] = self->s.origin[0] + crandom() * self->size[0];
	org[1] = self->s.origin[1] + crandom() * self->size[1];
	org[2] = self->s.origin[2] + crandom() * self->size[2];
	ThrowDebris (self, DEBRIS1_MODEL, spd, org);
	...
	ThrowDebris (self, DEBRIS3_MODEL, spd, org);
	VectorCopy (self->absmin, org);
	org[0] += self->size[0];
	ThrowDebris (self, DEBRIS3_MODEL, spd, org);
	VectorCopy (self->absmin, org);
	...
	org[1] = self->s.origin[1] + crandom() * self->size[1];
	org[2] = self->s.origin[2] + crandom() * self->size[2];
	ThrowDebris (self, DEBRIS2_MODEL, spd, org);
	org[0] = self->s.origin[0] + crandom() * self->size[0];
	org[1] = self->s.origin[1] + crandom() * self->size[1];
	org[2] = self->s.origin[2] + crandom() * self->size[2];
	ThrowDebris (self, DEBRIS2_MODEL, spd, org);
...
void misc_deadsoldier_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	ThrowHead (self, GIB_HEAD2_MODEL, damage, GIB_ORGANIC);
}

void SP_misc_deadsoldier (edict_t *ent)
{
	if (G_IsDeathmatch(ent)) return;
...
void SP_misc_viper (edict_t *ent)
{
	...
	ent->solid = SOLID_NOT;
	ent->s.modelindex = gi.modelindex (STROGG_SHIP_MODEL);
	VectorSet (ent->mins, -16, -16, 0);
	VectorSet (ent->maxs, 16, 16, 32);
...
void SP_misc_viper_bomb (edict_t *self)
{
	...
	self->s.modelindex = gi.modelindex (BOMB_MODEL);
...
{
	gi.setmodel (ent, GIB_HEAD_MODEL);
	ent->solid = SOLID_NOT;
	ent->s.effects |= EF_GIB;
	ent->takedamage = DAMAGE_YES;
	ent->die = gib_die;
...
void SP_misc_teleporter (edict_t *ent)
{
	...
	gi.setmodel (ent, DMSPAWN_MODEL);
	ent->s.skinnum = 1;
	ent->s.effects = EF_TELEPORTER;
	ent->s.sound = AMB10_SOUND;
	ent->solid = SOLID_BBOX;
