...
void soldier_attack1_refire1 (edict_t *self)
{
	...
	if ( ((CVAR_SKILL_LEVEL == 3) && (random() < 0.5)) || (range(self, self->enemy) == RANGE_MELEE) )
...
void soldier_sight(edict_t *self, edict_t *other)
{
	...
	if ((CVAR_SKILL_LEVEL > 0) && (range(self, self->enemy) >= RANGE_MID))
	{
		if (random() > 0.5)
...
// check for gib
	...
	{
		gi.sound (self, CHAN_VOICE, UDEATH_SOUND, 1, ATTN_NORM, 0);
		for (n= 0; n < 3; n++)
