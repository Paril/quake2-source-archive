...
void SpawnEntities (char *mapname, char *entities, char *spawnpoint)
{
	...
	skill_level = floor (CVAR_SKILL_LEVEL);
	if (skill_level < 0)
		skill_level = 0;
	if (skill_level > 3)
		skill_level = 3;
	if (CVAR_SKILL_LEVEL != skill_level)
...
// parse ents
	while (1)
	{
		...
		if (ent != g_edicts)
		{
			...
			{
				if (((CVAR_SKILL_LEVEL == 0) && (ent->spawnflags & SPAWNFLAG_NOT_EASY)) ||
					((CVAR_SKILL_LEVEL == 1) && (ent->spawnflags & SPAWNFLAG_NOT_MEDIUM)) ||
					(((CVAR_SKILL_LEVEL == 2) || (CVAR_SKILL_LEVEL == 3)) && (ent->spawnflags & SPAWNFLAG_NOT_HARD))
					)
					{
						G_FreeEdict (ent);
						inhibit++;
