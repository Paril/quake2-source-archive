...
void P_DamageFeedback (edict_t *player)
{
	...
	if ((level.time > player->pain_debounce_time) && !(player->flags & FL_GODMODE) && (client->invincible_framenum <= level.framenum))
	{
		...
		if (player->health < 25)
			...
			l = 100;
		gi.sound (player, CHAN_VOICE, RANDOM_PAIN_SOUND, 1, ATTN_NORM, 0);
	}
...
void SV_CalcViewOffset (edict_t *ent)
{
    ...
    edict_t *Bub;
	static int			bubble;
	static float YDirection, RDirection, PDirection;
...
//===================================
	...
	else if (ent->client->chasetoggle)
	{
		VectorSet (v, 0, 0, 0);
	...
	else
	{
		...
		if (ent->client->missile != NULL) {
			ent->client->ps.pmove.origin[0] = ent->client->missile->s.origin[0]*8;
			ent->client->ps.pmove.origin[1] = ent->client->missile->s.origin[1]*8;
			ent->client->ps.pmove.origin[2] = ent->client->missile->s.origin[2]*8;
			VectorCopy (ent->client->missile->s.angles, ent->client->ps.viewangles);
		}
	}
...
void SV_CalcBlend (edict_t *ent)
{
    ...
    else if (ent->client->quad_framenum > level.framenum)
	{
		...
		if (remaining == 30)	// beginning to fade
			gi.sound(ent, CHAN_ITEM, DAMAGE2_SOUND, 1, ATTN_NORM, 0);
	...
	else if (ent->client->invincible_framenum > level.framenum)
	{
		...
		if (remaining == 30)	// beginning to fade
			gi.sound(ent, CHAN_ITEM, PROTECT2_SOUND, 1, ATTN_NORM, 0);
		if (remaining > 30 || (remaining & 4) )
	...
	else if (ent->client->enviro_framenum > level.framenum)
	{
		...
		if (remaining == 30)	// beginning to fade
			gi.sound(ent, CHAN_ITEM, AIROUT_SOUND, 1, ATTN_NORM, 0);
		if (remaining > 30 || (remaining & 4) )
...
void P_FallingDamage (edict_t *ent)
{
	...
	if (delta > 30)
	{
		...
		if (!CVAR_DEATHMATCH || !((int)dmflags->value & DF_NO_FALLING) )
...
void P_WorldEffects (void)
{
	...
	if (!old_waterlevel && waterlevel)
	{
		...
		if (current_player->watertype & CONTENTS_LAVA)
			gi.sound (current_player, CHAN_BODY, LAVA_SCREAM_SOUND, 1, ATTN_NORM, 0);
		else if (current_player->watertype & CONTENTS_SLIME)
			gi.sound (current_player, CHAN_BODY, ENTER_WATER_SOUND, 1, ATTN_NORM, 0);
	...
	{
		PlayerNoise(current_player, current_player->s.origin, PNOISE_SELF);
		gi.sound (current_player, CHAN_BODY, EXIT_WATER_SOUND, 1, ATTN_NORM, 0);
		current_player->flags &= ~FL_INWATER;
	}
	...
	{
		gi.sound (current_player, CHAN_BODY, UNDER_WATER_SOUND, 1, ATTN_NORM, 0);
	}
	...
	if (old_waterlevel == 3 && waterlevel != 3)
	{
		...
		{	// gasp for air
			gi.sound (current_player, CHAN_VOICE, WATER_GASP1_SOUND, 1, ATTN_NORM, 0);
			PlayerNoise(current_player, current_player->s.origin, PNOISE_SELF);
		}
		else  if (current_player->air_finished < level.time + 11)
		{	// just break surface
			gi.sound (current_player, CHAN_VOICE, WATER_GASP2_SOUND, 1, ATTN_NORM, 0);
		}
	}
		...
		if (breather || envirosuit)
		{
			...
			if (((int)(current_client->breather_framenum - level.framenum) % 25) == 0)
			{
				if (!current_client->breather_sound)
					gi.sound (current_player, CHAN_AUTO, U_BREATHING1_SOUND, 1, ATTN_NORM, 0);
				else
					gi.sound (current_player, CHAN_AUTO, U_BREATHING2_SOUND, 1, ATTN_NORM, 0);
		...
		{	// drown!
				...
				&& current_player->health > 0)
			{
				...
				if (current_player->health <= current_player->dmg)
					gi.sound (current_player, CHAN_VOICE, WATER_DROWN1_SOUND, 1, ATTN_NORM, 0);
				else if (rand()&1)
					gi.sound (current_player, CHAN_VOICE, GURPING1_SOUND, 1, ATTN_NORM, 0);
				else
					gi.sound (current_player, CHAN_VOICE, GURPING2_SOUND, 1, ATTN_NORM, 0);
	...
	if (waterlevel && (current_player->watertype&(CONTENTS_LAVA|CONTENTS_SLIME)) )
	{
		if (current_player->watertype & CONTENTS_LAVA)
		{
			...
			{
				if (rand()&1)
					gi.sound (current_player, CHAN_VOICE, BURNING1_SOUND, 1, ATTN_NORM, 0);
				else
					gi.sound (current_player, CHAN_VOICE, BURNING2_SOUND, 1, ATTN_NORM, 0);
...
void G_SetClientSound (edict_t *ent)
{
	...
	if (ent->client->pers.helpchanged && ent->client->pers.helpchanged <= 3 && !(level.framenum&63) )
	{
		...
		gi.sound (ent, CHAN_VOICE, PC_UP_SOUND, 1, ATTN_STATIC, 0);
	}
	...
	if (ent->waterlevel && (ent->watertype&(CONTENTS_LAVA|CONTENTS_SLIME)) )
		...
		ent->s.sound = RG_HUM_SOUND;
	else if (strcmp(weap, "weapon_bfg") == 0)
		ent->s.sound = BFG_HUM_SOUND;
