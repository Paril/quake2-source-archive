	...
	else if (VectorLength(themove) > 4)	// running
	{
		...
		else
		{
			...
			if (!self->waterlevel && self->client->pickup_msg_time < (level.time - 0.3))
			{
				...
				if (K2_IsBFK(self))
				{
					gi.sound(self,CHAN_BODY,gi.soundindex("makron/step1.wav"), 1, ATTN_NORM, 0);
				}
				else if (!K2_IsStealth(self))
				//K2
...
//					break;
	...
	if (usevwep->value && (num_view_weapons == 0))
	{
		gi.dprintf("WARNING: usevwep enabled, but no [view weapon] section in bots.cfg\n  You should re-install Eraser to restore the [view weapons] section.\n\n");
	}
