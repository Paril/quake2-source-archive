...
int	RoamFindBestItem(edict_t	*self, edict_t	*list_head, int	check_paths)
{
	...
	for (trav = list_head ; trav ; trav = trav->node_target)
	{
		...
		else if (list_head == bonus_head)
		{
			...
			//K2:Begin
			else if ( (trav->item->pickup == Pickup_Key) )
			{
				if(trav->prev_owner == self)
					continue;

				//Don't have one, so we really want one!
				if(!self->client->key)
				{
					dist_divide = 4;
				}
				else  //If we have one and keys are droppable...
				{
					if(!droppable->value)
						continue;
					//If bot hasn't had the key for at least half of the key's time,
					//don't bother
					if (!K2_botCheckKeyTimer(self))
						continue;
					dist_divide = K2_botCompareKey(self,K2_KeyType(trav));
					if(dist_divide)
					{
						//gi.dprintf("Bot dropped key on purpose!\n");
						K2_DropKeyCommand(self);
						if(dist_divide < 4)
							dist_divide = 4;
