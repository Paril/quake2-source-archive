...
qboolean KillBox (edict_t *ent)
{
	...
	while (1)
	{
		...
		//K2:Begin
		//Need to check for reverse telefrag condition
...
//			return false;
	...
	while (++i < num_players)
	{
		...
		//K2:Begin
		//Need to check for reverse telefrag condition
		if(K2_IsProtected(trav) && !K2_IsProtected(ent) )
		{
			//Reverse Telefrag
			T_Damage (ent,trav,trav,vec3_origin, ent->s.origin, vec3_origin, 100000, 0, DAMAGE_NO_PROTECTION, MOD_REVERSE_TELEFRAG);
...
#else
	i = sprintf(filename, GAMEVERSION);
#endif
...
void my_bprintf (int printlevel, char *fmt, ...)
{
	...
	{
		cl_ent = g_edicts + 1 + i;
		if (!cl_ent->inuse || cl_ent->bot_client) //ERASER
			continue;
...
if (!ent || !ent->inuse || ent->isbot || ent->bot_client)
  return;
