...
void MoveClientToIntermission (edict_t *ent)
{
	...
	//K2:Begin - Init the client k2 vars
...
//ZOID
	...
	//K2:Begin - add the nextleveldelay time to nextlevelstart
	nextlevelstart = level.time + nextleveldelay->value;
	//K2:End
...
void DeathmatchScoreboardMessage (edict_t *ent, edict_t *killer)
{
	char	entry[1024];
	char	string[1400];
	char	string2[1400]=""; //K2:Begin and End
	int		stringlength;
	int		i, j, k;
	int		sorted[MAX_CLIENTS];
	int		sortedscores[MAX_CLIENTS];
...
//ZOID
	...
	for (i=0 ; i<total ; i++)
	{
		...
		//K2:Begin - Bonus scores
		Com_sprintf (string2, sizeof(string2),
					"xv %i yv %i string2 \"Bonus\" ",		
					x+32+85, y+8);
...
void G_SetStats (edict_t *ent)
{
	...
	//K2:Begin
	K2_SetClientStats(ent,hasPowerup);
	//K2:End
...
void G_CheckChaseStats (edict_t *ent)
{
	...
	for (i=0; i<num_players; i++)
	{
		...
		if (!players[i]->inuse || cl->chase_target != ent)
			continue;
		
		memcpy(cl->ps.stats, ent->client->ps.stats, sizeof(cl->ps.stats));
		
		G_SetSpectatorStats(players[i+1]);
