...
void berserk_attack_spike (edict_t *self)
{
	...
	fire_hit (self, aim, (15 + (rand() % 6)), 400, MOD_HIT);		//	Faster attack -- upwards and backwards
}
...
void berserk_attack_club (edict_t *self)
{
	...
	fire_hit (self, aim, (5 + (rand() % 6)), 400, MOD_HIT);		// Slower attack
}
...
void SP_monster_berserk (edict_t *self)
{
	...
	if (deathmatch->value && !hunt->value)
	{
		G_FreeEdict (self);
		return;
	}
