...
void ClipGibVelocity (edict_t *ent)
{
	if (ent->velocity[0] < -300)
		ent->velocity[0] = -300;
	else if (ent->velocity[0] > 1500)
		ent->velocity[0] = 1500;
	if (ent->velocity[1] < -300)
		ent->velocity[1] = -300;
	else if (ent->velocity[1] > 1500)
		ent->velocity[1] = 1500;
	if (ent->velocity[2] < 200)
		ent->velocity[2] = 200;	// always some upwards
	else if (ent->velocity[2] > 5000)
		ent->velocity[2] = 5000;
}
...
void ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	gib->nextthink = level.time + 10 + random()*2;

	gi.linkentity (gib);
}

//Make a head-kick HAH ! :)
static void ClientHead_Touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t dir;
	vec3_t forward, up;
	...
	if (other->client)
	{
		if (!other->client->resp.is_a_head)
			return;

		AngleVectors (other->s.angles, forward, NULL, up);

		VectorNormalize (forward); //not needed
		VectorScale (forward, 500, dir);
		VectorAdd (ent->velocity, dir, ent->velocity);
		VectorMA (ent->velocity, 250 + crandom() * 10.0, up, ent->velocity);
		gi.sound (ent, CHAN_VOICE, gi.soundindex ("mutant/mutatck3.wav"), 1, ATTN_NORM, 0);
	}
...
void ThrowHead (edict_t *self, char *gibname, int damage, int type)
{
	...
	self->solid = SOLID_TRIGGER;//SOLID_NOT;
...
void ThrowClientHead (edict_t *self, int damage)
{
	...
	self->solid = SOLID_TRIGGER;//### ADDED REMOVED ### SOLID_NOT;
	self->s.effects = EF_GIB;
	self->s.sound = 0;
	self->flags |= FL_NO_KNOCKBACK;

	self->movetype = MOVETYPE_BOUNCE;
	self->touch = ClientHead_Touch;	//added
	VelocityForDamage (damage, vd);
	...
	{
		//spk
		self->client->resp.is_a_head = true;
