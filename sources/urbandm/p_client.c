	...
	if (deathmatch->value || coop->value)
	{
		...
		if (attacker == self)
		{
			...
			case MOD_BFG_BLAST:
				...
				break;
			case MOD_LASER_DEFENCE:
				message = "walk on his own lasers";
				break;
			default:
				if (IsFemale(self))
		...
		if (attacker && attacker->client)
		{
			...
			if (message)
			{
				...
				if (deathmatch->value)
				{
					...
					{
						if (attacker->head_num < 1)
							attacker->client->resp.score++;
						else
						{
							attacker->client->resp.score = attacker->client->resp.score + 1 + attacker->head_num;
							gi.cprintf (attacker, PRINT_CHAT, "+%d head bonus.\n", attacker->head_num);
						}
					}
...
void player_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	//spk - remove the cam so there's no bug when Copying to body queue
...
//	self->solid = SOLID_NOT;
	...
	if (!self->deadflag)
	{
		self->client->respawn_time = level.time + 6.0;
	...
	if (self->health < GIB_HEALTH_COEF)
	{	// gib
		gi.sound (self, CHAN_BODY, gi.soundindex ("misc/udeath.wav"), 1, ATTN_NORM, 0);
		for (n= 0; n < 2; n++)
			ThrowGib (self, "models/objects/gibs/sm_meat/tris.md2", damage * 15, GIB_ORGANIC);
		for (n= 0; n < 1; n++)
			ThrowGib (self, "models/objects/gibs/bone/tris.md2", damage * 15, GIB_ORGANIC);
		for (n= 0; n < 1; n++)
			ThrowGib (self, "models/objects/gibs/chest/tris.md2", damage * 15, GIB_ORGANIC);
...
{
	//motd by SteQve... don't have the time to make one...
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
	...
	else if (ent->deadflag || ent->feign)	//spk added
...
//		gi.dprintf ("pmove changed!\n");
	...
	if (ent->head_num > 0)
	{
		for (i = 1; i < (ent->head_num+1); i++)
		{
			if (ent->cap_head[i] == NULL)
			{
				gi.dprintf ("Warning, Head %d is NULL\n", i);
				continue;
			}
			else
			{
			//	gi.dprintf ("Running CaptureHeadThink : head %d\n", i);
				CapturedHeadThink (ent, ent->cap_head[i]);
			}
		}
	}
	//### END SPK ###
	...
	//### SPK ### Jumping sparks
	if (ent->misc_time[JUMP_POWER_FINISHED] > level.time)
	{
		...
		VectorScale (forward, -7, back_pos);
		VectorAdd (back_pos, ent->s.origin, back_pos);
		back_pos[2] += (ent->viewheight / 2);	
		VectorScale (forward, -50, dir);

		G_Spawn_Splash (TE_SPLASH , 14, 4, back_pos, dir, back_pos);
	}

	if (ent->misc_time[PROTECT_PERIOD] > level.time)
	{
		vec3_t forward, right, dir;
		vec3_t back_pos;
		...
		AngleVectors(ent->client->v_angle, forward, right, NULL);
		VectorScale (forward, -1, back_pos);
		VectorAdd (back_pos, ent->s.origin, back_pos);
		back_pos[2] += (ent->viewheight);	
		VectorScale (forward, -50, dir);

		G_Spawn_Splash (TE_SPLASH , 15, 2, back_pos, dir, back_pos);
	}
	...
	if (ent->raising_up)
	{
		//gi.dprintf ("Death Reverse frame no : %d - ", ent->s.frame);
		//si on est en train de se relever
		if (ent->misc_time[7] < level.time)
		{
			
		//	gi.dprintf ("Stopped RAISING !");
