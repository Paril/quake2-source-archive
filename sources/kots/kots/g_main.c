...
//KOTS
cvar_t	*kots_password;
cvar_t	*kots_datapath;
cvar_t	*kots_realtime;
cvar_t	*kots_levelmin;
cvar_t	*kots_levelmax;
cvar_t	*kots_teamplay;
cvar_t	*kots_skin1;
cvar_t	*kots_skin2;
cvar_t	*kots_lives;
cvar_t	*kots_port;
...
void EndDMLevel (void)
{
	...
	for ( i = 0; i < maxclients->value; i++ )
	{
		...
		if ( 1 <= ent->kots_vote && ent->kots_vote <= 10 )
			maps[ ent->kots_vote ]++;
	...
	for ( i = 1; i < 10; i++ )
	{
		if ( maps[i] > maps[choose] )
			if ( count > 0 && maps[i] >= count / 4 )
				choose = i;
	}
...
void CheckDMRules (void)
{
	...
	if ( timelimit->value && !kots_lives->value )
	{
	...
	if ( fraglimit->value || kots_lives->value )
	{
		for (i=0 ; i<maxclients->value ; i++)
		{
			cl = game.clients + i;
			...
			if ( kots_lives->value )
			{
				...
				if ( g_edicts[i+1].kots_lives > 0 )
					bEnd++;
			}
			else if ( g_edicts[i+1].kots_kills >= fraglimit->value )
			{
	...
	if ( bEnd < 2 && level.time > 30.0 && kots_lives->value )
	{
		gi.bprintf (PRINT_HIGH, "KOTS Lives has completed.\n");
		EndDMLevel ();
		return;
	}
}
