...
//Next is added section for flash grenade
	if (ent->client->blindTime > 0)
  {
		...
		//uhm gee didnt they forget something? 
		//lets add a decrement so it doesnt last forever
		ent->client->blindTime--;
	}//end added section for flash

	//KOTS
	if ( ent->client->pers.kots_cloaking && ( ent->svflags & SVF_NOCLIENT ) )
...
void G_SetClientEffects (edict_t *ent)
{
	...
	if ( level.kots_spreewar &&	ent->client->pers.kots_streak_count < 25 )
	{
		ent->s.effects |= EF_COLOR_SHELL;

		ent->s.renderfx |= RF_SHELL_RED;
	}
	else if ( ent->client->pers.kots_streak_count > 5 )
	{
		ent->s.effects |= EF_COLOR_SHELL;
