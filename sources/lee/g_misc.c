...
{
//1 disable LA
//	gi.SetAreaPortalState (ent->style, 1);
...
void SP_light (edict_t *self)
{
	...
	if (!self->targetname || ctf->value || deathmatch->value)
	{
		G_FreeEdict (self);
		return;
	}
...
/*		gi.WriteByte (svc_temp_entity);
		...
		self->think = SP_misc_explobox;		
		//self->model = " ";
		//self->model = gi.modelindex ("sprites/s_explo2.sp2");		
	} else {
	if (self->groundentity)
