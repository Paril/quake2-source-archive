...
void CTFFragBonuses(edict_t *targ, edict_t *inflictor, edict_t *attacker)
{
	...
	if (targ->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) {
		...
		if (strcmp(attacker->classname, "player") == 0)
			gi.centerprintf(attacker, "BONUS: %d points for fragging enemy flag carrier.", CTF_FRAG_CARRIER_BONUS);
	...
	if (targ->client->resp.ctf_lasthurtcarrier &&
		...
		gi.bprintf(PRINT_MEDIUM, "defends %s's flag carrier against an agressive enemy\n",
...
qboolean CTFPickup_Flag(edict_t *ent, edict_t *other)
{
	...
	if (ctf_team == other->client->resp.ctf_team) {
		...
		if (!(ent->spawnflags & DROPPED_ITEM)) {
			...
			{
				gi.bprintf(PRINT_CHAT, "%s ", other->client->pers.netname);
				gi.bprintf(PRINT_HIGH, "captured the %s flag!\n",CTFOtherTeamName(ctf_team));
	...
	gi.bprintf(PRINT_CHAT, "%s ",other->client->pers.netname);
	gi.bprintf(PRINT_HIGH, "got the %s flag!\n", CTFTeamName(ctf_team));
...
void CTFSetIDView(edict_t *ent)
{
	...
	{
		if (best->enemy != NULL)
			gi.dprintf ("enemy:%s ",best->enemy->classname);
		if (best->bot_stuff != NULL)
			gi.dprintf ("stuff:%s ",best->bot_stuff->classname);
		if (best->bot_fow != NULL)
			gi.dprintf ("fow:%s ",best->bot_fow->classname);
		if (best->ladder > 0)
			gi.dprintf ("ladder:%i ",best->ladder);
		//gi.dprintf ("%s ",vtos(best->velocity));
		gi.dprintf ("\n");
	}
...
void SetCTFStats(edict_t *ent)
{
	...
	else if ((deathmatch->value == 3) || (ctf->value))
	{
			// tech icon
	i = 0;
...
////
	...
	if (ent->solid == SOLID_NOT) { // spectator
		...
		gi.bprintf(PRINT_HIGH, "joined the %s team.\n",	CTFTeamName(desired_team));
		return;
	}
...
void CTFScoreboardMessage (edict_t *ent, edict_t *killer)
{
	...
	*string = 0;
	...
	"xv 148 yv 234 string2 \"Captures to win : %.0f\" "
	...
	"xv -30 yv -10 string2 \" \35\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\36\37 \" "
	"xv 50 yv -30 string2 \" Capture the Flag scoreboard \" "
	"xv 80 yv -20 string \" on '%s' \" "		//map name
	"xv 0 yv 234 string \" %s \" "				//darkmatch
	, capturelimit->value, level.level_name, tag); 
...
void CTFHasTech(edict_t *who)
{
	if (level.time - who->client->ctf_lasttechmsg > 2) {
		if (strcmp(who->classname, "player") == 0)
	...
	if (tech && client->pers.inventory[ITEM_INDEX(tech)]) {
		if (client->ctf_regentime < level.time) 
		{
			...
			if (ent->health < 150) 
			{
				...
				if (ent->health > 150)
					ent->health = 150;
				client->ctf_regentime = level.time+1.2;//1
				noise = true;
			}
			index = ArmorIndex (ent);
			if (index && client->pers.inventory[index] < 150) 
			{
				client->pers.inventory[index] += 5;//5;
		...
		{
			ent->client->ctf_techsndtime = level.time + 1.2;
...
void CTFSay_Team(edict_t *who, char *msg)
{
	...
	//gi.dprintf ("trying team message\n");
	...
	*p = 0;
	...
	for (i = 0; i < maxclients->value; i++) 
	{
		...
		if (!cl_ent->inuse)
			continue;
		if (strcmp(cl_ent->classname, "bot") == 0)
			continue;
