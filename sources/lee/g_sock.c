...
qboolean InitSockets(void)
{
	...
	if ( err != 0 ) 
	{
		/* Tell the user that we couldn't find a usable 
		   WinSock DLL.                                 */    
		gi.dprintf("InitSocks::Could not find a usable Winsock dll\n");
		return false;
	} 
         ...
         HIBYTE( wsaData.wVersion ) != 1) 
	{
		/* Tell the user that we couldn't find a usable 
		   WinSock DLL.                                 */    
		WSACleanup();
		gi.dprintf("InitSocks:Could not find Winsock 2.0\n");
		return false;
	}  
	...
	if (hp == NULL)										
	{
		gi.dprintf("InitSocks:Bad Host Address\n");
		return false;
	}
	...
	if(!setsockopt(slisten,SOL_SOCKET,SO_REUSEADDR,"1",sizeof(int)))
		gi.dprintf("InitSocks:Socket Options set\n");
	else
		gi.dprintf("InitSocks:Error setting Socket Options:%d\n",WSAGetLastError());
	...
	{
		closesocket(slisten);
		gi.dprintf("InitSocks:Could not bind socket\n");
		return false;
	}
	gi.dprintf("InitSocks:socket bound and ready\n");
...
qboolean ReadMsg(gclient_t *client, char *out)
{
	int rc;

	if(!client->sock.sconnected)
	{
		gi.dprintf("ReadMsg:Error receiving - %s is not connected\n",client->pers.netname);
		return false;
	}
	...
	if (rc ==0) 
	{
		printf("ReadMsg: %s:Connection closed by client\n",client->pers.netname);
...
qboolean SendMsg(gclient_t *client, char msgid, char *midi)
{
	...
	if(!client->sock.sconnected)
	{
		gi.dprintf("SendMsg:Error - %s is not connected\n",client->pers.netname);
		return false;
	}
	...
	{
		gi.dprintf("SendMsg:Error sending midi info to %s\n",client->pers.netname);
		PrintSockError();
		return false;
	}
...
qboolean Connect(gclient_t *client)
{
...
//					if(!memcmp(cl->ipaddr,ip,sizeof(ip)))
		...
		gi.dprintf("ChkSockState::Error:%d\n",WSAGetLastError());
	...
	for(i=0;i<maxclients->value;i++)
	{
		...
		if(cl->pers.mstate > 4)
		{
				...
				(cl->pers.mtry < 4))
			{
				...
				{
				case CWPLAY:
					SendMsg(cl,MSG_PLAY,level.midiname);
					break;
				case CWPAUSE:
					SendMsg(cl,MSG_PAUSE,NULL);
					break;
				case CWRESUME:
					SendMsg(cl,MSG_RESUME,NULL);
					break;
				case CWSTOP:
					SendMsg(cl,MSG_STOP,NULL);
					break;
				case CWERROR:
					SendMsg(cl,MSG_ERROR,NULL);
					break;
				}
			}
		}
