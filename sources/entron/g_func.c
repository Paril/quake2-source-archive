...
void c_rotating_use (edict_t *self, edict_t *other, edict_t *activator);
...
//	ent->moveinfo.sound_middle = "doors/hydro1.wav";
   ...
   if (ent->delay && ent->wait)
	   ent->use = c_rotating_use;
   else
...
//shogun: lighting function, puts a constant beam between itself and its target

void lightning_think (edict_t *self)
{
	int damage;
	trace_t tr;
	vec3_t dir;
	...
	if (self->target_ent != NULL)
	{
		...
		VectorNormalize(dir);
		// find something to damage in path of beam
		tr = gi.trace (self->s.origin, NULL, NULL, self->target_ent->s.origin, self, MASK_SHOT);
		if(tr.fraction < 1)
		{
			...
			{ 
				...
				//T_Damage (edict_t *targ, edict_t *inflictor, edict_t *attacker, vec3_t inflictdir, vec3_t point, vec3_t normal, int damage, int knockback, int dflags, int mod)
				if(damage > 0)
					T_Damage (tr.ent, self, self, vec3_origin, tr.ent->s.origin, vec3_origin, damage, 1, 0, MOD_PLASMA);
			}
			else
			{
				gi.WriteByte (svc_temp_entity);
...
{
//	gi.dprintf ("func_lighting: being used: ");

	self->activator = activator;

	if(self->moveinfo.state == STATE_BOTTOM)
	{
//		gi.dprintf ("turning on!\n");
	...
	{
//		gi.dprintf ("turning off!\n");
		self->nextthink = 0;
		self->moveinfo.state = STATE_BOTTOM;
	}
}

void func_lightning_start (edict_t *self)
{
	if (!self->target)
		gi.dprintf ("func_lightning without a target at %s\n", vtos(self->absmin));
	else
	{
		self->target_ent = G_PickTarget (self->target);
		if (!self->target_ent)
		{
			gi.dprintf ("func_lighting: target %s not found\n", self->target);
			return;
		}
	}
}
...
void SP_func_lightning (edict_t *self)
{
	self->movetype = MOVETYPE_NONE;
	self->solid = SOLID_NOT;
	self->use = lightning_use;
	self->moveinfo.state = STATE_BOTTOM;
	self->think = func_lightning_start;
	self->nextthink = level.time + 0.3 + random() * 0.3; 
	gi.linkentity (self);
}
