...
// delay reaction so if the monster is teleported, its sound is still heard
	if (self->scr.num_intercepts)
		{
		script_use(self, other, activator);
		return;
		}
	kill_script(self);	
...
void walkmonster_start (edict_t *self)
{
	...
	self->monsterinfo.shots = 1;
	kill_script(self);
	monster_start (self);
	if (self->scriptfile)
		load_script(self);
	if (self->scr.on_touch >= 0)
