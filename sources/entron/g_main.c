...
void ClientEndServerFrames (void)
{
	...
	for (ent ; ent < &g_edicts[globals.num_edicts] ; ent++)
      {
      if (ent->flipping == FLIP_LIGHT)
         {
			//ent->s.frame = rand() & 7;
         ent->s.frame += ent->style;
         if (ent->s.frame > 10 || ent->s.frame < 0)
				{
            ent->style = -ent->style;
				ent->s.frame += ent->style;
				}
