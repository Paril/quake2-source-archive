...
void SP_light (edict_t *self)
{
   ...
   switch (self->team_data)
      {
      case 1:
         self->s.modelindex = gi.modelindex("models/objects/flame1/tris.md2");
         self->s.renderfx = RF_TRANSLUCENT;
         self->flipping = FLIP_LIGHT;
			self->s.frame = rand() & 7;
			self->style = 1;
         gi.linkentity(self);
         break;
      case 2:
         self->s.modelindex = gi.modelindex("models/objects/flame2/tris.md2");
         self->s.renderfx = RF_TRANSLUCENT;
         self->flipping = FLIP_LIGHT;
			self->s.frame = rand() & 7;
			self->style = 1;
         ...
         break;
      case 3:
         self->s.modelindex = gi.modelindex("models/objects/flame3/tris.md2");
         self->s.renderfx = RF_TRANSLUCENT;
         self->flipping = FLIP_LIGHT;
			self->s.frame = rand() & 7;
			self->style = 1;
         ...
         break;
       case 4:
         self->s.modelindex = gi.modelindex("sprites/l_w1.sp2");
         self->s.renderfx = RF_TRANSLUCENT;
         gi.linkentity(self);
         break;
      }
...

{
	...
	BOT_walkmove (self, vectoyaw(v), 20 * ratio * FRAMETIME);
}
...
void SP_misc_banner (edict_t *ent)
{
   ...
   switch (ent->style)
      ...
      case 0:
         ...
         break;
      case 1:
         ent->s.modelindex = gi.modelindex ("models/objects/banner/small.md2");
         ent->think = misc_banner_think;	
	      ent->nextthink = level.time + FRAMETIME;
         break;
      default:
         if (ent->style > 1 && ent->style <= 20)
            {
            if (ent->style & 1)
               {
               sprintf (x, "models/objects/banners/banner%d/tris2.md2", ent->style / 2);
               }
            else
               {
               sprintf (x, "models/objects/banners/banner%d/tris.md2", ent->style / 2);
               ent->think = misc_banner_think;	
	            ent->nextthink = level.time + FRAMETIME;
               }
            ent->s.modelindex = gi.modelindex(x);
            }
         else
            {
            G_FreeEdict(ent);
