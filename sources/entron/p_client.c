...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	if (deathmatch->value || coop->value)
	{
		...
		if (attacker == self)
		{
			...
			// CCH
			case MOD_RIPPER:
				message = "didn't get out of the way";
				break;
		...
		if (attacker && attacker->client)
		{
			...
			case MOD_HAMMER:
				message = "was hammered down by";
				break;
			case MOD_RIPPER:
				message = "just got sliced in half by";
				break;
			case MOD_PLASMA:
...
void player_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
   ...
   if (self->client->flashlight)
	...
	self->s.modelindex3 = 0;	// remove linked armor model
...
//	self->solid = SOLID_NOT;
	...
	if (self->client->locked)
		{
		self->client->locked->s.renderfx &= ~RF_SHELL_RED;
		gi.linkentity(self->client->locked);
		self->client->locked = NULL;
		}
		...
		if (!self->deadflag)
		   {
			...
			if (self->client->ps.pmove.pm_flags & PMF_DUCKED)
			   {
				self->s.frame = FRAME_CrouchDie_start - 1;
				self->client->anim_end = FRAME_CrouchDie_end;
			   }
			else switch (i)
			   {
			   case 0:
				   self->s.frame = FRAME_Die1_start - 1;
				   self->client->anim_end = FRAME_Die1_end - 1;
				   break;
			   case 1:
				   self->s.frame = FRAME_Die2_start - 1;
				   self->client->anim_end = FRAME_Die2_end;
				   break;
			   case 2:
				   self->s.frame = FRAME_Die2_start - 1;
				   self->client->anim_end = FRAME_Die2_end;
...
void InitClientPersistant (gclient_t *client)
{
   ...
   client->pers.weapon = item;
	...
	client->pers.max_srounds      = 10;
   client->pers.max_cells	      = 50;

   client->pers.max_liquidfire   = 50;
	client->pers.max_plasma		   = 50;
	client->pers.max_discs		   = 25;

   client->pers.max_rockets      = 20;
	client->pers.max_clip	      = 50;
...
void CopyToBodyQue (edict_t *ent)
{
   ...
   body->s.origin[2] -= 18;
   body->style = 0;
   if (gi.pointcontents (body->s.origin) & MASK_WATER)
      {
      body->nextthink = level.time + 3;
      body->think = floatbody;
      }
   body->s.origin[2] += 18;
   body->flipping &= ~FLIP_WATER;
   //body->flipping |= FLIP_DEADBODY;
	gi.linkentity (body);
...
void PutClientInServer (edict_t *ent)
{
   ...
   ent->client->anim_end = FRAME_Stand_end;
   ...
   if (!deathmatch->value && !coop->value)
      {
	   ent->s.frame = FRAME_stand01;
		ent->client->anim_end = FRAME_stand30;
...
void ClientThink (edict_t *ent, usercmd_t *ucmd)
{
   ...
   level.current_entity = ent;
	...
	if (inscript)
		{
		if (ent->client->fov_speed > 0)
			{
			ent->client->ps.fov += ent->client->fov_speed;
			if (ent->client->ps.fov >= ent->client->fov_destination)
				{
				ent->client->fov_speed = 0;
				ent->client->ps.fov = ent->client->fov_destination;
				}
			}
		else
			{
      ...
      if (ent->status & STATUS_SCENE_VIEW)
         camera_think_scene (ent->client->resp.camera);
		return;
		}
   ...
   if (ent->client->camera_model && !(ent->status & STATUS_3RD_VIEW)) 
      {
      ent->client->ps.pmove.pm_type = PM_FREEZE;
      VectorClear(ent->velocity);
      //if (ent->status & STATUS_SCENE_VIEW)
   ...
   client->sidemove = ucmd->sidemove;
	...
	if (client->latched_buttons & BUTTON_ATTACK)
      if (!client->weapon_thunk)
         {
         if (client->pers.weapon && client->reload_time >= client->pers.weapon->quantity)
            weapon_reload(ent);
         else 
            {
