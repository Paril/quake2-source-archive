...
field_t fields[] = {
   // Topaz:
   {"team_data", FOFS(team_data), F_INT},
   {"flame", FOFS(team_data), F_INT},
   {"skin", FOFS(team_data), F_INT},
	{"scriptfile", FOFS(scriptfile), F_LSTRING},
...
void InitGame (void)
{
	...
	// CCH
	beh_items = gi.cvar("beh_items", "0", CVAR_SERVERINFO | CVAR_LATCH);
	allow_grapple = gi.cvar("allow_grapple", "0", CVAR_SERVERINFO | CVAR_LATCH);
	allow_tech = gi.cvar("allow_tech", "0", CVAR_SERVERINFO | CVAR_LATCH);
...
void ReadLevel (char *filename)
{
   ...
   // Topaz: wipe all the old nodes
	memset (g_nodes, 0, MAX_NODES * sizeof(g_nodes[0]));
   num_nodes = 0;

   // Topaz: wipe all the old nodes
	memset (g_ambushes, 0, MAX_NODES * sizeof(g_ambushes[0]));
   num_ambushes = 0;
