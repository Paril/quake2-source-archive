...
void brain_tentacle_retract (edict_t *self)
{
...
/*	AngleVectors (self->s.angles, forward, right, NULL);
	...
	VectorMA(start, 8192, aim, end);
	tr = gi.trace(start, NULL, NULL, end, self, MASK_SHOT);
	if ( tr.ent )
	{
		VectorScale(aim, -300, aim);
		VectorAdd(aim, tr.ent->velocity, tr.ent->velocity);
	}*/

	VectorSet (aim, MELEE_DISTANCE, 0, 8);
	if (fire_hit (self, aim, (35 + (rand() %5)), 0) && skill->value > 0)
		self->spawnflags |= 65536;

	// Cheap and very buggy!
	//VectorSet (aim, 8000, 0, 8);
	//fire_hit (self, vec, 0, -300);
	//gi.sound (self, CHAN_WEAPON, sound_tentacles_retract, 1, ATTN_NORM, 0);
}
...
void brain_tentacle_attack_repeat (edict_t *self);
...
mframe_t brain_frames_attack2 [] =
{
	...
	ai_charge,	0,	brain_tentacle_retract,
	ai_charge,	13,	brain_tentacle_attack,
	ai_charge,	0,	brain_tentacle_attack,
	ai_charge,	2,	brain_tentacle_attack,
	ai_charge,	0,	brain_tentacle_attack,
...
mmove_t brain_move_attack2 = {FRAME_attak201, 80, brain_frames_attack2, brain_tentacle_attack_repeat};

mframe_t brain_frames_attack2_stop [] =
{
	ai_charge,	-9,	brain_chest_closed,
	ai_charge,	0,	NULL,
	ai_charge,	4,	NULL,
...
mmove_t brain_move_attack2_stop = {81, 87, brain_frames_attack2_stop, brain_run};
...
void brain_tentacle_attack_repeat (edict_t *self)
{
	...
	self->monsterinfo.currentmove = &brain_move_attack2_stop;
}
...
// regular death
	...
	if (self->activator->client->pers.selected == self)
...
void SP_monster_brain (edict_t *self)
{
	...
	self->s.modelindex = gi.modelindex ("players/brain/tris.md2");
	VectorSet (self->mins, -16, -16, -24);
	...
	self->monsterinfo.attack = brain_melee;
