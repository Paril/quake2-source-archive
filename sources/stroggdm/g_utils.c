...
float returnandfindspawnpoint (vec3_t mins, vec3_t maxs, vec3_t out)
{
	vec3_t loc = {0,0,0};
	vec3_t floor;
	int i;
	...
	do 
	{
		...
		if (gi.pointcontents(loc) == 0)
		{
			...
			if (tr.contents & MASK_WATER)
				continue; 
			VectorCopy (tr.endpos, loc);
			loc[2] += maxs[2] - mins[2]; // make sure the entity can fit!
		}
	} while (gi.pointcontents(loc) > 0 && j < 1000 && k < 500);
...
void G_FreeMon (edict_t *ed)
{

	ed->activator->client->resp.spawned_monsters--;
