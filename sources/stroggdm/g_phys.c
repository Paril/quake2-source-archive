...
// add gravity
	...
	&& ent->movetype != MOVETYPE_FLYMISSILE	// RAFAEL
	// move type for rippergun projectile
	&& ent->movetype != MOVETYPE_WALLBOUNCE
	&& ent->movetype != MOVETYPE_MORE_BOUNCE)
		SV_AddGravity (ent);
...
// move origin
	...
	if (trace.fraction < 1)
	{
		...
		// RAFAEL ( else )	
			...
			backoff = 1.5;
		else if (ent->movetype == MOVETYPE_MORE_BOUNCE)
			backoff = 3;
		else
			backoff = 1;
		...
		// RAFAEL
		if (ent->movetype == MOVETYPE_MORE_BOUNCE)
			vectoangles (ent->velocity, ent->s.angles);
	// stop if on ground
		// RAFAEL
		if (trace.plane.normal[2] > 0.7 && ent->movetype != MOVETYPE_WALLBOUNCE && ent->movetype != MOVETYPE_MORE_BOUNCE)
		{		
