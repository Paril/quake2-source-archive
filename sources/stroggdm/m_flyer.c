...
void flyer_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (self->owner == other)
	{
		other->client->resp.spawned_monsters --;
		safe_cprintf (other, PRINT_HIGH, "A spawned flyer has died!\n");
	}
...
void SP_monster_flyer (edict_t *self)
{
	...
	self->s.modelindex = gi.modelindex ("players/flyer/tris.md2");
