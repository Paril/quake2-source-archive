...
void parasite_drain_attack_health (edict_t *self)
{
	...
	if (self->client->ps.gunframe == 3)
	{
		gi.sound (self, CHAN_AUTO, gi.soundindex("parasite/paratck1.wav"), 1, ATTN_NORM, 0);
	}
	if (self->client->ps.gunframe == 4 && tr.ent->takedamage)
	{
		damage = 8;
		gi.sound (self, CHAN_AUTO, gi.soundindex("parasite/paratck2.wav"), 1, ATTN_NORM, 0);
	}
	else
	{
		if (self->client->ps.gunframe == 5 && tr.ent->takedamage)
			gi.sound (self, CHAN_WEAPON, gi.soundindex("parasite/paratck3.wav"), 1, ATTN_NORM, 0);
		damage = 3;
	}
	if (self->client->ps.gunframe == 15)
	{
...
// Medic stuff
void medicplain_heal (edict_t *self)
{
	vec3_t	offset, start, f, r, end, dir;
	trace_t	tr;
	int damage;
	...
	if (self->client->ps.gunframe == 9 && tr.ent->takedamage)
	{
		damage = 12;
		gi.sound (self, CHAN_AUTO, gi.soundindex("medic/medatck3.wav"), 1, ATTN_NORM, 0);
	}
	else
	{
		if (self->client->ps.gunframe == 10 && tr.ent->takedamage)
			gi.sound (self, CHAN_WEAPON, gi.soundindex("medic/medatck4.wav"), 1, ATTN_NORM, 0);
	...
	if (tr.ent->client)
	{
		...
		{
			if (self->client->resp.landtype == tr.ent->client->resp.landtype)
				tr.ent->health += damage;
		}
		else if (teamplay_classes->value)
		{
			if (self->client->resp.class == tr.ent->client->resp.class)
				tr.ent->health += damage;
		}
		else if (ctf->value)
		{
			if (self->client->resp.ctf_team == tr.ent->client->resp.ctf_team)
				tr.ent->health += damage;
		}
		// Beh, many combinations.
		else if (targ->client->resp.party.has_party && attacker->client->resp.in_party)
		{
		...
		else if (attacker->client->resp.in_party && targ->client->resp.in_party)
		{
			...
			T_Damage (self->enemy, self, self, dir, self->enemy->s.origin, vec3_origin, damage, 0, DAMAGE_NO_KNOCKBACK, MOD_BLASTER);
	}
	else if (tr.ent->team_owner == self->team_owner)
	{
		tr.ent->health += damage;
	}
	else
...
void medic_heal (edict_t *self)
{
	...
	{
		damage = 8;
		gi.sound (self, CHAN_AUTO, gi.soundindex("medic_commander/medatck3a.wav"), 1, ATTN_NORM, 0);
	}
	else
	{
		if (self->client->ps.gunframe == 10 && tr.ent->takedamage)
			gi.sound (self, CHAN_WEAPON, gi.soundindex("medic_commander/medatck4a.wav"), 1, ATTN_NORM, 0);
...
void parasite_drain (edict_t *self)
{
	vec3_t	offset, start, f, r, end, dir;
	trace_t	tr;
	int damage;
...
void SP_monster_parasite (edict_t *self)
{
	...
	self->s.modelindex = gi.modelindex ("players/parasite/tris.md2");
	VectorSet (self->mins, -16, -16, -24);
