...
edict_t *medic_FindHurtPlayer (edict_t *self)
{
	edict_t	*ent = NULL;
	edict_t	*best = NULL;
...
void medic_idle (edict_t *self)
{
	...
	//	FoundTarget (self);
	//}
	//ent = medic_FindHurtPlayer(self);
	//if (ent)
	//{
		//if (self->enemy)
			//return;
		//gi.bprintf (PRINT_HIGH, "Found someone..\n");
		//self->enemy = ent;
		//self->enemy->owner = self;
		//self->monsterinfo.aiflags |= AI_MEDIC;
		//FoundTarget (self);
...
void medic_fire_blaster (edict_t *self)
{
	...
	flash_number = MZ2_MEDIC_BLASTER_1;
...
/*	AngleVectors (self->s.angles, forward, right, NULL);
	...
	monster_fire_blaster (self, start, forward, dammulti, 1700, flash_number, effect);
}
...
mmove_t medic_move_attackHyperBlaster = {FRAME_attack19, FRAME_attack30, medic_frames_attackHyperBlaster, medic_run};
...
void medic_cable_heal (edict_t *self)
{
	self->enemy->health ++;
}
...
void SP_monster_medic (edict_t *self)
{
	...
	self->s.modelindex = gi.modelindex ("players/medic/tris.md2");
