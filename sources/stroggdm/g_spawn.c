...
// parse ents
	...
	if (logging->value)
		fprintf (logged, "SERVER: SpawnEntities successfully initiated.\n");
...
void SP_worldspawn (edict_t *ent)
{
  ...
  gi.configstring(30,va("%i",game.maxclients));

	if (logging->value)
		fprintf (logged, "SERVER: Finished setting up worldspawn settings\n");
...
//ZOID
	...
	if (logging->value)
		fprintf (logged, "SERVER: Finished precaching sounds\n");

	PrecacheItem (FindItem ("Soldier Blaster"));
	...
	if (logging->value)
		fprintf (logged, "SERVER: Finished precaching models\n");


	// Paril
	// Config Strings
	// Caused Index Overflow, can't use
	//InitSoundArray();
//
...
//
  ...
  if (logging->value)
		fprintf (logged, "SERVER: Finished setting up lights\n");
