...
void Chunk_Touch (edict_t *gib,edict_t *other,cplane_t *plane,csurface_t *surf) 
{
	if (other == world) 
		return;
...
void ThrowDebris (edict_t *self, char *modelname, float speed, vec3_t origin, edict_t *owner)
{
	...
	if (Q_stricmp(modelname, "models/objects/gibs/sm_meat/tris.md2") == 0)
...
void SP_misc_eastertank (edict_t *ent)
{
	...
	ent->s.modelindex = gi.modelindex ("players/tank/tris.md2");
	ent->s.frame = 254;
