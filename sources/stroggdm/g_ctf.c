...
int CTFApplyProjectile(edict_t *targ, edict_t *ent, int dmg, int dmg_type)
{
	...
	if (dmg && tech && targ->client && (dmg_type & 4) && targ->client->pers.inventory[ITEM_INDEX(FindItem ("Projectile Shield"))]) 
	{
		dmg = 0;
		gi.sound (targ, CHAN_AUTO, gi.soundindex ("brain/Brnpain2.wav"), 1, ATTN_NORM, 0);
	}
	return dmg;
}
	...
	for (i = 0; i < maxclients->value; i++) {
		...
		if (cl_ent->client->resp.ctf_team == who->client->resp.ctf_team)
		{
			if (!ent->client->resp.class)
				safe_cprintf (cl_ent, PRINT_CHAT, "([No Class]%s): %s\n", ent->client->pers.netname, outmsg);
			else
				safe_cprintf (cl_ent, PRINT_CHAT, "([%s]%s): %s\n", monstername, ent->client->pers.netname, outmsg);
...
void CTFJoinTeam(edict_t *ent, int desired_team)
{
	...
	//PutClientInServer (ent);
	// The observer mode will end automatically by new functions
	// -Paril
...
// Trapped tech
void Touch_Trapped_Item (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	qboolean	taken;
	...
	//T_RadiusDamage (ent, ent->owner, other->client->pers.health / 3, NULL, 80, MOD_R_SPLASH);
	T_Damage (other, ent, ent->owner, ent->s.origin, vec3_origin, vec3_origin, other->client->pers.health / 3, 20, DAMAGE_ENERGY, MOD_R_SPLASH);
}
...
void SpawnTrappedTech(edict_t	*owner)
{
	...
	if (j == 0)
		...
		model = "models/ctf/projectile/tris.md2";

	ent = G_Spawn();

	ent->classname = "trapped_tech";
	ent->spawnflags = DROPPED_ITEM;
	ent->s.effects = EF_ROTATE | EF_HYPERBLASTER;
	ent->s.renderfx = RF_GLOW;
	VectorSet (ent->mins, -15, -15, -15);
	VectorSet (ent->maxs, 15, 15, 15);
