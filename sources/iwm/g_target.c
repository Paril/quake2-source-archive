...
#ifdef _WIN32
extern __declspec(dllexport) void SP_target_speaker (edict_t *self);
#endif
...
#define LASER_CORNER_POINTER 1024
...
void target_laser_think (edict_t *self)
{
	...
	if (self->enemy)
	{
		...
		if (self->spawnflags & LASER_CORNER_POINTER)
		{
			point[2] += 14;
			point[0] += 32;
			point[1] += 24;
	...
	if (self->tempint)
		VectorMA (start, self->tempint, self->movedir, end);
	else
	...
	if (self->spawnflags & LASER_CORNER_POINTER)
	{
		...
		VectorSubtract (point, self->s.origin, hah);
		d = VectorLength(hah);
		VectorNormalize (hah);

		VectorMA (start, d, self->movedir, end);
		//end[2] += 14;
		//end[0] += 32;
		//end[1] += 24;
	}
