...
void CTFInit(void)
{
	ctf = gi.cvar("ctf", "0", CVAR_LATCH|CVAR_SERVERINFO);
...
qboolean CTFPickup_Flag(edict_t *ent, edict_t *other)
{
	...
	if (ctf_team == other->client->resp.ctf_team) {
		...
		if (!(ent->spawnflags & DROPPED_ITEM)) {
			...
			if (other->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) {
				...
				{
					ctfgame.team1++;
					SendSoundToAll (SoundIndex ("announcer/redscores.wav"), 1);
				}
				else
				{
					ctfgame.team2++;
					SendSoundToAll (SoundIndex ("announcer/bluescores.wav"), 1);
				}

				gi.sound (ent, CHAN_RELIABLE+CHAN_NO_PHS_ADD+CHAN_VOICE, SoundIndex("ctf/flagcap.wav"), 1, ATTN_NONE, 0);
		...
		gi.sound (ent, CHAN_RELIABLE+CHAN_NO_PHS_ADD+CHAN_VOICE, SoundIndex("ctf/flagret.wav"), 1, ATTN_NONE, 0);
	...
	for (i = 1; i <= maxclients->value; i++) 
	{
		...
		if (player->client->resp.ctf_team == other->client->resp.ctf_team)
			unicastSound (player, SoundIndex("announcer/yourteamhas.wav"), 1);
		else if (player->client->resp.ctf_team != other->client->resp.ctf_team)
			unicastSound (player, SoundIndex ("announcer/theenemyhas.wav"), 1);
		// "else" is spectators. We don't want to confuse them.
			
	}
	unicastSound (other, SoundIndex("announcer/youhaveflag.wav"), 1);
...
static void CTFDropFlagThink(edict_t *ent)
{
	...
	if (strcmp(ent->classname, "item_flag_team1") == 0) {
		...
		SendSoundToAll (SoundIndex("announcer/redreturned.wav"), 1);
	} else if (strcmp(ent->classname, "item_flag_team2") == 0) {
		CTFResetFlag(CTF_TEAM2);
		safe_bprintf(PRINT_HIGH, "The %s flag has returned!\n",
			CTFTeamName(CTF_TEAM2));
		SendSoundToAll (SoundIndex("announcer/bluereturned.wav"), 1);
	}
}
...
void CTFEffects(edict_t *player)
{
	...
	if (player->client->pers.inventory[ITEM_INDEX(flag1_item)])
		player->s.modelindex3 = ModelIndex("players/male/flag1.md2");
	else if (player->client->pers.inventory[ITEM_INDEX(flag2_item)])
		player->s.modelindex3 = ModelIndex("players/male/flag2.md2");
	else
...
void SetCTFStats(edict_t *ent)
{
	...
	ent->client->ps.stats[STAT_CTF_TEAM1_HEADER] = ImageIndex ("ctfsb1");
	ent->client->ps.stats[STAT_CTF_TEAM2_HEADER] = ImageIndex ("ctfsb2");
	...
	while (tnames[i]) {
		if ((tech = FindItemByClassname(tnames[i])) != NULL &&
			...
			ent->client->ps.stats[STAT_CTF_TECH] = ImageIndex(tech->icon);
			break;
		}
		i++;
	}
	...
	if (ent->client->resp.ctf_team == CTF_TEAM1 &&
		...
		(level.framenum & 8))
		ent->client->ps.stats[STAT_CTF_FLAG_PIC] = ImageIndex ("i_ctf2");

	else if (ent->client->resp.ctf_team == CTF_TEAM2 &&
		ent->client->pers.inventory[ITEM_INDEX(flag1_item)] &&
		(level.framenum & 8))
		ent->client->ps.stats[STAT_CTF_FLAG_PIC] = ImageIndex ("i_ctf1");

	ent->client->ps.stats[STAT_CTF_JOINED_TEAM1_PIC] = 0;
	ent->client->ps.stats[STAT_CTF_JOINED_TEAM2_PIC] = 0;
	if (ent->client->resp.ctf_team == CTF_TEAM1)
		ent->client->ps.stats[STAT_CTF_JOINED_TEAM1_PIC] = ImageIndex ("i_ctfj");
	else if (ent->client->resp.ctf_team == CTF_TEAM2)
		ent->client->ps.stats[STAT_CTF_JOINED_TEAM2_PIC] = ImageIndex ("i_ctfj");
...
void CTFResetGrapple(edict_t *self)
{
	if (self->owner->client->ctf_grapple) {
		...
		gi.sound (self->owner, CHAN_RELIABLE+CHAN_WEAPON, SoundIndex("weapons/grapple/grreset.wav"), volume, ATTN_NORM, 0);
...
void CTFGrappleTouch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage) {
		T_Damage (other, self, self->owner, self->velocity, self->s.origin, plane->normal, self->dmg, 1, 0, MOD_HOOK);
		CTFResetGrapple(self);
		return;
	}
	...
	gi.sound (self->owner, CHAN_RELIABLE+CHAN_WEAPON, SoundIndex("weapons/grapple/grpull.wav"), volume, ATTN_NORM, 0);
	gi.sound (self, CHAN_WEAPON, SoundIndex("weapons/grapple/grhit.wav"), volume, ATTN_NORM, 0);
...
void CTFGrappleDrawCable(edict_t *self)
{
	...
	P_ProjectSource (self->owner->client, self->owner->s.origin, offset, f, r, start, 0);
...
void CTFGrapplePull(edict_t *self)
{
	...
	if (self->enemy) {
		...
		if (self->enemy->takedamage &&
			...
			gi.sound (self, CHAN_WEAPON, SoundIndex("weapons/grapple/grhurt.wav"), volume, ATTN_NORM, 0);
		}
	...
	if (self->owner->client->ctf_grapplestate > CTF_GRAPPLE_STATE_FLY) {
		...
		if (self->owner->client->ctf_grapplestate == CTF_GRAPPLE_STATE_PULL &&
			...
			gi.sound (self->owner, CHAN_RELIABLE+CHAN_WEAPON, SoundIndex("weapons/grapple/grhang.wav"), volume, ATTN_NORM, 0);
...
void CTFFireGrapple (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect)
{
	...
	grapple->s.modelindex = ModelIndex ("models/weapons/grapple/hook/tris.md2");
//	grapple->s.sound = SoundIndex ("misc/lasfly.wav");
	grapple->owner = self;
...
//	VectorSet(offset, 24, 16, ent->viewheight-8+2);
	...
	gi.sound (ent, CHAN_RELIABLE+CHAN_WEAPON, SoundIndex("weapons/grapple/grfire.wav"), volume, ATTN_NORM, 0);
...
int CTFApplyResistance(edict_t *ent, int dmg)
{
	...
	if (dmg && tech && ent->client && ent->client->pers.inventory[ITEM_INDEX(tech)]) {
		// make noise
	   	gi.sound(ent, CHAN_VOICE, SoundIndex("ctf/tech1.wav"), volume, ATTN_NORM, 0);
		return dmg / 2;
	}
	return dmg;
}
	...
	if (tech && ent->client &&
		...
		if (ent->client->ctf_techsndtime < level.time) {
			...
			if (ent->client->quad_framenum > level.framenum)
				gi.sound(ent, CHAN_VOICE, SoundIndex("ctf/tech2x.wav"), volume, ATTN_NORM, 0);
			else
				gi.sound(ent, CHAN_VOICE, SoundIndex("ctf/tech2.wav"), volume, ATTN_NORM, 0);
		}
		return true;
	}
	return false;
...
void CTFApplyHasteSound(edict_t *ent)
{
	...
	if (tech && ent->client &&
		...
		ent->client->ctf_techsndtime = level.time + 1;
		gi.sound(ent, CHAN_VOICE, SoundIndex("ctf/tech3.wav"), volume, ATTN_NORM, 0);
	}
}
...
void CTFApplyRegenerationTech(edict_t *ent)
{
	...
	if (tech && client->pers.inventory[ITEM_INDEX(tech)]) {
		...
		if (noise && ent->client->ctf_techsndtime < level.time) {
			ent->client->ctf_techsndtime = level.time + 1;
			gi.sound(ent, CHAN_VOICE, SoundIndex("ctf/tech4.wav"), volume, ATTN_NORM, 0);
		}
	}
}
...
void SP_misc_ctf_banner (edict_t *ent)
{
	...
	ent->solid = SOLID_NOT;
	ent->s.modelindex = ModelIndex ("models/ctf/banner/tris.md2");
...
void SP_misc_ctf_small_banner (edict_t *ent)
{
	ent->movetype = MOVETYPE_NONE;
	ent->solid = SOLID_NOT;
	ent->s.modelindex = ModelIndex ("models/ctf/banner/small.md2");
	if (ent->spawnflags & 1) // team2
		ent->s.skinnum = 1;
...
void CTFObserver(edict_t *ent)
{
	...
	if (ent->movetype == MOVETYPE_NOCLIP) {
		safe_cprintf(ent, PRINT_HIGH, "You are already an observer.\n");
		return;
	}
...
void SP_trigger_teleport (edict_t *ent)
{
	...
	s->s.sound = SoundIndex ("world/hum1.wav");
	gi.linkentity(s);
