...
void fire_flechette (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int kick)
{
	...
	flechette->s.modelindex = ModelIndex ("models/proj/flechette/tris.md2");

//	flechette->s.sound = SoundIndex ("");			// FIXME - correct sound!
...
//	flechette->s.frame = flechette->s.skinnum = 6;
...
//			gi.dprintf ("%f - prox already gone off!\n", level.time);
	...
	{
		gi.sound (ent, CHAN_VOICE, SoundIndex ("weapons/proxwarn.wav"), 1, ATTN_NORM, 0);
...
//		ent->wait = level.time + PROX_TIME_TO_LIVE;
	...
	{
		if (ent->s.frame == 0)
			gi.sound (ent, CHAN_VOICE, SoundIndex ("weapons/proxopen.wav"), 1, ATTN_NORM, 0);
		//ent->s.sound = SoundIndex ("weapons/proxopen.wav");
		ent->s.frame++;
		ent->think = prox_open;
...
//		gi.dprintf ("start %s    aim %s   speed %d\n", vtos(start), vtos(aimdir), speed);
	...
	prox->s.modelindex = ModelIndex ("models/weapons/g_prox/tris.md2");
	prox->owner = self;
...
void fire_flame (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed)
{
	...
	flame->s.sound = SoundIndex ("weapons/flame.wav");
	flame->owner = self;
	flame->dmg = damage;
...
//	VectorScale (aimdir, speed, flame->velocity);
	...
	flame->s.sound = SoundIndex ("weapons/flame.wav");
	flame->s.modelindex = ModelIndex ("models/projectiles/puff/tris.md2");
	flame->owner = self;
...
edict_t *StartFire(edict_t *fireOwner, vec3_t fireOrigin, float fireDuration, float fireDamage)
{
	...
	fire->s.sound = SoundIndex ("weapons/incend.wav");
	fire->s.modelindex = ModelIndex ("models/objects/fire/tris.md2");
...
//	gi.sound (fire, CHAN_VOICE, SoundIndex ("weapons/incend.wav"), 1, ATTN_NORM, 0);
	return fire;
}
...
//	if (!other->takedamage)
	{
		...
		else
		{
			if (random() > 0.5)
				...
				gi.sound (ent, CHAN_VOICE, SoundIndex ("weapons/grenlb2b.wav"), 1, ATTN_NORM, 0);
		}
		return;
	}
...
//		grenade->s.effects &= ~EF_TELEPORT;
	...
	grenade->s.modelindex = ModelIndex ("models/projectiles/incend/tris.md2");
	grenade->owner = self;
...
void fire_player_melee (edict_t *self, vec3_t start, vec3_t aim, int reach, int damage, int kick, int quiet, int mod)
{
	...
	{
		if(!quiet)
			gi.sound (self, CHAN_WEAPON, SoundIndex ("weapons/swish.wav"), 1, ATTN_NORM, 0);
	...
	if(tr.ent->takedamage == DAMAGE_YES || tr.ent->takedamage == DAMAGE_AIM)
	{
		...
		if(!quiet)
			gi.sound (self, CHAN_WEAPON, SoundIndex ("weapons/meatht.wav"), 1, ATTN_NORM, 0);
	}
	else
	{
		if(!quiet)
			gi.sound (self, CHAN_WEAPON, SoundIndex ("weapons/tink1.wav"), 1, ATTN_NORM, 0);
...
//	VectorMA (ent->s.origin, -0.02, ent->velocity, origin);
	...
	gi.sound (ent, CHAN_NO_PHS_ADD+CHAN_VOICE, SoundIndex ("weapons/grenlx1a.wav"), 1, ATTN_NONE, 0);
/*
	gi.WriteByte (svc_temp_entity);
	if (ent->groundentity)
		gi.WriteByte (TE_GRENADE_EXPLOSION);
...
*/
	...
	ent->noise_index = SoundIndex ("world/rumble.wav");
...
//			gi.dprintf ("nuke frame %d\n", ent->s.frame);
		...
		{
/*			gi.sound (ent, CHAN_VOICE, SoundIndex ("weapons/nukewarn.wav"), 1, ATTN_NORM, 0);
			ent->timestamp += 10.0;
		}
*/		
			...
			{
//				ent->s.sound = SoundIndex ("weapons/nukewarn.wav");
//				gi.sound (ent, CHAN_NO_PHS_ADD+CHAN_VOICE, SoundIndex ("weapons/nukewarn2.wav"), 1, ATTN_NORM, 0);
				gi.sound (ent, CHAN_NO_PHS_ADD+CHAN_VOICE, SoundIndex ("weapons/nukewarn2.wav"), 1, attenuation, 0);
//				gi.sound (ent, CHAN_VOICE, SoundIndex ("weapons/nukewarn2.wav"), 1, ATTN_NORM, 0);
...
void fire_nuke (edict_t *self, vec3_t start, vec3_t aimdir, int speed)
{
	...
	nuke->s.modelindex = ModelIndex ("models/weapons/g_nuke/tris.md2");
	nuke->owner = self;
...
void tesla_think_active (edict_t *self)
{
	...
	for(i=0;i<num;i++)
	{
		...
		if(!(hit->svflags & (SVF_MONSTER | SVF_DAMAGEABLE)) && !hit->client)
			continue;
		// Paril: Don't hit owner.
		if (hit == self->teammaster)
			continue;
...
void tesla_think (edict_t *ent)
{
	...
	if(!(ent->s.frame))
		gi.sound (ent, CHAN_VOICE, SoundIndex ("weapons/teslaopen.wav"), 1, ATTN_NORM, 0); 
...
//	VectorClear (tesla->maxs);
	...
	tesla->s.modelindex = ModelIndex ("models/weapons/g_tesla/tris.md2");
...
void blaster2_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
	{
		...
		if (self->owner)
		{
			...
			T_Damage (other, self, self->owner, self->velocity, self->s.origin, plane->normal, self->dmg, 1, DAMAGE_ENERGY, MOD_BLASTER2);
	...
	else
	{
		...
		if (self->dmg >= 5)
			T_RadiusDamage(self, self->owner, self->dmg*3, self->owner, self->dmg_radius, MOD_BLASTER2);
...
void SpawnNeutSparkies (edict_t *self)
{
//	G_Spawn_Sparks (TE_SHIELD_SPARKS, self->s.origin, self->velocity, self->s.origin);
		gi.WriteByte (svc_temp_entity);
		gi.WriteByte (TE_FORCEWALL);
		gi.WritePosition (self->tempvector);
		gi.WritePosition (self->s.origin);
		gi.WriteByte (COLOR_GREEN);
		gi.multicast (self->tempvector, MULTICAST_PHS);
		gi.dprintf ("%s %s\n", vtos(self->tempvector), vtos(self->s.origin));

	self->nextthink4 = level.time + 0.1;
}
void SaveNOldOrigin (edict_t *self)
{
	VectorCopy (self->s.origin, self->tempvector);
...
void fire_blaster2 (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	...
	if (self->tempfloat != 666.767676)
	{
		for (i=1, cur=g_edicts+i; i < globals.num_edicts; i++,cur++)
		{
			...
			if (cur->s.modelindex == ModelIndex ("models/proj/neutron/shot.md2"))
				all++;
		}
	...
	bolt->think2 = KnifeArc;
	bolt->nextthink2 = level.time + 0.6;
	bolt->gravity = (sv_gravity->value / 1000) - 0.750;
		...
		bolt->dmg_radius = 128;
		bolt->s.modelindex = ModelIndex ("models/proj/neutron/shot.md2");
		bolt->touch = blaster2_touch;

		/*if (hyper)
		{
			bolt->s.effects = 0;
			VectorCopy (bolt->s.origin, bolt->tempvector);
			bolt->think3 = SaveNOldOrigin;
			bolt->nextthink3 = level.time + 0.1;
			bolt->think4 = SpawnNeutSparkies;
			bolt->nextthink4 = level.time + 0.1;
		}*/
...
void tracker_boom_spawn (vec3_t origin)
{
	...
	boom->s.modelindex = ModelIndex ("models/items/spawngro/tris.md2");
	boom->s.skinnum = 1;
	boom->s.frame = 2;
...
#define TRACKER_DAMAGE_FLAGS	(DAMAGE_NO_POWER_ARMOR | DAMAGE_ENERGY)
...
//			gi.dprintf("ouch %x\n", self);
			T_Damage (self->enemy, self, self->owner, vec3_origin, self->enemy->s.origin, pain_normal,
						self->dmg+15, 0, TRACKER_DAMAGE_FLAGS, MOD_TRACKER);

			self->enemy->tracker_framenum = level.framenum + 1;

			self->enemy->s.origin[2] += 12;
			gi.linkentity (self->enemy);

			VectorScale (self->enemy->velocity, 0.45, kvel);
			
			VectorAdd (self->enemy->velocity, kvel, self->enemy->velocity);
...
void tracker_explode (edict_t *self, cplane_t *plane)
{
	...
	gi.sound (self, CHAN_VOICE, SoundIndex ("weapons/disrupthit.wav"), 1, ATTN_NORM, 0);
...
void fire_tracker (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, edict_t *enemy)
{
	...
	bolt->speed = speed;
	bolt->s.effects = EF_TRACKER;
	bolt->s.sound = SoundIndex ("weapons/disrupt.wav");
	VectorClear (bolt->mins);
	VectorClear (bolt->maxs);
	
	bolt->s.modelindex = ModelIndex ("models/proj/disintegrator/tris.md2");
	bolt->touch = tracker_touch;
	bolt->enemy = enemy;
	bolt->owner = self;
	bolt->dmg = damage;
	bolt->classname = "tracker";
//	bolt->s.frame = bolt->s.skinnum = 7;
	gi.linkentity (bolt);
...
void chainfist_smoke (edict_t *ent)
{
	...
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, tempVec, 0);

	if (ent->is_bot)
		return;

	// Paril: Multicast this?
...
//	{
//		idle_index = SoundIndex("weapons/sawidle.wav");
//		attack_index = SoundIndex("weapons/sawhit.wav");
//	}
...
//		ent->client->weapon_sound = attack_index;
		ent->client->weapon_sound = SoundIndex("weapons/sawhit.wav");
	else if(ent->client->weaponstate == WEAPON_DROPPING)
		ent->client->weapon_sound = 0;
	else
//		ent->client->weapon_sound = idle_index;
		ent->client->weapon_sound = SoundIndex("weapons/sawidle.wav");
...
//	}
	...
	ent->client->ps.gunframe++;
	ent->client->ps.gunindex = ModelIndex ("models/weapons/v_beamer2/tris.md2");
...
//	if (!attack_index)
//	{
//		attack_index = SoundIndex ("weapons/bfg__l1a.wav");
//		off_model = ModelIndex ("models/weapons/v_beamer/tris.md2");
//		on_model = ModelIndex ("models/weapons/v_beamer2/tris.md2");
...
//	}
	...
	{
//		ent->client->weapon_sound = attack_index;
		ent->client->weapon_sound = SoundIndex ("weapons/bfg__l1a.wav");
...
//			ent->client->ps.gunindex = off_model;
			ent->client->ps.gunindex = ModelIndex ("models/weapons/v_beamer/tris.md2");
		}
	}
	else
	{
...
void Weapon_Tesla (edict_t *ent)
{
	...
	{
		ent->client->ps.gunindex = ModelIndex  ("models/weapons/v_tesla2/tris.md2");
	}
	else
	{
		ent->client->ps.gunindex = ModelIndex  ("models/weapons/v_tesla/tris.md2");
	}
