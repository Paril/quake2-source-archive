...
void gib_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (plane)
	{
		gi.sound (self, CHAN_VOICE, SoundIndex ("misc/fhit3.wav"), 1, ATTN_NORM, 0);
...
void ThrowDebris (edict_t *self, char *modelname, float speed, vec3_t origin)
{
	...
	for (i = 1; i < 5; i++)
	{
		if (Q_stricmp(modelname, va("models/objects/glass_gibs/gib%i.md2", i)) == 0)
...
edict_t *Find_Target (char *targetname)
{
	edict_t *h;
	
	for (h = g_edicts+1; h < &g_edicts[game.maxentities]; h++) 
	{
		if (h && h->targetname && Q_stricmp(h->targetname, targetname) == 0)
			return h;
	}
	return NULL;
}
...
//	vec3_t end;
	...
	model->s.modelindex = ModelIndex("models/objects/grenade2/tris.md2");
	VectorCopy (self->s.origin, model->s.origin);
	model->s.origin[2] += 14;
	model->s.origin[0] += 32;
	model->s.origin[1] += 24;
	gi.linkentity(model);
	...
	//if (self->target)
	//{
		/*edict_t *targ = Find_Target(self->target);
		if (!targ)
			return;
		...
		gi.WritePosition (model->s.origin);
		gi.WritePosition (end);
		gi.multicast (model->s.origin, MULTICAST_PHS);*/
	//}
}
...
void SP_light (edict_t *self)
{
	...
	if (self->style >= 32)
	{
		...
		if (self->spawnflags & START_OFF)
			...
			gi.configstring (CS_LIGHTS+63, "m");
	}
}
...
#ifdef _WIN32
extern __declspec(dllexport) void SP_misc_explobox (edict_t *self);
#endif
...
void commander_body_think (edict_t *self)
{
	...
	if (self->s.frame == 22)
		gi.sound (self, CHAN_BODY, SoundIndex ("tank/thud.wav"), 1, ATTN_NORM, 0);
}
...
void commander_body_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	self->nextthink = level.time + FRAMETIME;
	gi.sound (self, CHAN_BODY, SoundIndex ("tank/pain.wav"), 1, ATTN_NORM, 0);
}
...
//ZOID
	...
	if (other->laz_vehicle)
	{
		VectorCopy (dest->s.origin, other->laz_vehicle->s.origin);
		VectorCopy (dest->s.origin, other->laz_vehicle->s.old_origin);
		other->laz_vehicle->s.origin[2] += 24;
	}
	VectorCopy (dest->s.origin, other->s.origin);
	VectorCopy (dest->s.origin, other->s.old_origin);
	other->s.origin[2] += 10;
...
void FindOtherEntities (edict_t *self)
{
	edict_t *blip = NULL;
	edict_t *dest;
//	int i;
	...
	while (blip = findradius(blip, self->s.origin, 40))
	{
		if (!blip->inuse)
			continue;
		if (!blip->can_teleport_through_destinations)
			continue;
		if (blip->client)
			continue;
		...
		// unlink to make sure it can't possibly interfere with KillBox
		gi.unlinkentity (blip);

		VectorCopy (dest->s.origin, blip->s.origin);
		VectorCopy (dest->s.origin, blip->s.old_origin);
		blip->s.origin[2] += 20;

		//VectorClear (blip->s.angles);
...
*/
#ifdef _WIN32
extern __declspec(dllexport) void SP_misc_teleporter (edict_t *self);
#endif
void SP_misc_teleporter (edict_t *ent)
{
	edict_t		*trig;

	// Spawned from the editor.
	if (ent->tempint == 1)
	{
	...
	trig->owner = ent;
	trig->think = FindOtherEntities;
	trig->nextthink = level.time + .1;
...
*/
#ifdef _WIN32
extern __declspec(dllexport) void SP_misc_teleporter_dest (edict_t *self);
#endif
