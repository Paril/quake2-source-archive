...
static void GravGren_Explode (edict_t *ent)
{
	vec3_t		origin;
	int			mod;
	...
	if (ent->spawnflags & 2)
		...
		mod = MOD_G_SPLASH;
	T_RadiusDamage(ent, ent->owner, ent->dmg, ent->enemy, 400, mod);
...
static void GravGren_Touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if (other == ent->owner)
		return;
...
void fire_gravgrenade (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
{
	...
	grenade->owner = self;
	grenade->touch = GravGren_Touch;
	grenade->nextthink = level.time + timer;
	grenade->think = GravGren_Explode;
	grenade->dmg = damage;
	grenade->dmg_radius = damage_radius;
	grenade->classname = "gravity_grenade";
	if (held)
...
void weapon_gravgrenade_fire (edict_t *ent, qboolean held)
{
	vec3_t	offset;
	...
	fire_gravgrenade (ent, start, forward, damage, speed, timer, radius, held);
	...
	if (ent->client->weaponstate == WEAPON_FIRING)
	{
		...
		if (ent->client->ps.gunframe == 11)
		{
			...
			if (!ent->client->grenade_blew_up && level.time >= ent->client->grenade_time)
			{
				...
				weapon_gravgrenade_fire (ent, true);
		...
		{
			ent->client->weapon_sound = 0;
			weapon_gravgrenade_fire (ent, false);
		}
