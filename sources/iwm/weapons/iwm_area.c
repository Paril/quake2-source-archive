...
void ForceGrenade_Explode (edict_t *ent)
{
	...
	while ((sent = findradius(sent, ent->s.origin, 1200)) != NULL)
	{
		if (!sent->takedamage)
			continue;
		...
		if (CanDamage (sent, ent))
		{
			...
			T_Damage (sent, ent, ent->owner, dir, ent->s.origin, vec3_origin, 25+random()*5, 300, DAMAGE_RADIUS, mod);
			if (sent != ent->owner)
				T_Damage (sent, ent, ent->owner, dir, ent->s.origin, vec3_origin, 25+random()*5, 300, DAMAGE_RADIUS, mod);
			if (sent->client)
			{
				...
				T_Damage (sent, ent, ent->owner, dir, ent->s.origin, vec3_origin, 60, 300, DAMAGE_RADIUS, mod);
			}
		}
	}

	//(*positioned_sound) (vec3_t origin, edict_t *ent, int channel, int soundinedex, float volume, float attenuation, float timeofs)
	gi.positioned_sound(ent->s.origin,ent,CHAN_WEAPON,SoundIndex("makron/bfg_fire.wav"),1,ATTN_NORM,0);
