...
void SP_monster_tank (edict_t *self)
{
	...
	self->s.modelindex = ModelIndex ("models/monsters/tank/tris.md2");
	VectorSet (self->mins, -32, -32, -16);
	VectorSet (self->maxs, 32, 32, 72);
	self->movetype = MOVETYPE_STEP;
	self->solid = SOLID_BBOX;

//	sound_pain = SoundIndex ("tank/tnkpain2.wav");
	sound_thud = SoundIndex ("tank/tnkdeth2.wav");
//	sound_idle = SoundIndex ("tank/tnkidle1.wav");
	sound_die = SoundIndex ("tank/death.wav");
	sound_step = SoundIndex ("tank/step.wav");
	sound_windup = SoundIndex ("tank/tnkatck4.wav");
	sound_strike = SoundIndex ("tank/tnkatck5.wav");
//	sound_sight = SoundIndex ("tank/sight1.wav");
	...
	if ((!self->is_spawned && strcmp(self->classname, "monster_tank_commander") == 0))
	{
		self->health = 1000;
		self->gib_health = -225;
	}
	else if (self->is_spawned && Q_stricmp(self->name, "a Tank Commander") == 0)
	{
		self->s.skinnum = 2;
		self->health = 1000;
		self->gib_health = -225;
