...
void Jet_BecomeExplosion( edict_t *ent, int damage )
{
  ...
  gi.sound( ent, CHAN_BODY, SoundIndex("misc/udeath.wav"), 1, ATTN_NORM, 0 );
...
void FollowPlayer (edict_t *s)
{
	if (s->owner->deadflag || !Jet_Active(s->owner))
	{
		s->owner->carpet_active = 0;
		G_FreeEdict (s);
		return;
	}

	VectorCopy (s->owner->s.origin, s->s.origin);
	VectorCopy (s->owner->s.angles, s->s.angles);

	//gi.dprintf ("%f\n", s->owner->velocity[2]);

	if (s->owner->velocity[2] < 0)
		s->s.origin[2] -= 30;
	...
	s->s.effects |= EF_SPHERETRANS;
}

void CreateCarpetEntity (edict_t *ent)
{
	edict_t *c = ent->carpet;

	c = G_Spawn();

	c->movetype = MOVETYPE_NONE;
	c->s.modelindex = ModelIndex ("models/items/carpet/tris.md2");
	c->s.frame = 1; // Player frame
	c->think = FollowPlayer;
	c->nextthink = level.time + .1;
	c->owner = ent;
	c->inuse = true;
	VectorCopy (c->owner->s.origin, c->s.origin);
	VectorCopy (c->owner->s.angles, c->s.angles);
	ent->carpet_active = 1;
	c->s.renderfx |= RF_FRAMELERP;
...
void Jet_ApplyJet( edict_t *ent, usercmd_t *ucmd )
{
  ...
  //Jet_ApplySparks( ent );
  if (!ent->carpet_active)
	  CreateCarpetEntity (ent);
