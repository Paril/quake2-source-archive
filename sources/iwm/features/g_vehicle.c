	...
	if(vspeed < 200) {
		...
		if(tr.startsolid)
			...
			T_Damage (other, self, self->owner, dir, self->s.origin, vec3_origin,
						other->health - other->gib_health + 1, 0, 0, MOD_VEHICLE); // Fixme
...
void vehicle_think (edict_t *self)
{
	...
	if (self->owner)
	{
		...
		if(speed != 0)
		{
			...
			if (y2 > 90)
				y2 = 90;
//			self->owner->client->ps.pmove.delta_angles[ROLL] += ANGLE2SHORT(yaw);
...
//	self->class_id = ENTITY_FUNC_VEHICLE;
	...
	self->nextthink  = level.time + FRAMETIME;
	self->noise_index  = SoundIndex("engine/engine.wav");
	self->noise_index2 = SoundIndex("engine/idle.wav");
	VectorClear(self->velocity);
	VectorClear(self->avelocity);
	self->moveinfo.current_speed = 0;
	self->moveinfo.state = STOP;
