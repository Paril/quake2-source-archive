...
{
	fire_rail (self, start, aimdir, damage, kick, 4);
...
static void M_FliesOn (edict_t *self)
{
	if (self->waterlevel)
		return;
	self->s.effects |= EF_FLIES;
	self->s.sound = SoundIndex ("infantry/inflies1.wav");
...
void M_CheckGround (edict_t *ent)
{
	...
	if (Q_stricmp(ent->classname, "monster_car") == 0 || Q_stricmp(ent->classname, "sheep") == 0)
		return;
...
void M_WorldEffects (edict_t *ent)
{
	...
	if (ent->waterlevel == 0)
	{
		...
		{	
			gi.sound (ent, CHAN_BODY, SoundIndex("player/watr_out.wav"), 1, ATTN_NORM, 0);
	...
	{	
		if (!(ent->svflags & SVF_DEADMONSTER))
		{
			if (ent->watertype & CONTENTS_LAVA)
				if (random() <= 0.5)
					gi.sound (ent, CHAN_BODY, SoundIndex("player/lava1.wav"), 1, ATTN_NORM, 0);
				else
					gi.sound (ent, CHAN_BODY, SoundIndex("player/lava2.wav"), 1, ATTN_NORM, 0);
			else if (ent->watertype & CONTENTS_SLIME)
				gi.sound (ent, CHAN_BODY, SoundIndex("player/watr_in.wav"), 1, ATTN_NORM, 0);
...
void monster_think (edict_t *self)
{
	...
	{
		if (!self->transformer->vehicle)
		{
			G_FreeEdict(self);
			return;
		}
//		gi.dprintf ("CAR ALIVE!\n");
	}

// CYBERSLASH: check if inverted monster has hit ceiling
	if (self->gravity < 0)
		pig_checksplat (self);
// CYBERSLASH
	...
	// Special fire code for monsters
	if (self->fire_framenum > level.framenum + 80)
		self->fire_framenum = level.framenum + 80;
	if (self->fire_hurtframenum > level.framenum + 80)
		self->fire_hurtframenum = level.framenum + 80;
	
	if (self->fire_hurtframenum < level.framenum && self->fire_framenum > level.framenum && self->fire_entity)
	{
		T_Damage (self, world, world, vec3_origin, self->s.origin, vec3_origin, 5, 0, 0, 0); // FIXMEH
		self->fire_hurtframenum = level.framenum + 10;
	}
	if (self->fire_framenum < level.framenum && self->fire_entity) // Pssssss!
	{
		G_FreeEdict (self->fire_entity);
		self->fire_framenum = -1;
		self->fire_entity = NULL;
	}
...
void monster_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (!activator)
		return;
	if (activator && activator->flags & FL_NOTARGET)
		return;
	if (activator && !(activator->client) && !(activator->monsterinfo.aiflags & AI_GOOD_GUY))
		return;
...
{
	if (deathmatch->value && !self->is_spawned)
	{
		G_FreeEdict (self);
		return false;
	}
