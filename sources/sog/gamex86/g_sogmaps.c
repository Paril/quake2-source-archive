...
void InitMapList(void)
{
	...
	//open it the file
	if(sog_ctf->value ||
	   ((int)sogflags->value & SOG_CTF_ONLY))
		fp = fopen("./sog/ctfmaps.lst", "r" );
	else
		fp = fopen("./sog/dmmaps.lst", "r" );
				...
				case 1:
					{
						...
						if(Q_stricmp(temp,"idg3")==0)
							entry->game = CLASS_Q1;
						else if (Q_stricmp(temp,"idg2")==0)
							entry->game = CLASS_DOOM;
						else if (Q_stricmp(temp,"idg1")==0)
...
char * GenNextMap(void)
{
	...
	if(i >= 0)
	{
		if(((int)sogflags->value & SOG_DM_ONLY)
			|| sog_team->value)
			index =FindNextMap(i,DM);
		else if((int)sogflags->value & SOG_CTF_ONLY)
