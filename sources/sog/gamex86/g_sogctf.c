...
// Skid-  Don't allow them to change teams if they havent got a class	
	...
	{
		safe_cprintf(ent, PRINT_HIGH, "Please select a Class.\n");
		return;	
	}
...
void CTFDeadDropFlag(edict_t *self)
{
	...
	// Arno - Clear flagmodel from dead player
...
*/
	...
	ent->client->ps.stats[STAT_CTF_TEAM1_CAPS] = gengame.total1;
	ent->client->ps.stats[STAT_CTF_TEAM2_CAPS] = gengame.total2;
...
void CTFScoreboardMessage (edict_t *ent, edict_t *killer)
{
	...
	for (i=0 ; i<game.maxclients ; i++)
	{
		...
		// Arno added - spectators are not in any team
