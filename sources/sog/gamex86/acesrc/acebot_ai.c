...
void ACEAI_PickLongRangeGoal(edict_t *self)
{
	...
	for(i=0;i<num_items;i++)
	{
		...
		if(sog_ctf->value && (item_table[i].item == ITEMLIST_FLAG2 || item_table[i].item == ITEMLIST_FLAG1) &&
		  (self->client->resp.team == CTF_TEAM1 && self->client->pers.inventory[ITEMLIST_FLAG2] ||
		   self->client->resp.team == CTF_TEAM2 && self->client->pers.inventory[ITEMLIST_FLAG1]))
			weight = 10.0;
	...
	if (!(coop->value)) 
	{
		...
		for(i=0;i<num_players;i++)
		{
			...
			if(sog_ctf->value && (players[i]->client->pers.inventory[ITEMLIST_FLAG2] || players[i]->client->pers.inventory[ITEMLIST_FLAG1]))
			  weight = 2.0;
			else
			  weight = 0.3; 
...
qboolean ACEAI_FindEnemy(edict_t *self)
{
	...
	for(i=0;i<=num_players;i++)
	{
		...
		// CTF and Red vs Blue - teamplay
		if(((sog_team->value && ((int)sogflags->value & SOG_REDBLUE_TEAMS)) || sog_ctf->value) && 
		   self->client->resp.team == players[i]->client->resp.team)
		   continue;

		// Classbased - teamplay
		if((sog_team->value && !((int)sogflags->value & SOG_REDBLUE_TEAMS)) && 
		   self->client->resp.player_class == players[i]->client->resp.player_class)
		   continue;
...
{	
	...
	switch(self->client->resp.player_class)
	{
		case CLASS_WOLF:
		{
			...
			if(range < 200)
			{
				if(ACEAI_CheckShot(self) && ACEIT_ChangeWeapon(self,FindItem("flamethrower")))
					return;
			}				
			...
			// Only use double gatling gun when ammo > 100
			if(self->client->pers.inventory[AMMO_BULLETS_INDEX] >= 100)
			if(ACEIT_ChangeWeapon(self,FindItem("gatling guns")))
				return;		
		
				// Only use gatling gun when ammo > 50
			if(self->client->pers.inventory[AMMO_BULLETS_INDEX] >= 50)
			if(ACEIT_ChangeWeapon(self,FindItem("Gatling Gun")))
				return;		
		
			if(ACEIT_ChangeWeapon(self,FindItem("Machine Pistol 40")))
				return;
				
			if(ACEIT_ChangeWeapon(self,FindItem("walter p-38")))
				return;
				
			if(ACEIT_ChangeWeapon(self,FindItem("knife")))
				return;		
		...
		case CLASS_DOOM:
		{
			...
			if(range > 300)
			{
				...
				if(self->client->pers.inventory[AMMO_CELLS_INDEX] > 50)
				if(ACEAI_CheckShot(self) && ACEIT_ChangeWeapon(self, FindItem("Goddamn Big Gun")))
					return;

				if(ACEAI_CheckShot(self) && ACEIT_ChangeWeapon(self,FindItem("RL3000")))
					return;
			}				
			// Only use double barreled shotgun when close enough
			if(range > 30 && range < 200)
			if(ACEIT_ChangeWeapon(self,FindItem("Double Barreled Shotgun")))
				return;

			// Only use chainsaw when really close
			if(range < 64)
			if(ACEIT_ChangeWeapon(self,FindItem("Chainsaw")))
				return;

			if(ACEIT_ChangeWeapon(self,FindItem("Plasma Rifle")))
				return;
		
			// Only use Minigun when ammo > 50
			if(self->client->pers.inventory[AMMO_BULLETS_INDEX] >= 50)
			if(ACEIT_ChangeWeapon(self,FindItem("Mini Gun")))
				return;

			if(ACEIT_ChangeWeapon(self,FindItem("Combat Shotgun")))
				return;

			if(ACEIT_ChangeWeapon(self,FindItem("Pistol")))
				return;
			
			if(ACEIT_ChangeWeapon(self,FindItem("Fists")))
				return;
		}
		case CLASS_Q1:
		{
			...
			if(range < 300)
			{
				if(ACEAI_CheckShot(self) && ACEIT_ChangeWeapon(self,FindItem("discharger")))
					return;
			}				
			...
			if(range > 300)
			{
				if(ACEAI_CheckShot(self) && ACEIT_ChangeWeapon(self,FindItem("RPG")))
					return;
			}		

			// Only use heavy spiker when ammo > 50
			if(self->client->pers.inventory[AMMO_BULLETS_INDEX] >= 50)
			if(ACEIT_ChangeWeapon(self,FindItem("heavy spiker")))
				return;		
		
			if(ACEIT_ChangeWeapon(self,FindItem("sawed-off shotgun")))
   	   			return;
   	   			
			if(ACEIT_ChangeWeapon(self,FindItem("spiker")))
   	   			return;
   	   			
			if(ACEIT_ChangeWeapon(self,FindItem("Boomstick")))
   	   			return;
   	   			
 			if(ACEIT_ChangeWeapon(self,FindItem("axe")))
   	   			return;   	   						
