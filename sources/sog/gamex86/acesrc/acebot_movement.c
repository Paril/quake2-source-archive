...
void ACEMV_Attack (edict_t *self, usercmd_t *ucmd)
{
	...
	// Move forward in case of a melee-weapon
	if ((self->client->pers.weapon == FindItem("axe")) ||
		(self->client->pers.weapon == FindItem("Fists")) ||
		(self->client->pers.weapon == FindItem("Chainsaw")) ||
		(self->client->pers.weapon == FindItem("knife")))
	{
	...
	if (skill->value == 0) 
	{
		target[0] += (random()-0.5) * 100;
		target[1] += (random()-0.5) * 100;
	}
	else if (skill->value == 1)
	{	
		target[0] += (random()-0.5) * 60;
		target[1] += (random()-0.5) * 60;
	}
