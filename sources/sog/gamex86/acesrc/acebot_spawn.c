...
void ACESP_SaveBots()
{
	...
	if((pOut = fopen("sog\\bots.tmp", "wb" )) == NULL)
		return; // bail
...
void ACESP_LoadBots()
{
	...
	if((pIn = fopen("sog\\bots.tmp", "rb" )) == NULL)
		return; // bail
...
void ACESP_HoldSpawn(edict_t *self)
{
	...
	safe_bprintf(PRINT_MEDIUM, "%s joined the %s team.\n",
		self->client->pers.netname, CTFTeamName(self->client->resp.team));
	else
...
void ACESP_PutClientInServer (edict_t *bot, qboolean respawn, int team)
{
	...
	if((sog_team->value && ((int)sogflags->value & SOG_REDBLUE_TEAMS)) || sog_ctf->value)
	{
		...
		client->resp.state = START_GEN;
		s = Info_ValueForKey (client->pers.userinfo, "skin");
		GenAssignModel(bot, s);
	}
...
// end Skid	
    ...
    if(!respawn)
	{
		...
		bot->nextthink = level.time + 2 + random()*5.0; // up to five seconds
	}
	else
	{
		if (!KillBox (bot))
...
void ACESP_SetName(edict_t *bot, char *name, char *skin, char *team)
{
	...
	// idg*_only-sogflags
	if ((int)sogflags->value & SOG_IDG1_ONLY) skin = "idg1/base";
	else if ((int)sogflags->value & SOG_IDG2_ONLY) skin = "idg2/blue";
	else if ((int)sogflags->value & SOG_IDG3_ONLY) skin = "idg3/base"; 
	...
	if(strlen(skin) == 0)
	{
		...
		rnd = random();
		if(rnd  < 0.05)
			sprintf(bot_skin,"idg1/base");
		else if(rnd < 0.1)
			sprintf(bot_skin,"idg1/wolf");
		else if(rnd < 0.15)
			sprintf(bot_skin,"idg1/revenge");
		else if(rnd < 0.2)
			sprintf(bot_skin,"idg2/blue");
		else if(rnd < 0.25)
			sprintf(bot_skin,"idg2/brown");
		else if(rnd < 0.3)
			sprintf(bot_skin,"idg2/black");
		else if(rnd < 0.35)
			sprintf(bot_skin,"idg2/dead");
		else if(rnd < 0.4)
			sprintf(bot_skin,"idg3/base");
		else if(rnd < 0.45)
			sprintf(bot_skin,"idg3/blue-white");
		else if(rnd < 0.5)
			sprintf(bot_skin,"idg3/red");
		else if(rnd < 0.55)
			sprintf(bot_skin,"idg3/purple");
		else if(rnd < 0.6)
...
// determine player_class upon skin
	if((bot_skin[0] == 'i') &&
	   (bot_skin[1] == 'd') &&
	   (bot_skin[2] == 'g')) {
	   		if (bot_skin[3] == '1') bot->client->resp.player_next_class = CLASS_WOLF;
	   		else if (bot_skin[3] == '2') bot->client->resp.player_next_class = CLASS_DOOM;
	   		else if (bot_skin[3] == '3') bot->client->resp.player_next_class = CLASS_Q1;
	   		else bot->client->resp.player_next_class = CLASS_Q2;
	   }
	else
		bot->client->resp.player_next_class = CLASS_Q2;

	bot->client->resp.player_class = bot->client->resp.player_next_class;	
