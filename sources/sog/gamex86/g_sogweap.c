	...
	if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))    
    {
        ...
        {            
            if (tr.ent->takedamage)            
            {
			   ...
			   if (!(tr.ent->svflags & SVF_MONSTER) && tr.ent->client)
			   {
				   if(tr.ent->client->resp.player_class == CLASS_Q1)
				   {
						...
						gi.sound (tr.ent, CHAN_VOICE, gi.soundindex("idg3weapons/axe/axhit1.wav") , 1.0 , ATTN_NORM, 0);
				   }
			   }
            ...
            else
			{
				...
				gi.WriteDir (tr.plane.normal);
				gi.multicast (tr.endpos, MULTICAST_PVS);
				gi.sound (self, CHAN_AUTO, gi.soundindex("idg3weapons/axe/axhit2.wav") , 0.8 , ATTN_NORM, 0);
			}
        }
    }
    return;
...
void nail_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	else
	{
		...
		if(!deathmatch->value)
		{
			...
			{
				if (sound < 0.1)
					gi.sound (self, CHAN_WEAPON, gi.soundindex ("idg3weapons/nails/ric3.wav"), 1, ATTN_STATIC, 0);
				else if (sound < 0.2)
					gi.sound (self, CHAN_WEAPON, gi.soundindex ("idg3weapons/nails/ric2.wav"), 1, ATTN_STATIC, 0);
				else 
					gi.sound (self, CHAN_WEAPON, gi.soundindex ("idg3weapons/nails/ric1.wav"), 1, ATTN_STATIC, 0);
			}
			else if (sound < 0.5)
				gi.sound (self, CHAN_WEAPON, gi.soundindex ("idg3weapons/nails/tink1.wav"), 1, ATTN_STATIC, 0);
		}
...
void q1_fire_nail (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, qboolean sng)
{
	...
	nail->s.modelindex = gi.modelindex ("models/objects/idg3nail/tris.md2");
	nail->owner = self;
...
void q1_grenade_explode (edict_t *ent)
{
	...
	gi.sound (ent, CHAN_AUTO, gi.soundindex ("idg3weapons/rocket/r_exp3.wav"), 1.0, ATTN_NORM, 0);	
...
static void q1_grenade_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	{
		gi.sound (ent, CHAN_RELIABLE|CHAN_WEAPON, gi.soundindex ("idg3weapons/grenades/bounce.wav"), 1, ATTN_NORM, 0);
		return;
	}
...
void q1_fire_grenade (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius)
{
	...
	grenade->s.modelindex = gi.modelindex ("models/objects/idg3grenade/tris.md2"); 
	grenade->owner = self;
...
void q1_fire_lightning (edict_t *self, vec3_t start, vec3_t dir, int damage)
{
	...
	else
	{
		...
		{
			gi.WriteByte (svc_temp_entity);
		//Knightmare- use the plasma beam temp event instead
		//this is normally drawn from the side and not from the center
		//but you will have to make the new lightning model 
		//line up with the plasma beam segment model
		//the model for this is models/proj/beam/tris.md2

		//you could also use the TE_MONSTER_HEATBEAM temp event for the
		//shabler and target_q1_bolt lightning entities
		//the model for this is models/proj/widowbeam/tris.md2

		//You could also investigate using the TE_LIGHTNING temp event 
		//for the target_q1_bolt lightning entity
		//the model for this is models/proj/lightning/tris.md2
...
{    
	...
	if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))    
    {
        if (tr.fraction < 1.0)        
        {            
			gi.sound (self, CHAN_VOICE, gi.soundindex("idg2weapons/punch.wav") , 1.0 , ATTN_NORM, 0);
            if (tr.ent->takedamage)            
            {
...
//gi.dprintf("CLIENT BEING CHAINSAWED\n"); 
	...
	if(hit==true) 
	{
		gi.sound (self, CHAN_WEAPON, gi.soundindex("idg2weapons/sawhit.wav") , 1.0 , ATTN_NORM, 0);
	}
	else if(self->client->oldweapon < level.time) 
	{
		gi.sound (self, CHAN_WEAPON, gi.soundindex("idg2weapons/sawon.wav") , 1.0 , ATTN_NORM, 0);
...
void plasma_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	gi.sound (self, CHAN_AUTO, gi.soundindex ("idg2weapons/phit.wav"), 1, ATTN_NORM, 0);
...
void d_rocket_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	gi.sound (self, CHAN_AUTO, gi.soundindex ("idg2weapons/rockexp.wav"), 1.0, ATTN_NORM, 0);	
...
void d_fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	VectorClear (rocket->mins);
	VectorClear (rocket->maxs);
	rocket->s.modelindex = gi.modelindex ("models/objects/idg2rocket/tris.md2");
	rocket->owner = self;
...
//void T_RadiusDamage (edict_t *inflictor, edict_t *attacker, float damage, edict_t *ignore, float radius, int mod)
	...
	gi.sound (self, CHAN_VOICE, gi.soundindex ("idg2weapons/bfgexp.wav"), 1, ATTN_NORM, 0);
	self->solid = SOLID_NOT;
	self->touch = NULL;
...
void w_rocket_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	gi.sound (self, CHAN_AUTO, gi.soundindex ("idg1weapons/explode.wav"), 1.0, ATTN_NORM, 0);	
...
void w_fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	VectorClear (rocket->mins);
	VectorClear (rocket->maxs);
	rocket->s.modelindex = gi.modelindex ("models/objects/idg1rocket/tris.md2");
	rocket->owner = self;
...
void flame_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	gi.sound (self, CHAN_VOICE, gi.soundindex ("idg1weapons/fthit.wav"), 1, ATTN_NORM, 0);
	...
	if (!((tr.surface) && (tr.surface->flags & SURF_SKY)))    
    {
        ...
        {            
            if (tr.ent->takedamage)            
            {
			   ...
			   if (!(tr.ent->svflags & SVF_MONSTER) && tr.ent->client)
			   {
				   if(tr.ent->client->resp.player_class == CLASS_WOLF)
				   {
				   		tr.ent->pain_debounce_time = level.time;
						gi.sound (tr.ent, CHAN_VOICE, gi.soundindex("idg1weapons/knifehit.wav") , 1.0 , ATTN_NORM, 0);
				   }
			   }
				   gi.WriteByte (svc_temp_entity);
				   gi.WriteByte (TE_MOREBLOOD);
            ...
            else
			{
				...
				gi.WriteDir (tr.plane.normal);
				gi.multicast (tr.endpos, MULTICAST_PVS);
				gi.sound (self, CHAN_AUTO, gi.soundindex("idg1weapons/knifehit.wav") , 0.8 , ATTN_NORM, 0);
			}
        }
    }
    return;
...
void keen_raygunshot_think (edict_t *self)
{
	
	// animation raygun shot
...
static void keen_raygunshot_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t		origin;
	...
	{
		// FIX MOD
		T_Damage (other, ent, ent->owner, ent->velocity, ent->s.origin, plane->normal, ent->dmg, 0, 0, MOD_KEEN_RAYGUN);
	}
	...
	VectorClear (ent->velocity);
	ent->s.modelindex = gi.modelindex ("sprites/raygun.sp2"); 
	ent->s.frame = 0;
	ent->s.sound = gi.soundindex ("idg3weapons/lightning/lhit.wav");
	ent->s.effects &= ~EF_ANIM_ALLFAST; 
...
void keen_fire_raygunshot (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed)
{
	...
	raygun = G_Spawn();
	VectorCopy (start, raygun->s.origin);
	VectorCopy (dir, raygun->movedir);
	vectoangles (dir, raygun->s.angles);
	VectorScale (dir, speed, raygun->velocity);
	raygun->movetype = MOVETYPE_FLYMISSILE;
	raygun->clipmask = MASK_SHOT;
	raygun->solid = SOLID_BBOX;
	raygun->s.frame = 0;
	raygun->s.renderfx |= RF_TRANSLUCENT;

	VectorClear (raygun->mins);
	VectorClear (raygun->maxs);
	raygun->s.modelindex = gi.modelindex ("models/proj/beam/tris.md2");
	raygun->owner = self;
	raygun->touch = keen_raygunshot_touch;
	raygun->nextthink = level.time + FRAMETIME;
	raygun->think = keen_raygunshot_think;
	raygun->dmg = damage;
	//needle->s.sound = gi.soundindex ("weapons/rockfly.wav");
	raygun->classname = "raygunshot";
	
	gi.linkentity (raygun);
}
