...
// Doom                 
                        ...
                        case MOD_DOOM_BFG_LASER:
                                ...
                                message2 = "'s Goddamn Big Gun";
                                break;
                        case MOD_DOOM_BFG_BLAST:
                                message = "was disintegrated by the blast of";
                                message2 = "'s Goddamn Big Gun";
                                break;
...
// Skid added 
// MONSTER MESSAGES !
        else if(!deathmatch->value && attacker && (attacker->svflags & SVF_MONSTER))
        {
                if(Q_stricmp(attacker->classname,"monster_idg3_monkey")==0)
                        message ="was eviscerated by a Fiend";
                else if(Q_stricmp(attacker->classname,"monster_idg3_soldier")==0)
                        message = "was shot by a Grunt";
                else if(Q_stricmp(attacker->classname,"monster_idg3_saw")==0)
                        message = "was destroyed by an Ogre";
                else if(Q_stricmp(attacker->classname,"monster_idg3_knight")==0)
                        message = "was slashed by a Knight";
                else if(Q_stricmp(attacker->classname,"monster_idg3_flying")==0)
                        message = "was scragged by a Scrag";
                else if(Q_stricmp(attacker->classname,"monster_idg3_zombie")==0)
                        message = "joins the Zombies";
                else if(Q_stricmp(attacker->classname,"monster_idg3_electric")==0)
                        message = "was smashed by a Shambler";
                else if(Q_stricmp(attacker->classname,"monster_idg3_dog")==0)
                        message = "was mauled by a Rottweiler";
                else if(Q_stricmp(attacker->classname,"monster_idg3_boss")==0)
                        message = "couldn't face Chton";
				else if(Q_stricmp(attacker->classname,"monster_idg2_super")==0)
				{
						message = "bows down before the Cyberdemon";
						cyberdemon_salute (attacker);
				}
				else if(Q_stricmp(attacker->classname,"monster_idg2_monkey")==0)
                        message = "couldn't handle a little devil";
				else if(Q_stricmp(attacker->classname,"monster_idg2_pinky")==0)
                        message = "served as breakfast for the demons";
				else if(Q_stricmp(attacker->classname,"monster_idg2_uberdog")==0)
                        message = "kneels before a huge demon";
				else if(Q_stricmp(attacker->classname,"monster_idg1_sa")==0)
                        message = "was shot by a guard";
				else if(Q_stricmp(attacker->classname,"monster_idg1_ss")==0)
                        message = "was executed by the SS";
				else if(Q_stricmp(attacker->classname,"monster_idg1_dog")==0)
                        message = "was killed by a dog";
				else if(Q_stricmp(attacker->classname,"monster_idg1_officer")==0)
                        message = "was shot by an officer";
				else if(Q_stricmp(attacker->classname,"monster_idg1_bd19")==0)
                        message = "couldn't win from the Big Guard";
				else if(Q_stricmp(attacker->classname,"monster_idg1_mutant")==0)
                        message = "was assimilated by the zombie's";
				else if(Q_stricmp(attacker->classname,"monster_idg1_commander")==0)
                        message = "died by the rockets of a nazi commander";
				else if(Q_stricmp(attacker->classname,"monster_idg1_general")==0)
                        message = "was defeated by a nazi general";
                if(message)
                {
                        safe_bprintf(PRINT_MEDIUM,"%s %s.\n", self->client->pers.netname,message);
                        return;
...
// Skid
	...
	if ((self->health < -40) && (strcmp(self->classname,"bot")!=0))
    {
// Old gibs- Skid
		switch(player_class)
		{
		case CLASS_WOLF:
			{
				gi.sound (self, CHAN_BODY, gi.soundindex ("idg2/gib.wav"), 1, ATTN_NORM, 0);
...
//GIB MODEL IF STANDING
				...
				{
					gi.sound (self, CHAN_VOICE, gi.soundindex("*death1.wav"), 1, ATTN_NORM, 0);
					ThrowDoomguy (self);
				}
                break;
			}
		case CLASS_Q1:
			{
				if(damage >= 100000)
					gi.sound (self, CHAN_BODY, gi.soundindex ("idg3/teledth.wav"), 1, ATTN_NORM, 0);
				else
					gi.sound (self, CHAN_BODY, gi.soundindex ("idg3/gib.wav"), 1, ATTN_NORM, 0);
				ThrowGib (self, "models/objects/gibs/leg/tris.md2", damage, GIB_ORGANIC);
                for (n= 0; n < 4; n++)
                {
                  if ((n % 2) == 0) 
...
//spawning weapon
        ...
        //Knightmare- lithium carrying capacity cvars
        client->pers.max_bullets        = idg4_max_bullets->value;
        client->pers.max_shells         = idg4_max_shells->value;
        client->pers.health             = 100;
        client->pers.max_health         = idg4_max_health->value;
...
// Setup state, first base respawn - Skid
        ...
        // Checking score, so that player will not respawn in the base all the time 
        
        if (sog_ctf->value) // && (ent->client->resp.score == 0) || ((int)genflags->value & SOG_CTF_BASE_RESPAWN))
...
void spectator_respawn (edict_t *ent)
{
        ...
        if (ent->client->pers.spectator) 
		{
                ...
                // Remove the chasecam, otherwise it will leave a ghost
                if (ent->client->oldplayer) ChasecamRemove(ent);
...
// set new class
        if(ent->client->resp.player_class != ent->client->resp.player_next_class)
        {
				...
				if(deathmatch->value && !sog_ctf->value)
				{
				...
				case CLASS_WOLF:
					{
						stuffcmd(ent,"exec idg1.cfg\n");
						break;
					}
				case CLASS_DOOM:
					{
						stuffcmd(ent,"exec idg2.cfg\n");
						break;
					}
				case CLASS_Q1:
					{
						stuffcmd(ent,"exec idg3.cfg\n");
						break;
					}
				case CLASS_Q2:
					{
						stuffcmd(ent,"exec idg4.cfg\n");
						break;
					}
				}
	     }
...
// ACEBOT_END
    ...
    else
    {
              ...
              (ent->client->resp.player_class == CLASS_Q1))
        {
             ...
             gi.sound (ent, CHAN_NO_PHS_ADD+CHAN_VOICE, gi.soundindex("idg3world/teleport/r_tele1.wav"), 1, ATTN_NORM, 0);
        }
        else 
        {
//End Skid
...
void ClientUserinfoChanged (edict_t *ent, char *userinfo)
{
        ...
        if (!Info_Validate(userinfo))
        {
			...
			{
			case CLASS_WOLF:
				strcpy (userinfo, "\\name\\badinfo\\skin\\idg1/base");
				break;
			case CLASS_DOOM:
				strcpy (userinfo, "\\name\\badinfo\\skin\\idg2/green");
				break;
			case CLASS_Q1:
				strcpy (userinfo, "\\name\\badinfo\\skin\\idg3/base");
				break;
			case CLASS_Q2:
			default:
		...
		{
		case CLASS_WOLF:
				newskin= Info_ValueForKey(userinfo, "idg1skin");
				break;
		case CLASS_DOOM:
				newskin= Info_ValueForKey(userinfo, "idg2skin");
				break;
		case CLASS_Q1:
				newskin= Info_ValueForKey(userinfo, "idg3skin");
				break;
		case CLASS_Q2:
		default:
...
//Skid added
		...
		if(!deathmatch->value && !coop->value && !player_class)
		{
...
// Arno : I have no idea what this is supposed to do, but it allows players to
// jump higher in Chasecam-mode, so I commented it out.                
/*
