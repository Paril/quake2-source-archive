...
//Skid added
	switch(player_class)
	{	//Knightmare- fixed the view turning black
		//not all the correct damage_armor, damage_parmor and damage_blood checks were done
...
//Arno changed - decreased blend, otherways Flynn is blinded !

				SV_AddBlend (0.9, 0, 0, 0.08 /* remaining/450.0 */, ent->client->ps.blend);	// red
			else
...
// FIXME - QUAKE DOOM
		...
		case CLASS_DOOM:
			{
				if (remaining == 30)	// beginning to fade
					gi.sound(ent, CHAN_ITEM, gi.soundindex("idg2items/powerups/godfade.wav"), 1, ATTN_NORM, 0);
				if (remaining > 30 || (remaining & 4) )
...
//===================================
	else if (ent->client->invis_framenum > level.framenum)
	{
		...
		{
			if (remaining == 30)	// beginning to fade
				gi.sound(ent, CHAN_ITEM, gi.soundindex("idg3items/powerups/invisfade.wav"), 1, ATTN_NORM, 0);
			if (remaining > 30 || (remaining & 4) )
		...
		{
			if (remaining == 30)	// beginning to fade
				gi.sound(ent, CHAN_ITEM, gi.soundindex ("idg2items/powerups/invisfade.wav"), 1, ATTN_NORM, 0);
		}
...
//===================================
	...
	else if (ent->client->enviro_framenum > level.framenum)
	{
		...
		{
			if(ent->client->resp.player_class == CLASS_Q1)
				gi.sound(ent, CHAN_ITEM, gi.soundindex("idg3items/powerups/envsuitfade.wav"), 1, ATTN_NORM, 0);
			else
...
//=======================
	if (ent->client->invis_framenum > level.framenum)
	{
		...
		else
		{
			...
			if(pclass == CLASS_Q1)
			{
				ent->s.modelindex = gi.modelindex("models/items/idg3powerups/invis/eyes/tris.md2");
				ent->s.skinnum = 0;
			}
			else if(pclass == CLASS_DOOM)
			{
				ent->s.effects = EF_SPHERETRANS;
				ent->s.modelindex = gi.modelindex("models/objects/idg2invis/tris.md2");
				ent->s.skinnum = 0;
			}
...
void G_SetClientSound (edict_t *ent)
{
	...
	{
		if ((strcmp(weap, "weapon_idg2_saw") == 0))
			ent->s.sound = gi.soundindex("idg2weapons/sawidle.wav");
		else 
			ent->s.sound = 0;
	}
}
...
//Skid added
	if(!ent->deadflag && 
	   !(ent->flags & FL_INVIS) &&
	   (ITEM_INDEX(client->pers.weapon) == 18))		// Q1_AXE == 18
		axe=true;
//End Skid
...
//End Skid
		...
		else 
		{
			...
			if(axe)
			{
				if (ent->s.frame != FRAME_axe_jump2)
					ent->s.frame = FRAME_axe_jump1;
				client->anim_end = FRAME_axe_jump6;
			}
			else
			{
