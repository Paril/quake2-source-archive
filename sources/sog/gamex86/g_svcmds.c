...
// ACEBOT_ADD
	...
	{ 
		...
		if(((int)sogflags->value & SOG_REDBLUE_TEAMS) || sog_ctf->value)
			ACESP_SpawnBot (gi.argv(4), gi.argv(2), gi.argv(3), NULL);
		else
...
//Skid added
		...
		PrintMapList();
	else if (Q_stricmp(cmd, "listest")==0)
		GenMapListAlive();
	else if (Q_stricmp(cmd, "curmap")==0)
		gi.dprintf("Current MAP name: %s\n", FindCurrentMap());
	else if (Q_stricmp(cmd, "nextmap")==0)
		gi.dprintf("Next MAP name : %s\n", GenNextMap());
	else if(Q_stricmp(cmd, "teamscore")==0)
		gi.dprintf("Team1 :%d\nTeam2 :%d\nTeam3 :%d\nTeam4 :%d\n",
		gengame.team1,gengame.team2,gengame.team3,gengame.team4);
//end Skid
	else
