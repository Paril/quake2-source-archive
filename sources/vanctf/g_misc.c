...
// CTF - ZOID
	//flags are important
	if (strcmp(self->classname, flag_classnameforteam[CTF_TEAM1]) == 0) {
		flag_reset(CTF_TEAM1); // this will free self!
		gi.bprintf(PRINT_HIGH, "The %s flag has returned!\n",
			team_nameforteam[CTF_TEAM1]);
		return;
	}
	if (strcmp(self->classname, flag_classnameforteam[CTF_TEAM2]) == 0) {
		flag_reset(CTF_TEAM2); // this will free self!
		gi.bprintf(PRINT_HIGH, "The %s flag has returned!\n",
			team_nameforteam[CTF_TEAM2]);
		return;
	}
