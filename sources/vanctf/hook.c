...
edict_t *hook_laser_start (edict_t *ent)
{
	...
	// set the color
	if (IS_TEAM1(ent->owner))		// on red team
	...
	if (rhook->owner) { // && rhook->owner != world) {
		if (rhook->owner->client) {
			...
			if (rhook->owner->client->pers.weapon &&
				strcmp(rhook->owner->client->pers.weapon->pickup_name, "Grapple") == 0) {
...
void fire_hook (edict_t *owner, vec3_t start, vec3_t forward) {
		...
		hook->waterlevel = owner->waterlevel;	// fix splashing sounds
		hook->watertype = owner->watertype;
...
void hook_fire (edict_t *ent) {
	...
	if (ent->client->pers.weapon &&
		strcmp(ent->client->pers.weapon->pickup_name, "Grapple") == 0)
