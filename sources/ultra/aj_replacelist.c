...
void read_replacelist(void)
{
	...
	char	filename[256];
	cvar_t	*game_dir;
	char	strbuf[256], from_weapon[256];
	int		newline;
	...
	i += sprintf(filename + i, replace_file->string);
      ...
      strcat(filename, "/");
	  strcat(filename, replace_file->string);
#endif
...
//		gi.error("Unable to read the replace file.\n");
	...
	do 
	{
		...
		else // start of some data
		{
			...
			aj_replace(from_weapon, strbuf); // try adding this weapon
...
fail_line:;
	...
	sprintf(strbuf, "Read %d mappings\n", mappings);
	gi.dprintf(strbuf);
...
void aj_onegun(int weap)
{
	char *ammo;
	char *weapon;
	float grens=start_grenades->value;
	char number[4];
	int newdmflags;
