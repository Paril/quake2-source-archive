...
// vengeance - waits until < 50% health and vore ball tracks person who hurt you
// hunter - kills person who killed you.
...
void defender_shoot (edict_t *self, edict_t *enemy)
{
	...
	start[2] += 2;
	fire_blaster2 (self->owner, start, dir, defender_blaster_damage->value, defender_blaster_speed->value, EF_BLASTER, 0);
...
void body_gib (edict_t *self)
{
	...
	{
		ThrowGib (self, "models/objects/gibs/arm/tris.md2", 50, GIB_ORGANIC);
		ThrowGib (self, "models/objects/gibs/arm/tris.md2", 50, GIB_ORGANIC);
		ThrowGib (self, "models/objects/gibs/leg/tris.md2", 50, GIB_ORGANIC);
		ThrowGib (self, "models/objects/gibs/leg/tris.md2", 50, GIB_ORGANIC);
		ThrowGib (self, "models/objects/gibs/bone/tris.md2", 50, GIB_ORGANIC);
		ThrowGib (self, "models/objects/gibs/bone2/tris.md2", 50, GIB_ORGANIC);
	}
...
void vengeance_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if(!(self->spawnflags & SPHERE_DOPPLEGANGER))
	{
		if(self->owner->health >= vengeance_health_threshold->value)
			return;
...
edict_t *Sphere_Spawn (edict_t *owner, int spawnflags)
{
	...
	switch(spawnflags & SPHERE_TYPE)
	{
		case SPHERE_DEFENDER:
			...
			sphere->wait = level.time + defender_time->value;
		...
		case SPHERE_HUNTER:
			...
			sphere->wait = level.time + hunter_time->value;
		...
		case SPHERE_VENGEANCE:
			...
			sphere->wait = level.time + vengeance_time->value;
...
void Own_Sphere (edict_t *self, edict_t *sphere)
{
	...
	if(self->client)
	{
		//self->client->owned_sphere->spawnflags = sphere->spawnflags;
