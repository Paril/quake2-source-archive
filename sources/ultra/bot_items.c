...
int	RoamFindBestItem(edict_t	*self, edict_t	*list_head, int	check_paths)
{
	...
	for (trav = list_head ; trav ; trav = trav->node_target)
	{
		...
		else if (list_head == health_head)
		{
			if (trav->count == 100)
				dist_divide = 4;
			else if (self->health > (max_health->value - 10))	// don't need it     // AJ - changed health relative to cvar
				continue;
			else if ((self->health > (max_health->value /2)) && check_paths)	// only check routes for health when really low
				continue;
		}
