...
void Grapple_Touch(edict_t *hook, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	// AJ changed exp_flags & EXPERT_SKY_HOOK to hook_sky->value
	if (!hook_sky->value) {
...
// AJ changed constant to HOOK_DAMAGE to cvar of the same name
		T_Damage(other, hook, hook->owner, hook->velocity, hook->s.origin, plane->normal, hook_damage->value, 0, 0, MOD_GRAPPLE);
	}
	...
	hook->delay = level.time + hook_maxtime->value;
...
void Think_Grapple(edict_t *hook)
{
	...
	else
	{
		if (hook->owner->client->hook_touch) {
			...
			T_Damage(obj, hook, hook->owner, hook->velocity, hook->s.origin, vec3_origin, hook_damage->value, 0, 0, MOD_GRAPPLE);
		}
...
// AJ: changed constant PULL_SPEED to cvar of the same name
	VectorScale(hookDir, /*player->scale * */ hook_pullspeed->value, player->velocity);
...
{
	if (!hook_offhand->value || ent->deadflag || ent->client->hook_frame > level.framenum)
		return;
