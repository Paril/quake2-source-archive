...
// AJ
	if (!OnSameTeam(targ, attacker))
		lithium_applyvampire(attacker, damage);
// end AJ
...
// AJ knockback_*->value multipliers added....
			if (targ->client  && attacker == targ)
				VectorScale (dir, 1600.0 * (float)knockback * knockback_self->value / mass, kvel);	// the rocket jump hack...
			else
				VectorScale (dir, 500.0 * (float)knockback * knockback_adjust->value / mass, kvel);
//end AJ
