...
// FIXME: need pack_armor->value reference... but add to what type of armor?
	if (other->client->pers.max_health < pack_health->value)
		other->client->pers.max_health = pack_health->value;
...
void Use_Breather (edict_t *ent, gitem_t *item)
{
	...
	if (client->breather_framenum > level.framenum)
		client->breather_framenum += (breather_time->value * 10);
	else
		client->breather_framenum = level.framenum + (breather_time->value * 10);
...
void Use_Envirosuit (edict_t *ent, gitem_t *item)
{
	...
	if (client->enviro_framenum > level.framenum)
		client->enviro_framenum += (enviro_time->value * 10);
	else
		client->enviro_framenum = level.framenum + (enviro_time->value * 10);
...
void	Use_Invulnerability (edict_t *ent, gitem_t *item)
{
	...
	if (client->invincible_framenum > level.framenum)
		client->invincible_framenum += (inv_time->value * 10);
	else
		client->invincible_framenum = level.framenum + (inv_time->value * 10);
...
void	Use_Silencer (edict_t *ent, gitem_t *item)
{
	...
	client->pers.inventory[ITEM_INDEX(item)]--;
	ValidateSelectedItem (ent);
	client->silencer_shots += silencer_shots->value;
...
qboolean Pickup_Armor (edict_t *ent, edict_t *other)
{
	...
	{
		if (!old_armor_index)
			client->pers.inventory[jacket_armor_index] = armor_bonus_value->value;
		else
			client->pers.inventory[old_armor_index] += armor_bonus_value->value;
	}
