...
// end AJ
	...
	if (delta > 30)
	{
		if (ent->health > 0)
		{
			...
			else if (hook_style->value==1 && ent->client->hook_on && ent->client->hook)
				return;
...
// AJ hack to save bots from suiciding in lava
			if (current_player->bot_client)
			{
				if (envirosuit)	// take 1/3 damage with envirosuit
					T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, bot_lava_mod->value*1*waterlevel, 0, 0, MOD_LAVA);
				else
					T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, bot_lava_mod->value*3*waterlevel, 0, 0, MOD_LAVA);
			}
			else
			{
				if (envirosuit)	// take 1/3 damage with envirosuit
					T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, lava_mod->value*1*waterlevel, 0, 0, MOD_LAVA);
				else
					T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, lava_mod->value*3*waterlevel, 0, 0, MOD_LAVA);
			}
// end AJ
		}
...
// AJ hack to save bots from suiciding in slime
				if (current_player->bot_client)
					T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, bot_slime_mod->value*1*waterlevel, 0, 0, MOD_SLIME);
				else
					T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, slime_mod->value*1*waterlevel, 0, 0, MOD_SLIME);
// end AJ
			}
		}
	}
...
//ZOID
	...
	if (ent->client->showscores && (!(level.framenum & 31) || (level.intermissiontime == level.time))) // AJ added extra clause to get that last update
	{
//ZOID
		if (ent->client->menu) {
			PMenu_Update(ent);
