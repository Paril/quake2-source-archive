...
// AJ changed constants to cvars. un-did Ridah's optimisation (/4 changed to /2)
	monster_fire_shotgun (self, start, forward, damage, 4, bot_sshotgun_hspread->value, bot_sshotgun_vspread->value, bot_sshotgun_count->value/2, MZ_SSHOTGUN, MOD_SSHOTGUN);

	angles[YAW] -= 10;
	AngleVectors(angles, forward, NULL, NULL);
// AJ changed constants to cvars. un-did Ridah's optimisation (/4 changed to /2)
	monster_fire_shotgun (self, start, forward, damage, 4, bot_sshotgun_hspread->value, bot_sshotgun_vspread->value, bot_sshotgun_count->value/2, MZ_SSHOTGUN | MZ_SILENCED, MOD_SSHOTGUN);
...
// AJ changed constant (3) to cvar 
	damage = bot_chaingun_damage->value * shots;
	kick = 2;
...
// AJ changed constants (*0.5!!) to cvars (hspread, vspread)
		fire_bullet (self, start, forward, damage, kick, bot_chaingun_hspread->value, bot_chaingun_hspread->value, MOD_CHAINGUN);
//	}
...
// AJ changed constant (100) to cvar
	damage	= bot_railgun_damage->value;
	kick	= 200;
...
// AJ changed constant (100) to bot_rocket_damage, 20.0 to bot_rocket_damage2
	damage = bot_rocket_damage->value + (int)(random() * bot_rocket_damage2->value);
// AJ changed constant (120) to bot_rocket_rdamage
	radius_damage = bot_rocket_rdamage->value;
// AJ changed constant (12) to bot_rocket_radius
	damage_radius = bot_rocket_radius->value;
...
// AJ changed constant (120) to cvar
	damage = bot_grenade_damage->value;
// AJ - changed algorithm for calculating grenade radius (cvar)
//	radius = damage+40;
	radius = bot_grenade_radius->value;
// end AJ
...
// AJ changed constant 1000 to cvar bot_hyperblaster_speed->value
	fire_blaster (self, start, forward, damage, bot_hyperblaster_speed->value, effect, true);
	self->client->pers.inventory[self->client->ammo_index]--;
