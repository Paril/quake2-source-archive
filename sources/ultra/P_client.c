...
// AJ
	...
	if (hook_style->value==0 && self->client->hook)
...
// special case it so that bots still get CTF grapple when hook_offhand set
	...
	if (hook_offhand->value && ent->bot_client)
	{
		client->pers.inventory[ITEM_INDEX(FindItem("Grapple"))] = 1;
...
// end AJ - zkick
	...
	if (client->hook_on && client->hook && hook_style->value==1)
...
// AJ
	...
	if (hook_style->value==0 && client->on_hook == true)
		Pull_Grapple(ent);
// end AJ
...
// AJ
	...
	if (hook_style->value==0 && client->on_hook)
...
// AJ abandon hope hook
	if (hook_style->value==1 && client->hook_on && VectorLength(ent->velocity) < 1) 
		  client->ps.pmove.gravity = 0;
...
// AJ added
	...
	if (hook_offhand->value && //|| expflags & EXPERT_POGO) && 
	...
	if (hook_offhand->value && Ended_Grappling (client) && 
