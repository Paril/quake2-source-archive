...
void ThrowClientHead (edict_t *self, int damage)
{
	...
	// Expert: prevent gibs floating near horizontal surfaces 
...
{
// CTF
	// flags are important, don't let plats/doors destroy them
...
/*QUAKED misc_block (0 .5 .8) (-16 -16 0) (16 16 40)
 * Expert: This ent simply does nothing. Its sole purpose is to block off
 * regions of maps from gameplay. A message will be printed to all players
 * in the vicinity of the block that the area is cut off.
 */

void misc_block_think (edict_t *self)
{
       edict_t *player;                // Temporary player ent
       vec3_t delta;                   // Difference in origin from player to block
       vec_t distance;                 // Distance from player to block

       // Print a message to all players in the near vicinity
       // For every player in the game:
       for (player = g_edicts+1; player < g_edicts + game.maxclients + 1; player++) {
               if (player->inuse && player->client != NULL) {
                       VectorSubtract(self->s.origin, player->s.origin, delta);
                       distance = VectorLength(delta);

                       if (distance < 80)              // If the player is within 80u:
                               // If the player hasn't been recently informed of a block:
                               if (player->client->block_time < level.time) {
                                       // Inform the player
                                       gi.centerprintf(player, "Area Blocked Off.\n");
                                       // Note that the player was informed
                                       player->client->block_time = level.time + 20*FRAMETIME; // 2 secs/message
