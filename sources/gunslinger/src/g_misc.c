...
{
	// Preserve special items -- Stone
	if (self->item) {
		if (self->item->flags & IT_ARTIFACT) {
//			debugmsg("Artifact crushed... respawning\n");
			GQ_RespawnSpecial(self);
			return;
		}
		if (((int)playmode->value == PM_BADGE_WARS) && GQ_MatchItem(self->item, "Sheriff Badge")) {
//			debugmsg("Sheriff Badge crushed... respawning\n");
			GQ_RespawnSpecial(self);
			return;
		}
		if (GQ_MatchItem(self->item, "Money Bag")) {
//			debugmsg("Money bag crushed... respawning\n");
			GQ_RespawnSpecial(self);
			return;
		}
		if ((self->item->flags & IT_WEAPON) && (weapons_unique->value)) {
			GQ_DelayedRespawn (self);
			return;
		}
	}
...
//		gi.sound (self, CHAN_AUTO, gi.soundindex("world/brkglas.wav") , 1, ATTN_NORM, 0); 
		gi.sound (self, CHAN_AUTO, gi.soundindex(va("gunslinger/glass%i.wav",(rand()%5)+1)) , 1, ATTN_NORM, 0); 
		G_FreeEdict (self);
	}
}
...
{
/*	What? Dropping this would take all the fun out of it...
	if (deathmatch->value)
