...
void jorgBlackHole (edict_t *self);
...
mframe_t jorg_frames_attack3 []=
{
	...
	{ai_run_circle,	10,	NULL},
	{ai_run_circle,	10,	NULL},
	{ai_run_circle,	10,	jorgBlackHole},
	{ai_run_circle,	10,	NULL},
	{ai_run_circle,	10,	NULL},
	{ai_run_circle,	10,	NULL},
	{ai_run_circle,	10,	NULL},
...
mmove_t jorg_move_attack3 = {FRAME_attak201, FRAME_attak213, jorg_frames_attack3, jorg_run};
...
{
	{ai_run_circle,	22,	jorg_firebullet},
	{ai_run_circle,	22,	jorg_firebullet},
	{ai_run_circle,	22,	jorg_firebullet},
	{ai_run_circle,	22,	jorg_firebullet},
	{ai_run_circle,	22,	jorg_firebullet},
...
//	gi.dprintf("%s: %f %f\n", self->classname, level.time, self->pain_debounce_time);
	if (level.time < self->pain_debounce_time)
			return;

	// Lessen the chance of him going into his pain frames if he takes little damage
	if (damage <= 40 + 2 * self->monsterinfo.skill)
		if (random()<=0.6)
			return;
	...
	*/
	...
	self->pain_debounce_time = level.time + (5 + 0.25 * self->monsterinfo.skill);
	...
	} else if (damage <= 100 + 18 * self->monsterinfo.skill) {
...
void jorgBFG (edict_t *self) {
	...
	if (self->radius_dmg)
		damage = 200 + 16 * self->monsterinfo.skill;
	else
		damage = 100 + 8 * self->monsterinfo.skill;
...
//	gi.sound (self, CHAN_VOICE, sound_attack2, 1, ATTN_NORM, 0);
	/*void monster_fire_bfg (edict_t *self,
							 ...
							 float damage_radius,
							 int flashtype)*/
	monster_fire_bfg(self, start, dir, damage, 600 + 6 * self->monsterinfo.skill, 150 + 1 * self->monsterinfo.skill, 200 + 1 * self->monsterinfo.skill, MZ2_JORG_BFG_1);
}

void monster_fire_blackhole(edict_t *ent);
void jorgBlackHole (edict_t *self) {
	monster_fire_blackhole(self);
}
...
void jorg_firebullet_right (edict_t *self)
{
	...
	monster_fire_bullet (self, start, forward, damage, 4, DEFAULT_BULLET_HSPREAD * spreadmult, DEFAULT_BULLET_VSPREAD * spreadmult, MZ2_JORG_MACHINEGUN_R1);
}
...
void jorg_attack(edict_t *self) {
	...
	if ((self->monsterinfo.currentmove == &jorg_move_start_attack1) ||
		(self->monsterinfo.currentmove == &jorg_move_attack1) ||
		(self->monsterinfo.currentmove == &jorg_move_attack2) ||
		(self->monsterinfo.currentmove == &jorg_move_attack3)) {
		return;
	}
...
//		return false;
	...
	if (random () < chance) {
		self->monsterinfo.attack_state = AS_MISSILE;
		self->monsterinfo.attack_finished = level.time + 0.4*random();
		return true;
	}
