...
void use_target_secret (edict_t *ent, edict_t *other, edict_t *activator)
{
	...
	if (level.found_secrets == level.total_secrets) {
		gi.bprintf(PRINT_HIGH, "All secrets found, %d bonus exp awarded!\n", 2500 + 500 * level.total_secrets);
		giveExpToAll(2500 + 500 * level.total_secrets);
	}
...
void use_target_goal (edict_t *ent, edict_t *other, edict_t *activator)
{
	...
	if (level.found_goals == level.total_goals) {
		gi.bprintf(PRINT_HIGH, "All goals accomplished, %d bonus exp awarded!\n", 1000 + 1000 * level.total_goals);
		giveExpToAll(1000 + 1000 * level.total_goals);
...
void target_explosion_explode (edict_t *self)
{
	...
	T_RadiusDamage (NULL, self, self->activator, self->dmg, 0, NULL, self->dmg+40, true, MOD_EXPLOSIVE);
...
//		T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, 10 * other->max_health, 1000, 0, MOD_EXIT);
	...
	// if coop, make sure all clients are nearby.
	if (coop->value) {
		for (i=0 ; i<maxclients->value ; i++) {
			...
			VectorSubtract (activator->s.origin, scan->s.origin, v);
			if (VectorLength (v) > 500) {
				if (level.time < other->pain_debounce_time) {
					gi.cprintf(activator, PRINT_HIGH, "%s is too far away\n", scan->client->pers.netname);
				}
				ret = true;
			}
		}
		if (ret) {
			other->pain_debounce_time = level.time + 5.0;
			return;
		}
	}
...
void use_target_splash (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (self->dmg)
		T_RadiusDamage (NULL, self, activator, self->dmg, 0, NULL, self->dmg+40, true, MOD_SPLASH);
}
