...
void mutant_hit_left (edict_t *self)
{
	...
	int damage;
	if (self->radius_dmg)
		damage = 50 + 10 * self->monsterinfo.skill;
	else
		damage = 25 + 5 * self->monsterinfo.skill;
...
void mutant_check_refire (edict_t *self)
{
	...
	if ((range(self, self->enemy) == RANGE_MELEE) && (random() < (0.3 + 0.02 * self->monsterinfo.skill)))
...
void mutant_jump_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf) {
	...
	if (other->takedamage)
	{
		if ((other == self->enemy) && (VectorLength(self->velocity) > 400))
		{
			vec3_t	point;
			vec3_t	normal;
			int		damage;
			...
			damage = (int) ceil((float)(30 + 3 * self->monsterinfo.skill) * (float)(0.5 + random()));
			T_Damage (other, self, self, self->velocity, point, normal, damage, 0, 0, MOD_UNKNOWN);
			mutant_melee(self);
			return;
		}
	}
	...
	if (!M_CheckBottom (self)) {
		if (self->groundentity) {
			self->monsterinfo.nextframe = FRAME_attack03; // was 02
		}
		return;
	}
}
...
//	self->s.origin[2] += 1;
	VectorScale (forward, 600 + 10 * self->monsterinfo.skill, self->velocity);
	self->velocity[2] = 225;
	if (self->enemy->s.origin[2] > self->s.origin[2])
		self->velocity[2] += (self->enemy->s.origin[2] - self->s.origin[2]) * 1.5;
	self->groundentity = NULL;
	self->monsterinfo.aiflags |= AI_DUCKED;
	self->monsterinfo.attack_finished = level.time + 1.2;
	self->touch = mutant_jump_touch;
	self->yaw_speed = 10; // default 27
}
...
void mutant_check_landing (edict_t *self)
{
	...
	tmp = self->velocity[2];
	AngleVectors (self->s.angles, forward, NULL, NULL);
	VectorScale (forward, 900, self->velocity);
	self->velocity[2] = tmp;

	if (level.time > self->monsterinfo.attack_finished)
		self->monsterinfo.nextframe = FRAME_attack04; // was 02
	else {
		self->yaw_speed = 27;
...
qboolean mutant_check_jump (edict_t *self)
{
	...
	if (!self->enemy)
		return false;
	if (self->enemy->s.origin[2] > self->s.origin[2] + 512)
		return false;
...
{
	if (random() < (0.55 + 0.07 * self->monsterinfo.skill))
		mutant_checkattack(self);
	if (!self->radius_dmg || !(self->monsterinfo.ability & GIEX_MABILITY_SILENCED)) {
		gi.sound (self, CHAN_VOICE, sound_sight, 1, ATTN_NORM, 0);
	}
}
...
void mutant_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (random() < 0.7 - 0.03 * self->monsterinfo.skill)
		return;
...
//	sound_step3 = gi.soundindex ("mutant/step3.wav");
	...
	self->mass = 300;
	self->health = 360 + 45 * skill->value;
	self->max_health = self->health;
	self->monsterinfo.level = 495 + 285 * skill->value;
	self->monsterinfo.skill = skill->value;
	self->gib_health = -180 - 20 * skill->value;
	strcpy(self->monsterinfo.name, "a mutant");
