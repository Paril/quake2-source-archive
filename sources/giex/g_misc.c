...
void ClipGibVelocity (edict_t *ent)
{
	...
	if (ent->velocity[1] < -300)
		...
		ent->velocity[1] = 300;
	if (ent->velocity[2] < 70)
		ent->velocity[2] = 70;	// always some upwards
...
//	vec3_t	normal_angles, right;
	...
	if (self->classid == CI_PIZZAGIB) {
		if (other && other->inuse && other->client && other != self->owner) {
			other->client->barf_time = level.time + 30;
			other->client->barf_frames = random() * 20 + 30;
			self->touch = NULL;
		}
	}
...
edict_t *ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	gib->s.origin[0] = (float) origin[0] + (float) crandom() * (float) size[0];
	gib->s.origin[1] = (float) origin[1] + (float) crandom() * (float) size[1];
	gib->s.origin[2] = (float) origin[2] + (float) crandom() * (float) size[2];
	...
	ClipGibVelocity (gib);
	gib->avelocity[0] = (float) random()*600.0;
	gib->avelocity[1] = (float) random()*600.0;
	gib->avelocity[2] = (float) random()*600.0;

	gib->think = G_FreeEdict;
	gib->nextthink = level.time + 0.5 + random();
...
void ThrowHead (edict_t *self, char *gibname, int damage, int type)
{
	...
	self->think = G_FreeEdict;
	self->nextthink = level.time + 1 + random()*2;
...
void ThrowDebris (edict_t *self, char *modelname, float speed, vec3_t origin)
{
	...
	chunk->nextthink = level.time + 0.5 + random();
	chunk->s.frame = 0;
	chunk->flags = 0;
	chunk->classid = CI_DEBRIS;
...
void func_explosive_explode (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	if (self->dmg)
		T_RadiusDamage (NULL, self, attacker, self->dmg, 0, NULL, self->dmg+40, true, MOD_EXPLOSIVE);
...
void barrel_explode (edict_t *self)
{
	...
	T_RadiusDamage (NULL, self, self->activator, self->dmg, 0, NULL, self->dmg+40, true, MOD_BARREL);
...
void SP_misc_deadsoldier (edict_t *ent)
{
	...
	ent->health = 0;
	ent->gib_health = -100 - 25 * skill->value;
...
void misc_viper_bomb_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	T_RadiusDamage (NULL, self, self, self->dmg, 0, NULL, self->dmg+40, true, MOD_BOMB);
	BecomeExplosion2 (self);
}
...
void misc_viper_bomb_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	viper = G_FindClassId (NULL, CI_MISC_VIPER);
...
void setLight(void) {
	...
	for (i = 0; i < 63; i++) {
		sprintf(s, "%c", c);
		gi.configstring(CS_LIGHTS + i, s);
	}
}
void decLight(void) {
	if (level.light_level > 0)
		level.light_level--;
	setLight();
}
void incLight(void) {
	if (level.light_level < 12)
		level.light_level++;
	setLight();
}
...
void addStealth(void) {
	level.stealthcount++;
//	gi.dprintf("Stealths: %d\n", level.stealthcount);
}

void removeStealth(void) {
	if (level.stealthcount > 0)
		level.stealthcount--;
//	gi.dprintf("Stealths: %d\n", level.stealthcount);
}
