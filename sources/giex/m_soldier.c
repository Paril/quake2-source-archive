...
//	int		n;
	...
	self->pain_debounce_time = level.time + 2 + 0.1 * self->monsterinfo.skill;
	...
	if (random() < 1 - 0.02 * self->monsterinfo.skill)
		return;
...
//	if ((random() < (0.3 + 0.05 * self->monsterinfo.skill)) || (range(self, self->enemy) == RANGE_MELEE))
//		self->monsterinfo.nextframe = FRAME_attak402;
	self->monsterinfo.attack_finished = level.time + 0.2 + 0.4 * random();
}
...
mframe_t soldier_frames_attack4 [] =
{
	...
	{ai_run_circle, 16, soldier_fire4},
...
void soldier_attack7_refire (edict_t *self) {
	if ((!self->enemy) || (self->enemy->health <= 0))
		return;
/*	if (random() < 0.75)
		return;*/
...
mframe_t soldier_frames_attack6 [] =
{
	...
	{ai_run_circle, 14, NULL},
	{ai_run_circle, 14, NULL},
	{ai_run_circle, 14, soldier_fire8},
	{ai_run_circle, 14, NULL},
	{ai_run_circle, 14, NULL},
	{ai_run_circle, 14, NULL},
	{ai_run_circle, 14, NULL},
	...
	{ai_run_circle, 14, NULL},
	{ai_run_circle, 14, NULL},
	{ai_run_circle, 14, soldier_attack6_refire}
};
mmove_t soldier_move_attack6 = {FRAME_runs01, FRAME_runs14, soldier_frames_attack6, soldier_run};

mframe_t soldier_frames_attack7 [] =
{
	{ai_run_circle, 10, NULL},
	{ai_run_circle, 11, NULL},
	{ai_run_circle, 13, NULL},
	{ai_run_circle, 15, soldier_fire8},
	{ai_run_circle, 17, soldier_fire8},
	{ai_run_circle, 17, soldier_fire8},
	{ai_run_circle, 17, soldier_fire8},
	{ai_run_circle, 17, soldier_fire8},
	{ai_run_circle, 17, soldier_fire8},
	...
	{ai_run_circle, 17, NULL},
	{ai_run_circle, 17, soldier_attack7_refire}
};
mmove_t soldier_move_attack7 = {FRAME_runs01, FRAME_runs14, soldier_frames_attack7, soldier_run};

void soldier_attack(edict_t *self) {
	if (self->s.skinnum < 2) {
/*		if (random() <= 0.25) {
			self->monsterinfo.pausetime = level.time + (8 + rand() % 8) * FRAMETIME;
			self->monsterinfo.currentmove = &soldier_move_attack4;
		} else {*/
			self->monsterinfo.currentmove = &soldier_move_attack7;
//		}
	} else if (self->s.skinnum < 4) {
...
void soldier_sight(edict_t *self, edict_t *other) {
	...
	if (random() > 0.5 + 0.03 * self->monsterinfo.skill)
		soldier_attack(self);
}
...
void soldier_dodge (edict_t *self, edict_t *attacker, float eta)
{
	...
	if ((!self->enemy) && (attacker->client))
			self->enemy = attacker;

	if (self->monsterinfo.skill == 0)
	{
...
// regular death
	...
	if (n == 0) {
		...
		self->monsterinfo.pausetime = level.time + (6 + rand() % 10) * FRAMETIME;
	} else if (n == 1)
...
//	sound_sight2 =	gi.soundindex ("soldier/solsrch1.wav");
	...
	self->monsterinfo.dodge = NULL; //soldier_dodge;
	self->monsterinfo.attack = soldier_attack;
	self->monsterinfo.melee = NULL;
	self->monsterinfo.sight = soldier_sight;
	self->monsterinfo.aggressive = true;

	self->monsterinfo.cts_any = 0.4;
	self->monsterinfo.cts_class1 = 0.005;
	self->monsterinfo.cts_class2 = 0.03;
	self->monsterinfo.cts_class3 = 0.15;
...
//	sound_death_light =	gi.soundindex ("soldier/soldeth2.wav");
	...
	self->s.skinnum = 0;
	self->health = 70 + 16 * skill->value;
	self->max_health = self->health;
	self->monsterinfo.level = 270 + 155 * skill->value;
	self->gib_health = -100 - 15 * skill->value;
	strcpy(self->monsterinfo.name, "a light soldier");

	self->monsterinfo.prefered_range = 60;
}
...
//	sound_death = gi.soundindex ("soldier/soldeth1.wav");
	...
	self->s.skinnum = 2;
	self->health = 50 + 12 * skill->value;
	self->max_health = self->health;
	self->monsterinfo.level = 250 + 145 * skill->value;
	self->monsterinfo.skill = skill->value;
	self->gib_health = -100 - 15 * skill->value;
	strcpy(self->monsterinfo.name, "a shotgun guard");

	self->monsterinfo.prefered_range = 40;
}
...
//	sound_death_ss = gi.soundindex ("soldier/soldeth3.wav");
	...
	self->s.skinnum = 4;
	self->health = 60 + 14 * skill->value;
	self->max_health = self->health;
	self->monsterinfo.level = 245 + 140 * skill->value;
	self->gib_health = -100 - 15 * skill->value;
	strcpy(self->monsterinfo.name, "a heavy soldier");
