...
void Boss2Rocket (edict_t *self)
{
	...
	vec3_t	dir;
	vec3_t	vec;
	int speed = 500 * 5 * self->monsterinfo.skill;
	int damage;
	if (self->radius_dmg)
		damage = (175 + 26 * self->monsterinfo.skill);
	else
		damage = (90 + 13 * self->monsterinfo.skill);
...
void boss2_firebullet_right (edict_t *self)
{
	...
	int damage;
	if (self->radius_dmg)
		damage = (18 + 3.6 * self->monsterinfo.skill);
	else
		damage = (9 + 1.8 * self->monsterinfo.skill);
	...
	monster_fire_bullet (self, start, forward, damage, 4, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, MZ2_BOSS2_MACHINEGUN_R1);
}	
...
void boss2_firebullet_left (edict_t *self)
{
	...
	monster_fire_bullet (self, start, forward, damage, 4, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, MZ2_BOSS2_MACHINEGUN_L1);
}	
...
void boss2_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	self->pain_debounce_time = level.time + (4 + 0.2 * self->monsterinfo.skill);
}
...
//	sound_pain3 = gi.soundindex ("bosshovr/bhvpain3.wav");
	...
	self->health = 2800 + 850 * skill->value;
	self->max_health = self->health;
	self->monsterinfo.level = 410 + 320 * skill->value;
	self->monsterinfo.skill = skill->value;
	self->gib_health = -2000;
	self->mass = 1000;
	self->yaw_speed = 18;
	strcpy(self->monsterinfo.name, "a big flying thingie");
	...
	self->monsterinfo.cts_any = 0.85;
	self->monsterinfo.cts_class1 = 0.15;
	self->monsterinfo.cts_class2 = 0.35;
	self->monsterinfo.cts_class3 = 0.65;
	self->monsterinfo.item_any = 0.75;
