...
void weapon_railgun_fire (edict_t *ent)
{
	...
	//VectorScale (forward, -600, ent->velocity);

	VectorSet(offset, 0, 7,  ent->viewheight-8);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
	fire_rail (ent, start, forward, damage, kick);

	ent->client->damage_alpha += 0.05;
	ent->client->damage_blood = 5;
	ent->client->v_dmg_time = 0;

	if (ent->client->damage_alpha > 1)
	{
		ent->health = 0;
		T_RadiusDamage(ent,ent,100,ent,100,MOD_SHOTGUN);
		player_die (ent,world,ent,10000,ent->s.origin);
	...
	if (ent->client->damage_blood)
		VectorMA (v, (float)ent->client->damage_blood,  bcolor, v);

	VectorCopy (v, ent->client->damage_blend);
...
void Weapon_Railgun (edict_t *ent)
{
	...
	Weapon_Generic (ent, 3, 3, 56, 61, pause_frames, fire_frames, weapon_railgun_fire);
}
