...
void ClientBeginDeathmatch (edict_t *ent)
{
	...
	else if (!level.seeker)
	{
		...
		ent->client->resp.spectator = ent->client->pers.spectator = true;

	// Get gl_modulate
	gi.WriteByte (SVC_STUFFTEXT);
	gi.WriteString ("set \"glmod\" $gl_modulate u\n");
	gi.unicast(ent, true);
...
void ClientUserinfoChanged (edict_t *ent, char *userinfo)
{
	...
	ent->client->resp.glmod = atoi(Info_ValueForKey (userinfo, "glmod"));
...
void ClientThink (edict_t *ent, userCmd_t *ucmd)
{
	...
	if (ent->client->chase_target) {
		...
		if (level.seeker == ent && !level.hiding && level.hider)
			client->ps.pMove.pmType = PMT_FREEZE;
	...
	//		gi.dprintf ("pmove changed!\n");
		...
		if (level.hider == ent)
		{
			if (ucmd->forwardMove > 0 || ucmd->forwardMove < 0 || ucmd->sideMove < 0 || ucmd->sideMove > 0)
				level.hiderMoved = true;
			else
				level.hiderMoved = false;
		}
