...
// GRIM
void soldier_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf);
// GRIM
...
// GRIM 20/10/2001 12:12AM
void soldier_sts_think (edict_t *self)
{
	...
	/*
	if (self->s.frame == FRAME_death401)
		self->s.frame = FRAME_death402;
	else if (self->s.frame == FRAME_death402)
		self->s.frame = FRAME_pain101;
	else if (self->s.frame == FRAME_pain101)
		self->s.frame = FRAME_pain301;
	else 
		self->s.frame = FRAME_death401;
	*/
	if (self->s.frame == FRAME_pain301)
		self->s.frame = FRAME_pain101;
	else
		self->s.frame = FRAME_pain301;
...
qboolean soldier_sts (edict_t *self)
{
	...
	if ((self->last_mod != MOD_CHAINGUN) && (self->last_mod != MOD_MACHINEGUN))
		return false;
	...
	self->think = soldier_sts_think;
	self->nextthink = level.time + FRAMETIME;
	self->timestamp = level.time + FRAMETIME * 3;
...
mframe_t soldier_frames_quick_death [] =
{
	ai_move, 0,   NULL,
	ai_move, 0,   NULL,
	ai_move, 0,   NULL,
...
mmove_t soldier_move_quick_death = {FRAME_death131, FRAME_death136, soldier_frames_quick_death, soldier_dead};

mframe_t soldier_frames_quick_death2 [] =
{
	ai_move, 0,   NULL,
	ai_move, 0,   NULL,
	ai_move, 0,   NULL
};
mmove_t soldier_move_quick_death2 = {FRAME_death450, FRAME_death453, soldier_frames_quick_death, soldier_dead};
...
// check for gib
	...
	{
		// GRIM 19/10/2001 5:12PM
		if (self->touch && (random() < 0.5))
		{
			gitem_t *item;
			int count;
			...
			if (self->s.skinnum <= 1)
			{
	        		item = GetItemByTag(II_STROGG_BLASTER);
				count = 7 + (random() * 9);
			}
			else if (self->s.skinnum <= 3)
			{
	        		item = GetItemByTag(II_STROGG_SHOTGUN);
				count = 4 + (random() * 7);
			}
			else
			{
	        		item = GetItemByTag(II_STROGG_SUBMACH);
				count = 15 + (random() * 17);
			}
	
        		if (item)
				MonsterDropItem (self, item, count, 0, item->ammoTag);
		}
		// GRIM
...
// GRIM 20/10/2001 12:09AM
	if (self->deadflag == DEAD_DYING)
	{
		...
		if (self->monsterinfo.currentmove->firstframe == FRAME_death401)
		{
			if (self->s.frame < FRAME_death414)
			{
				self->monsterinfo.currentmove = &soldier_move_death6;
				return;
			}
			self->monsterinfo.currentmove = &soldier_move_quick_death2;
			self->deadflag = DEAD_DEAD;
		}

		if ((self->monsterinfo.currentmove->firstframe == FRAME_death101) && (self->s.frame >= FRAME_death107))
		{
			self->monsterinfo.currentmove = &soldier_move_quick_death;
			self->deadflag = DEAD_DEAD;
		}

		if ((self->monsterinfo.currentmove->firstframe == FRAME_death201) && (self->s.frame >= FRAME_death205))
		{
			self->monsterinfo.currentmove = &soldier_move_quick_death;
			self->deadflag = DEAD_DEAD;
		}

		if ((self->monsterinfo.currentmove->firstframe == FRAME_death301) && (self->s.frame >= FRAME_death306))
		{
			self->monsterinfo.currentmove = &soldier_move_quick_death;
			self->deadflag = DEAD_DEAD;
		}

		if ((self->monsterinfo.currentmove->firstframe == FRAME_death501) && (self->s.frame >= FRAME_death507))
		{
			self->monsterinfo.currentmove = &soldier_move_quick_death;
			self->deadflag = DEAD_DEAD;
		}
...
void SP_monster_soldier_x (edict_t *self)
{
	...
	// GRIM 19/10/2001 12:01AM
	gi.modelindex ("models/monsters/soldier/dead/tris.md2");
...
void soldier_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	ent->s.modelindex = gi.modelindex ("models/monsters/soldier/dead/tris.md2");
	if (ent->s.frame == FRAME_death136)
		ent->s.frame = 0;
	else if (ent->s.frame == FRAME_death235)
		ent->s.frame = 1;
	else if (ent->s.frame == FRAME_death345)
		ent->s.frame = 2;
	else if (ent->s.frame == FRAME_death453)
		ent->s.frame = 3;
	else if (ent->s.frame == FRAME_death524)
		ent->s.frame = 4;
	else if (ent->s.frame == FRAME_death610)
		ent->s.frame = 5;
	...
	if (Pickup_BAItem(item, count, 0, item->ammoTag, other))
	{
		...
		other->client->ps.stats[STAT_PICKUP_STRING] = CS_ITEMS + ITEM_INDEX(item);
