...
// GRIM 26/06/2001 3:32PM - more states, and shorter names for them
typedef enum 
{
	W_READY, 
	W_IDLE, 
...
// GRIM 26/06/2001 10:18AM - new inventory system
#define IT_CLIP         64
#define IT_SLING        128
// GRIM
...
typedef struct
{
	...
	int			power_cubes;		// ugly necessity for coop
        ...
        // GRIM 26/06/2001 1:06PM
        int dropped_item_count;
        int dropped_item_index;  
        // GRIM 
} level_locals_t;
...
typedef struct
{
	...
	// GRIM 4/11/2001 6:46PM
	char *equipment;	// replaces sv_equipment string
	// GRIM
} spawn_temp_t;
...
// GRIM 26/06/2001 1:06PM
extern	cvar_t	*sv_maxdropped;
extern	cvar_t	*sv_maxknives;
extern	cvar_t	*sv_edit;
extern	cvar_t	*sv_equipment;
// GRIM
...
typedef struct
{
	...
	// GRIM 6/10/2001 1:58PM - new inventory system
	int	cstats[8];
        int     item_bodyareas[BA_MAX];
        int     item_quantities[BA_MAX];
        int     item_flags[BA_MAX];
        int     item_ammotypes[BA_MAX];
	int 	hgren_type;
	// GRIM
} client_persistant_t;
...
struct gclient_s
{
	...
	// GRIM 26/06/2001 4:48PM
	int             newRightWeapon;
	int             newLeftWeapon;
	int             newRightAmmoType;
	int             newLeftAmmoType;
	qboolean        action;

	float           cycleItems;
	int             itemSelect;
        int             sortedItems[BA_MAX];
        int             sortedItemBodyAreas[BA_MAX];
        int		deadflag;
        qboolean		weapon_halfspeed;
	float ir_framenum;
	int previous_right_tag;
	int previous_right_bodyarea;
	int previous_left_tag;
	int previous_left_bodyarea;
	float action_msg_time;
// GRIM
};
...
struct edict_s
{
	...
	monsterinfo_t	monsterinfo;
        ...
        // GRIM 26/06/2001 10:20AM - new defs
        float   last_fire;
        int     last_hitloc;
	int	(*dmgadjust)(edict_t *ent, vec3_t point, vec3_t normal, int damage, int dflags, int mod);
	vec3_t 	spawnpoint;
	qboolean decoy;
	edict_t *storage_item;
	int last_mod;
