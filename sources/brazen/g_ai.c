...
void AI_SetSightClient (void)
{
	...
	while (1)
	{
		...
		{
			// GRIM 8/01/2002 1:06PM
			if (ent->movetype == MOVETYPE_NOCLIP)
			{
				if (ent->target_ent)
				{
					level.sight_client = ent->target_ent;
					return;		// got one
				}
			}
			else
...
void FoundTarget (edict_t *self)
{
	...
	// GRIM 8/01/2002 1:16PM - or if it is a coopview decoy
	if (self->enemy->client || self->enemy->decoy)
	// GRIM
	{
...
//			level.disguise_violation_framenum = level.framenum + 5;
	...
	// GRIM 20/10/2001 12:38PM - if we have a kill target, attack it
	if (self->killtarget)
	{
	...
	{
		// GRIM 27/10/2001 8:59PM 
		if (self->emp_framenum > level.framenum)
			return; // getting fucked up by an emp grenade still
		// GRIM
...
qboolean FindTarget (edict_t *self)
{
	...
	// GRIM 22/10/2001 10:40PM
	if ((self->monsterinfo.aiflags & AI_GOOD_GUY) && (!self->killtarget))
	// GRIM
	{
		...
		// GRIM 22/10/2001 10:48PM
		if (strcmp(self->classname, "misc_actor") != 0)
			return false;

		FindSomeMonster(self);
		if (self->enemy)
			return true;
	...
	// GRIM 27/10/2001 8:59PM 
	if (self->emp_framenum > level.framenum)
		return false; // getting fucked up by an emp grenade still
	// GRIM

	// GRIM 20/10/2001 12:11PM
	if (self->killtarget && (!(self->spawnflags & 1)))
	{
...
// but not weapon impact/explosion noises
	...
	/* GRIM else if (level.disguise_violation_framenum > level.framenum)
	{
...
//			gi.dprintf ("ignoring coop sound target\n");
	...
	// GRIM 8/01/2002 1:13PM - always go for coopview decoys
	else if (client->decoy && client->target_ent)
	{
		if (client->target_ent->flags & FL_NOTARGET)
			return false;
	}
	// GRIM
...
//			self->monsterinfo.aiflags &= ~AI_MEDIC;
	...
	if (hesDeadJim)
	{
		// GRIM 20/10/2001 12:37PM - NULL out killtarget if we had one
		if (self->killtarget)
		{
			self->killtarget = NULL;
			self->enemy = NULL;

			// if we have a combat target, head for that now
			if (self->combattarget)
			{
				self->goalentity = self->movetarget = G_PickTarget(self->combattarget);
