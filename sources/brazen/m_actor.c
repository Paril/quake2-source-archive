...
void actorFireGun (edict_t *self)
{
	vec3_t	start, target;
	...
	case II_SHOTGUN:
		...
		fire_shotgun (self, start, forward, 6, 12, 1000, 500, 15, MOD_SHOTGUN);
		break;
	...
	case II_SUBMACH:
		...
		gi.multicast (self->s.origin, MULTICAST_PVS);
        	fire_bullet (self, start, forward, 7, 30, 100, 100, MOD_MACHINEGUN);
		break;
	...
	case II_ASSAULT_RIFLE:
		...
		gi.multicast (self->s.origin, MULTICAST_PVS);
        	fire_bullet (self, start, forward, 13, 30, 100, 100, MOD_ARIFLE);
		break;
...
void SP_misc_actor (edict_t *self)
{
	...
	*/
	...
	if (self->item)
	{
		...
		case II_ASSAULT_RIFLE:
			self->s.modelindex2 = gi.modelindex("players/male/w_hyperblaster.md2");
			break;
