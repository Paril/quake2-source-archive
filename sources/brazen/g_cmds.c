...
void Cmd_Inven_f (edict_t *ent)
{
	...
	*/
	for (i = 0; i < MAX_ITEMS; i++)
	{
		...
		if (item && item->tag)
			gi.WriteShort (CountItemByTag(ent, item->tag));
		else
			gi.WriteShort (0);
	}
	// GRIM
...
void ClientCommand (edict_t *ent)
{
	...
	if (Q_stricmp (cmd, "use") == 0)
		Cmd_Use_f (ent);
	/* GRIM 6/10/2001 8:33PM else if (Q_stricmp (cmd, "drop") == 0)
		Cmd_Drop_f (ent);
		...
		Cmd_InvDrop_f (ent);
	/* GRIM 4/10/2001 3:21PM
	else if (Q_stricmp (cmd, "weapprev") == 0)
		Cmd_WeapPrev_f (ent);
	else if (Q_stricmp (cmd, "weapnext") == 0)
		Cmd_WeapNext_f (ent);
	*/ // GRIM 4/10/2001 3:22PM
