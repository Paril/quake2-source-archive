...
int BrainDMGAdjust(edict_t *ent, vec3_t point, vec3_t normal, int damage, int dflags, int mod)
{
	int save = 0;

	if ((dflags & DAMAGE_BULLET) && (ent->health > (ent->max_health / 2)))
	{	// bullet has landed on crappy metal...  reduce damage some
		save = (damage / 4);
	}
