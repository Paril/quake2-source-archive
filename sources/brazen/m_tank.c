...
void tank_footstep (edict_t *self)
{
	...
	StepShake(self->s.origin, 512, 50);
	// GRIM
}
...
void tank_thud (edict_t *self)
{
	...
	// GRIM 17/10/2001 8:28PM
	StepShake(self->s.origin, 512, 200);
	// GRIM
}
...
void tank_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
        gitem_t *item;
        int count;
        vec3_t up = {0, 0, 1};
	...
	item = GetItemByTag(II_TANK_ROCKET_LAUNCHER);
	count = 4 + (random() * 2);
	count *= 3;
...
// check for gib
	if (self->health <= self->gib_health)
	{
		...
		// GRIM 29/10/2001 7:18PM
		ThrowGib (self, "models/monsters/tank/gibs/larm/tris.md2", damage < 50 ? 50 : damage, GIB_METALLIC);
		ThrowGib (self, "models/monsters/tank/gibs/rarm/tris.md2", damage < 50 ? 50 : damage, GIB_METALLIC);
		ThrowGib (self, "models/monsters/tank/gibs/lleg/tris.md2", damage < 50 ? 50 : damage, GIB_METALLIC);
		ThrowGib (self, "models/monsters/tank/gibs/rleg/tris.md2", damage < 50 ? 50 : damage, GIB_METALLIC);
		// GRIM
...
// regular death
	...
	// GRIM 29/10/2001 5:56PM
	self->s.modelindex = gi.modelindex ("models/monsters/tank/dead/tris.md2");
	self->s.frame = 0;
	// GRIM
...
int TankDMGAdjust(edict_t *ent, vec3_t point, vec3_t normal, int damage, int dflags, int mod)
{
	int save = 0;

	if ((dflags & DAMAGE_BULLET) && (ent->health > (ent->max_health / 2)) && (!(ent->last_hitloc & LOCATION_HEAD)))
	{	// bullet has landed on metal...  reduce damage done significantly
		save = damage - (damage / 4);
	}
...
void SP_monster_tank (edict_t *self)
{
	...
	// GRIM
	gi.modelindex("models/monsters/tank/dead/tris.md2");
	gi.modelindex("models/monsters/tank/gibs/larm/tris.md2");
	gi.modelindex("models/monsters/tank/gibs/rarm/tris.md2");
	gi.modelindex("models/monsters/tank/gibs/lleg/tris.md2");
	gi.modelindex("models/monsters/tank/gibs/rleg/tris.md2");
	// GRIM
	...
	// GRIM 8/10/2001 9:42PM
	self->dmgadjust = TankDMGAdjust;
	...
	// GRIM 8/10/2001 10:09PM - Tank Commanders come equiped with power screen armor
	if (strcmp(self->classname, "monster_tank_commander") == 0) // (and a lot of it!)
	{
