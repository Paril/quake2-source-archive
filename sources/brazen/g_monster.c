...
{
	// GRIM fire_heat (self, start, dir, offset, damage, kick, true);
...
{
	// GRIM 8/10/2001 3:37PM
	//fire_grenade (self, start, aimdir, damage, speed, 2.5, damage+40);
	fire_cgrenade (self, start, aimdir, damage, speed, 2.5, damage+40, II_FRAG_GRENADES);
	// GRIM
...
void M_SetEffects (edict_t *ent)
{
	...
	// GRIM 27/10/2001 8:52PM
	if (ent->emp_framenum > level.framenum)
	{
		remaining = ent->emp_framenum - level.framenum;

		if (remaining & 8)
		{
			ent->pain_debounce_time = level.time;
			ent->pain (ent, ent, 0, 999);
...
void monster_triggered_spawn (edict_t *self)
{
	...
	// GRIM 13/10/2001 3:43PM
	// make everything in edit mode non-solid, but touchable/shootable
	if (sv_edit->value)
...
void monster_death_use (edict_t *self)
{
	...
	if (self->item)
	{
		// GRIM 6/10/2001 12:59PM - new inventory system
		...
		VectorCopy(self->s.origin, org);
	        ...
	        dropped = LaunchItem(self, self->item, org, velocity);
	        if (dropped)
	        {
			dropped->nextthink = level.time + 30;
			dropped->count = dropped->item->quantity;
			if (dropped->item->tag == II_ASSAULT_RIFLE)
			{
				dropped->viewheight |= (SHF_EXTRA_AMMO1 | SHF_EXTRA_AMMO2 | SHF_EXTRA_AMMO3 | SHF_EXTRA_AMMO4);
			}

			dropped->last_fire = dropped->item->ammoTag;
...
//		gi.dprintf("fixed spawnflags on %s at %s\n", self->classname, vtos(self->s.origin));
	...
	if (st.item)
	{
		// GRIM 8/10/2001 10:10AM
		if (Q_stricmp(st.item, "weapon_machinegun") == 0)
	        	st.item = "weapon_submach";
		if (Q_stricmp(st.item, "ammo_bullets") == 0)
	        	st.item = "ammo_submachclip";
		if (Q_stricmp(st.item, "ammo_shells") == 0)
	        	st.item = "ammo_shotgunclip";
		if (Q_stricmp(st.item, "ammo_rockets") == 0)
	        	st.item = "ammo_grenades";
		if (Q_stricmp(st.item, "weapon_supershotgun") == 0)
			st.item = "weapon_shotgun";
		if (Q_stricmp(st.item, "weapon_hyperblaster") == 0)
			st.item = "weapon_arifle";
		if (Q_stricmp(st.item, "ammo_cells") == 0)
	        	st.item = "ammo_arifleclip";
		// GRIM
...
void monster_start_go (edict_t *self)
{
	...
	// GRIM 20/10/2001 12:56PM
	if (self->killtarget)
	{
		self->movetarget = G_PickTarget(self->killtarget);
		if (!self->movetarget)
		{
			gi.dprintf ("%s can't find killtarget %s at %s\n", self->classname, self->killtarget, vtos(self->s.origin));
			self->target = NULL;
		}
