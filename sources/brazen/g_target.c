...
void use_target_changelevel (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (coop->value)
	{
		qboolean falling = false;
		
		if ((!activator->groundentity) && (activator->client->oldvelocity[2] < -40) && (activator->velocity[2] < -40))
			falling = true;

		// if this is a new unit or the player is falling, put activator in spectator 
		// mode and check to see if everyone else is dead or not
		if (strstr(self->map, "*") || falling) 
		{
			activator->movetype = MOVETYPE_NOCLIP;
			activator->solid = SOLID_NOT;
			activator->svflags |= SVF_NOCLIENT;
			activator->client->ps.gunindex = 0;
			activator->deadflag = DEAD_NO;
			...
			for (player = 1; player <= game.maxclients; player++)
			{
				...
				// there is someone out there who is dead
				if (ent->deadflag)
				{
					CheckCoopAllDead();
					return;
...
void target_laser_think (edict_t *self)
{
	...
	// GRIM 8/01/2002 9:04PM
	if (self->decoy && self->enemy)
