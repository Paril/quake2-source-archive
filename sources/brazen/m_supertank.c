...
void TreadSound (edict_t *self)
{
	...
	StepShake(self->s.origin, 1024, 55);
	// GRIM
}
...
void BossExplode (edict_t *self)
{
	...
	// GRIM 17/10/2001 8:28PM
	StepShake(self->s.origin, 1024, 200);
	// GRIM
...
int SuperTankDMGAdjust(edict_t *ent, vec3_t point, vec3_t normal, int damage, int dflags, int mod)
{
	int save = 0;
...
//	self->s.sound = gi.soundindex ("bosstank/btkengn1.wav");
	...
	// GRIM 27/10/2001 7:32PM
	self->dmgadjust = SuperTankDMGAdjust;
