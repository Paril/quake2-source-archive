...
//PGM
		...
		// GRIM 13/10/2001 3:45PM
		// make everything in edit mode non-solid, but touchable/shootable
		if (sv_edit->value)
		{
			if (strstr(ent->classname, "func_door") || (Q_stricmp(ent->classname, "func_explosive") == 0) || (Q_stricmp(ent->classname, "func_wall") == 0))
			{
				ent->svflags |= SVF_DEADMONSTER;
				ent->s.effects |= EF_SPHERETRANS;
			}
			else if (strstr(ent->classname, "trigger_"))
			{
