...
int GladiatorDMGAdjust(edict_t *ent, vec3_t point, vec3_t normal, int damage, int dflags, int mod)
{
	int save = 0;

	if ((dflags & DAMAGE_BULLET) && (ent->health > (ent->max_health / 2)))
	{
		if ((ent->last_hitloc & (LOCATION_HEAD | LOCATION_CHEST | LOCATION_STOMACH)))
			return 0;
...
void SP_monster_gladiator (edict_t *self)
{
	...
	self->dmgadjust = GladiatorDMGAdjust;
