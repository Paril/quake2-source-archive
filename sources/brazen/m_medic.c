...
void medic_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
        gitem_t *item;
        int count;
        vec3_t up = {0, 0, 1};
	...
	item = GetItemByTag(II_MEDIC_HYPER_BLASTER);
	count = 30 + (random() * 30);
	...
	ent->s.modelindex = gi.modelindex ("models/monsters/medic/dead/tris.md2");
	ent->s.frame = 0;
...
int MedicDMGAdjust(edict_t *ent, vec3_t point, vec3_t normal, int damage, int dflags, int mod)
{
	int save = 0;

	if ((dflags & DAMAGE_BULLET) && (ent->health > (ent->max_health / 2)))
	{
		if (((ent->last_hitloc & LOCATION_HEAD) || (ent->last_hitloc & LOCATION_CHEST)) && (ent->last_hitloc & LOCATION_FRONT))
			return 0;

		// bullet has landed on metal...  reduce damage some
		save = damage - (damage / 3);
	}
...
void SP_monster_medic (edict_t *self)
{
	...
	gi.modelindex ("models/monsters/medic/dead/tris.md2");
	// GRIM
