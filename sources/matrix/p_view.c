...
//SKULL
        if (ent->camon)
		{
			ent->client->ps.pmove.origin[0] = ent->cam->s.origin[0]*8;
			ent->client->ps.pmove.origin[1] = ent->cam->s.origin[1]*8;
			ent->client->ps.pmove.origin[2] = ent->cam->s.origin[2]*8;
		}
...
// MATRIX
	...
	if (delta > 30)
	{
		...
		if (!deathmatch->value || !((int)dmflags->value & DF_NO_FALLING) )
			...
			T_Damage (ent, world, world, dir, ent->s.origin, vec3_origin, MatrixFallingRoll(ent, damage), 0, 0, MOD_FALLING);
			// MATRIX
...
// MATRIX
	if (ent->matrixflip == NOFLIP)
	{
		...
		ent->s.angles[ROLL] = 0;
		//painberry
	//	ent->s.angles[ROLL] = (ent->client->v_angle[YAW] - ent->oldvangles[YAW]);//lean into turns
		//painberry
		if (ent->velocity[0] || ent->velocity[1])
		ent->s.angles[ROLL] += SV_CalcRoll (ent->s.angles, ent->velocity)* -8;//lean into strafes
	}
	MatrixRunUpWalls(ent);
...
//END
	MatrixClientFrame(ent);
	

	
	ent->oldgroundentity = ent->groundentity;
}
