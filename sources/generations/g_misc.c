...
// Diff head models -Skid
	if(self->style ==3)
	{
		gibname = "models/objects/q1gibs/q1head/tris.md2";
		self->s.skinnum = 0;
	}
	else if (self->style ==2)
	{
		gibname ="models/objects/dhead/tris.md2";
		self->s.skinnum = 0;
	}
	else if(self->style == 1)
	{
		gibname ="models/objects/whead/tris.md2";
		self->s.skinnum = 0;
	}
	else
	{
...
//	G_FreeEdict (self);
//Skid added
	gi.sound (self, CHAN_AUTO, gi.soundindex ("weapons/q2rockex.wav"), 1.0, ATTN_NORM, 0);	
	gi.unlinkentity(self);
	GenRockExplode(self, self->s.origin);
}
...
//Skid
	if(self->style==1 || level.game == CLASS_Q1)
	{
	...
	else if(self->style==2 || level.game == CLASS_DOOM)
	{
		other->client->ps.pmove.pm_time = 240>>3;	
		other->client->ps.pmove.pm_flags |= PMF_TIME_TELEPORT;
		DoomTeleporterSplash(self->s.origin, self->s.angles);
		DoomTeleporterSplash(dest->s.origin, dest->s.angles);
	}
	else
	{
//End Skid
		...
		//Skid added
		gi.sound (self, CHAN_AUTO, gi.soundindex("misc/q2tele.wav"), 1, ATTN_NORM, 0);
		gi.sound (other, CHAN_AUTO, gi.soundindex("misc/q2tele.wav"), 1, ATTN_NORM, 0);
	}
...
//push velocity	
	if((self->style == 1) || (level.game == CLASS_Q1))
	{
		...
		VectorMA(other->velocity,250,forward,other->velocity);
	}
//End Skid
...
void SP_misc_teleporter (edict_t *ent)
{
	  ...
	  (level.game == CLASS_Q2))
	{
		...
		gi.soundindex("misc/q2tele.wav");
	}
	else if(ent->style == 1 ||
		    level.game == CLASS_Q1)
	{
		gi.soundindex("q1world/teleport/r_tele1.wav");
		gi.soundindex("q1world/teleport/r_tele2.wav");
		gi.soundindex("q1world/teleport/r_tele3.wav");
		gi.soundindex("q1world/teleport/r_tele4.wav");
		gi.soundindex("q1world/teleport/r_tele5.wav");
		ent->s.sound = gi.soundindex ("q1world/hum1.wav");
	}
