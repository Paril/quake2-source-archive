...
//	flame_shots			=	gi.cvar("flame_shots",			"1",  0);
	...
	spawn_camp_check	=	gi.cvar("spawn_camp_check",		"1",  CVAR_SERVERINFO);
	spawn_camp_time		=	gi.cvar("spawn_camp_time",		"2",  CVAR_SERVERINFO);
	invuln_medic		=	gi.cvar("invuln_medic",			"0",  CVAR_SERVERINFO | CVAR_LATCH);
//	teamgren			=	gi.cvar("teamgren",				"1",  CVAR_SERVERINFO | CVAR_LATCH);
	death_msg			=	gi.cvar("death_msg",			"2",  CVAR_SERVERINFO);
	level_wait			=	gi.cvar("level_wait",			"20",  CVAR_SERVERINFO);
	invuln_spawn		=	gi.cvar("invuln_spawn",			"2", CVAR_SERVERINFO);
...
//	gi.cvar ("dll_version", "v .08b",  CVAR_SERVERINFO | CVAR_LATCH); //faf
	...
	player_scores   = gi.cvar ("player_scores",  "0", 0);//faf
	campaign_mode   = gi.cvar  ("campaign_mode",  "1", 0);//faf

	max_gibs		= gi.cvar  ("max_gibs",   "20", 0);//faf
	extra_anims		= gi.cvar  ("extra_anims", "0", 0);//faf
