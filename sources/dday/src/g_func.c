 ...
 ************************************
 *   $Source: /usr/local/cvsroot/dday/src/g_func.c,v $
 *   $Revision: 1.5 $
...
void door_blocked  (edict_t *self, edict_t *other)
{
	...
	// dday doors don't do damage if not set to
	//T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, self->dmg, 1, 0, MOD_CRUSH);
...
/*	edict_t *tempent=NULL;
	...
	for(tempent=&g_edicts[0],i=0;i<game.maxentities;i++,tempent++)
	{
		if(		tempent 
			&&	tempent->inuse 
			&&	tempent->client 
			&&	tempent->client->resp.team_on
			&&  tempent->client->resp.mos) //are they a person and on a team?
		{
			if( (tempent->client->resp.team_on->index==ent->obj_owner) &&
				(tempent->client->limbo_mode) ) //are they on this team and dead?
			{
				ent->nextspawn = 0;
				EndObserverMode(tempent);	//Christ 'em
			}
		}
	}
