...
extern	cvar_t	*noweap_medic;
extern	cvar_t	*new_aim_system;
//extern	cvar_t	*teamgren;
extern	cvar_t	*death_msg;
extern	cvar_t	*level_wait;
extern	cvar_t	*invuln_spawn;

extern	cvar_t  *stats_update_time;		//vanwilder, time between stats update
extern  cvar_t  *kill_scale;			//vanwilder, kills map scaling
extern  cvar_t  *low_lag;				//vanwilder, low lag
...
gitem_t *FindTeamItem (char *dllname, int position); //faf
...
typedef struct
{
	...
	int total;		//total on team. add stuff to count upon joinging
...
//	client_respawn_t	old_resp;
	...
	qboolean	showpscores;		// playerscores
	qboolean	showhelp;
...
//bcass end
	...
	qboolean	aim;			// hawkins truesight
...
//bcass end
	...
	float		last_jump_time;		//vanwilder, slope bug.

	int			naderange;			//vanwilder, nade range
	int			stat_shots;			//vanwilder, how many shots
	int			stat_hits;			//vanwilder, how many hits
	int			stat_kills;			//vanwilder, enemy kills
	int			stat_deaths;		//vanwilder, deaths from enemy
	char		stuff_text[256];	//stuffed command to test for stuffing
	int			stuff_verified;		//check to see if they have already been verified
	float		stuff_time;			//time to be checked by
	char		client_ip[30];		//clients ip for kicking and whatnot
...
//	qboolean		ident;
	...
	edict_t			*reflection;	//vanwilder, reflections
	int				timer;
};
