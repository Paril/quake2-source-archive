...
{
	// ARTIFACT QUAKE 2: We need tank! :)
//	if (deathmatch->value)
//	{
//		G_FreeEdict (self);
//		return;
//	}
	// So instead, it's "if deathmatch, and the tank DOESN'T have a owner that's a client"
	if (deathmatch->value && !(self->owner && self->owner->client))
	{
		G_FreeEdict (self);
		return;
	}
