...
//ZOID
			...
			if (message)
			{
				...
				if (ff)
					...
					attacker->client->resp.score++;
//Azrael			
				attacker->client->resp.mana = 1; //Can cast spells
				CheckforDivine(attacker);
//Azrael		
			}
			return;
		}
...
void InitClientPersistant (gclient_t *client)
{
	gitem_t		*item;
//Azrael
	int max_health = client->pers.max_health;

	int max_bullets = client->pers.max_bullets;
	int max_shells = client->pers.max_shells;
	int max_rockets = client->pers.max_rockets;
	int max_grenades = client->pers.max_grenades;
	int max_cells = client->pers.max_cells;
	int max_slugs = client->pers.max_slugs;
//Azrael


	memset (&client->pers, 0, sizeof(client->pers));

	item = FindItem("Holy Fire");
...
//Azrael
	if(!max_health)
		...
		client->pers.max_health	= max_health;
	
	client->pers.health			= max_health;
	client->next_fire = level.time;	
	if(!max_bullets)
		client->pers.max_bullets = 100;
	else
		client->pers.max_bullets = max_bullets;
	
	if(!max_shells)
		client->pers.max_shells = 50;
	else
		client->pers.max_shells	= max_shells;
	
	if(!max_rockets)
		client->pers.max_rockets = 5;
	else
		client->pers.max_rockets = max_rockets;

	if(!max_grenades)
		client->pers.max_grenades = 5;
	else
		client->pers.max_grenades = max_grenades;

	if(!max_cells)
		client->pers.max_cells = 100;
	else
		client->pers.max_cells = max_cells;
	
	if(!max_slugs)
		client->pers.max_slugs = 5;
	else
		client->pers.max_slugs		= max_slugs;
...
//Azrael
	int holylevel = client->resp.holylevel;
	int captures = client->resp.captures;
//Azrael
//ZOID
...
//ZOID
//Azrael
	client->resp.holylevel = holylevel;
	client->resp.captures = captures;
//Azrael	
...
//Azrael
	if (motd_file = fopen("faith/motd.txt", "r"))
	{
