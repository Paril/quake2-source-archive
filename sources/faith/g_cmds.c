...
//ZOID--special case for tech powerups
	if (Q_stricmp(gi.args(), "rune") == 0 && (it = CTFWhat_Tech(ent)) != NULL) {
		it->drop (ent, it);
		return;
	}
//ZOID
...
//Azrael
	...
	else if (Q_stricmp(cmd, "spellone") == 0){
		SpellOne(ent);
	}
	else if (Q_stricmp(cmd, "spelltwo") == 0){
		SpellTwo(ent);
	}
	else if (Q_stricmp(cmd, "spellthree") == 0){
		SpellThree(ent);
	}
	else if (Q_stricmp(cmd, "spellfour") == 0){
		SpellFour(ent);
	}
	else if (Q_stricmp(cmd, "body") == 0){
		Shockwave(ent);
	}
