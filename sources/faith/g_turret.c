...
void turret_breach_think (edict_t *self)
{
	...
	if (self->owner)
	{
		...
		if (self->owner->target_ent == self) {
		...
		else {		// a player is controlling the turret, move towards view angles
			...
			if ((self->owner->client->buttons & BUTTON_USE) && (self->delay < level.time))	// 3 second break between shots
			{
				turret_breach_fire (self);
		...
		for (i=0 ; i<maxclients->value ; i++, ent++)
		{
			...
			if (VectorLength(dir) < 16)	{	// player has taken control of turret
				self->owner = ent;
