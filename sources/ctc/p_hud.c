...
void DeathmatchScoreboardMessage (edict_t *ent, edict_t *killer)
{
	...
	if (Chicken_TCTCScoreboard (ent, killer)) return;
	...
	{
		cl_ent = g_edicts + 1 + i;
		if (!cl_ent->inuse || cl_ent->movetype == MOVETYPE_NOCLIP || !strcmp(cl_ent->classname, "camera")) // Chicken
			continue;
	...
	for (i=0 ; i<total ; i++)
	{
		...
		// add a dogtag
		if (cl_ent->client && cl_ent->client->pers.inventory[chickenItemIndex] > 0 && !cl_ent->client->isThrowing) // Chicken
			tag = "tag3";													// Chicken
		else if (cl_ent == ent)
			tag = "tag1";
