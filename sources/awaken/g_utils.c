...
FILE* OpenConfiglistFile(qboolean report)
{
	...
	game = gi.cvar("game", "", 0);
	if (!*game->string)
		sprintf(filename, "%s/%s", GAMEVERSION, sv_config_file->string);
	else
		sprintf(filename, "%s/%s", game->string, sv_config_file->string);
...
FILE* OpenAGMDropFile(qboolean report, qboolean readonly)
{
	...
	game = gi.cvar("game", "", 0);
	if (!*game->string)
		sprintf(filename, "%s/%s", GAMEVERSION, sv_agm_drop_file->string);
	else
		sprintf(filename, "%s/%s", game->string, sv_agm_drop_file->string);
...
FILE* OpenDiscLauncherDropFile(qboolean report, qboolean readonly)
{
	...
	game = gi.cvar("game", "", 0);
	if (!*game->string)
		sprintf(filename, "%s/%s", GAMEVERSION, sv_disc_drop_file->string);
	else
		sprintf(filename, "%s/%s", game->string, sv_disc_drop_file->string);
...
FILE* OpenBotConfigFile(qboolean report, qboolean readonly)
{
	...
	game = gi.cvar("game", "", 0);
	if (!*game->string)
		sprintf(filename, "%s/%s", GAMEVERSION, sv_bots_config_file->string);
	else
		sprintf(filename, "%s/%s", game->string, sv_bots_config_file->string);
