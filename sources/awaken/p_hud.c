...
//CW++
			if (cl->spectator)
			{
				sprintf(entry, "xv %d yv %d string2 \"%s\" ", x+32, y, cl->pers.netname);
				sprintf(entry2, "xv %d yv %d string \"(Spectator)\" ", x+32, y+8);
				strcat(entry, entry2);
				if (cl->chase_target)
				{
					sprintf(entry2, "xv %d yv %d string \"Watching:\" ", x+32, y+16);
					strcat(entry, entry2);
					sprintf(entry2, "xv %d yv %d string \"%s\" ", x+32, y+24, cl->chase_target->client->pers.netname);
					strcat(entry, entry2);
				}
			}
			else
...
==================
*/
void ShowGaussTarget(edict_t *ent, char *string)
{
	edict_t		*cl_ent;
	edict_t		*targ = NULL;
	vec3_t		targ_vec;
