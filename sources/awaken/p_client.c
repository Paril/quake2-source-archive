...
//CW++
	else if (attacker && (!Q_stricmp(attacker->classname, "turret_breach") || !Q_stricmp(attacker->classname, "model_turret")))
	{
		...
		if (message)
		{
			sprintf(killer, "an auto-tracking");
			if (self->noise_index)
				sprintf(killer, "their very own");
			else
			{
				for (i = 0, player = g_edicts + 1; i < (int)maxclients->value; ++i, ++player)
				{
					...
					{
						sprintf(killer, "%s's", player->client->pers.netname);
						if (ff)
...
void SaveClientData(void)
{
	...
	for (i = 0; i < (int)game.maxclients; i++)
	{
		...
		game.clients[i].pers.savedFlags = (ent->flags & (FL_GODMODE|FL_NOTARGET|FL_POWER_ARMOR));	//CW
	}
}
