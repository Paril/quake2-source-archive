...
void Use_Siphon(edict_t *ent, gitem_t *item)
{
	...
	if (ent->client->siphon_framenum > level.framenum)
		ent->client->siphon_framenum += 300;
	else
		ent->client->siphon_framenum = level.framenum + 300;
...
void Use_Needle(edict_t *ent, gitem_t *item)
{
	...
	if (remaining <= 0)
		ent->client->needle_framenum = level.framenum + 600;
	else if (remaining <= 600)
		ent->client->needle_framenum = level.framenum + 600 + (int)(remaining * 0.5);
	else
...
void Use_AntiBeam(edict_t *ent, gitem_t *item)
{
	...
	if (ent->client->antibeam_framenum > level.framenum)
		ent->client->antibeam_framenum += 300;
	else
		ent->client->antibeam_framenum = level.framenum + 300;
