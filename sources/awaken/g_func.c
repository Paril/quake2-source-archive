...
void Move_Calc(edict_t *ent, vec3_t dest, void(*func)(edict_t*))
{
	...
	{
		// accelerative
		ent->moveinfo.current_speed = 0.0;
...
==============
*/
#define AccelerationDistance(target, rate)	(target * ((target / rate) + 1) * 0.5)					//CW
...
void plat_CalcAcceleratedMove(moveinfo_t *moveinfo)
{
	...
	if ((moveinfo->remaining_distance - accel_dist - decel_dist) < 0.0)
	{
		float f;

		f = (moveinfo->accel + moveinfo->decel) / (moveinfo->accel * moveinfo->decel);
		moveinfo->move_speed = (-2.0 + sqrt(4.0 - 4.0 * f * (-2.0 * moveinfo->remaining_distance))) / (2.0 * f);
...
void plat_Accelerate(moveinfo_t *moveinfo)
{
	...
	if (moveinfo->current_speed < moveinfo->speed)
	{
		...
		// during this move we will accelrate from current_speed to move_speed and 
		// cross over the decel_distance; figure the average speed for the entire move
		p1_distance = moveinfo->remaining_distance - moveinfo->decel_distance;
		p1_speed = (old_speed + moveinfo->move_speed) * 0.5;										//CW
...
void Think_AccelMove(edict_t *ent)
{
	...
	if (ent->moveinfo.current_speed == 0.0)			// starting or blocked
	...
	VectorScale(ent->moveinfo.dir, ent->moveinfo.current_speed * 10.0, ent->velocity);
	ent->nextthink = level.time + FRAMETIME;
	ent->think = Think_AccelMove;
}
...
void Touch_Plat_Center(edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (ent->moveinfo.state == STATE_BOTTOM)
		...
		ent->nextthink = level.time + 1.0;		// the player is still on the plat, so delay going down
}
...
void plat_spawn_inside_trigger(edict_t *ent)
{
	...
	tmin[0] = ent->mins[0] + 25.0;
	tmin[1] = ent->mins[1] + 25.0;
	tmin[2] = ent->mins[2];

	tmax[0] = ent->maxs[0] - 25.0;
...
// Avert your eyes - it's ugly hack time!
	...
	if (!Q_stricmp(level.mapname, "ecdm1awak"))
	{
		G_FreeEdict(ent);
		return;
	}
...
//	Set the axis of rotation.
	...
	if (ent->spawnflags & 4)		// X_AXIS
		ent->movedir[2] = 1.0;
	else if (ent->spawnflags & 8)	// Y_AXIS
		ent->movedir[0] = 1.0;
	else							// Z_AXIS
		ent->movedir[1] = 1.0;
...
//Pon++
	if (self->activator)
	{
		if ((int)chedit->value && (CurrentIndex < MAXNODES) && !self->activator->deadflag && (self->activator == &g_edicts[1]))
		{
...
void door_use_areaportals(edict_t *self, qboolean open)
{
	...
	while ((t = G_Find(t, FOFS(targetname), self->target)) != NULL)									//CW
	{
...
//CW++
	ent->style = ENT_ID_DOOR_ROTATING;
//CW--
}
...
void SP_func_door_secret(edict_t *ent)
{
	...
	ent->moveinfo.accel = ent->moveinfo.decel =	ent->moveinfo.speed = 50.0;
	...
	if (ent->spawnflags & SECRET_1ST_DOWN)
		VectorMA(ent->s.origin, -1.0 * width, up, ent->pos1);
	else
