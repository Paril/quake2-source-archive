	...
	if (!(tr.fraction < 1.0))
	{
		...
		if (tr.contents & MASK_WATER)
		{
			...
			if (!VectorCompare(start, tr.endpos))
			{
				...
				VectorMA(water_start, 8192.0, forward, end);
				VectorMA(end, r, right, end);
				VectorMA(end, u, up, end);
			}
...
//	the target to be this endpoint. Otherwise, the target is the end of the line.
	...
	VectorMA(self->owner->s.origin, 8192.0, forward, end);
...
//	endpoint for the particle beam.
	...
	for (i = 0; i < 3; ++i)
	{
		if (end[i] >= 4095.0)
		{
			scale *= (4095.0 - start[i]) / (end[i] - start[i]);
			VectorMA(start, scale, aimdir, end);
		}
		else if (end[i] <= -4095.0)
		{
			scale *= (4095.0 + start[i]) / (start[i] - end[i]);
			VectorMA(start, scale, aimdir, end);
		}
	}
