...
// BEGIN Teamplay : need to access this from hook.c
...
// END Teamplay : need to access this from hook.c
...
void ChangeWeapon (edict_t *ent)
{
	...
	// set visible model
	// BEGIN TEAMPLAY
	if ((ent->s.modelindex == 255) || (ent->client->invisible) || (ent->client->quadcloak)) {
	// END TEAMPLAY
...
void Use_Weapon (edict_t *ent, gitem_t *item)
{
	...
	if (item == ent->client->pers.weapon)
	{
		...
		teamplay_WeaponToggle(ent, item);
		return;
	}
	else
	{
...
void Weapon_Generic (edict_t *ent, int FRAME_ACTIVATE_LAST, int FRAME_FIRE_LAST, int FRAME_IDLE_LAST, int FRAME_DEACTIVATE_LAST, int *pause_frames, int *fire_frames, void (*fire)(edict_t *ent))
{
	...
	if ((ent->deadflag || ent->s.modelindex != 255) && (!ent->client->invisible) && (!ent->client->quadcloak))// VWep animations screw up corpses
	{
		return;
	}
...
void weapon_grenade_fire (edict_t *ent, qboolean held)
{
	...
	if (IsFemale(ent))
		...
		speed = GRENADE_MINSPEED + (GRENADE_TIMER * 2 - timer) * ((GRENADE_MAXSPEED - GRENADE_MINSPEED) / GRENADE_TIMER);
	// END Teamplay
...
void weapon_grenadelauncher_fire (edict_t *ent)
{
	...
	{
		// BEGIN Teamplay
		if ((ent->client->pers.classlevel > 1) && (ent->client->clustermode))
			ent->client->pers.inventory[ent->client->ammo_index] -= modifier;
		else
			ent->client->pers.inventory[ent->client->ammo_index]--;
		// END Teamplay
	}
...
void Weapon_RocketLauncher_Fire (edict_t *ent)
{
	...
	// BEGIN Teamplay
	if ((teamplay == 1) && (ent->hasturret == true))
		return;
	// END Teamplay
...
//	Blaster_Fire (ent, vec3_origin, damage, false, EF_BLASTER);
	Blaster_Fire (ent, vec3_origin, damage, false, 0);
	// END Teamplay
...
void Weapon_HyperBlaster_Fire (edict_t *ent)
{
	...
	if ((ent->client->special) && (ent->client->pers.player_class == 6))
...
//			else
			// END Teamplay
				effect = 0;
			if (deathmatch->value)
				damage = 12;	// BEGIN Teamplay Lowered damage from 15 to 13 END
			else
				damage = 20;
...
void Chaingun_Fire (edict_t *ent)
{
	...
	for (i=0 ; i<shots ; i++)
	{
		...
		if ((ent->client->pers.classlevel > 2) && (ent->client->pers.playmode == 0))
		{
...
void weapon_shotgun_fire (edict_t *ent)
{
	...
	damage = 4 + ent->client->pers.classlevel;
	// END Teamplay
...
void weapon_supershotgun_fire (edict_t *ent)
{
	...
	damage = 6 + (ent->client->pers.classlevel * 2);
	// END Teamplay
...
//	Weapon_Generic (ent, 6, 17, 57, 61, pause_frames, fire_frames, weapon_supershotgun_fire);
	...
	if (ent->client->pers.classlevel > 2)
		Weapon_Generic (ent, 6, 12, 57, 61, pause_frames, fire_frames, weapon_supershotgun_fire);
	else
		Weapon_Generic (ent, 6, 17, 57, 61, pause_frames, fire_frames, weapon_supershotgun_fire);
}
			...
			if (ent->client->pers.player_class == 3) 
				if (ent->lasersight)
				{
					if (ent->client->invisible)
						damage = (int)(300 * modifier);
					else
						damage = (int)(150 * modifier);
				}
				else if (ent->client->invisible)
					damage = (int)(150 * modifier);
				else
					damage = (int)(100 * modifier);
			else
				damage = 100;
		}
		else
...
void sniper_flare (edict_t *self)
{
	...
	if (teamplay == 1)
	{
		...
		{
			gsutil_centerprint(self, "%s", "Only a Sniper can throw flares!\n");
			return;
		}
	}

	if (self->client->proxdet > 3)
	{
		gsutil_centerprint(self, "%s", "You have 4 flares active already!\n");
		return;
	}
	...
	{ 		
		gsutil_centerprint(self, "%s", "Not enough cells for a flare.\n");
		return;	
	}
	else
	...
	fire_flare (self, start, forward, damage, speed, timer, damage_radius);
