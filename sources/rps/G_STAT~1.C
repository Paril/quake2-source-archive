 ...
 *  Public header file.
 *
 * $Id: g_statlog.c 1.2 1998/04/06 09:22:08 mdavies Exp mdavies $
 *
 */
...
int sl_Logging( game_import_t  *gi, char *pPatchName )
{
...
void StartGSLogFile(void)
{
	...
	int		i, j, lines=0;
	char	*pdest;
	cvar_t  *filename	= gi.cvar( "stdlogname", "rps/StdLog.log", CVAR_SERVERINFO );
	...
	if( !tempFile )
    {
         printf("Couldn't open file, logging OFF\n", tName );
	...
	if(pdest)
		gi.dprintf("Good GameEnd\n");
	else
	{
		printf("Bad GameEnd, Adjusting\n");
		isbad=true;
		lines--;
	}
