...
void SP_light (edict_t *self)
{
	...
	// IGNORE THAT:  DeadLode REALLY wants switchable lights...
...
void func_object_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage == DAMAGE_NO)
		return;
	T_Damage (other, self, self, vec3_origin, self->s.origin, vec3_origin, self->dmg, 1, 0, MOD_CRUSH, NULL);
}
...
void teleporter_think (edict_t *self) {
...
//ZOID
	// DEADLODE
	if (other->grapple)
		Grapple_Release(other->grapple);
	// DEADLODE

	// unlink to make sure it can't possibly interfere with KillBox
	gi.unlinkentity (other);

	VectorCopy (other->s.origin, old_dest);
	...
	// If we can't kill it, then don't teleport the player
	if (!KillBox (other)) {
		VectorCopy(old_dest, other->s.origin);
		VectorCopy(old_dest, other->s.old_origin);
		self->touch = NULL;
		self->think = teleporter_think;
		self->nextthink = level.time + 1.0;
		gi.linkentity (self);
	} else {
		// draw the teleport splash at source and on the player
		if (!(self->spawnflags & TELEPORT_NOEFFECT)) {
			if (self->owner)
...
void SP_misc_teleporter (edict_t *ent)
{
	...
	if (!(ent->spawnflags & TELEPORT_NOPAD)) {
