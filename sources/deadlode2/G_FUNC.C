...
void plat_blocked (edict_t *self, edict_t *other)
{
	...
	{
// DEADLODE
		// Don't destroy goal items....
		if (other->supply && other->spawnflags &0x04) {
...
// DEADLODE
		...
		T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, 100000, 1, 0, MOD_CRUSH, NULL);
	...
	T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, self->dmg, 1, 0, MOD_CRUSH, NULL);
...
//	
	...
	trigger->enemy = ent;
	trigger->team_mask = ent->team_mask;
...
void rangetarg_flash (edict_t *self)
{
	if (level.time > self->timestamp) {
		self->svflags |= SVF_NOCLIENT;
		self->timestamp = level.time + 5.0;
...
void rangetarg_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	self->svflags |= SVF_NOCLIENT;
	self->health = self->max_health;
	self->takedamage = DAMAGE_NO;
	self->timestamp = level.time + 3.0;
	self->old_think = self->think;
	self->think = rangetarg_flash;
	self->nextthink = level.time + 0.2;
}

void rangetarg_use (edict_t *ent, edict_t *other, edict_t *activator)
{
...
void SP_dl_func_rangetarget (edict_t *ent)
{
	VectorClear (ent->s.angles);
	ent->solid = SOLID_BSP;
	ent->movetype = MOVETYPE_PUSH;
	...
	ent->use = rangetarg_use;

	if (ent->health)
	{
		ent->max_health = ent->health;
		ent->die = rangetarg_die;
		ent->takedamage = DAMAGE_YES;
	}
...
void button_fire (edict_t *self)
{
	...
	if (self->activator->client) {
		if (self->team_mask && (self->team_mask-1 != self->activator->client->resp.team - &dlsys.teams[0]))
			return;
	} else if (self->team_mask && (self->team_mask-1 != self->activator->team - &dlsys.teams[0]))
		return;
...
void door_use_areaportals (edict_t *self, qboolean open)
{
	...
	{
		if (DL_strcmp(t->classname, "func_areaportal", -1, false) == 0)
		{
...
void door_go_down (edict_t *self)
{
	...
	self->moveinfo.state = STATE_DOWN;
	if (DL_strcmp(self->classname, "func_door", -1, false) == 0)
		Move_Calc (self, self->moveinfo.start_origin, door_hit_bottom);
	else if (DL_strcmp(self->classname, "func_door_rotating", -1, false) == 0)
...
void door_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (activator->client) {
		if (self->team_mask && (self->team_mask-1 != activator->client->resp.team - &dlsys.teams[0]))
			return;
	} else if (self->team_mask && (self->team_mask-1 != activator->team - &dlsys.teams[0]))
		return;
...
void trigger_elevator_init (edict_t *self)
{
	...
	if (DL_strcmp(self->movetarget->classname, "func_train", -1, false) != 0)
	{
