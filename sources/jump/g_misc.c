...
=================
*/
void VelocityForDamage (int damage, vec3_t /*@out@*/v)
{
	v[0] = 100.0 * crandom();
	v[1] = 100.0 * crandom();
...
void SP_misc_teleporter (edict_t *ent)
{
	...
	VectorSet (trig->mins, -24, -24, 8);
	VectorSet (trig->maxs, 24, 24, 24);
...
void ClipBBoxToEights (edict_t *ent)
{
	...
	for (i=0 ; i<3 ; i++)
	{
		temp = ent->s.origin[i]*8.0;
		if (temp > 0.0)
			temp += 0.5;
		else
			temp -= 0.5;
		ent->s.origin[i] = 0.125 * (int)temp;
	}
}
