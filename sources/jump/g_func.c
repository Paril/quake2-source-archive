...
void button_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (self->health) {
		T_Damage (self, other, other, vec3_origin, vec3_origin, plane->normal, 1, 0, 0, MOD_UNKNOWN);
		return;
	}
...
void door_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (self->spawnflags & 128) {
		T_Damage (other, self, self, vec3_origin, vec3_origin, plane->normal, self->dmg, 0, 0, MOD_TRIGGER_HURT);
		return;
	}

	if (self->takedamage == DAMAGE_YES) {
		T_Damage (self, other, other, vec3_origin, vec3_origin, plane->normal, 15, 0, DAMAGE_NO_PROTECTION, MOD_UNKNOWN);
		return;
	}
	...
	if (self->message) {
		...
		unicastSound (other, gi.soundindex ("misc/talk1.wav"), 1.0);
	}
