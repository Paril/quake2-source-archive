...
void ED_CallSpawn (edict_t *ent)
{
	...
	if (Q_strncasecmp (ent->classname, "ammo_", 5) == 0)
		return;
...
void JumpLoadScores(void)
{
	...
	in = fopen (va("jump/scores/%s", level.mapname), "rb");
	if (!in) {
		gi.dprintf ("WARNING: Was not able to load scores!!\n");
		return;
	}

	while (!feof(in)) {
		if ((fread (&entnum, sizeof(entnum), 1, in)) == 0)
			break;

		ent = g_edicts + entnum;

		if (!ent->jumpinfo) {
			gi.dprintf ("WARNING: Corrupt score file! Resetting.\n");
			fclose (in);
			remove (va("jump/scores/%s", level.mapname));
			return;
		}

		fread (&s, sizeof(s), 1, in);

		fread (ent->jumpinfo->name, (size_t)s, 1, in);
		fread (&ent->jumpinfo->date, sizeof(ent->jumpinfo->date), 1, in);
		fread (&ent->jumpinfo->time, sizeof(ent->jumpinfo->time), 1, in);
	}

	fclose (in);
}

void JumpSaveScores (void)
{
	unsigned char s;
	...
	out = fopen (va("jump/scores/%s", level.mapname), "wb");
	if (!out) {
		gi.dprintf ("WARNING: Was not able to save scores!!\n");
		return;
	}
	...
	for ( ; from < &g_edicts[globals.num_edicts]; from++)
	{
		...
		fwrite (&from->jumpinfo->date, sizeof(from->jumpinfo->date), 1, out);
		fwrite (&from->jumpinfo->time, sizeof(from->jumpinfo->time), 1, out);
	}
...
char * ReadEntFile (char *mapname) {
	...
	//prepare file
	filename = va("jump/maps/%s.ent", mapname);
...
void SpawnEntities (char *mapname, char *entities, char *spawnpoint)
{
	...
	if (game.maxclients < 2) {
		gi.cprintf (NULL, PRINT_HIGH, "***************\nWARNING: Jump is multiplayer only. Restarting in multiplayer mode.\n***************\n");
		gi.AddCommandString (va ("set deathmatch 1\nset maxclients 8\nkillserver\nmap \"%s\"\n", level.mapname));
	}

	if (sv_debug->value >= 3) {
		FILE *file;
		file = fopen (va("jump/%s.txt", level.mapname),"wb");
		if (file) {
			fprintf (file, "%s",entities);
			fclose (file);
			gi.dprintf ("Entities have been dumped to %s.txt\n", level.mapname);
		} else {
...
// parse ents
	...
	while ((t = G_Find (t, FOFS(classname), "misc_teleporter"))) {
		edict_t *s = NULL;
		qboolean ok = false;
		while ((s = G_Find (NULL, FOFS(targetname), t->target))) {
			ok = true;
			break;
		}
		if (!ok) {
...
//	InitBodyQue ();
	...
	level.default_gravity = (int)sv_gravity->value;
