...
//WF Weapons Factory Game Flags
// wfflags->value flags
#define	WF_ALLOW_FRIENDLY_FIRE		1
#define	WF_FRAG_LOGGING				2
#define	WF_NO_FORT_RESPAWN			4
#define	WF_NO_HOMING				8
#define	WF_NO_FLYING				16
#define WF_DECOY_PURSUE				32
#define WF_NO_DECOYS				64
//#define WF_NO_RAILGUN_EFFECT		64	//no longer used
#define WF_NO_TURRET				128
#define WF_NO_EARTHQUAKES			256
#define WF_NO_GRAPPLE				512
#define WF_MAP_VOTE					1024
#define WF_ANARCHY					2048
#define WF_ZOID_FLAGCAP				4096
#define WF_NO_PLAYER_CLASSES		8192
#define WF_ZBOT_DETECT				16384
#define WF_AUTO_TEAM_BALANCE		32768
#define WF_SPECIAL_LIGHTS			131072
#define WF_TMP5						262144
#define WF_TMP6						524288


/* Explaination of WF flags:

DF_ALLOW_FRIENDLY_FIRE - If set, this will allow homing rockets,
	decoys, laserballs, proximity bombs to target teammates

DF_NO_FAST_WEAPONS_SWITCH - If set, this will turn OFF fast
	weapons switching
...
WF_PLAYER_CLASSES - (no longer used. see WF_NO_PLAYER_CLASSES)
...
WF_VWEP_ON - Turns on vwep (visible weapons) support for all players

WF_NO_EARTHQUAKES - disable earthquake grenades

WF_NO_DECOYS - disabled the use of decoys

WF_NO_TURRET - Disables turret grenade

WF_NO_GRAPPLE - Disables the use of the grappling hook

WF_RESPAWN_ARMED - If player classes are not used, setting this flag will
   give the player all weapons except bfg on respawn.

WF_MAP_VOTE - Allow players to vote for the next map

WF_STD_LOG - Write to standard log output (like GibStats)

WF_ZOID_FLAGCAP	- Use standard CTF flag capture instead of TF style

WF_NO_PLAYER_CLASSES - If set, this will turn OFF player classes. If
   NOT set, all weapons will be removed from maps and players only get
   what comes with their class.

WF_FRAG_LOGGING - If set, it will turn on frag logging (gslog format)

WF_ZBOT_DETECT - Turn on zbot detection and log it

WF_AUTO_TEAM_BALANCE - If set, it will modify damage amounts based on score
*/

//The following flags are reserved for wfadmin
#define WF_ADMIN_MAP_LIST       1

/*
WF_ADMIN_MAP_LIST - Use a map list for map rotation?  RESERVED
   by L. Allan Campbell (Geist)
*/
//WF
