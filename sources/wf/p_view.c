...
//===================================
	...
	else
	{
		...
		if (bobcycle & 1)
			delta = -delta;
		angles[ROLL] += delta;
//WF24 S STWF - Concusion grenade
		//add angles based on drunk
...
//WF24 S ST
	...
	{   
		if (ent->remotecam)
		{
            ...
            ent->client->ps.pmove.origin[0] = ent->remotecam->camposition[0]*8;
            ent->client->ps.pmove.origin[1] = ent->remotecam->camposition[1]*8;
            ent->client->ps.pmove.origin[2] = ent->remotecam->camposition[2]*8;
			VectorCopy(ent->s.angles,ent->remotecam->s.angles);
/*db		ent->client->oldplayer->s.frame = ent->s.frame;
...
//WF S
	...
	{               
		...
		VectorSet(offset,ent->s.origin[0]+forward[0]*-80,ent->s.origin[1]+forward[1]*-80,36+ent->s.origin[2]+forward[2]*-40);
		tr = gi.trace (ent->s.origin, NULL, NULL, offset, ent, MASK_SHOT);

		ent->client->ps.pmove.origin[0] = (tr.endpos[0]+forward[0]*10)*8;
		ent->client->ps.pmove.origin[1] =(tr.endpos[1]+forward[1]*10)*8;
		ent->client->ps.pmove.origin[2] =(tr.endpos[2]+forward[2]*10)*8;
/*db		ent->client->oldplayer->s.frame = ent->s.frame;
...
//WF24 S ST - screen turns orange if on fire
	if(ent->Flames)
		SV_AddBlend (1.0, 0.5, 0.0, 0.4, ent->client->ps.blend);
       //ST

    //WF - Flash grenade
	//Are we done with flash yet?
...
//WF
	//WF Disguise Stuff
	if(ent->disguiseshots>2)
		SV_AddBlend (0.0, 1.0, 0.0, 0.3, ent->client->ps.blend);
	else if (ent->disguiseshots==2)
		SV_AddBlend (1.0, 1.0, 0.0, 0.3, ent->client->ps.blend);
	else if (ent->disguiseshots==1)
		SV_AddBlend (0.0, 0.5, 0.0, 0.3, ent->client->ps.blend);
...
void P_FallingDamage (edict_t *ent)
{
	...
	// TeT The game stopped them cold, don't take damage for it
	if (ent->cantmove)
	{
		return;
	}
	// TeT The jet pack can cause massive velocity changes
	if (Jet_Active(ent))
	{
		return;
	}
...
void P_WorldEffects (void)
{
	...
	int			waterlevel, old_waterlevel;
                                                      //ERASER ADDED || (CURRENT  
	if ((current_player->movetype == MOVETYPE_NOCLIP) || (current_player->health <= 0))
	{
	...
	if (current_client->player_items & ITEM_REBREATHER)
		breather = 1;//WF34 2 LINES
	//
...
//ZOID
//WF24 S
	//Add effects for respawn protection
	if (ent->client->protecttime > level.time)
		ent->s.effects |= EF_TELEPORTER;	//set it
	else
		ent->s.effects &= ~EF_TELEPORTER;	//clear it
//WF24 E
	if (ent->client->quad_framenum > level.framenum
//ZOID
		&& (level.framenum & 8)
...
//ZOID
	...
	if (ent->disease)
		ent->s.effects |= EF_FLIES;//WF34 2 LINES
	//acrid 3/99
...
//ZOID
	...
	{	// standing
		...
		else
		{
			ent->s.frame = FRAME_stand01;
			client->anim_end = FRAME_stand40;
//WF24 S - put decoy back to standing
			if (ent->decoy) decoy_stand(ent->decoy);
//WF24 E
		}
	}
}
...
//ZOID
//FIXME ACRID WF24 USES THIS
//update chasecam follower stats
//	for (i = 1; i <= maxclients->value; i++) {
//		edict_t *e = g_edicts + i;
//FIXME ACRID WF24 USES THIS

// ERASER START, use player list
	for (i=0; i<num_players; i++)
	{
		e = players[i];
// ERASER END//FIXME ACRID

		if (!ent->inuse ||
			e->client->chase_target != ent)//crash here was changing maps
			continue;
...
//ZOID
	...
	//See if there are any friends in the area
	ShowCloseFriend_f (ent);
...
//WF24 S - SCANNER
//	if (ent->client->showscores && !(level.framenum & 31) )//NORMAL LINE
...
//		!(level.framenum & SCANNER_UPDATE_FREQ)) || (ent->client->pers.scanner_active & 2))
	if (  ((ent->client->showscores) && !(level.framenum & 31)) ||
		  ((ent->client->pers.scanner_active) && !(level.framenum & 7))   )
//WF34 2 LINES
//WF24 E
	{
//ZOID
		if (ent->client->menu) 
		{
//WF CO	    PMenu_Update(ent);//NORMAL LINE WF24 USES BELOW SECTION
			ent->menutime++;//JR Added this so it updates less since it was always updating not good
			if(ent->menutime>85)//WF34 WAS 55
			{
				PMenu_Update(ent);
				ent->menutime = 0;
			}
