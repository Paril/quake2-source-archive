...
/*FIXME ACRID WF24 USES THESE 4 LINES
#define	STATE_TOP			0
...
edict_t	*plat_ent = NULL, *start_trail;	//ERASER used for node laying

void plat_go_down (edict_t *ent);

void plat_hit_top (edict_t *ent)//acrid fixme look for grapple fix
{//ERASER START
	if ((bot_calc_nodes->value) && (ent->movetarget) && (ent->frags != 1) && plat_ent && (ent->movetarget->groundentity == ent))
	{
		int i;
		vec3_t	save_org;
		...
		VectorCopy(ent->jump_velocity, ent->s.origin);
		gi.linkentity(ent);

		// make sure the start_trail is directly below current position of player
		plat_ent->s.origin[0] = ent->movetarget->s.origin[0];
		plat_ent->s.origin[1] = ent->movetarget->s.origin[1];

		// drop the starting position node
		PlayerTrail_Add(plat_ent, plat_ent->s.origin, NULL, false, false, NODE_PLAT);
		start_trail = plat_ent->last_trail_dropped;

		if (start_trail)
			start_trail->target_ent = ent;

		// restore position
		VectorCopy(save_org, ent->s.origin);
		gi.linkentity(ent);

		// drop the top node
		PlayerTrail_Add(ent->movetarget, ent->movetarget->s.origin, NULL, false, false, NODE_NORMAL);

		// set visibility of nodes
		NodeDebug("Connecting PLAT nodes\n");

		if (start_trail)
		{
			for (i=0; i < MAX_PATHS; i++)
			{
				if ((start_trail->paths[i] == -1) || (start_trail->paths[i] == ent->movetarget->last_trail_dropped->trail_index))
					break;
			}
			if (i >= MAX_PATHS)	// max_nodes, so override first slot
				start_trail->paths[0] = ent->movetarget->last_trail_dropped->trail_index;
			else
				start_trail->paths[i] = ent->movetarget->last_trail_dropped->trail_index;

			CalcRoutes(start_trail->trail_index);
		}


NodeDebug("Plat start node has %i visible nodes\n", i);

		CalcRoutes(ent->movetarget->last_trail_dropped->trail_index);

		G_FreeEdict(plat_ent);
		plat_ent = NULL;

		ent->frags = 1;		// don't drop more than 1 start node for each platform
	}

	if (ent->movetarget)
	{	// turn off player riding plat flag, so they can resume dropping nodes

		if (!ent->movetarget->bot_client && bot_calc_nodes->value)
		{
			int i;

			if ((i = ClosestNodeToEnt(ent->movetarget, false, true)) > -1)
			{
				ent->movetarget->last_trail_dropped = trail[i];
			}
		}

		ent->movetarget->flags &= ~FL_TEAMSLAVE;
		ent->movetarget = NULL;
	}
//ERASER END
...
{//ERASER START
	if (bot_calc_nodes->value && ent->movetarget)
	{	// set plat_ent position
		vec3_t	org, orgdest, dir;
		trace_t	trace;

		// make sure the plat is somewhat below us
		AngleVectors(ent->movetarget->s.angles, dir, NULL, NULL);
		VectorMA(ent->movetarget->s.origin, 12, dir, org);
		VectorSubtract(org, tv(0,0,32), orgdest);

		trace = gi.trace(org, VEC_ORIGIN, VEC_ORIGIN, orgdest, ent->movetarget, MASK_SOLID);

		if (trace.ent != ent)
			goto not_on_plat;

		ent->movetarget->flags |= FL_TEAMSLAVE;	// always set this, so we don't drop a shitload of trails while riding the plat

		if (ent->frags != 1)
		{
NodeDebug("Setting plat_ent\n");
			if (!plat_ent)
				plat_ent = G_Spawn();

			plat_ent->classname = "plat_ent";
			VectorCopy(tv(-16, -16, -24), plat_ent->mins);
			VectorCopy(tv(16, 16, 32), plat_ent->maxs);

			VectorCopy(VEC_ORIGIN, plat_ent->velocity);
			VectorCopy(ent->movetarget->s.origin, plat_ent->s.origin);

			// store the starting position for node dropping
			VectorCopy(ent->s.origin, ent->jump_velocity);
	...
	if (!(other->svflags & SVF_MONSTER) && (!other->client))
	{
		...
		{//WF34
			if (!strcmp(other->classname, "SentryGun") )
				turret_self_remove(other);
			else
				BecomeExplosion1 (other);//NORMAL LINE
		}//WF34
		return;
	}

	T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, self->dmg, 1 + (other->bot_client)*60, 0, MOD_CRUSH);
...
//ERASER START hack, don't go up if the bot doesn't specifically want to
	if (other->bot_client && (!other->goalentity || ((other->goalentity->s.origin[2] + 48) < other->s.origin[2])))
		return;
//ERASER END
//WF34See if there is a team specific indicator
	if ((ent->wf_team) && (ent->wf_team != other->wf_team))
		return;	//WF34
	ent = ent->enemy;	// now point at the plat, not the trigger

	ent->movetarget = other;	//ERASER NODES: tell the plat who is riding it
...
{
	if (wfdebug) gi.dprintf("button_return.\n");//WF34
...
{
	if (wfdebug) gi.dprintf("button_wait.\n");//WF34
	self->moveinfo.state = STATE_TOP;
	self->s.effects &= ~EF_ANIM01;
	self->s.effects |= EF_ANIM23;

if (wfdebug) gi.dprintf("Self team=%d, activator_team = %d, activator=%s.\n",
						self->wf_team, self->activator->wf_team, self->activator->classname);
	if ((self->wf_team) && (self->wf_team != self->activator->wf_team))
	{}
	else
	{
...
{
if (wfdebug) gi.dprintf("button_fire.\n");
	if (self->moveinfo.state == STATE_UP || self->moveinfo.state == STATE_TOP)
		return;
if (wfdebug) gi.dprintf("button_fire-1.\n");
	...
	//Should we award a bonus?
	if (self->bonustype) 
	{
if (wfdebug) gi.dprintf("button_fire-2.\n");
		WFAddBonus(self, self->activator);
	}
...
{
if (wfdebug) gi.dprintf("button_use.\n");
...
{
if (wfdebug) gi.dprintf("button_touch.\n");
	if (!other->client)
		return;
...
//ERASER START nodes, drop a node here if not touched yet
	if (self->frags < 1)
	{
		self->frags = 1;
 		PlayerTrail_Add(other, other->s.origin, NULL, false, true, NODE_BUTTON);
	}
	else if (other->bot_client)
	{
		other->activator = NULL;
		other->activator_time = level.time + 8;
	}
//ERASER END
...
if (wfdebug) gi.dprintf("button_touch-1.\n");
...
{
	if (wfdebug) gi.dprintf("button_killed.  Inflictor=%s,Attacker=%s \n", inflictor->classname, attacker->classname);
...
void botButtonThink(edict_t *ent);//ERASER
...
void SP_func_button (edict_t *ent)
{
	...
	gi.setmodel (ent, ent->model);
	//WF34
if (wfdebug) gi.dprintf("func_button: bonustype = %d\n",
						ent->bonustype);//WF34
	if (ent->sounds != 1)
...
3)	medium
4)	heavy
"wf_team" the team the door is for 0 means not a team door
...
//ERASER START Bot stuff
	if (activator && activator->bot_client)
	{
		activator->bot_plat_pausetime = level.time + 0.8;
		activator->checkstuck_time = level.time + 1.5;	// don't worry about not moving
	}
...
void Touch_DoorTrigger (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (!(other->svflags & SVF_MONSTER) && (!other->client))
		return;
//WF34 S
//gi.dprintf("trigger team=%d, entity team = %d\n", self->wf_team, other->wf_team);

	if(self->wf_team)//WF John added this for team doors 6/26/98
	{
		if(self->wf_team != other->wf_team)
			return;
	}
//WF34 E
//	if ((self->owner->spawnflags & DOOR_NOMONSTER) && (other->svflags & SVF_MONSTER))
//		return;//WF24 FIXME USES THESE LINES
...
//gi.dprintf("SP_func_door.   = %d\n",ent->wf_team);//WAS ALREADY CO
...
//		gi.dprintf ("train_next: no next target\n");
	...
	if (ent->spawnflags & 1)
	{
		...
		self->s.event = EV_OTHER_TELEPORT;//WF34
		gi.linkentity (self);
		goto again;
	}
...
static void rotating_light_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	//Don't do rotating light if WF_SPECIAL_LIGHTS is not enabled
	if (((int)wfflags->value & WF_SPECIAL_LIGHTS)  == 0) 
		return;
...
void SP_rotating_light (edict_t *self)
{
	...
	if (!self->speed)//speed doesnt appear to work acrid
	{
		self->speed = 32;
	...
	if (self->spawnflags & 2)
	{
		self->moveinfo.sound_start = gi.soundindex ("alarm.wav");	
	}
