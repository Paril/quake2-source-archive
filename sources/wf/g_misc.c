...
void ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	gib->solid = SOLID_NOT;
	//freezer additions
//	gib->s.effects |= EF_GIB;//acrid 3/99 orig
	gib->s.effects = self->s.effects |= EF_GIB;//acrid 3/99 set ef to body ef
	gib->s.renderfx = self->s.renderfx;//acrid 3/99 set fx to body fx

	gib->flags |= FL_NO_KNOCKBACK;
	gib->takedamage = DAMAGE_YES;
	gib->die = gib_die;
	//WF JR Water FLoating stuff
	gib->CanFloat=1;
	gib->waterlevel = 0;
	gib->HitTopOfWater = 0;
	gib->turrettime=-1;
	gib->watertype=0;
	//WF JR End Water Floating Stuff
...
void ThrowHead (edict_t *self, char *gibname, int damage, int type)
{
	...
	self->avelocity[YAW] = crandom()*500;

	self->think = G_FreeEdict;
	self->nextthink = level.time + 10 + random()*4;
...
if (!self->bot_client)//ERASER
	...
	self->solid = SOLID_NOT;
	self->s.effects |= EF_GIB;//acrid 3/99 fixme added | for freeze code//looks like a old mistake
	self->s.sound = 0;
	self->flags |= FL_NO_KNOCKBACK;
	//WF JR Water FLoating stuff
	self->CanFloat=1;
...
void SP_light (edict_t *self)
{
	...
	if (!self->targetname)//acrid lightcode 1/29/99 || deathmatch->value)
	{
		G_FreeEdict (self);
		return;
	}

	//Don't do targeted light if WF_SPECIAL_LIGHTS is not enabled
...
{
	edict_t		*dest, *last_trail;//ERASER ADDED *LAST_TRAIL
	int			i;
	...
	//See if we pass the team check WF34
	if ( PassTeamCheck(self,other) == false) return;


//ZOID
	if(other->bot_client)//newgrap 4/99
		CTFPlayerResetGrapple(other);
	else//newgrap 4/99
		CTFPlayerResetGrapple2(other);//newgrap 4/99
...
//	dest->nextthink = level.time + 0.2;
	...
	if ((!other->bot_client) && (bot_calc_nodes->value))
	{
		vec3_t org;

		VectorCopy(dest->s.origin, org);
		org[2] += 9;

		if (!self->owner->frags && (!matching_trail(org) || !(self->owner->frags = 1)))
		{
			NodeDebug("Dropping teleporter route..\n");

			// drop a teleport route
			PlayerTrail_Add(other, other->s.origin, NULL, false, true, NODE_TELEPORT);
			last_trail = other->last_trail_dropped;
		}
		else	// look for a node to use for future checking
		{
			int i;

			if ((i = ClosestNodeToEnt(other, false, true)) > -1)
			{
				other->last_trail_dropped = trail[i];
			}
		}
	}
//ERASER END
	...
	{
		CTFPlayerResetGrapple2(other->client->ctf_grapple);
	}
	...
	if (!other->bot_client && (bot_calc_nodes->value))	
	{
		if (!self->owner->frags)	// only add one route per teleporter
		{
			int i;

			self->owner->frags = 1;

			// drop a teleport dest node
			PlayerTrail_Add(other, other->s.origin, NULL, false, false, NODE_NORMAL);

			// add the route
			for (i=0; i<MAX_PATHS; i++)
				if (last_trail->paths[i] == -1)
					break;

			if (i == MAX_PATHS)
			{
//				gi.dprintf("WARNING: Unable to add teleport route, MAX_PATHS reached\n");
				i--;	// just use the last spot
			}

			last_trail->paths[i] = other->last_trail_dropped->trail_index;
			last_trail->flags = TRAIL_TELEPORT_SOURCE;

			last_trail->routes->route_dist[other->last_trail_dropped->trail_index] = 0;
			last_trail->routes->route_path[other->last_trail_dropped->trail_index] = (short int) other->last_trail_dropped->trail_index;

			CalcRoutes(other->last_trail_dropped->trail_index);
			CalcRoutes(last_trail->trail_index);
		}
	}

	// Bots should check for a new node after teleporting
	if (other->bot_client)
...
//gi.dprintf("SP_misc_teleporter.   = %d\n",ent->wf_team);
