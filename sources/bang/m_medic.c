...
{
	gi.sound (self, CHAN_AUTOON, sound_hook_launch, 1, ATTN_NORM, 0);
}
...
void medic_cable_attack (edict_t *self)
{
	...
	else
	{
		if (self->s.frame == FRAME_attack44)
			gi.sound (self, CHAN_AUTOON, sound_hook_heal, 1, ATTN_NORM, 0);
	}
...
{
	gi.sound (self, CHAN_AUTOON, sound_hook_retract, 1, ATTN_NORM, 0);
	self->enemy->monsterinfo.aiflags &= ~AI_RESURRECTING;
}
