...
//	gi.sound (self, CHAN_AUTO, actor.sound_pain, 1, ATTN_NORM, 0);
...
// check for gib
	if (self->health <= -80)
	{
//		gi.sound (self, CHAN_AUTO, actor.sound_gib, 1, ATTN_NORM, 0);
		for (n= 0; n < 4; n++)
...
// regular death
//	gi.sound (self, CHAN_AUTO, actor.sound_die, 1, ATTN_NORM, 0);
	self->deadflag = DEAD_DEAD;
...
void target_actor_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (self->spawnflags & 1)		//jump
	{
		...
		if (other->groundentity)
		{
			...
			gi.sound(other, CHAN_AUTO, gi.soundindex("player/male/jump1.wav"), 1, ATTN_NORM, 0);
		}
	}
