...
void ClipGibVelocity (edict_t *ent)
{
	if (ent->velocity[0] < -300)
		ent->velocity[0] = -300 + ((crandom()*30)-15);
	else if (ent->velocity[0] > 300)
		ent->velocity[0] = 300 + ((crandom()*30)-15);
	if (ent->velocity[1] < -300)
		ent->velocity[1] = -300 + ((crandom()*30)-15);
	else if (ent->velocity[1] > 300)
		ent->velocity[1] = 300 + ((crandom()*30)-15);
	if (ent->velocity[2] < 400)
		ent->velocity[2] = 400 + ((crandom()*30)-15);	// always some upwards
	else if (ent->velocity[2] > 300)
		ent->velocity[2] = 300 + ((crandom()*30)-15);
}
...
void gib_think (edict_t *self)
{
	...
	if (self->s.frame == 10)
	{
		...
		self->nextthink = level.time + 6 + random()*10;
	}
}
...
void gib_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	//if (self->touch_debounce_time < level.time) {
		n = (rand() + 1) % 3;
		if (n == 0)
			gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gibimp1.wav"), 1, ATTN_STATIC, 0);
		else if (n == 1)
			gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gibimp2.wav"), 1, ATTN_STATIC, 0);
		else
			gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gibimp3.wav"), 1, ATTN_STATIC, 0);
	//}
...
void ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	n = (rand() + 1) % 3;
	if (n == 0)
		gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gibimp1.wav"), 1, ATTN_NORM, 0);
	else if (n == 1)
		gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gibimp2.wav"), 1, ATTN_NORM, 0);
	else
		gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gibimp3.wav"), 1, ATTN_NORM, 0);
	...
	gib->nextthink = level.time + 6 + random()*10;
...
void ThrowRandomAmmo (edict_t *self)
{
	int i;
	int newitem;
	edict_t *ent;
	...
	for (i = 0; i < 6 + random() * 5; i++) {
		VectorSet(aimdir, crandom() * 20, crandom() * 20,random()*20+10);
		ent = G_Spawn();
		newitem = random() * 8;

		switch(newitem) {
			case 0:
				ent->classname = "ammo_infestgrenades";
				break;
			case 1:
				ent->classname = "ammo_normalgrenades";
				break;
			case 2:
				ent->classname = "ammo_firegrenades";
				break;
			case 3:
				ent->classname = "ammo_biogrenades";
				break;
			case 4:
				ent->classname = "ammo_forcegrenades";
				break;
			case 5:
				if (no_nukes->value == 0)
					ent->classname = "ammo_nukegrenades";
				else
					ent->classname = "ammo_forcegrenades";
				break;
			case 6:
				ent->classname = "ammo_lasergrenades";
				break;
			case 7:
				ent->classname = "ammo_swarmgrenades";
				break;
			default:
				ent->classname = "ammo_normalgrenades";
				break;
		...
		VectorScale (aimdir, 20, ent->velocity);
		ent->s.effects = ent->item->world_model_flags;
		ent->s.renderfx = RF_GLOW;
		VectorSet (ent->mins, -15, -15, -15);
		VectorSet (ent->maxs, 15, 15, 15);
		gi.setmodel (ent, ent->item->world_model);
		ent->s.skinnum = itemlist[ITEM_INDEX(ent->item)].skinnum;
...
static void ClusterB_Explode (edict_t *ent, int type)
{
	...
	T_RadiusDamage(ent, ent->activator, ent->dmg, NULL, ent->dmg_radius,mod);

	VectorSet(aimdir[0], 20, 20, random() * 20 + 30);
	VectorSet(aimdir[1],20,-20,random()*20+30);
	VectorSet(aimdir[2],-20,20,random()*20+30);
	VectorSet(aimdir[3],-20,-20,random()*20+30);
	VectorSet(aimdir[4],30,30,random()*20+40);
	VectorSet(aimdir[5],30,-30,random()*20+40);
	VectorSet(aimdir[6],-30,30,random()*20+40);
	VectorSet(aimdir[7],-30,-30,random()*20+40);
	
	for (i = 0; i < 8; i++) {
		switch (type) {
			case EXPLODE_FIRE:
				fire_fire(ent->activator, ent->s.origin, aimdir[i], 90 + random() * 10, 7 + random() * 3, random()*3+1, 120, false);
				break;
			case EXPLODE_BIO:
				fire_spore (ent->activator, ent->s.origin, aimdir[i], 90 + random() * 10, 7 + random() * 3, random() * 3, 90, false);
				break;
			case EXPLODE_BLASTER:
				launch_blaster (ent->activator, ent->s.origin, aimdir[i], 1500, 15, 0, false);
				break;
		}
	}
}
...
void barrel_explode (edict_t *self)
{
	...
	switch (tpe) {
		case 1:
			//fire grenade style explosion
			ClusterB_Explode(self, EXPLODE_FIRE);
			break;
		case 2:
			//bio grenade style explosion
			ClusterB_Explode (self, EXPLODE_BIO);
			break;
		case 3:
			//laser blaster - NASTY!
			ClusterB_Explode (self, EXPLODE_BLASTER);
			break;
		case 4:
			//whee, free ammo!
			ThrowRandomAmmo (self);
			break;
		default:
			//boom.
...
void SP_misc_deadsoldier (edict_t *ent)
{
	...
	if (st.item)
	{
		...
		if (!ent->item)
			gi.dprintf("%s at %s has bad item: %s\n", ent->classname, vtos(ent->s.origin), st.item);
	} else {
		int newitem = random() * 7;
...
void ThrowUpNow(edict_t *hack)
{
	...
	// cough up some gibs.
	T_Damage(self,hack,hack->owner,forward,forward,forward,random()*25,0,0,MOD_MINI_INFEST_HG_SPLASH);
	...
	n = (rand() + 1) % 3;
	if (n == 0)
		gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gib1.wav"), 1, ATTN_NORM, 0);
	else if (n == 1)
		gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gib2.wav"), 1, ATTN_NORM, 0);
	else
		gi.sound (self, CHAN_VOICE, gi.soundindex ("misc/gib3.wav"), 1, ATTN_NORM, 0);
