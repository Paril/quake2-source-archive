...
//Wyrm: if homing... look for target!!!
        if (ent->movetype == MOVETYPE_HOMING)
        {
                edict_t *target = NULL;
                edict_t *blip = NULL;
                vec3_t  targetdir, blipdir, movedir;
                vec_t   speed;
                int     onlyclients=false;
                ...
                while ((blip = findradius(blip, ent->s.origin, 1000)) != NULL)
                {
                        ...
                        //check if a monster homing rocket (only target players...)
                        if (onlyclients && !(blip->client))
                                continue;

                        //FIXME: add a team check here?
                ...
                if (target != NULL)
                {
                        ...
                        VectorCopy(ent->velocity, movedir);
                        VectorNormalize(movedir);
                        VectorNormalize(targetdir);
                        VectorSubtract(targetdir, movedir, blipdir);
                        if (VectorLength(blipdir) < 0.2)
                        {
                                VectorCopy(targetdir, movedir);
                        }
                        else
                        {
                                VectorNormalize(blipdir);
                                VectorMA(movedir, 0.2, blipdir, movedir);
                        }
                        speed = VectorLength(ent->velocity);
                        VectorScale(movedir, speed, ent->velocity);
                }
        }
    ...
    if(ent->movetype == MOVETYPE_BOUNCEFLY)
    {
	    VectorNormalize (ent->velocity);
            vectoangles(ent->velocity, ent->s.angles); //Wyrm: adjust angle!!
...
void SV_Physics_FallFloat (edict_t *ent)
{
	...
	qboolean hitsound = false;
        vec3_t entmins, entmaxs, entsize;

        if (ent->s.effects & EF_GIB)
        {
                VectorSet (entmins, -8, -8, -6);
                VectorSet (entmaxs, 8, 8, 10);
                VectorSet (entsize, 16, 16, 16);
        }
        else
        {
                VectorCopy(ent->maxs, entmaxs);
                VectorCopy(ent->mins, entmins);
                VectorCopy(ent->size, entsize);
        }
	...
	if (!ent->waterlevel)
	{
		...
		vec3_t gravity;
                ...
                VectorCopy(entmins, min);
                VectorCopy(entmaxs, max);
	...
	//if (ent->waterlevel)
	{
		...
		// calculate massPerCubicMetre
                VectorScale(entsize, 1.0/32.0, volume);
		...
		for (i = 0.0; i <= 1.0; i += 0.05)
		{
			...
			int watertype;
                        ...
                        VectorAdd(ent->s.origin, entmins, midpoint);
                        VectorMA(midpoint, i, entmaxs, midpoint);
