...
//======
	...
	if (blindfire)
	{
		...
		if (!(trace.startsolid || trace.allsolid || (trace.fraction < 0.5)))
                {
                        if (self->s.skinnum & 2)
                                monster_fire_flamerocket (self, start, dir, 50, rocketSpeed, MZ2_CHICK_ROCKET_1);
                        else
...
//	self->monsterinfo.dodge = chick_dodge;
	...
	// pmm
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_chick_normal;
...
//	self->monsterinfo.dodge = chick_dodge;
	...
	// pmm
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_chick_fire;
...
{
        if (random() > 0.3)
                SP_monster_chick_normal (self);
        else
                SP_monster_chick_fire (self);
}
