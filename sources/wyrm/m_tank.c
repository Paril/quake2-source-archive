...
void TankBlaster (edict_t *self)
{
	...
	VectorSubtract (end, start, dir);
        ...
        if (self->s.skinnum > 3)
        {
                monster_fire_shotgun(self, start, dir, 4, 2, DEFAULT_SHOTGUN_HSPREAD, DEFAULT_SHOTGUN_VSPREAD, DEFAULT_SHOTGUN_COUNT, flash_number);
...
void TankStrike (edict_t *self)
{
	...
	VectorSet (aim, MELEE_DISTANCE, self->mins[0], -4);
        fire_hit (self, aim, (90 + (rand() %20)), 800);

	gi.sound (self, CHAN_WEAPON, sound_strike, 1, ATTN_NORM, 0);
}	

static void tank_fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int flashtype, float seed)
{
        if (self->s.skinnum < 4)
        ...
        else
        {
                ...
                if (seed < 0.25)
                        monster_fire_flamerocket (self, start, dir, damage, speed, flashtype);
                else if (seed < 0.5)
                        monster_fire_homingrocket (self, start, dir, damage, speed, flashtype);
                else if (seed < 0.75)
                        monster_fire_rocket(self, start, dir, damage, speed, flashtype);
                else //if (seed <= 1)
                        monster_fire_perforator (self, start, dir, damage, speed, flashtype);
        }
}
...
void TankRocket (edict_t *self)
{
	...
	if(!self->enemy || !self->enemy->inuse)		//PGM
        ...
        if (((r=random()) >= 0.75)&&(self->s.skinnum > 3))
                procket = true;
...
//PGM
	...
	// pmm
        ...
        //Wyrm: don't shoot at feet if using perf rockets
        else if (random() < 0.66 || (start[2] < self->enemy->absmin[2]) || procket)
	{
...
//			gi.multicast (start, MULTICAST_ALL);
	...
	if (blindfire)
	{
		...
		if (!(trace.startsolid || trace.allsolid || (trace.fraction < 0.5)))
                        tank_fire_rocket (self, start, dir, 50, rocketSpeed, flash_number, r);
		else 
		{
	...
	else
	{
		...
		if(trace.ent == self->enemy || trace.ent == world)
		{
			if(trace.fraction > 0.5 || (trace.ent && trace.ent->client))
                                tank_fire_rocket (self, start, dir, 50, rocketSpeed, MZ2_CHICK_ROCKET_1, r);
	//		else
...
void TankMachineGun (edict_t *self)
{
	...
	AngleVectors (dir, forward, NULL, NULL);
        ...
        //Wyrm: bigger bullets for hammerstein
        if (self->s.skinnum > 3)
                monster_fire_bullet (self, start, forward, 34, 6, DEFAULT_BULLET_HSPREAD, DEFAULT_BULLET_VSPREAD, flash_number);
        else
...
//				gi.dprintf ("blindfire - NO SHOT\n");
	...
	r = random();
        ...
        //Wyrm: added melee attack to the hammerstein
        if (range <= MELEE_DISTANCE && (self->s.skinnum > 3))
        {
                if (r < 0.5)
                        self->monsterinfo.currentmove = &tank_move_attack_strike;
                else if (r < 0.75)
...
void SP_monster_tank_normal (edict_t *self)
{
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_tank_normal;
...
/*QUAKED monster_hammerstein (1 .5 0) (-32 -32 -16) (32 32 72) Ambush Trigger_Spawn Sight
*/
void SP_monster_hammerstein (edict_t *self)
{
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_hammerstein;
...
{
//        if (random() > 0.3)
//                SP_monster_tank_normal (self);
//        else
                SP_monster_hammerstein (self);
}
