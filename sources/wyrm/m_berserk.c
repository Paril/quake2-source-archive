...
//Wyrm:
void doberserk_run (edict_t *self);
void berserk_run (edict_t *self);
void berserk_reattack(edict_t *self);
#define AI_BERSERK AI_IGNORE_SHOTS
...
mmove_t berserk_move_runb1 = {FRAME_r_att6, FRAME_r_att11, berserk_frames_runb1, NULL};

mframe_t berserk_frames_start_runb1 [] =
{
        ai_run, 9, NULL,
        ai_run, 6, NULL,
        ai_run, 18.4, NULL,
...
mmove_t berserk_move_start_runb1 = {FRAME_r_att1, FRAME_r_att5, berserk_frames_start_runb1, berserk_run};
...
void berserk_run (edict_t *self)
{
	monster_done_dodge (self);
        //Wyrm: berserk mode
        if (self->monsterinfo.aiflags & AI_BERSERK)
                self->monsterinfo.currentmove = &berserk_move_runb1;
...
void berserk_attack_club (edict_t *self)
{
	...
	VectorSet (aim, MELEE_DISTANCE, self->mins[0], -4);
        //Wyrm: this attack is slower, so does more damage
        fire_hit (self, aim, (20 + (rand() % 6)), 400);          // Slower attack
}
...
//Wyrm: new berserk attack
mframe_t berserk_frames_attack_running [] =
{	
        ai_charge, 19, NULL,
        ai_charge, 3.7, berserk_swing,
        ai_charge, 2.6, NULL,
        ai_charge, 19, NULL,
        ai_charge, 25, berserk_attack_club,
        ai_charge, 19.6, NULL,
        ai_charge, 7.8, berserk_reattack
};                                                
mmove_t berserk_move_attack_running = {FRAME_r_att12, FRAME_r_att18, berserk_frames_attack_running, berserk_run};
...
void berserk_melee (edict_t *self)
{
	monster_done_dodge (self);
        ...
        if (self->monsterinfo.aiflags & AI_BERSERK)
        {
                self->monsterinfo.currentmove = &berserk_move_attack_running;
        }
        else
        {
                if ((rand() % 2) == 0)
...
void berserk_reattack(edict_t *self)
{
	...
	{
                if ( (range (self, self->enemy) == RANGE_MELEE) && (random() <= 0.9) )
                {
                        self->monsterinfo.currentmove = &berserk_move_attack_running;
                        return;
                }
	...
	if (self->health < (self->max_health / 2))
        {
                self->s.skinnum |= 1;
                //Wyrm: be berserk!
                if (self->enemy) self->monsterinfo.aiflags |= AI_BERSERK;
        }
...
{
        if (self->monsterinfo.aiflags & AI_BERSERK)
                self->monsterinfo.currentmove = &berserk_move_start_runb1;
        else
                berserk_run(self);
}
...
mmove_t berserk_move_jump2 = { FRAME_run1, FRAME_run6, berserk_frames_jump2, berserk_run };
...
//	self->monsterinfo.dodge = NULL;
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_berserk;
