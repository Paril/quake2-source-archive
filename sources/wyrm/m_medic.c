...
void medic_pain (edict_t *self, edict_t *other, float kick, int damage)
{
	...
	if (self->mass > 400)
	{
		...
		gi.sound (self, CHAN_VOICE, commander_sound_pain2, 1, ATTN_NORM, 0);
                ...
                if (random() < (wyrmmin(((float)damage * 0.005), 0.5)))             // no more than 50% chance of big pain
...
//			gi.dprintf ("medic_commander - aborting heal due to beamus interruptus\n");
	...
	{
		vec3_t	maxs;
/*-----/ PM /-----/ NEW:  Kill any flames during resurrection. /-----*/
...
//	self->monsterinfo.dodge = medic_dodge;
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_medic;
