...
void mutant_hit_left (edict_t *self)
{
	...
	if (fire_hit (self, aim, (10 + (rand() %5)), 100))
		...
		gi.sound (self, CHAN_WEAPON, sound_hit, 1, ATTN_NORM, 0);
                //Wyrm: demon!
                if ((self->s.skinnum & 2) && (random() > 0.5))
                        PBM_Ignite(self->enemy, self, self->enemy->s.origin);
        }
	else
...
void mutant_finish_demonattack (edict_t *self);

void mutant_init_demonattack (edict_t *self)
{
        //Wyrm : Big Fucking Ball!
        MakePositron(self, self, self->s.origin, 0);
...
mframe_t mutant_frames_attack_demon [] =
{
        ai_charge,      0,      mutant_init_demonattack,
        ai_charge,      0,      mutant_finish_demonattack,
        ai_charge,      0,      NULL
};
mmove_t mutant_move_attack_demon = {FRAME_stand101, FRAME_stand103, mutant_frames_attack_demon, mutant_run};

void mutant_melee (edict_t *self)
{
        if (self->s.skinnum > 1 && random() > 0.25)
        {
                self->monsterinfo.currentmove = &mutant_move_attack_demon;
        }
        else //normal attack
...
mmove_t mutant_move_jump_up = { FRAME_run03, FRAME_run07, mutant_frames_jump_up, mutant_run };
...
mmove_t mutant_move_jump_down = { FRAME_run03, FRAME_run07, mutant_frames_jump_down, mutant_run };
...
void SP_monster_mutant_normal (edict_t *self)
{
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_mutant_normal;

	walkmonster_start (self);

        //Wyrm: napalm
        self->fireflags |= (24 | FIREFLAG_DOWN | FIREFLAG_DELTA_ALIVE);
}
...
void SP_monster_mutant_demon (edict_t *self)
{
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_mutant_demon;
...
{
        if (random() > 0.3)
                SP_monster_mutant_normal (self);
        else
                SP_monster_mutant_demon (self);
}
