...
//	pmm		
			...
			if (ent->flags & FL_SWIM)
			{
				...
				//Wyrm: the mosters go into water... but not to lava/slime
                if ((contents & CONTENTS_LAVA) && !(ent->flags & FL_IMMUNE_LAVA))
                        return false;
                if ((contents & CONTENTS_SLIME) && !(ent->flags & FL_IMMUNE_SLIME))
						return false;
/*
