...
//Wyrm: tech
extern int tech_resistance_index;
extern int tech_strength_index;
extern int tech_haste_index;
extern int tech_regeneration_index;
...
{
        gitem_t *item;
        int ammo_index, ammo2_index;
	int p1, p2;
	edict_t *e;

        //special ammo hud
        ammo_index = ent->client->ammo_index;
        ammo2_index = ent->client->ammo2_index;
        if (ammo_index && ent->client->pers.weapon)
        {
                if (ammo2_index)
                {
                        int ammount, ammount2;

                        ammount = ent->client->pers.inventory[ammo_index] / ent->client->pers.weapon->quantity;

                        ammount2 = ent->client->pers.inventory[ammo2_index] / ent->client->pers.weapon->quantity2;
                        if (ammount2 >= ammount)
                                ent->client->ps.stats[STAT_CTF_AMMO_STRING] = ammount;
                        else
                                ent->client->ps.stats[STAT_CTF_AMMO_STRING] = ammount2;

                        item = &itemlist[ammo_index];
                        ent->client->ps.stats[STAT_CTF_AMMO1_PIC] = gi.imageindex(item->icon);
                        item = &itemlist[ammo2_index];
                        ent->client->ps.stats[STAT_CTF_AMMO2_PIC] = gi.imageindex(item->icon);
                }
                else
                {
                        item = &itemlist[ammo_index];
                        ...
                        if (ent->client->pers.weapon == FindItem("Grenades"))
                                ent->client->ps.stats[STAT_CTF_AMMO_STRING] = ent->client->pers.inventory[ammo_index];
                        else
                                ent->client->ps.stats[STAT_CTF_AMMO_STRING] = ent->client->pers.inventory[ammo_index] / ent->client->pers.weapon->quantity;
                        ent->client->ps.stats[STAT_CTF_AMMO1_PIC] = gi.imageindex(item->icon);
                        ent->client->ps.stats[STAT_CTF_AMMO2_PIC] = 0;
                }
        }
        else
        {
                ent->client->ps.stats[STAT_CTF_AMMO_STRING] =
                ent->client->ps.stats[STAT_CTF_AMMO1_PIC] =
                ent->client->ps.stats[STAT_CTF_AMMO2_PIC] = 0;
        }
        ...
        if (ent->client->ammo_index)
        {
                ...
                ent->client->ps.stats[STAT_CTF_AMMO1_PIC] = 0;

        if (ent->client->ammo2_index)
        {
                item = &itemlist[ent->client->ammo_index];
                ent->client->ps.stats[STAT_CTF_AMMO1_PIC] = gi.imageindex(item->icon);
        }
...
//Wyrm: one pic for both pic teams :P
        ent->client->ps.stats[STAT_CTF_JOINED_TEAM_PIC] = 0;
	if (ent->client->resp.ctf_team == CTF_TEAM1)
                ent->client->ps.stats[STAT_CTF_JOINED_TEAM_PIC] = gi.imageindex ("i_ctfj1");
	else if (ent->client->resp.ctf_team == CTF_TEAM2)
                ent->client->ps.stats[STAT_CTF_JOINED_TEAM_PIC] = gi.imageindex ("i_ctfj2");
...
void CTFResetGrapple(edict_t *self)
{
	if (self->owner->client->ctf_grapple) {
		...
		cl->ctf_grapplestate = CTF_GRAPPLE_STATE_FLY; // we're firing, not on hook
                ...
                if (!cl->onturret && !cl->missile && !cl->chasetoggle)
	...
	*string = 0;
	...
	// team one
		...
		"xv 200 yv 28 string \"%4d/%-3d\" "
                ...
                show1 ? "xv 8 yv 8 picn ctfsb1" : "if 11 xv 8 yv 8 pic 11 endif",
		totalscore[0], total[0],
                show2 ?  "xv 168 yv 8 picn ctfsb2" : "if 11 xv 168 yv 8 pic 11 endif",
...
void CTFSetupTechSpawn(void)
{
	edict_t *ent;
        ...
        if (techspawn || ((int)dmflags->value & DF_CTF_NO_TECH) || !deathmatch->value)
		return;
...
int CTFApplyResistance(edict_t *ent, int dmg)
{
	...
	if (ent->client && ent->client->silencer_shots)
        ...
        if (dmg && ent->client && ent->client->pers.inventory[tech_resistance_index]) {
		// make noise
...
{
        if (dmg && ent->client && ent->client->pers.inventory[tech_strength_index]) {
		return dmg * 2;
	}
	return dmg;
}
...
qboolean CTFApplyStrengthSound(edict_t *ent)
{
	...
	if (ent->client && ent->client->silencer_shots)
        ...
        if (ent->client &&
                ent->client->pers.inventory[tech_strength_index]) {
...
{
        if (ent->client &&
                ent->client->pers.inventory[tech_haste_index])
		return true;
	return false;
}
...
void CTFApplyRegeneration(edict_t *ent)
{
	...
	if (ent->client->silencer_shots)
        ...
        if (client->pers.inventory[tech_regeneration_index]) {
		if (client->ctf_regentime < level.time) {
			client->ctf_regentime = level.time;
                        if (ent->health < (ent->max_health * 1.5)) {
                                ent->health += (ent->max_health / 20);
                                if (ent->health > (ent->max_health * 1.5))
                                        ent->health = (ent->max_health * 1.5);
...
{
        if (ent->client &&
                ent->client->pers.inventory[tech_regeneration_index])
		return true;
	return false;
}
...
pmenu_t joinmenu[] = {
        ...
        { NULL,                                 PMENU_ALIGN_LEFT, 0, NULL },
        { "CTF Credits",                PMENU_ALIGN_LEFT, 0, CTFCredits },
        { NULL,                                 PMENU_ALIGN_LEFT, 0, NULL },
...
//Wyrm: classes
        ...
        {
                joinmenu[8].text="Select Class";
                joinmenu[8].SelectFunc = EnterClassMenu;
                joinmenu[11].text="Bind Class Keys";
                joinmenu[11].SelectFunc = BindClassKeys;
        }
