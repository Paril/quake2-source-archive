...
void removeOldestGib()
{
	...
	while(1)
	{
        ...
        if (count > MAX_GIBS && oldestEnt != NULL)
...
//                gib->movetype = MOVETYPE_TOSS;
                gib->movetype = MOVETYPE_FALLFLOAT;
//                gib->touch = gib_touch;
		vscale = 0.5;
	}
	else
...
/*QUAKED misc_blood (0 0.5 0) (-4 -4 -4) (4 4 4)
makes a trail of blood at destination
*/
static void bloodtrail_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	G_FreeEdict(self);
}

void BloodTrail(vec3_t start, vec3_t end)
{
        edict_t *bloodtrail;
        vec3_t dir;
        ...
        bloodtrail = G_Spawn();
        VectorCopy (start, bloodtrail->s.origin);
        VectorCopy (start, bloodtrail->s.old_origin);
        VectorScale (dir, 500 + random()*500, bloodtrail->velocity);
        bloodtrail->solid = SOLID_NOT;
        VectorClear(bloodtrail->mins);
        VectorClear(bloodtrail->maxs);
        bloodtrail->movetype = MOVETYPE_FLYMISSILE;
        bloodtrail->clipmask = false;
        bloodtrail->s.effects |= EF_GIB;
        bloodtrail->s.renderfx |= RF_TRANSLUCENT;
        bloodtrail->s.renderfx |= RF_FULLBRIGHT;

        bloodtrail->s.modelindex = gi.modelindex("models/fire/tris.md2");
        bloodtrail->s.frame      = FRAME_FLAMEOUT;
        bloodtrail->s.skinnum    = 0;

        bloodtrail->touch = bloodtrail_touch;
        bloodtrail->nextthink = level.time + .2;
        bloodtrail->think = G_FreeEdict;
        bloodtrail->classname = "blood";

        gi.linkentity (bloodtrail);
}

void bloodthink (edict_t *ent)
{
//        gi.dprintf("hi soc");
        vec3_t spot;
        ...
        spot[2] -= 10;
        BloodTrail(ent->s.origin, spot);
        ent->nextthink = level.time + .1;
}

void SP_misc_blood (edict_t *ent)
{
        ent->solid = SOLID_NOT;
        ent->movetype = MOVETYPE_NONE;
        ent->flags = FL_NO_KNOCKBACK;
        VectorSet(ent->velocity,0,0,200);
        ent->s.effects |= EF_GIB;
        ent->think = bloodthink;
        ent->nextthink = level.time + FRAMETIME;
	gi.linkentity (ent);
}
