	...
	// so the view can never be outside the player box
        ...
        else
        {
                ...
                if (ent->client->missile != NULL)
                {
                        ...
                        VectorMA(ent->client->missile->s.origin, -32, forward, neworigin);
                        ...
                        VectorScale(forward, 400, ent->client->missile->velocity );
                }
                else
...
void SV_CalcBlend (edict_t *ent)
{
        ...
        if ( Jet_Active(ent) )
  {
    ...
    Jet_remaining = ent->client->Jet_framenum - level.framenum;

    /*if no fuel remaining, remove jetpack from inventory*/ 
      ent->client->pers.inventory[ITEM_INDEX(FindItem("Jetpack"))] = Jet_remaining;
    if (!Jet_remaining)
      ValidateSelectedItem (ent);

    /*Play jetting sound every 0.6 secs (sound of monster icarus)*/
    if ( ((int)Jet_remaining % 6) == 0 )
    ...
    if (Jet_remaining <= 40)
    {
      /*play on/off sound every sec*/
        if ( ((int)Jet_remaining % 10) == 0 )
                gi.sound(ent, CHAN_ITEM, gi.soundindex("items/protect.wav"), 1, ATTN_NORM, 0);
        if ( ((int)Jet_remaining & 4) == 0 )
  ...
  /*ATTILA end*/
        ...
        else if (ent->client->cloak_framenum >= level.framenum)
	{
                if (ent->client->cloak_framenum == level.framenum)
                        gi.sound (ent, CHAN_ITEM, gi.soundindex("cloak/off.wav"), 1, ATTN_NORM, 0);
                else
                {
                        remaining = ent->client->cloak_framenum - level.framenum;
                        if ((!(remaining % 64)) && (remaining > 30))
                                gi.sound (ent, CHAN_ITEM, gi.soundindex("cloak/running.wav"), 1, ATTN_NORM, 0);
...
//Wyrm: steroids
        ...
        {
                remaining = ent->client->steroids_framenum - level.framenum;
		if (remaining > 30 || (remaining & 4) )
                        SV_AddBlend (1, 1, 0.4, 0.04, ent->client->ps.blend);
        }
//ROGUE
...
//Wyrm: Can of Pork'N'Beans
        ...
        {
                remaining = ent->client->beans_framenum - level.framenum;

                if (!remaining)
                {
                        //Play finish sound
                        if (random() > 0.5)
                                gi.sound (ent, CHAN_ITEM, gi.soundindex("copb/copb_4.wav"), 1, ATTN_NORM, 0);
                        else
                                gi.sound (ent, CHAN_ITEM, gi.soundindex("copb/copb_5.wav"), 1, ATTN_NORM, 0);

                }
                else if (!((int)remaining % 50))
                {
                        float i;

                        ent->velocity[2] += 300 + random() * 100;
                        ...
                        i = random();
                        if (i < 0.33)
                                gi.sound (ent, CHAN_ITEM, gi.soundindex("copb/copb_1.wav"), 1, ATTN_NORM, 0);
                        else if (i < 0.66)
                                gi.sound (ent, CHAN_ITEM, gi.soundindex("copb/copb_2.wav"), 1, ATTN_NORM, 0);
                        else
                                gi.sound (ent, CHAN_ITEM, gi.soundindex("copb/copb_3.wav"), 1, ATTN_NORM, 0);
	...
	// PM:  Blend in flash if applicable.
...
void P_WorldEffects (void)
{
	...
	if (waterlevel == 3)
	{
            ...
            if ( Jet_Active(current_player) ) /*dont jet and dive and stay alive*/
              T_Damage (current_player, world, world, vec3_origin, current_player->s.origin, vec3_origin, current_player->health+1, 0, DAMAGE_NO_ARMOR, MOD_UNKNOWN);
            /*ATTILA end*/
...
/*-----/ PM /-----/ MODIFIED:  Use PPM's drowning sound. /-----*/
...
//Wyrm: cloaker!
        if (ent->client->cloak_framenum > level.framenum)
        {
                ...
                if (((ent->s.frame >= FRAME_crstnd01)&&(ent->s.frame <= FRAME_crstnd19))||((ent->s.frame >= FRAME_stand01)&&(ent->s.frame <=FRAME_stand40)))
...
void G_SetClientEvent (edict_t *ent)
{
	...
	if ( ent->groundentity && xyspeed > 225)
	{
		if ( (int)(current_client->bobtime+bobmove) != bobcycle )
                        //Wyrm: footsteps?
                        if (footsteps->value && !(ent->flags & FL_FROZEN))
                                ent->s.event = EV_FOOTSTEP;
	}
}
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	P_WorldEffects ();
        ...
        //Wyrm: BEGIN! - Turret! Check stuff...
        if (current_client->onturret)
                turret_client_check(current_client->turret);
        //END! - Turret!
	...
	// chase cam stuff
        if (ent->client->resp.spectator || ent->movetype == MOVETYPE_NOCLIP)
...
//ZOID
	...
	if ( (ent->client->pers.scanner_active) && !((level.framenum & SCANNER_UPDATE_FREQ) || (ent->client->pers.scanner_active & 2)) )
	{
		DeathmatchScoreboardMessage (ent, ent->enemy);
		gi.unicast (ent, false);
