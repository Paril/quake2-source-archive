...
/*-----------------------------------------------------------------------*/
	...
	if (deathmatch->value || coop->value)
	{
		...
		if (attacker == self)
		{
			...
			case MOD_HG_SPLASH:
			case MOD_G_SPLASH:
                        case MOD_CLUSTERGRENADES:
                        case MOD_RAILGRENADES:
                        case MOD_PROXGRENADES:
                        case MOD_STICKINGGRENADES:
                        case MOD_BFGRENADES:
                                if (IsNeutral(self))
...
/*-----------------------------------------------------------------*/
                        case MOD_PISTOL:
                                message = "took one bullet too many from";
                                message2 = "'s pistol";
                                break;
                        case MOD_AIRFIST:
                        ...
                        case MOD_EXPLOSIVEMACHINEGUN:
                                ...
                                message2 = "'s explosive bullets";
                                break;
                        case MOD_PULSERIFLE:
                                message = "gets ripped apart by";
                                message2 = "'s pulse rifle";
                                break;
                        case MOD_FIRETHROWER:
                                message = "was fried by";
                                break;
                        case MOD_STREETSWEEPER:
                                message = " has his insides now on the outside, thanx to ";
                                message2 = "'s Street Sweeper";
                                break;
                        case MOD_CLUSTERGRENADES:
...
//
				...
				message2 = "'s green grenade";
                                break;
                        case MOD_GUIDEDROCKETS:
                                message = " was hunted down by ";
                                message2 = "'s guided missile";
                                break;
                        case MOD_PLASMA:
                                message = " got a taste of plasma from ";
                                break;
                        case MOD_DISRUPTOR:
				...
				message2 = "'s disruptor";
                                break;
                        case MOD_ANTIMATTER:
                                message = " was reaped apart by ";
                                message2 = "'s antimatter cannon";
                                break;
                        case MOD_POSITRON:
                                message = " is annhilated by ";
                                message2 = "'s positron beam.";
                                break;
                        case MOD_TRIPBOMB:
				message = "tripped over";
                                message2 = "'s Tripbomb";
                                break;
                        case MOD_LASERTRIPBOMB:
                                message = "tripped over";
                        ...
                        case MOD_NUKE:
                                ...
                                message2 = "";
                                break;
                        case MOD_SENTRYEXPLODE:
                                message = "";           //FIXME
                                message2 = "";          //FIXME
                                break;
                        case MOD_SENTRYBULLET:
                                message = "";           //FIXME
                                message2 = "";          //FIXME
                                break;
                        }
...
//Wyrm: initial weapon can be thrown
/*
...
//	self->solid = SOLID_NOT;
	...
	if (!self->deadflag)
	{
                ...
                if (self->client->onturret)  //if linked to turret
				{
						...
						// level the gun
                        self->client->turret->move_angles[0] = 0;
                        self->client->turret->owner = NULL;
...
\* ¯¯¯¯¯¯¯¯¯ */
                if (self->client->flashlight)
                        nightmarePlayerResetFlashlight(self);
//Nightmare
...
//Wyrm:
        ...
        self->client->beans_framenum = 0;

	self->flags &= ~FL_POWER_ARMOR;
        self->flags &= ~FL_POWER_SCREEN;
...
//ZOID
	int ctf_team = client->resp.ctf_team;
        ...
        qboolean class_selected = client->resp.class_selected;
        int keysbinded = client->resp.keysbinded;
//ZOID
	memset (&client->resp, 0, sizeof(client->resp));
//ZOID
	client->resp.ctf_team = ctf_team;
        client->resp.class = class;
        client->resp.class_selected = class_selected;
        client->resp.keysbinded = keysbinded;
//ZOID
...
void SaveClientData (void)
{
	...
	for (i=0 ; i<game.maxclients ; i++)
	{
		...
		game.clients[i].pers.max_health = ent->max_health;
                game.clients[i].pers.savedFlags = (ent->flags & (FL_GODMODE|FL_NOTARGET|FL_POWER_ARMOR|FL_POWER_SCREEN));
		if (coop->value)
	...
	if (deathmatch->value)
	{
		...
		ClientUserinfoChanged (ent, userinfo);
                if (!enableclass->value)
                        ClassInitPersistant(client, 0);
                else
                        ClassInitPersistant(client, client->resp.class);
	}
	else if (coop->value)
	{
//		int			n;
...
//		}
	...
	else
	{
		memset (&resp, 0, sizeof(resp));
                //Wyrm: single player classes
                resp.class = client->resp.class;
                resp.class_selected = client->resp.class_selected;
                resp.keysbinded = client->resp.keysbinded;
	}
...
/*----------------------------------------------------------------------*/
        ...
        //Wyrm: freeze
        ent->client->iceball = NULL;
        //Wyrm: flashlight
        ent->client->flashlight = NULL;
        ent->client->flashtype = 0;

        //Wyrm: class
        if (!enableclass->value)
                SetClassAttributes(client, 0);
        else
                SetClassAttributes(client, client->resp.class);
...
//ZOID
        ...
        else if (enableclass->value)
        {
                if (!ent->client->resp.class_selected)
                {
                        // start as 'observer'
...
//ZOID
	...
	if (client->pers.spectator) {
		...
		client->resp.spectator = false;
                //Wyrm: if we aren't an spectator, check force chasecam
                if (cam_force->value)
                {
                        ChasecamStart(ent);
                        if (ent->client->chasecam)
                                ent->client->chasecam->chasedist1 = ent->client->cammaxdistance;
                }
        }
	...
	ChangeWeapon (ent);
        ...
        //check for start invulnerable time!
        if (start_666_time->value)
                client->invincible_framenum = level.framenum + start_666_time->value*10;
}
...
void ClientUserinfoChanged (edict_t *ent, char *userinfo)
{
	...
	s = Info_ValueForKey (userinfo, "skin");
        ...
        //set skin
        s = ClassSkin(s, ent->client->resp.class);
...
//ZOID
                ent->client->resp.class_selected = false;
		...
		if (!game.autosaved || !ent->client->pers.weapon)
			...
			InitClientPersistant (ent->client);
                        if (!enableclass->value)
                                ClassInitPersistant(ent->client, 0);
                        else
...
//Wyrm: class velocity
	...
	// otherwise, I can't do anything
        if( ent->flags & FL_FROZEN ) {
                if(level.time < ent->reallynextthink)
			return;
		else
                {
                        int n;
                        ...
                        ent->flags &= ~FL_FROZEN;
                        gi.sound (ent, CHAN_AUTO, gi.soundindex ("weapons/shatter.wav"), 1, ATTN_IDLE, 0);
                        n = rand() % 4 + 1;
                        while(n--)
                                ThrowDebris (ent, "models/objects/debris2/tris.md2", 1, ent->s.origin);

                        n = rand() % 3 + 1;
                        while(n--)
                                ThrowDebris (ent, "models/objects/debris1/tris.md2", 1, ent->s.origin);
                }
	}
	// **********************************
        ClassSpeedModifer = ent->client->ClassSpeed;
        //Wyrm: Steroids Powerup
        if (ent->client->steroids_framenum > level.framenum)
                ClassSpeedModifer *= 2;

        if (ClassSpeedModifer > 1)
        {
        ...
        else if (ClassSpeedModifer < 1)
        {
                ClassSpeedModifer *= 300;

                if (ucmd->forwardmove > ClassSpeedModifer)
                        ucmd->forwardmove = ClassSpeedModifer;
                else if (ucmd->forwardmove < -ClassSpeedModifer)
                        ucmd->forwardmove = -ClassSpeedModifer;

                if (ucmd->sidemove > ClassSpeedModifer)
                        ucmd->sidemove = ClassSpeedModifer;
                else if (ucmd->sidemove < -ClassSpeedModifer)
                        ucmd->sidemove = -ClassSpeedModifer;
        }
	...
	//		gi.dprintf ("pmove changed!\n");
		...
		{
                        //Wyrm: class, apply jumping :P
                        ent->velocity[2] *= ent->client->ClassJump;
                        //Wyrm: in steroids, jump more
                        if (ent->client->steroids_framenum > level.framenum)
                                ent->velocity[2] *= 1.35;
                        //Wyrm: only play sound if the class has jumping...
                        if (ent->client->ClassJump)
                        {
                                gi.sound(ent, CHAN_VOICE, gi.soundindex("*jump1.wav"), 1, ATTN_NORM, 0);
                                PlayerNoise(ent, ent->s.origin, PNOISE_SELF);
                        }
                }
                else if ((client->onturret) && (pm.cmd.upmove >= 10))  //Wyrm: if jumping off turret...
                {
                        client->onturret = 3;
                }
...
void ClientBeginServerFrame (edict_t *ent)
{
	...
	client = ent->client;
        ...
        //Wyrm: freeze gun
        if (client->iceball) IceThink(client->iceball);
...
//Wyrm: check if frozen...
	...
	if (ent->deadflag)
	{
        ...
        //bind keys if necessary
        if (!client->resp.spectator && ent->movetype != MOVETYPE_NOCLIP)
        if (autokeybinding->value)
        {
                if (client->resp.keysbinded == 5)
                {
                        Cmd_BindKeys(ent);
                        client->resp.keysbinded++;
                }
                else if (client->resp.keysbinded < 5)
                        client->resp.keysbinded++;
        }
