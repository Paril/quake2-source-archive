...
{
        fire_flamerocket (self, start, dir, damage, speed, damage+20, damage);
...
{
        fire_homing (self, start, dir, damage, speed, damage+20, damage);
...
{
        fire_perforator (self, start, dir, damage, speed, damage+20, damage);
		...
		if (!(ent->flags & FL_SWIM))
		{
                        if (ent->waterlevel < 2)
			{
				ent->air_finished = level.time + 12;
                                //Wyrm: if fallfloat, set step
                                if (ent->movetype == MOVETYPE_FALLFLOAT)
                                {
			...
			else
                        {
                                if (ent->air_finished < (level.time + 3))
                                        ent->movetype = MOVETYPE_FALLFLOAT;
...
//Wyrm: Napalm 2
        /* All combustible monsters have these fireflags. */
...
void monster_start_go (edict_t *self)
{
	...
	if (self->health <= 0)
        ...
        //NEW-->Wyrm!! Respawn...
        if (!self->monsterinfo.respawned)
        {
                VectorCopy(self->s.origin, self->monsterinfo.startorigin);
                VectorCopy(self->s.angles, self->monsterinfo.startangles);
        }
...
{
        if ((skill->value < 3)||(self->monsterinfo.spawn == NULL))
        {
                if (self->s.effects & EF_GIB)
                        G_FreeEdict (self);
                return;
        }
/*
        //If there is someone, wait...
        if (FindAnyOne(self))
        {
...
*/
        VectorCopy(self->monsterinfo.startorigin, self->s.origin);
        VectorCopy(self->monsterinfo.startangles, self->s.angles);
...
//free targets...
        ...
        self->monsterinfo.respawned = true;
        ...
        self->s.event = EV_PLAYER_TELEPORT; //fog!
...
{
        //Wyrm: finish sounds :P
        self->s.sound = 0;

        self->think = MonsterRespawn;
        if (self->monsterinfo.spawn != NULL)
        {
                self->think = MonsterRespawn;
                self->nextthink = level.time + 10 + (80 * random()); //Just like DooM...
        }
}
