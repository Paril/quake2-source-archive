...
void flyer_kamikaze_check2 (edict_t *self);
...
*/
int flyer_canassalt (edict_t *self)
{
        vec3_t mins, maxs;
        trace_t         tr;
	...
	// used to be 32 tall .. was WAY too big
        ...
        tr = gi.trace (self->s.origin, mins, maxs, self->enemy->s.origin, self, MASK_SHOT);
...
mframe_t flyer_frames_kamizake_look [] =
{
        ai_run, 25,  flyer_kamikaze_check2,
        ai_run, 25,  flyer_kamikaze_check2,
        ai_run, 25,  flyer_kamikaze_check2,
        ai_run, 25,  flyer_kamikaze_check2,
        ai_run, 25,  flyer_kamikaze_check2
};
mmove_t flyer_move_kamikaze_look = {FRAME_rollr02, FRAME_rollr06, flyer_frames_kamizake_look, flyer_kamikaze};
...
void flyer_kamikaze (edict_t *self)
{
        if (flyer_canassalt(self))
                self->monsterinfo.currentmove = &flyer_move_kamikaze;
        else
                self->monsterinfo.currentmove = &flyer_move_kamikaze_look;
}
...
//NEW-->Wyrm!! Respawn...
	...
	gi.multicast (self->s.origin, MULTICAST_PVS);
        self->s.modelindex = gi.modelindex("");
...
void SP_monster_flyer_normal (edict_t *self)
{
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_flyer_normal;
...
void SP_monster_kamikaze (edict_t *self)
{
	...
	self->monsterinfo.scale = MODEL_SCALE;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_kamikaze;
