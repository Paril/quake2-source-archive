...
void soldier_fire (edict_t *self, int in_flash_number)
{
	...
	else
	{
		VectorCopy (self->enemy->s.origin, end);
                ...
                //Wyrm: tech soldiers fire a bit down
                if (self->s.skinnum != 6 && self->s.skinnum != 7)
                {
	...
	else
	{
		...
		if (!(self->monsterinfo.aiflags & AI_HOLD_FRAME))
			self->wait = level.time + (3 + rand() % 8) * FRAMETIME;
                ...
                //Wyrm: tech soldier hack
                if (self->s.skinnum == 6 || self->s.skinnum == 7)
                        fire_laser(self, start, aim, 4);
                else
...
mmove_t soldier_move_jump = { FRAME_run01, FRAME_run12, soldier_frames_jump, soldier_run };
...
mmove_t soldier_move_jump2 = { FRAME_run01, FRAME_run12, soldier_frames_jump2, soldier_run };
...
void soldier_jump (edict_t *self)
{
	...
	if(self->enemy->s.origin[2] > self->s.origin[2])
                self->monsterinfo.currentmove = &soldier_move_jump2;
	else
...
void SP_monster_soldier_light (edict_t *self)
{
	...
	self->monsterinfo.blindfire = true;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_soldier_light;
...
void SP_monster_soldier (edict_t *self)
{
	...
	self->gib_health = -30;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_soldier;
}
...
void SP_monster_soldier_ss_normal (edict_t *self)
{
	...
	self->gib_health = -30;
        ...
        //Wyrm: respawn
        self->monsterinfo.spawn = SP_monster_soldier_ss_normal;
}
...
void SP_monster_soldier_tech (edict_t *self)
{
	...
	self->gib_health = -30;
        ...
        //Respawn
        self->monsterinfo.spawn = SP_monster_soldier_tech;
...
{
        if (random() > 0.3)
                SP_monster_soldier_ss_normal (self);
        else
                SP_monster_soldier_tech (self);
}
