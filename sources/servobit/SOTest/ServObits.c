/* *** STRIPPED DOWN ServObits.c *** */

#define SERV_OBIT_TESTER 1
...
#ifdef SERV_OBIT_TESTER
#include <string.h>
#include "external.h"
#endif

#ifndef SERV_OBIT_TESTER
#include "g_local.h"
#endif
...
int ServObitInsertValue(char *mess, char *format_str, char *field, char *value, int max_size)
{
	...
	while (format_str[x] && (x < max_size))
	{
		...
		{
			if (x + (int)strlen(value) < max_size)
			{
...
/* FIXME: boundary checks for Obit->Messages */
...
void ReadServerObits ()
{
	...
	DPRINT1("*** %s ***\n  (steqve@shore.net)\n", SERVOBIT_VERSION);
	DPRINT0("  thanks to newtonD, Eradicator, Hawkeye,\n");
	DPRINT0("  and Sati, \"Queen of Death Messages\"\n");
...
#ifdef SERV_OBIT_TESTER
		PRINTLOG0("*** ERROR: Diagnosed enemy kill, but no obit match found.\n");
#else
		BPRINT2("%s killed %s\n", victim, killer);
#endif
	}
...
#ifdef SERV_OBIT_TESTER
		PRINTLOG0("*** ERROR: Diagnosed suicide, but no obit match found.\n");
#else
...
#ifdef SERV_OBIT_TESTER
		PRINTLOG0("*** ERROR: Diagnosed environmental death, but no obit match found.\n");
#else
...
#ifdef SERV_OBIT_TESTER
		PRINTLOG0("*** ERROR: unrecognized death.\n");
#else
...
#ifdef SERV_OBIT_TESTER
	ServObit.Path = "";
	WriteServObitPathname(path, "SrvObTst.txt");
#else
