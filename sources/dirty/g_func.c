...
void plat_blocked (edict_t *self, edict_t *other)
{
	if (!(other->svflags & SVF_MONSTER) && (!other->client) )
	{
		// give it a chance to go away on it's own terms (like gibs)
                // GRIM - Bug in this.  Thing won't disappear, so replaced.
                T_Damage (other, world, self, vec3_origin, other->s.origin, vec3_origin, 100000, 1, 0, 0, MOD_CRUSH);
		// if it's still there, nuke it
		if (other)
                        ToastThatFucker (other);
                // GRIM
                return;
	}

        T_Damage (other, world, self, vec3_origin, other->s.origin, vec3_origin, self->dmg, 1, 0, 0, MOD_CRUSH);
...
void Touch_DoorTrigger (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if ((self->owner->spawnflags & DOOR_NOMONSTER) && (other->svflags & SVF_MONSTER))
        ...
        if (other->client && (other->client->pers.grabbing == -1))
        {
        ...
        // GRIM
        if (!AllyCheckOk (self->owner, other))
                return;
        // GRIM
	...
	self->touch_debounce_time = level.time + 1.0;
        ...
        // Dirty - god, I can't believe I am using some code from Action Quake2!
                // That just sux 
        if (other->client)
        {
                if (other->client->pers.grabbing != -1) // player not trying to open the door
                        return;
                else
                        door_use (self->owner, other, other);
                self->attack_time = level.time = 0.3;
        }
        // Dirty
}
...
void func_train_restart (edict_t *ent)
{
        VectorClear (ent->velocity);

        ent->target = ent->deathtarget;
...
void func_train_find (edict_t *self)
{
	...
	if (!ent)
	{
        ...
        // GRIM
        self->deathtarget = self->target;
        self->flags |= FL_RESPAWN;
        self->restart = func_train_restart;
        // GRIM
...
// GRIM
void func_timer_restart (edict_t *ent)
{
        // Start on
        if (ent->spawnflags & 1)
                ent->nextthink = level.time + 3.0 + ent->delay + ent->wait + crandom() * ent->random;
        else
                ent->nextthink = 0;
}
// GRIM
...
void SP_func_timer (edict_t *self)
{
	...
	self->svflags = SVF_NOCLIENT;
        ...
        // GRIM
        self->restart = func_timer_restart;
        self->flags |= FL_RESPAWN;
        if (turns_on)
                self->nextthink = 0;
        // Check if fire once...
...
// GRIM
void secret_door_restart (edict_t *ent)
{
        if ((ent->think == door_secret_move2) && (ent->wait == -1))
                door_secret_move4 (ent);
}
// GRIM
...
void SP_func_door_secret (edict_t *ent)
{
	...
	ent->classname = "func_door";
        ...
        // GRIM
        ent->flags |= FL_RESPAWN;
        ent->restart = secret_door_restart;
        Set_Forteam_n_Ally (ent);
        // GRIM
