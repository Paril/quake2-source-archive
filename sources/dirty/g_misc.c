...
// Originally, gibs, heads and debris' were handled here...
// Moved to z_junk.c
// Dirty
...
// GRIM
void func_wall_restart (edict_t *self)
{
...
//		gi.dprintf("func_wall missing TRIGGER_SPAWN\n");
	...
	else
	{
        ...
        self->restart = func_wall_restart;
        // GRIM
...
void func_object_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage == DAMAGE_NO)
		return;
        T_Damage (other, self, self, vec3_origin, self->s.origin, vec3_origin, self->dmg, 1, 0, 0, MOD_CRUSH);
}
...
// GRIM
void func_object_restart (edict_t *self)
{
	...
	else
	{
        ...
        VectorCopy (self->corpse_pos, self->s.origin);
	gi.linkentity (self);
}
// GRIM
...
void SP_func_object (edict_t *self)
{
	...
	self->clipmask = MASK_MONSTERSOLID;
        ...
        // GRIM
        self->flags |= FL_RESPAWN;
        self->restart = func_object_restart;
        VectorCopy (self->s.origin, self->corpse_pos);
        // GRIM
...
void func_explosive_explode (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	G_UseTargets (self, attacker);
        ...
        // respawn if rounds not on. Cool eh? 8)
        if (!turns_on)
        {
                self->think = self->restart;
                self->nextthink = level.time + self->wait;
        }

        // only make explosive if we have damage
	if (self->dmg)
        {
                edict_t *explode;

                explode = G_Spawn();
                VectorCopy(self->s.origin, explode->s.origin);
                BecomeExplosion1 (explode);
        }
        // GRIM
}
...
// GRIM
void func_explosive_restart (edict_t *self)
{
        self->nextthink = 0;
        self->solid = SOLID_BSP;
        self->svflags &= ~SVF_NOCLIENT;
...
void SP_func_explosive (edict_t *self)
{
	...
	if (self->use != func_explosive_use)
	{
        ...
        VectorCopy(self->s.origin, self->corpse_pos);
        self->restart = func_explosive_restart;
        // GRIM
...
void DoRespawnBarrel (edict_t *ent)
{
...
void SetRespawn2 (edict_t *ent, float delay, int h, int m, int d)
{
//        ent->flags |= FL_RESPAWN;
	ent->svflags |= SVF_NOCLIENT;
	ent->solid = SOLID_NOT;
	ent->nextthink = level.time + delay;
        ent->think = DoRespawnBarrel;
...
void BarrelExplosion1 (edict_t *self)
{
	gi.WriteByte (svc_temp_entity);
	gi.WriteByte (TE_EXPLOSION1);
	gi.WritePosition (self->s.origin);
	gi.multicast (self->s.origin, MULTICAST_PVS);
        ...
        if (deathmatch->value)
                SetRespawn2 (self, 40, 10, 400, 150);
	else
		G_FreeEdict (self);
}

void BarrelExplosion2 (edict_t *self)
{
	gi.WriteByte (svc_temp_entity);
	gi.WriteByte (TE_EXPLOSION2);
	gi.WritePosition (self->s.origin);
...
void barrel_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	vec3_t	v;
        ...
        {
                self->die (self, self, self, 10000, self->s.origin);
                return;
        }
...
void barrel_explode (edict_t *self)
{
	...
	if (self->groundentity)
                BarrelExplosion2 (self);
	else
                BarrelExplosion1 (self);
}
...
// GRIM
void viper_bomb_restart (edict_t *self)
{
	self->svflags |= SVF_NOCLIENT;
	self->movetype = MOVETYPE_NONE;
        VectorCopy (self->corpse_pos, self->s.origin);
...
void SP_misc_viper_bomb (edict_t *self)
{
	...
	self->svflags |= SVF_NOCLIENT;
        ...
        self->flags |= FL_RESPAWN;
        VectorCopy (self->s.origin, self->corpse_pos);
        self->restart = viper_bomb_restart;
        // GRIM
...
// GRIM
void satellite_dish_restart (edict_t *ent)
{
        ent->s.frame = 0;
        ent->solid = SOLID_BBOX;
        ent->svflags &= ~SVF_NOCLIENT;
...
void SP_misc_satellite_dish (edict_t *ent)
{
	...
	ent->use = misc_satellite_dish_use;
        ...
        if (ent->health)
        {
                ...
                ent->die = func_explosive_explode; // Hey, why not :)
                ent->flags |= FL_NO_KNOCKBACK;
        }
                            
        ent->flags |= FL_RESPAWN;
        VectorCopy (ent->s.origin, ent->corpse_pos);
        ent->restart = satellite_dish_restart;
        // GRIM
