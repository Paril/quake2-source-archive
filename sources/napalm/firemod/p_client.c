	...
	if (deathmatch->value || coop->value)
	{
		...
		if (attacker == self)
		{
			...
			case MOD_BFG_BLAST:
				...
				break;
// PM:  Fire related obituaries for suicides.
                        case MOD_FIRE:
				...
				break;
                        case MOD_CLOUD_BURST:
                        case MOD_CLOUD_FIRE:
                                message = "burst into flames";
				break;
                        case MOD_FIRESTORM:
                                message = "took a hot shower";
				break;
                        case MOD_FLASH:
                                message = "died in a flash";
				break;
                        case MOD_FLAME_BURST:
                                message = "stood too close to the fireworks";
				break;
                        case MOD_FLAMETHROWER:
                                message = "had a meltdown";
				break;
                        case MOD_METEOR_BLAST:
				if (IsNeutral(self))
                                        message = "ate its meteor blast";
                                else if (IsFemale(self))
                                        message = "ate her meteor blast";
				else
                                        message = "ate his meteor blast";
				break;
// PM
			default:
				if (IsNeutral(self))
		...
		if (attacker && attacker->client)
		{
			...
			case MOD_TELEFRAG:
				...
				break;
// PM:  Fire related obituaries for frags.
                        case MOD_FIRE:
				...
				break;
                        case MOD_CLOUD_FIRE:
                                message = "was firebombed by";
				break;
                        case MOD_FIRESTORM:
                                message = "burns in";
                                message2 = "'s firestorm";
				break;
                        case MOD_FLASH:
                                message = "was killed in a flash by";
				break;
                        case MOD_FLAME_BURST:
                                message = "got a light from";
                                message2 = "'s flame burst";
				break;
                        case MOD_FLAMETHROWER:
                                message = "was melted by";
...
// PM:  Don't toggle scores off if they were already up.
...
// PM:  Fix a few things...
                        // Play drown sound if underwater.
                        if (self->waterlevel == 3)
                                PBM_SfxDrown (self);
                        else
                                gi.sound (self, CHAN_VOICE, gi.soundindex(va("*death%i.wav", (rand()%4)+1)), 1, ATTN_NORM, 0);

                        // Make sure body is level.
...
// PM:  Set fire weapons on.
        client->pers.flameon = true;
// PM
...
// PM:  Don't let body make sounds.
        body->s.sound = 0;
// PM

// PM:  Treat body as dead monster so it bleeds and not spark.
...
// PM:  Tranfer fire from entity to body.
        if (ent->health > 0)
                body->health = 0;
        else
...
// PM:  Kill any flames and set new attributes.
        ent->gib_health = -41;
	ent->burnout = 0;
...
// PM:  Give respawn protection in DM.
        if (deathmatch->value)
                ent->client->invincible_framenum = level.framenum + 29;
// PM
