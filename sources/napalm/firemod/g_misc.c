...
// PM:  Don't shift head up if too close to ceiling.
        VectorCopy (self->s.origin, vd);
        vd[2] += 64;
        if (!(gi.pointcontents(vd) & CONTENTS_SOLID))
                self->s.origin[2] += 32;
// PM
	self->s.frame = 0;
	gi.setmodel (self, gibname);
...
void SP_misc_explobox (edict_t *self)
{
	...
	if (!self->dmg)
		self->dmg = 150;
// PM:  Set gib health and fireflags.
        self->gib_health = 0;
...
// PM:  Set gib health, mass, and fireflags.
        ent->gib_health = -80;
        ent->mass       = 200;
