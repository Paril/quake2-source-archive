...
//!! allowing CTF on any level probably involves something related to this code

	if (!G_Find(NULL, FOFS(classname), "item_flag_team1"))
	{	// not a CTF level (no flags)
		gi.cvar_forceset("ctf", "0");
	}
	else	// make sure CTF is enabled, since there is a flag
	{
		// niq: note that ctf must have been set for the flags to still be 
		// in the map since NIQ leaves/removes these prior to this point.
...
//#ifdef NIQ
		if(niq_enable->value)
			gi.configstring (CS_STATUSBAR, niq_ctf_statusbar);
//#else
		else
...
// niq: we use our own status bars for sp/dm/ctf
//#ifndef NIQ
...
void SP_worldspawn (edict_t *ent)
{
...
#ifdef EBOTS
	// niq 20/07/98: fix for spawning outside map etc.
	// num_dm_spots was never initialized unless niq_ebots was set
	// for now, letting all the following get initialized all the time.
	if(1 /*niq_ebots->value*/)
	{
		num_players		= 0;
		bot_count		= 0;
	...
	// niq 20/07/98: always initialize this!
	num_dm_spots = 0;
...
//#ifdef NIQ
	...
	{
	if (deathmatch->value)
		gi.configstring (CS_STATUSBAR, niq_dm_statusbar);
#ifdef NIQSP
	else
		gi.configstring (CS_STATUSBAR, niq_single_statusbar);
#endif
	}
//#else
else
...
//#endif
	...
	if(niq_ebots->value)
	{
		...
		{
		// !!! TBD: is this needed? Is num_view_weapons even used if there are no bots?
		total_bots			= 1;
		total_teams			= 0;
		teambot_list		= NULL;
		num_view_weapons	= 0;
	...
	{
		gi.modelindex ("players/male/a_grenades.md2");
		gi.modelindex ("players/male/w_bfg.md2");
		gi.modelindex ("players/male/w_blaster.md2");
		gi.modelindex ("players/male/w_chaingun.md2");
		gi.modelindex ("players/male/w_glauncher.md2");
		gi.modelindex ("players/male/w_grapple.md2");
		gi.modelindex ("players/male/w_hyperblaster.md2");
		gi.modelindex ("players/male/w_machinegun.md2");
		gi.modelindex ("players/male/w_railgun.md2");
		gi.modelindex ("players/male/w_rlauncher.md2");
		gi.modelindex ("players/male/w_shotgun.md2");
		gi.modelindex ("players/male/w_sshotgun.md2");

		gi.modelindex ("players/female/a_grenades.md2");
		gi.modelindex ("players/female/w_bfg.md2");
		gi.modelindex ("players/female/w_blaster.md2");
		gi.modelindex ("players/female/w_chaingun.md2");
		gi.modelindex ("players/female/w_glauncher.md2");
		gi.modelindex ("players/female/w_grapple.md2");
		gi.modelindex ("players/female/w_hyperblaster.md2");
		gi.modelindex ("players/female/w_machinegun.md2");
		gi.modelindex ("players/female/w_railgun.md2");
		gi.modelindex ("players/female/w_rlauncher.md2");
		gi.modelindex ("players/female/w_shotgun.md2");
		gi.modelindex ("players/female/w_sshotgun.md2");
	}
