...
*/
	...
	if(niq_enable->value)
	{
		// support some of the other hook 'types' out there
//!!!		if(!strcmp(s, "hook") || !strcmp(s, "grappling hook"))
		if(!strcmp(s, "grappling hook"))
			s = "grapple";
	}
#endif
...
//#ifdef NIQ
	// give NIQ the first crack at the command
	if (niq_clientcommand(ent, cmd))
		return;
//#endif
	...
	{
	bdidcmd = true;
	if (Q_stricmp (cmd, "botname") == 0) {
		if (!bot_allow_client_commands->value)
		{
			// niq: this should go to the client, not the server
...
/*
		...
		{
			gi.dprintf("You must specify a value from 1 -> 32\n");
			return;
		}

		if ((max_bots = atoi(gi.argv(1))) > MAX_BOTS)
		{
			my_bprintf(PRINT_HIGH, "Bot limit is %i!\n", MAX_BOTS);
//			gi.cvar_set("bot_num", MAX_BOTS);
			return;
		}
		else if (max_bots < 1)
		{
			my_bprintf(PRINT_HIGH, "Specify a value more than 0.\n");
			return;
		}
...
//ZOID
	...
	{
		CTFTeam_f (ent);
	} else if (niq_playerid->value && Q_stricmp(cmd, "id") == 0) {
		CTFID_f (ent);
	}
//ZOID
