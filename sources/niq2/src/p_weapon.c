...
{
#ifdef EBOTS
	// ebots code allows a list of supported models
	int	i;
	for (i=0; i<num_view_weapons; i++)
	{
		if (!strcmp(model, view_weapon_models[i]))
			return true;
	}
#else
	if (!strcmp(model, "male"))
		return true;
	if (!strcmp(model, "female"))
		return true;
	if (!strcmp(model, "cyborg"))
		return true;
	if (!strcmp(model, "crakhor"))
		return true;
#endif
...
//#ifdef NIQ
	if(niq_enable->value)
	{
	    // niq: when using blaster, damage is much higher than normal
