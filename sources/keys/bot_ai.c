...
*/
//K2:Begin
qboolean K2_botCanSeeStealth(edict_t *); 
void K2_botBFKInformDanger(edict_t *);
//K2:End
	...
	{	// don't attack someone else if current enemy has our flag!
		...
		while (player_index < num_players)
		{
			if (	!(search->flags & FL_NOTARGET) && (search != self) && (self->enemy != search) && (search->health > 0) && (search->bot_client || (search->light_level > 5)) && search->solid && !SameTeam(self, search)
					//K2:begin = Check to see if player is Stealth and if we can see him
					&& (K2_botCanSeeStealth(search)) 
					//Don't attack protected players of Feigning players
					&& (!K2_IsProtected(search))
					&& (!search->client->is_feigning)
					//If blinded, can't see other players
...
//		}
	...
	//K2:Begin - If bot has BFK, tell other bots to avoid
	K2_botBFKInformDanger(self);
	//take keys from other players
	if(K2_IsAnti(self))
	{
		if ( (rand() % 3) == 1)
			K2_TakePlayerKey(self);
	}
	
	//Regeneration check
	K2_Regeneration(self);
	//Check for exired key
	K2_KeyExpiredCheck(self);
	
	//Fire/Burn effects 
	//If bot gets into water, no more burn
	if (self->waterlevel >= 2)
