...
//K2:Begin - Changed  to non-static
...
void CTFResetGrapple(edict_t *self)
{
	if (self->owner->client->ctf_grapple) {
		...
		if (self->owner->client->silencer_shots || K2_IsStealth(self->owner)) //K2: Begin End
			volume = 0.2;
...
//	VectorSet(offset, 24, 16, ent->viewheight-8+2);
	...
	if (ent->client->silencer_shots || K2_IsStealth(ent)) //K2:Begin End
		volume = 0.2;
...
int CTFApplyResistance(edict_t *ent, int dmg)
{
	...
	if ( ent->client && (ent->client->silencer_shots || K2_IsStealth(ent))) //K2:Begin End
		volume = 0.2;
...
pmenu_t joinmenu[] = {
	{ "*Quake II",			PMENU_ALIGN_CENTER, NULL, NULL },
	{ "*Keys2 - ThreeWave CTF",	PMENU_ALIGN_CENTER, NULL, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL, NULL },
	{ NULL,					PMENU_ALIGN_CENTER, NULL, NULL },
...
qboolean CTFCheckRules(void)
{
	...
	if (capturelimit->value)
		if (ctfgame.team1 >= ctfgame.team2)
			k2_capsleft = (int)capturelimit->value - ctfgame.team1;
		else 
			k2_capsleft = (int)capturelimit->value - ctfgame.team2;
...
//ZOID
	...
	//K2:Begin - Hook - Set grappling time
