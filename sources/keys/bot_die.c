...
void bot_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
	...
	//Reset to old aggresion value
	//gi.dprintf("%s sggression reset to %f after death\n",self->client->pers.netname,self->botdata->bot_stats.aggr);
	self->bot_stats->aggr = self->botdata->bot_stats.aggr;
...
//	self->solid = SOLID_NOT;
	...
	if (!self->deadflag)
	{
		...
		//K2:Begin
		if(!attacker->bot_client && (inflictor != world)  && (meansOfDeath != MOD_CRUSH)  && botfraglogging->value  ) //Only log death if real player killed bot and if not a suicide.
		{
