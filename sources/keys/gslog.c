 ...
 *  Public header file.
 *
 * $Id: gslog.c 1.2 1998/04/06 09:22:08 mdavies Exp $
 *
 */
    ...
    if( deathmatch->value )
    {
        if( sl_Logging( gi, pPatch ) )
        {
            ...
            if( attacker == self )
            {
				...
				if (mod == MOD_FLAME)
					pWeaponName = "Flames";
				else if ( mod == MOD_DRUNK_SPLASH)
					pWeaponName = "Drunk Rocket";
				//K2:End
                ...
                if( attacker && attacker->client )
				{
					...
					switch (mod)
                    {
						...
						case MOD_HOMING_HYPERBLASTER:
						{
                            ...
                            /* Set weapon name - mdavies */
                            pWeaponName = "Homing Hyperblaster";
                            break;
                        }
