...
qboolean Pickup_Bandolier (edict_t *ent, edict_t *other)
{
	...
	if(swaat->value)
	{
	if (other->client->pers.max_bullets < (int)maxbullets->value*.833) //K2:was 250 - 83.3% of max
		other->client->pers.max_bullets = (int)maxbullets->value*.833; //K2:was 250
	if (other->client->pers.max_shells < (int)maxshells->value*.75)//K2:was 150 - 75% of max
		other->client->pers.max_shells = (int)maxshells->value*.75;//K2:was 150
	if (other->client->pers.max_cells < (int)maxcells->value*.833)//K2:was 250 - 83.3% of max
		other->client->pers.max_cells = (int)maxcells->value*.833;//K2:was 250
	if (other->client->pers.max_slugs < (int)maxslugs->value*.75)//K2:was 75 - 75% of max
		other->client->pers.max_slugs = (int)maxslugs->value*.75;//K2:was 75
	}
	else //K2:End
	{
...
qboolean Pickup_Pack (edict_t *ent, edict_t *other)
{
	...
	if(swaat->value)
	{
	if (other->client->pers.max_bullets < (int)maxbullets->value) //K2: was 300
		other->client->pers.max_bullets = (int)maxbullets->value; //k2:was 300
	if (other->client->pers.max_shells < (int)maxshells->value) //K2: was 200
		other->client->pers.max_shells = (int)maxshells->value;//K2: was 200
	if (other->client->pers.max_rockets < (int)maxrockets->value)//K2: was 100
		other->client->pers.max_rockets = (int)maxrockets->value;//K2: was 100
	if (other->client->pers.max_grenades < (int)maxgrenades->value)//K2: was 100
		other->client->pers.max_grenades = (int)maxgrenades->value;//K2: was 100
	if (other->client->pers.max_cells < (int)maxcells->value)//K2: was 300
		other->client->pers.max_cells = (int)maxcells->value;//K2: was 300
	if (other->client->pers.max_slugs < (int)maxslugs->value)//K2: was 100
		other->client->pers.max_slugs = (int)maxslugs->value;//K2: was 100
	}
	else //K2:End
	{
...
{
	//K2:Begin - Don't subtract from megahealth if have REGENERATION or BFK Key
		if ( K2_IsRegen(self->owner) || K2_IsBFK(self->owner) )
			 return;
	//K2:End
...
//K2:Begin - Adds other little health items to max health
	if ( (ent->style & HEALTH_IGNORE_MAX) && !(ent->style & HEALTH_TIMED))
	{
		if ( (other->max_health < ((int)maxhealth->value + 100)) || (other->health < (other->max_health + 100)) )
		{
			other->health += ent->count;		
			other->max_health += ent->count;

			if (other->health > ((int)maxhealth->value + 100) )
				other->health = (int)maxhealth->value + 100;

			if(other->max_health > ((int)maxhealth->value + 100) )
				other->max_health = (int)maxhealth->value + 100;
		}
		else
			return false;
	}
	...
	//K2:End
/*//K2:Begin: Remmed out Zoids stuff here
//ZOID
...
void Touch_Item (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (taken)
	{
		...
		{
			if (ent->count == 2)
				gi.sound(other, CHAN_ITEM, gi.soundindex("items/s_health.wav"), volume, ATTN_NORM, 0);
			else if (ent->count == 10)
				gi.sound(other, CHAN_ITEM, gi.soundindex("items/n_health.wav"), volume, ATTN_NORM, 0);
			else if (ent->count == 25)
				gi.sound(other, CHAN_ITEM, gi.soundindex("items/l_health.wav"), volume, ATTN_NORM, 0);
			else // (ent->count == 100)
				gi.sound(other, CHAN_ITEM, gi.soundindex("items/m_health.wav"), volume, ATTN_NORM, 0);
		}
...
void SpawnItem (edict_t *ent, gitem_t *item)
{
	...
	//K2:Begin - No BFG and other weapon  and powerup options
	if (Q_stricmp(item->pickup_name, "BFG10K") == 0)
	{
		if( nobfg->value)
		{
			G_FreeEdict(ent);
	...
	{
		if( nohyperblaster->value )
		{
			G_FreeEdict(ent);
			return;
		}
	}
	else if (Q_stricmp(item->pickup_name, "Railgun") == 0)
	{
		if( norailgun->value )
		{
			G_FreeEdict(ent);
	...
	else if (Q_stricmp(item->pickup_name, "Quad Damage") == 0)
	{
		if( noquad->value )
		{
			G_FreeEdict(ent);
	...
	else if (Q_stricmp(item->pickup_name, "Invulnerability") == 0)
	{
		if( noinvulnerability->value )
		{
			G_FreeEdict(ent);
			return;
		}
...
void SP_item_health_mega (edict_t *self)
{
	if ( deathmatch->value && ((int)dmflags->value & DF_NO_HEALTH) )
	{
		...
		//K2:Begin
	//  Exclude megahelath if nomegahealth=true
	//
	if ( nomegahealth->value )
	{
		G_FreeEdict (self);
		return;
	}
