...
qboolean SV_RunThink (edict_t *ent)
{
	...
	if (!ent->think)
	{
		//K2:Null ent-thinks don't crash the server any more
...
//retry:
	...
	for (part = ent ; part ; part=part->teamchain)
	{
		...
		//K2:Begin - Hook - If one of the ents is a grapple, move it.
		if (part->mynoise2 && part->mynoise2->think == Think_Grapple)
		{
			edict_t *hook = part->mynoise2;
			vec3_t org, org2, forward, right, up, move2;

			// If the grapple has already been released or something, continue
...
void SV_Physics_Step (edict_t *ent)
{
	...
	//ERASER	
	if ((!ent->groundentity || !ent->bot_client) && (ent->velocity[2] || ent->velocity[1] || ent->velocity[0]))
	{
		// apply friction
