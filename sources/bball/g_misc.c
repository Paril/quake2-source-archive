...
void ThrowClientHead (edict_t *self, int damage)
{
	...
	//M.S. throwing bots head
	else if (self->bot)
	{
		self->bot->anim_priority = ANIM_DEATH;
		self->bot->anim_end = self->s.frame;
	}
	//M.S.
	else
	{
...
void SP_path_corner (edict_t *self)
{
	...
	//M.S. spawn rim guard
	if( (Q_stricmp (self->targetname, "rim1") == 0) ||
		(Q_stricmp (self->targetname, "rim2") == 0) )
		SP_rim_guard (self);
	//M.S.
...
void SP_misc_teleporter_dest (edict_t *ent)
{
	...
	ent->s.skinnum = 0;
	//M.S. change from SOLID_BBOX to SOLID_NOT, don't want player interaction
	ent->solid = SOLID_NOT;
	//M.S.
