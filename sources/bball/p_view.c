...
void G_SetClientFrame (edict_t *ent)
{
	...
	//M.S. if hands are up, update weapons model every frame
	//check for health first or we will crash
	if(ent->health > 0 &&
	   ent->client->pers.weapon->weapmodel == WEAP_HANDS)
		BallShowCheck(ent);
	//M.S.
	...
	if (!ent->groundentity && client->anim_priority <= ANIM_WAVE
		//M.S. No newanim if dunking
		&& client->anim_priority != ANIM_DUNK)
		//M.S.
		goto newanim;
...
newanim:
	//M.S. for dunk scoring, only place I could find for this code
	if (client->anim_priority == ANIM_DUNK)
	{
		...
		if (ent->client->resp.team == 1)
		{
			...
			level.ballteam1 += 2;
			gi.bprintf(PRINT_HIGH, "%s slams 2 points for %s\n",ent->client->pers.netname, team1name->string);
			Inbound(2);
		}
		else
		{
			level.ballteam2 += 2;
			gi.bprintf(PRINT_HIGH, "%s slams 2 points for %s\n",ent->client->pers.netname, team2name->string);
			Inbound(1);
		}
	...
	{	// standing
		...
		else
		{
					...
					&& (ent->client->pers.inventory[ent->client->ammo_index]))
			{
				...
				client->anim_end = FRAME_ballstand10;
			}
			else
			{
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	//
	//M.S. don't turn if dunking (screen turns, model doesn't)
	if(ent->client->anim_priority != ANIM_DUNK)
	{
