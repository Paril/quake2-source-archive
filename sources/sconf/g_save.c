...
{
    // SCONFIG START
	//sl_Logging( &gi, NULL );	// StdLog - Mark Davies (Only required to set patch name)
	sl_Logging( &gi, "ServerConfigMOD v3.2 DM" );	// StdLog - Mark Davies (Only required to set patch name)
	
	gi.dprintf ("==== ServerConfigMOD v3.2 DM ====\n");
...
//	stdlogname = gi.cvar ("stdlogname", "0", 0);
	...
	// SCONFIG START
	serverconfigmodinfo = gi.cvar("MOD","ServerConfigMOD v3.2", CVAR_SERVERINFO|CVAR_NOSET);
	// SCONFIG STOP
	...
	// ServObit
	if (deathmatch) ServObitInitGame();
	// End ServObit

	curLevInCycle = J_ImportLevelCycle();
	// SCONFIG STOP
}
