...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	if (coop->floatVal && attacker->client)
...
void TossClientWeapon (edict_t *self)
{
	...
	if (!((int)(dmflags->floatVal) & DF_QUAD_DROP))
		quad = false;
	else
...
//	self->solid = SOLID_NOT;
	...
	if (!self->deadflag)
	{
		...
		{
			if (coop->floatVal && itemlist[n].flags & IT_KEY)
	...
	{	// normal death
		if (!self->deadflag)
		{
			...
			gi.sound (self, CHAN_VOICE, gi.soundindex(Q_VarArgs ("*death%i.wav", (rand()%4)+1)), 1, ATTN_NORM, 0);
		}
	}
...
void InitClientResp (gclient_t *client)
{
	...
	client->resp.aimType = 0;
	client->resp.aimingLeft = true;
	client->resp.camDist = 2500;
}
...
float	PlayersRangeFromSpot (edict_t *spot)
{
	...
	for (n = 1; n <= maxclients->floatVal; n++)
	{
		player = &g_edicts[n];
...
{
	if ( (int)(dmflags->floatVal) & DF_SPAWN_FARTHEST)
...
void CopyToBodyQue (edict_t *ent)
{
	...
	body = &g_edicts[(int)maxclients->floatVal + level.body_que + 1];
...
void spectator_respawn (edict_t *ent)
{
	...
	if (ent->client->pers.spectator) {
		...
		// count spectators
		for (i = 1, numspec = 0; i <= maxclients->floatVal; i++)
			if (g_edicts[i].inUse && g_edicts[i].client->pers.spectator)
				numspec++;

		if (numspec >= maxspectators->floatVal) {
...
//		}
	...
	ent->clipMask = CONTENTS_MASK_SOLID|CONTENTS_MONSTER;
	...
	if (deathmatch->floatVal && ((int)dmflags->floatVal & DF_FIXED_FOV))
	{
		client->ps.fov = 90;
	}
	else
...
void ClientUserinfoChanged (edict_t *ent, char *userinfo)
{
	...
	if (deathmatch->floatVal && *s && strcmp(s, "0"))
		ent->client->pers.spectator = true;
	else
		ent->client->pers.spectator = false;
...
BOOL ClientConnect (edict_t *ent, char *userinfo)
{
	...
	if (deathmatch->floatVal && *value && strcmp(value, "0")) {
		int i, numspec;
		...
		// count spectators
		for (i = numspec = 0; i < maxclients->floatVal; i++)
...
void ClientDisconnect (edict_t *ent)
{
	...
	G_FreeEdict(ent->client->resp.followDude);
...
cmTrace_t	PM_trace (vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end)
{
	...
	{
		int cliptype = (CONTENTS_SOLID|CONTENTS_WINDOW|CONTENTS_PLAYERCLIP);
		if (pm_passent->client->resp.aimType == 1)
			cliptype |= CONTENTS_MIST;

		return gi.trace (start, mins, maxs, end, pm_passent, cliptype);
	}
	else
...
#define CLOSE_CAMERA 120
#define FAR_CAMERA 360
// Fills origin and angles with the finalized origin and angles for player's camera
...
static void GI_Pmove (pMove_t *pMove, edict_t *ent)
{
	float airAccel = 0;
	pMoveNew_t	epm;
...
void ClientThink (edict_t *ent, userCmd_t *ucmd)
{
	...
	// set the delta angle
	vec3_t realAngle = {10,90,0};
	for (i=0 ; i<3 ; i++)
	{
		client->ps.pMove.deltaAngles[i] = ANGLE2SHORT(realAngle[i] - client->resp.cmd_angles[i]);
	}
	...
	if (ent->client->chase_target) {
		...
		client->ps.pMove.gravity = sv_gravity->floatVal;
	...
	//		gi.dprintf ("pmove changed!\n");
		...
		//else
		{
			...
			if (ent->client->resp.aimingLeft && ucmd->sideMove < 0 && !(ent->client->buttons & BUTTON_ATTACK))
				ent->client->resp.aimingLeft = false;
			else if (!ent->client->resp.aimingLeft && ucmd->sideMove > 0 && !(ent->client->buttons & BUTTON_ATTACK))
				ent->client->resp.aimingLeft = true;

			client->v_angle[0] = 0;
			client->ps.viewAngles[0] = 0;
			if (!ent->deadflag)
			{
				if (ent->client->resp.aimingLeft)
				{
					client->v_angle[1] -= 90;
					client->ps.viewAngles[1] -= 90;
				}
				else
				{
					client->v_angle[1] += 90;
			...
			if (ent->client->resp.aimType == 1)
			{
				client->v_angle[0] -= 35;
				client->ps.viewAngles[1] -= 35;
			}
			else if (ent->client->resp.aimType == 2)
			{
				client->v_angle[0] += 35;
				client->ps.viewAngles[1] += 35;
			}
			else if (ent->client->resp.aimType == 3)
			{
				client->v_angle[0] -= 90;
				client->ps.viewAngles[1] -= 90;
			}
			else if (ent->client->resp.aimType == 4)
			{
				client->v_angle[0] += 90;
				client->ps.viewAngles[1] += 90;
			}
		...
		/*if (ent->s.origin[1] != 64 && !ent->deadflag)
		{
			gi.bprintf (PRINT_HIGH, "%s attempted to cheat!!\n", client->pers.netname);
			player_die(ent, ent, ent, 9999, ent->s.origin);
		}*/
...
==============
*/
void CreateFollowDude (edict_t *ent)
{
	ent->client->resp.followDude = G_Spawn();

	ent->client->resp.followDude->s = ent->s;
	ent->client->resp.followDude->s.effects |= RF_FRAMELERP;
	gi.linkentity(ent->client->resp.followDude);
}
...
void ClientBeginServerFrame (edict_t *ent)
{
	...
	if (ent && ent->client)
	{
		if (!ent->client->resp.followDude)
			CreateFollowDude(ent);
		else
		{
			gi.linkentity(ent->client->resp.followDude);
			ent->client->resp.followDude->s = ent->s;
	...
	if (ent->deadflag)
	{
		...
		if ( level.time > client->respawn_time)
		{
			...
			if ( ( client->latched_buttons & buttonMask ) ||
				(deathmatch->floatVal && ((int)dmflags->floatVal & DF_FORCE_RESPAWN) ) )
			{
				respawn(ent);
				client->latched_buttons = 0;
			}
