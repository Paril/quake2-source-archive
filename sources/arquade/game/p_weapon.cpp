...
void weapon_grenadelauncher_fire (edict_t *ent)
{
	...
	gi.WriteShort (ent->client->resp.followDude-g_edicts);
...
void Blaster_Fire (edict_t *ent, vec3_t g_offset, int damage, bool hyper, int effect)
{
	...
	if (is_quad)
		damage *= 4;
	Angles_Vectors (ent->client->v_angle, forward, right, NULL);
	Vec3Set (offset, 16, 8, ent->viewheight-15);
	...
	fire_blaster (ent, start, forward, damage, hyper ? 600 : 1000, effect, hyper);
...
void Machinegun_Fire (edict_t *ent)
{
	...
	Vec3Set (offset, 13, 8, ent->viewheight-14);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
...
void Chaingun_Fire (edict_t *ent)
{
	...
	for (i=0 ; i<shots ; i++)
	{
		...
		r = 7 + crandom()*4;
		u = crandom()*4;
		Vec3Set (offset, u+13, r, u + ent->viewheight-13);
		P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
...
void weapon_shotgun_fire (edict_t *ent)
{
	...
	Vec3Set (offset, 10, 8,  ent->viewheight-12);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
...
void weapon_supershotgun_fire (edict_t *ent)
{
	...
	Vec3Set (offset, 10, 6,  ent->viewheight-12);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
	gi.WriteByte (SVC_TEMP_ENTITY);
	gi.WriteByte (TE_GUNSHOT);
	gi.WritePosition (start);
...
void weapon_railgun_fire (edict_t *ent)
{
	...
	Vec3Set (offset, 15, 7,  ent->viewheight-12);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
