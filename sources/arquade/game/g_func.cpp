...
void Move_Final (edict_t *ent)
{
	...
	Vec3Scale (ent->moveinfo.dir, ent->moveinfo.remaining_distance / FRAMETIME, ent->velocity);
...
void AngleMove_Final (edict_t *ent)
{
	...
	Vec3Scale (move, 1.0/FRAMETIME, ent->avelocity);
...
void Think_AccelMove (edict_t *ent)
{
	...
	Vec3Scale (ent->moveinfo.dir, ent->moveinfo.current_speed*10, ent->velocity);
	ent->nextthink = level.time + FRAMETIME;
	ent->think = Think_AccelMove;
}
...
//	
	...
	Vec3Copy (tmin, trigger->mins);
	Vec3Copy (tmax, trigger->maxs);
...
void button_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	if (!other->client)
		return;
...
void Touch_DoorTrigger (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	if (other->health <= 0)
		return;
...
void Think_SpawnDoorTrigger (edict_t *ent)
{
	...
	Vec3Copy (ent->absMin, mins);
	Vec3Copy (ent->absMax, maxs);
	...
	other = G_Spawn ();
	Vec3Copy (mins, other->mins);
	Vec3Copy (maxs, other->maxs);
	other->owner = ent;
...
void door_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	if (!other->client)
		return;
...
void door_secret_move6 (edict_t *self)
{
	Move_Calc (self, vec3Origin, door_secret_done);
}
