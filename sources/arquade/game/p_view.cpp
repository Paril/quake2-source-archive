...
void P_DamageFeedback (edict_t *player)
{
	...
	if ((level.time > player->pain_debounce_time) && !(player->flags & FL_GODMODE) && (client->invincible_framenum <= level.framenum))
	{
		...
		if (player->health < 25)
			...
			l = 100;
		gi.sound (player, CHAN_VOICE, gi.soundindex(Q_VarArgs ("*pain%i_%i.wav", l, r)), 1, ATTN_NORM, 0);
	}
...
void P_FallingDamage (edict_t *ent)
{
	...
	if (delta > 30)
	{
		...
		if (!deathmatch->floatVal || !((int)dmflags->floatVal & DF_NO_FALLING) )
...
void P_WorldEffects (void)
{
	...
	breather = (bool)(current_client->breather_framenum > level.framenum);
	envirosuit = (bool)(current_client->enviro_framenum > level.framenum);
	...
	if (waterlevel == 3)
	{
		...
		{	// drown!
				...
				&& current_player->health > 0)
			{
				...
				T_Damage (current_player, world, world, vec3Origin, current_player->s.origin, vec3Origin, current_player->dmg, 0, DAMAGE_NO_ARMOR, MOD_WATER);
			}
		}
	}
	else
	...
	if (waterlevel && (current_player->watertype&(CONTENTS_LAVA|CONTENTS_SLIME)) )
	{
		if (current_player->watertype & CONTENTS_LAVA)
		{
			...
			if (envirosuit)	// take 1/3 damage with envirosuit
				T_Damage (current_player, world, world, vec3Origin, current_player->s.origin, vec3Origin, 1*waterlevel, 0, 0, MOD_LAVA);
			else
				T_Damage (current_player, world, world, vec3Origin, current_player->s.origin, vec3Origin, 3*waterlevel, 0, 0, MOD_LAVA);
		}
		...
		if (current_player->watertype & CONTENTS_SLIME)
		{
			...
			{	// no damage from slime with envirosuit
				T_Damage (current_player, world, world, vec3Origin, current_player->s.origin, vec3Origin, 1*waterlevel, 0, 0, MOD_SLIME);
			}
		}
	}
}
...
bool PlayerOnFloor (edict_t *player)
{
	...
	Vec3MA (player->s.origin, 50, end, end);
    tr = gi.trace (player->s.origin, NULL, NULL, end, player, CONTENTS_MASK_ALL);
    //Com_Printf("%f\n", tr.fraction);
...
newanim:
	...
	if (client->resp.followDude)
	{
		client->resp.followDude->s = ent->s;
		client->resp.followDude->s.renderFx &= ~RF_FRAMELERP;
	}
}
...
void ClientEndServerFrame (edict_t *ent)
{
	...
	current_client->ps.pMove.origin[1] -= ent->client->resp.camDist;
	current_client->ps.pMove.origin[2] += ent->client->resp.camDist/4.3;

	if (current_client->resp.followDude)
	{
		Vec3Copy (ent->s.origin, current_client->resp.followDude->s.origin);
		Vec3Copy (ent->velocity, current_client->resp.followDude->velocity);
	}
	...
	//{
		...
		Vec3MA (ent->s.origin, 50, end, end);
		tr = gi.trace (ent->s.origin, NULL, NULL, end, ent, CONTENTS_MASK_ALL);
		//Com_Printf("%f\n", tr.fraction);
		if (tr.plane.normal && tr.fraction < 0.50f)
		{
			vec3_t oldAngles;
			Vec3Copy (ent->s.angles, oldAngles);
			//vec3_t ang;
			VecToAngles(tr.plane.normal, ent->s.angles);
			ent->s.angles[1] = oldAngles[1];

			ent->s.angles[PITCH] += 90;
			if (ent->client->resp.aimingLeft)
				ent->s.angles[0] = -ent->s.angles[0];
		}*/
	//}
