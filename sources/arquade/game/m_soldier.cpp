...
void soldier_fire (edict_t *self, int flash_number)
{
	...
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[flash_index], forward, right, start);
...
void soldier_sight(edict_t *self, edict_t *other)
{
	...
	if ((skill->floatVal > 0) && (range(self, self->enemy) >= RANGE_MID))
	{
		if (random() > 0.5)
