...
//	gi.sound (self, CHAN_VOICE, actor.sound_pain, 1, ATTN_NORM, 0);
	...
	if ((other->client) && (random() < 0.4))
	{
		...
		Vec3Subtract (other->s.origin, self->s.origin, v);
...
void actorMachineGun (edict_t *self)
{
	...
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_ACTOR_MACHINEGUN_1], forward, right, start);
	if (self->enemy)
	{
		...
		{
			Vec3Copy (self->enemy->absMin, target);
...
void target_actor_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	vec3_t	v;
	...
	{
		Vec3Subtract (other->movetarget->s.origin, other->s.origin, v);
