...
void makronBFG (edict_t *self)
{
	...
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_MAKRON_BFG], forward, right, start);
...
void MakronSaveloc (edict_t *self)
{
	Vec3Copy (self->enemy->s.origin, self->pos1);	//save for aiming the shot
...
void MakronRailgun (edict_t *self)
{
	...
	Angles_Vectors (self->s.angles, forward, right, NULL);
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_MAKRON_RAILGUN_1], forward, right, start);
	
	// calc direction to where we targted
	Vec3Subtract (self->pos1, start, dir);
	VectorNormalizef (dir, dir);
...
void makron_dead (edict_t *self)
{
	Vec3Set (self->mins, -60, -60, 0);
	Vec3Set (self->maxs, 60, 60, 72);
...
void MakronSpawn (edict_t *self)
{
	...
	Vec3Subtract (player->s.origin, self->s.origin, vec);
	self->s.angles[YAW] = VecToYaw(vec);
	VectorNormalizef (vec, vec);
	Vec3MA (vec3Origin, 400, vec, self->velocity);
	self->velocity[2] = 200;
