...
void use_target_changelevel (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if (deathmatch->floatVal && !( (int)dmflags->floatVal & DF_ALLOW_EXIT) && other != world)
	{
...
void use_target_spawner (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	gi.linkentity (ent);
	if (self->speed)
		Vec3Copy (self->movedir, ent->velocity);
}
...
void target_laser_think (edict_t *self)
{
	...
	while(1)
	{
		...
		if ((tr.ent->takedamage) && !(tr.ent->flags & FL_IMMUNE_LASER))
			T_Damage (tr.ent, self, self->activator, self->movedir, tr.endPos, vec3Origin, self->dmg, 1, DAMAGE_ENERGY, MOD_TARGET_LASER);
