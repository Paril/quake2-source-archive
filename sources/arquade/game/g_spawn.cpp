...
{
	int i = 0;
	allowDMMonsters = true;
	edict_t *m;

	for (; i < 1; i++)
	{
		m = G_Spawn();
		edict_t *spot = SelectFarthestDeathmatchSpawnPoint();

		Vec3Copy (spot->s.origin, m->s.origin);
		Vec3Copy (spot->s.angles, m->s.angles);
		m->s.origin[2] += 4;
		SP_monster_tank(m);

		gi.linkentity(m);
	}
	allowDMMonsters = false;
...
void InitSpawner ()
{
	...
	//tmp->think = SpawnMonster;
	//tmp->nextthink = level.time + 15;
