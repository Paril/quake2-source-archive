...
void mutant_hit_left (edict_t *self)
{
	...
	Vec3Set (aim, MELEE_DISTANCE, self->mins[0], 8);
...
void mutant_hit_right (edict_t *self)
{
	...
	Vec3Set (aim, MELEE_DISTANCE, self->maxs[0], 8);
	if (fire_hit (self, aim, (10 + (rand() %5)), 100))
...
void mutant_check_refire (edict_t *self)
{
	...
	if ( ((skill->floatVal == 3) && (random() < 0.5)) || (range(self, self->enemy) == RANGE_MELEE) )
...
void mutant_jump_touch (edict_t *self, edict_t *other, plane_t *plane, cmBspSurface_t *surf)
{
	if (self->health <= 0)
	{
		self->touch = NULL;
	...
	if (other->takedamage)
	{
		if (Vec3Length(self->velocity) > 400)
		{
			vec3_t	point;
			vec3_t	normal;
			int		damage;
...
void mutant_jump_takeoff (edict_t *self)
{
	...
	self->s.origin[2] += 1;
	Vec3Scale (forward, 600, self->velocity);
	self->velocity[2] = 250;
