...
bool Pickup_Powerup (edict_t *ent, edict_t *other)
{
	...
	if (deathmatch->floatVal)
	{
		...
		if (((int)dmflags->floatVal & DF_INSTANT_ITEMS) || ((ent->item->use == Use_Quad) && (ent->spawnflags & DROPPED_PLAYER_ITEM)))
		{
...
{
	if ( deathmatch->floatVal && ((int)dmflags->floatVal & DF_NO_HEALTH) )
	{
		G_FreeEdict (self);
		return;
	}
