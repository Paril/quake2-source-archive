...
void M_WorldEffects (edict_t *ent)
{
		...
		if (!(ent->flags & FL_SWIM))
		{
			...
			{	// drown!
				if (ent->pain_debounce_time < level.time)
				{
					...
					if (dmg > 15)
						dmg = 15;
					T_Damage (ent, world, world, vec3Origin, ent->s.origin, vec3Origin, dmg, 0, DAMAGE_NO_ARMOR, MOD_WATER);
	...
	if ((ent->watertype & CONTENTS_LAVA) && !(ent->flags & FL_IMMUNE_LAVA))
	{
		if (ent->damage_debounce_time < level.time)
		{
			...
			T_Damage (ent, world, world, vec3Origin, ent->s.origin, vec3Origin, 10*ent->waterlevel, 0, 0, MOD_LAVA);
		}
	}
	if ((ent->watertype & CONTENTS_SLIME) && !(ent->flags & FL_IMMUNE_SLIME))
	{
		if (ent->damage_debounce_time < level.time)
		{
			...
			T_Damage (ent, world, world, vec3Origin, ent->s.origin, vec3Origin, 4*ent->waterlevel, 0, 0, MOD_SLIME);
		}
	}
...
{
	if (deathmatch->floatVal && !allowDMMonsters)
	{
		G_FreeEdict (self);
		return false;
	}
