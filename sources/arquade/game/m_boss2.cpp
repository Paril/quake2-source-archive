...
//1
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_BOSS2_ROCKET_1], forward, right, start);
	Vec3Copy (self->enemy->s.origin, vec);
...
//2
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_BOSS2_ROCKET_2], forward, right, start);
	Vec3Copy (self->enemy->s.origin, vec);
	vec[2] += self->enemy->viewheight;
	Vec3Subtract (vec, start, dir);
	VectorNormalizef (dir, dir);
...
//3
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_BOSS2_ROCKET_3], forward, right, start);
	Vec3Copy (self->enemy->s.origin, vec);
	vec[2] += self->enemy->viewheight;
	Vec3Subtract (vec, start, dir);
	VectorNormalizef (dir, dir);
...
//4
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_BOSS2_ROCKET_4], forward, right, start);
	Vec3Copy (self->enemy->s.origin, vec);
	vec[2] += self->enemy->viewheight;
	Vec3Subtract (vec, start, dir);
	VectorNormalizef (dir, dir);
...
void boss2_firebullet_right (edict_t *self)
{
	...
	Angles_Vectors (self->s.angles, forward, right, NULL);
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_BOSS2_MACHINEGUN_R1], forward, right, start);

	Vec3MA (self->enemy->s.origin, -0.2, self->enemy->velocity, target);
...
void boss2_firebullet_left (edict_t *self)
{
	...
	Angles_Vectors (self->s.angles, forward, right, NULL);
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[MZ2_BOSS2_MACHINEGUN_L1], forward, right, start);
...
/*	vec3_t	forward, right;
	...
	G_ProjectSource (self->s.origin, dumb_and_hacky_monster_MuzzFlashOffset[flash_number], forward, right, start);

	Vec3Copy (self->enemy->s.origin, vec);
	vec[2] += self->enemy->viewheight;
	Vec3Subtract (vec, start, dir);
	VectorNormalizef (dir);
...
void boss2_dead (edict_t *self)
{
	Vec3Set (self->mins, -56, -56, 0);
	Vec3Set (self->maxs, 56, 56, 80);
