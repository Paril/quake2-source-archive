...
void ClipGibVelocity (edict_t *ent)
{
	...
	if (ent->velocity[2] < 500)
		ent->velocity[2] = 500;	// always some upwards
	else if (ent->velocity[2] > 900)
		ent->velocity[2] = 900;
}
...
void gib_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->client) {
		if (other->client->is_bunny)
		{
			gi.sound (other, CHAN_VOICE, gi.soundindex ("misc/fhit3.wav"), 1, ATTN_NORM, 0);
			gi.cprintf (other,PRINT_HIGH,"Yummy! Gibs! +%d health!\n",i);
			other->health +=i;
			G_FreeEdict(self);
			return;
		}
...
void gib_radiuscheck (edict_t *self)
{
	int i = random() * 6 + 1;
	//edict_t	*target = NULL;
	edict_t *blip = NULL;
	...
	while ((blip = findradius(blip, self->s.origin, 50)) != NULL)
	{
		if (!(blip->client))
			continue;
		if (!(blip->client->is_bunny))
			continue;

		gi.sound (blip, CHAN_VOICE, gi.soundindex ("misc/fhit3.wav"), 1, ATTN_NORM, 0);
		gi.cprintf (blip,PRINT_HIGH,"Yummy! Gibs! +%d health!\n",i);
		blip->health +=i;
		G_FreeEdict(self);
		return;
	}
	self->nextthink2 = level.time + .2;
}
...
void ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	gib->die = gib_die;
	//gib->health = 200;
	gib->think2 = gib_radiuscheck;
	gib->nextthink2 = level.time + 1.2;
	...
	VectorSet(gib->velocity,200+(crandom()*900) - 450,200+(crandom()*900) - 450,crandom()*400+400);
	//ClipGibVelocity (gib);
	VectorScale(gib->velocity,1.4,gib->velocity);
	...
	gib->nextthink = level.time + 10 + random()*12;
