...
void bfg_think (edict_t *self)
{
	T_RadiusDamage(self,self->owner,100,NULL,140,MOD_BFG_EFFECT);
...
void fire_bfg (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius)
{
	...
	bfg->movetype = MOVETYPE_BIG_BOUNCE;
	...
	bfg->owner = self;
	bfg->touch = bfg_touch;
	bfg->nextthink = level.time + 60;
	bfg->think = bfg_explode;
	bfg->think2 = bfg_think;
	bfg->nextthink2 = level.time + crandom();
	bfg->radius_dmg = damage;
	bfg->dmg_radius = damage_radius;
	bfg->classname = "bfg-shot";
