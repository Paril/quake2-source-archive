...
void plat_hit_top (edict_t *ent)
{
        if (!(ent->flags & FL_TEAMSLAVE))
        {
                if (ent->moveinfo.sound_end)
                        gi.sound (ent, CHAN_AUTO, ent->moveinfo.sound_end, 1, ATTN_STATIC, 0);
                ent->s.sound = 0;
        }
...
void plat_go_down (edict_t *ent)
{
        if (!(ent->flags & FL_TEAMSLAVE))
        {
                if (ent->moveinfo.sound_start)
                        gi.sound (ent, CHAN_AUTO, ent->moveinfo.sound_start, 1, ATTN_STATIC, 0);
...
void plat_blocked (edict_t *self, edict_t *other)
{
        if (!other->client)
        {
                ...
                // if it's in the bodyque, kill it in a special way
                /*if ((int)other <= (int)game.maxclients + BODY_QUEUE_SIZE){
...
void SP_func_plat (edict_t *ent)
{
        ...
        ent->moveinfo.sound_start = SoundIndex (plats_pt1_strt);
        ent->moveinfo.sound_middle = SoundIndex (plats_pt1_mid);
        ent->moveinfo.sound_end = SoundIndex (plats_pt1_end);
}
...
{
        T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, self->dmg, 1, DAMAGE_IGNORE_RESISTANCES, MOD_CRUSH);
}
...
{
        if (self->style)
                if (self->activator && self->activator->client && !((self->style) & classtypebit[self->activator->client->resp.class_type]))
                        return;
        ...
        if (self->moveinfo.sound_start && !(self->flags & FL_TEAMSLAVE))
                gi.sound (self, CHAN_AUTO, self->moveinfo.sound_start, 1, ATTN_STATIC, 0);
...
void SP_func_button (edict_t *ent)
{
        ...
        if (!ent->target && !ent->targetname) {
                gi.dprintf ("REMOVED: func_button with no target and no targetname\n");
                G_FreeEdict (ent);
                return;
        }

        if (!ent->target)
                gi.dprintf ("WARNING: func_button with no target\n");
        ...
        if (ent->sounds != 1)
                ent->moveinfo.sound_start = SoundIndex (switches_butn2);
        ...
        while ((t = G_Find (t, FOFS(targetname), self->target)))
        {
                if (t->enttype == ENT_AREAPORTAL)
                {
                        if (mapdebugmode->value)
                                gi.dprintf ("areaportal %d: open = %d, targetname %s, triggered by %s\n", t->style, open, t->targetname, self->targetname);
...
{
        if (!(self->flags & FL_TEAMSLAVE) && self->movetarget)
        {
                /*if (self->moveinfo.sound_end)
                        gi.sound (self, CHAN_AUTO, self->moveinfo.sound_end, 1, ATTN_STATIC, 0);
                self->s.sound = 0;*/

                if (!(self->activator && self->activator->client && self->activator->client->resp.class_type == CLASS_COMMANDO))
                {
                        if (self->moveinfo.sound_end)
                                gi.sound (self->movetarget, CHAN_AUTO, self->moveinfo.sound_end, 1, ATTN_STATIC, 0);
                }
                self->movetarget->s.sound = 0;
        }
        self->moveinfo.state = STATE_TOP;
...
{
        // fixing premature ap trigger = hom
        if (self->think == door_hit_bottom) {
...
void door_go_down (edict_t *self)
{
        if (!(self->flags & FL_TEAMSLAVE) && self->movetarget)
        {
                ...
                self->s.sound = self->moveinfo.sound_middle;*/
                if (self->moveinfo.sound_start)
                        gi.sound (self->movetarget, CHAN_AUTO, self->moveinfo.sound_start, 1, ATTN_STATIC, 0);
                self->movetarget->s.sound = self->moveinfo.sound_middle;
        }
        if (self->max_health)
        {
                self->takedamage = DAMAGE_YES;
        ...
        //if (self->classname[9] == '\0')
        if (self->enttype == ENT_FUNC_DOOR)
                Move_Calc (self, self->moveinfo.start_origin, door_hit_bottom);
        else //if (strcmp(self->classname, "func_door_rotating") == 0)
                AngleMove_Calc (self, door_hit_bottom);

        //gi.bprintf (PRINT_HIGH, "door origin close: %s\n", vtos2(self));
}
...
void door_go_up (edict_t *self, edict_t *activator)
{
        ...
        {
                /*if (self->moveinfo.sound_start)
                        gi.sound (activator, CHAN_AUTO, self->moveinfo.sound_start, 1, ATTN_STATIC, 0);
                self->s.sound = self->moveinfo.sound_middle;*/
                if (self->moveinfo.sound_start)
                        self->movetarget->nextthink = level.time + FRAMETIME;
                self->movetarget->s.sound = self->moveinfo.sound_middle;
        }
...
void door_use (edict_t *self, edict_t *other, edict_t *activator)
{
        ...
        // block triggers while door is rising
        if (self->moveinfo.state == STATE_UP)
                return;
...
{
        //gi.bprintf (PRINT_HIGH, "doortrigger %d touched by %s\n",self->s.number, other->classname);
        ...
        {
                gi.dprintf ("WARNING: DoorTrigger lost its door at %s!!\n", vtos2(self));
                G_FreeEdict (self);
                return;
        }
...
void door_blocked  (edict_t *self, edict_t *other)
{
        ...
        //gi.bprintf (PRINT_HIGH, "door %d blocked by %s\n",self->s.number, other->classname);

        //r1: removed SVF_MONSTER check (allows spider structures to be killed in doors
        //    like humans)
        if (!other->client)
        {
                // give it a chance to go away on it's own terms (like gibs)
                T_Damage (other, self, self, vec3_origin, other->s.origin, vec3_origin, 100000, 0, DAMAGE_NO_PROTECTION, MOD_CRUSH);

                // if it's still there, nuke it
                if (other->inuse && other->takedamage)
                        G_FreeEdict (other);
                return;
        }
...
{
        //gi.bprintf (PRINT_HIGH, "door %d touched by %s\n",self->s.number, other->classname);
        ...
        gi.sound (other, CHAN_AUTO, SoundIndex (misc_talk1), 1, ATTN_NORM, 0);
}

void door_play_opensound (edict_t *self)
{
        if (self->owner->activator && self->owner->activator->client && self->owner->activator->client->resp.class_type == CLASS_COMMANDO)
                return;

        if (self->owner->moveinfo.sound_start)
                gi.sound (self, CHAN_AUTO, self->owner->moveinfo.sound_start, 1, ATTN_STATIC, 0);
}

void door_set_soundent (edict_t *ent)
{
        vec3_t mins, maxs;
        edict_t *other, *snd;

        if (ent->flags & FL_TEAMSLAVE)
                return;         // only the team leader

        // create sound ent and hijack
        snd = G_Spawn ();
        ent->movetarget = snd;

        snd->svflags = SVF_NOCLIENT;
        snd->owner = ent;
        snd->think = door_play_opensound; // hax for ap + opening door
        ...
        VectorSubtract (maxs, mins, snd->s.origin);
        VectorMA (mins, 0.5, snd->s.origin, snd->s.origin);
...
void SP_func_door (edict_t *ent)
{
        ...
        if (ent->sounds != 1)
        {
                ent->moveinfo.sound_start = SoundIndex (doors_dr1_strt);
                ent->moveinfo.sound_middle = SoundIndex (doors_dr1_mid);
                ent->moveinfo.sound_end = SoundIndex (doors_dr1_end);
        }
...
void SP_func_door_rotating (edict_t *ent)
{
        ...
        if (!st.distance)
        {
                gi.dprintf("WARNING: %s at %s with no distance set\n", ent->classname, vtos2(ent));
                st.distance = 90;
        }
...
void SP_func_water (edict_t *self)
{
        ...
        switch (self->sounds)
        {
                ...
                case 1: // water
                        self->moveinfo.sound_start = SoundIndex (world_mov_watr);
                        self->moveinfo.sound_end = SoundIndex (world_stp_watr);
                        break;
...
//              gi.dprintf ("train_next: no next target\n");
        ...
        if (!ent)
        {
                gi.dprintf ("WARNING: train_next: bad target %s\n", self->target);
                return;
        }
        ...
        if (ent->spawnflags & 1)
        {
                if (!first)
                {
                        gi.dprintf ("WARNING: connected teleport path_corners, see %s at %s\n", ent->classname, vtos2(ent));
                        return;
                }
                first = false;
...
void func_train_find (edict_t *self)
{
        ...
        if (!self->target)
        {
                gi.dprintf ("WARNING: train_find: no target\n");
                return;
        }
        ent = G_PickTarget (self->target);
        if (!ent)
        {
                gi.dprintf ("WARNING: train_find: target %s not found\n", self->target);
                return;
        }
        self->target = ent->target;
...
void SP_func_train (edict_t *self)
{
        ...
        {
                gi.dprintf ("WARNING: func_train without a target at %s\n", vtos(self->absmin));
        }
}
...
//              gi.dprintf("elevator busy\n");
        ...
        if (!other->pathtarget)
        {
                gi.dprintf("WARNING: elevator used with no pathtarget\n");
                return;
        }
        ...
        if (!target)
        {
                gi.dprintf("WARNING: elevator used with bad pathtarget: %s\n", other->pathtarget);
                return;
        }
...
void trigger_elevator_init (edict_t *self)
{
        if (!self->target)
        {
                gi.dprintf("WARNING: trigger_elevator has no target\n");
                return;
        }
        self->movetarget = G_PickTarget (self->target);
        if (!self->movetarget)
        {
                gi.dprintf("WARNING: trigger_elevator unable to find target %s\n", self->target);
                return;
        }
        if (strcmp(self->movetarget->classname, "func_train") != 0)
        {
                gi.dprintf("WARNING: trigger_elevator target %s is not a train\n", self->target);
                return;
        }
...
void func_timer_think (edict_t *self)
{
        if (mapdebugmode->value >= 2)
                gi.dprintf ("[ timer ] func_timer {%s} has timed out\n",self->targetname);
...
{
        if (mapdebugmode->value)
                gi.dprintf ("[ timer ] func_timer {%s} was used by %s {%s}\n",self->targetname, other->classname, other->targetname);
        self->activator = activator;
...
void SP_func_timer (edict_t *self)
{
        ...
        if (self->random >= self->wait)
        {
                ...
                gi.dprintf("WARNING: func_timer at %s has random >= wait\n", vtos2(self));
        }
...
/*  when the game ends, it searches for this, if it finds it, it fires its targets  */
/*  both on_humans_wins and on_aliens_win use this function because the only difference is the classname  */

void SP_on_win(edict_t *self)
{
        self->svflags |= SVF_NOCLIENT;
        self->activator = self; 
}
