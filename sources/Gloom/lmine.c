...
void tripwire_think(edict_t *self) {
    ...
    while (++num <= 20) {
        ...
        // first trace starts in something solid, no need to trace further
        if (tr.startsolid && num == 1) {
            self->svflags &= ~SVF_30FPS;
            self->nextthink = level.time + FRAMETIME;
            VectorCopy(start, tr.endpos);
            break;
        // activate mine in case its not blocked at the very start
        } else if (num == 1 && !tr.startsolid)
            self->svflags |= SVF_30FPS;
        
        // new trace started in something solid that is not a client. stop tracing here
