...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
  ...
  if (SETTING_INT(fragmode) == FM_DPOINTS || attacker->client && score_override) { //those where modified on G_Damage by override code, dont modify twice, above code should update just SD, stats, etc
        ...
        if (ent->client->resp.score > 0 && ent->client->resp.team != TEAM_NONE && !(level.intermissiontime)) {
                ...
                for (i = 0;i <= 64;i++) {
                        if (fragsave[i].inuse == 0) {
                                ...
                                fragsave[i].dmg_points = ent->client->resp.dmg_points;
