...
void PostSpawnSetup (edict_t *ent, int class_type)
{
        ...
        if (!(sv_cheats->value) && SETTING_INT(fragmode) != FM_UNLIMITED)
...
qboolean TeamStartCheck(edict_t *ent, int class_type)
{
	...
	if (SETTING_INT(fragmode) != FM_UNLIMITED)
          i = classlist[class_type].frags_needed;
...
*/
                ...
                if (t) {
                        ...
                        //reset their frags if positive and count team change
                        if (!(ent->client->pers.adminpermissions & (1 << PERMISSION_BENEFITS))) {
                           if (ent->client->resp.score > 0 && SETTING_INT(fragmode) != FM_UNLIMITED) ent->client->resp.score = 0;
                           if (ent->client->resp.total_score > 0) ent->client->resp.total_score = 0;
                           if (ent->client->resp.dmg_points) ent->client->resp.dmg_points = 0;
                           //If admin doesnt get cleared, don't count teamchanges so @setteam to self isnt needed either
